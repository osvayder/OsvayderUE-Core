 ...
 */
async function simulateListTools({ toolCacheTtlMs = DEFAULT_TTL_MS, allowedToolNames = [] } = {}) {
  ...
  const getExposedToolName = (rawToolName) => {
    if (hasScopedToolFilter) return rawToolName;
    return rawToolName === "restart_survival" ? rawToolName : `unreal_${rawToolName}`;
  };
  ...
  for (const tool of unrealTools) {
    if (!isScopedToolAllowed(tool.name) || !shouldExposeListedTool(tool.name)) {
      continue;
    }

    mcpTools.push({
      name: getExposedToolName(tool.name),
...
describe("ListTools — router filtering", () => {
  ...
  it("scoped mode exposes allowed mega tools by raw name", async () => {
    const result = await simulateListTools({ allowedToolNames: ["enhanced_input", "blueprint_modify"] });
    expect(result.tools.find(t => t.name === "enhanced_input")).toBeDefined();
    expect(result.tools.find(t => t.name === "blueprint_modify")).toBeDefined();
  });

  it("scoped mode omits router and context tools", async () => {
    const result = await simulateListTools({ allowedToolNames: ["enhanced_input"] });
    expect(result.tools.find(t => t.name === "unreal_ue")).toBeUndefined();
    expect(result.tools.find(t => t.name === "unreal_get_ue_context")).toBeUndefined();
  });

  it("scoped mode excludes disallowed tools", async () => {
    const result = await simulateListTools({ allowedToolNames: ["enhanced_input"] });
    expect(result.tools.find(t => t.name === "enhanced_input")).toBeDefined();
    expect(result.tools.find(t => t.name === "blueprint_query")).toBeUndefined();
    expect(result.tools.find(t => t.name === "unreal_spawn_actor")).toBeUndefined();
  });
});
