	...
	TArray<FString> ExtractExecutionTruthArtifactPaths(const FString& Text)
	{
		...
		for (FString Token : Tokens)
		{
			...
			if (LowerToken.Contains(TEXT(".log"), ESearchCase::CaseSensitive)
				|| LowerToken.Contains(TEXT("saved/unrealclaude"), ESearchCase::CaseSensitive)
...
FString ExecutionTruthCategoryToString(const EUnrealClaudeExecutionTruthCategory Category)
{
	switch (Category)
	{
	case EUnrealClaudeExecutionTruthCategory::ReadOnlyInspection:
		return TEXT("read_only_inspection");
	case EUnrealClaudeExecutionTruthCategory::ApprovedBuildOrTestExecution:
		return TEXT("approved_build_or_test_execution");
	case EUnrealClaudeExecutionTruthCategory::ApprovedProjectMutation:
		return TEXT("approved_project_mutation");
	case EUnrealClaudeExecutionTruthCategory::ManagedStateWrite:
		return TEXT("managed_state_write");
	case EUnrealClaudeExecutionTruthCategory::StaleOrOutOfRun:
		return TEXT("stale_or_out_of_run");
	case EUnrealClaudeExecutionTruthCategory::UnsafeOrUnknown:
	default:
...
bool IsManagedUnrealClaudeStatePathMentioned(const FString& Text)
{
	...
	return Normalized.Contains(TEXT("saved/unrealclaude/active_plan.json"), ESearchCase::CaseSensitive)
		|| Normalized.Contains(TEXT("saved/unrealclaude/planarchives"), ESearchCase::CaseSensitive)
		|| Normalized.Contains(TEXT("saved/unrealclaude/closeoutdecisions"), ESearchCase::CaseSensitive)
		|| Normalized.Contains(TEXT("saved/unrealclaude/closeout_decision.json"), ESearchCase::CaseSensitive)
		|| Normalized.Contains(TEXT("saved/unrealclaude/visible_session"), ESearchCase::CaseSensitive)
		|| Normalized.Contains(TEXT("saved/unrealclaude/agent_trace"), ESearchCase::CaseSensitive);
}

FString FUnrealClaudeExecutionTruthDecision::ToSummaryString() const
{
...
TSharedPtr<FJsonObject> FUnrealClaudeExecutionTruthDecision::ToJsonObject() const
{
...
FUnrealClaudeExecutionTruthDecision ClassifyExecutionTruth(
	const FUnrealClaudeExecutionTruthInputs& Inputs)
{
	FUnrealClaudeExecutionTruthDecision Decision;
	...
	const bool bManagedStateTouched = IsManagedUnrealClaudeStatePathMentioned(CommandEnvelope);
	...
	{
		Decision.Category = EUnrealClaudeExecutionTruthCategory::StaleOrOutOfRun;
	...
	{
		Decision.Category = EUnrealClaudeExecutionTruthCategory::ManagedStateWrite;
		Decision.ReasonCode = TEXT("command_mutates_managed_unrealclaude_state");
		return Decision;
	}

	if (bBuildOrTestExecution)
	{
		Decision.Category = EUnrealClaudeExecutionTruthCategory::ApprovedBuildOrTestExecution;
	...
	{
		Decision.Category = EUnrealClaudeExecutionTruthCategory::ReadOnlyInspection;
	...
	{
		Decision.Category = EUnrealClaudeExecutionTruthCategory::ApprovedProjectMutation;
		Decision.ReasonCode = TEXT("canon_routing_classified_project_mutation");
		return Decision;
	}

	Decision.Category = EUnrealClaudeExecutionTruthCategory::UnsafeOrUnknown;
