...
FString FClaudeCodeRunner::CachedCliJsPath;
...
namespace
{
	...
	/** Singleton persistent-session manager owned by the ClaudeCodeRunner translation unit. */
	FClaudeSessionManager& GetPersistentSessionManagerSingleton()
	{
		static FClaudeSessionManager Manager;
		return Manager;
	}
}

FString FClaudeCodeRunner::ComposePersistentSessionFlags(
...
FString FClaudeCodeRunner::GetClaudeLanguageDisplayName(const EUnrealClaudeDictationLanguage Language)
{
...
FString FClaudeCodeRunner::ComposeLanguageHintFlag(const FString& LanguageDisplayName)
{
...
FString FClaudeCodeRunner::ComposeModelFlag(const FString& ModelName)
{
...
FString FClaudeCodeRunner::ComposeEffortFlag(const FString& EffortLevel)
{
...
FString FClaudeCodeRunner::ComposeStdinPayloadForClaudePrint(const FString& PromptText)
{
...
bool FClaudeCodeRunner::ShouldAutoResetOnExit(const int32 ExitCode, const FString& Output, const bool bUsedResume)
{
...
FAgentBackendCapabilities FClaudeCodeRunner::GetCapabilities() const
{
	FAgentBackendCapabilities Capabilities;
	Capabilities.Backend = EUnrealClaudeProviderBackend::ClaudeCli;
...
FAgentBackendStatus FClaudeCodeRunner::GetStatus() const
{
	FAgentBackendStatus Status;
	Status.Backend = EUnrealClaudeProviderBackend::ClaudeCli;
		...
		if (IFileManager::Get().FileExists(*CliJsPath))
		{
			...
			for (const FString& NodePath : NodePaths)
			{
				if (IFileManager::Get().FileExists(*NodePath))
				{
					...
					UE_LOG(LogUnrealClaude, Log, TEXT("Found Claude CLI (direct node mode): node=%s, cli=%s"), *NodePath, *CliJsPath);
					return CachedClaudePath;
				}
			}
		}
...
FString FClaudeCodeRunner::BuildCommandLine(const FClaudeRequestConfig& Config)
{
	...
	const UUnrealClaudeSettings* ClaudeSettings = UUnrealClaudeSettings::Get();
	...
	{
		const FString RequestModel = ClaudeSettings->GetConfiguredModelForBackend(EUnrealClaudeProviderBackend::ClaudeCli);
		bool bWasExisting = false;
		const FString SessionId = GetPersistentSessionManagerSingleton()
			.GetOrCreatePersistentSessionId(EUnrealClaudeProviderBackend::ClaudeCli, RequestModel, &bWasExisting);
	...
	if (ClaudeSettings)
	{
		const FString ConfiguredModel = ClaudeSettings->GetConfiguredModelForBackend(EUnrealClaudeProviderBackend::ClaudeCli);
		const FString ModelFragment = ComposeModelFlag(ConfiguredModel);
		if (!ModelFragment.IsEmpty())
		{
			CommandLine += ModelFragment;
			UE_LOG(LogUnrealClaude, Log, TEXT("Claude model fragment: %s"), *ModelFragment.TrimStartAndEnd());
		}
		...
		if (!EffortFragment.IsEmpty())
		{
			...
			UE_LOG(LogUnrealClaude, Log, TEXT("Claude effort fragment: %s"), *EffortFragment.TrimStartAndEnd());
		}
	}
	...
	if (ConfigRequestsUnrealMcpBridge(Config))
	{
		...
		if (!PluginDir.IsEmpty())
		{
			...
			const UUnrealClaudeSettings* PortSettings = UUnrealClaudeSettings::Get();
			int32 McpPort = (PortSettings) ? PortSettings->MCPServerPort : UnrealClaudeConstants::MCPServer::DefaultPort;
			...
			const UUnrealClaudeSettings* EyeSettings = UUnrealClaudeSettings::Get();
			...
			if (EyeSettings && EyeSettings->bEnableOsvayderEye && !EyeServerPath.IsEmpty() && IFileManager::Get().FileExists(*EyeServerPath))
			{
				...
				UE_LOG(LogUnrealClaude, Log, TEXT("OsvayderEye enabled in MCP config: %s"), *EyeServerPath);
			}
			...
			auto McpScopeCheck = FUnrealClaudeScopePolicy::IsAbsoluteWriteAllowed(MCPConfigPath);
			...
			{
				UE_LOG(LogUnrealClaude, Warning, TEXT("Scope denied for MCP config write: %s"), *MCPConfigPath);
				FExecutionReceipt Receipt;
				Receipt.Tool = TEXT("claude_runner");
				Receipt.bSuccess = false;
				Receipt.TargetType = TEXT("file");
	...
	if (!MaterializedSystemPrompt.IsEmpty())
	{
		...
		auto SysScopeCheck = FUnrealClaudeScopePolicy::IsAbsoluteWriteAllowed(SystemPromptPath);
		if (SysScopeCheck.bAllowed)
		{
			if (FFileHelper::SaveStringToFile(MaterializedSystemPrompt, *SystemPromptPath, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM))
			{
				...
				UE_LOG(LogUnrealClaude, Log, TEXT("System prompt written to: %s (%d chars)"), *SystemPromptPath, MaterializedSystemPrompt.Len());
				...
				FUnrealClaudeExecutionLog::Get().AddReceipt(R);
			}
			else
			{
	...
	auto PromptScopeCheck = FUnrealClaudeScopePolicy::IsAbsoluteWriteAllowed(PromptPath);
...
void FClaudeCodeRunner::ParseAndEmitNdjsonLine(const FString& JsonLine)
{
	...
	if (Type == TEXT("system"))
	{
		...
		{
			UE_LOG(LogUnrealClaude, Warning, TEXT("NDJSON System: subtype=%s, attempt=%.0f, error=%s, error_status=%s, session_id=%s, raw=%s"),
...
bool FClaudeCodeRunner::LaunchProcess(const FString& FullCommand, const FString& WorkingDir)
{
	...
	// Diagnostic logging
	UE_LOG(LogUnrealClaude, Log, TEXT("Process env: USERPROFILE=%s, APPDATA=%s, LOCALAPPDATA=%s"),
	...
	UE_LOG(LogUnrealClaude, Log, TEXT("Credentials check: %s (exists=%d)"), *CredsPath, bCredsExist);
	if (!bCredsExist)
	{
		UE_LOG(LogUnrealClaude, Warning, TEXT("Claude credentials not found! Run 'claude login' in terminal first."));
	}
	...
	UE_LOG(LogUnrealClaude, Log, TEXT("Launching via cmd.exe: %s %s"), *ActualExe, *Params);
#else
...
void FClaudeCodeRunner::ExecuteProcess()
{
	...
	if (StdInWritePipe)
	{
		...
		if (!StdinPayload.IsEmpty())
		{
			...
			const UUnrealClaudeSettings* DiagSettings = UUnrealClaudeSettings::Get();
	...
	const UUnrealClaudeSettings* RetrySettings = UUnrealClaudeSettings::Get();
	...
	if (bShouldReset)
	{
		UE_LOG(LogUnrealClaude, Warning,
			...
			ExitCode);
		GetPersistentSessionManagerSingleton().ResetPersistentSession(EUnrealClaudeProviderBackend::ClaudeCli);
