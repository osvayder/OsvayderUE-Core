...
namespace
{
	...
	FString DetermineExecutionModeFromTools(const TArray<FString>& ToolNames)
	{
		...
		{
			MCPServer = FUnrealClaudeModule::Get().GetMCPServer();
		}
...
TSharedPtr<FJsonObject> FUnrealClaudeReportTruthSummary::ToJson() const
{
...
int32 FUnrealClaudeReportTruthSummary::GetTotalClaimCount() const
{
...
bool FUnrealClaudeReportTruthSummary::HasAnyClaims() const
{
	return GetTotalClaimCount() > 0;
}

FString FUnrealClaudeReportArtifacts::GetReportsDirectory()
{
	FString SavedDir = FPaths::ConvertRelativePathToFull(FPaths::ProjectSavedDir());
	FPaths::NormalizeDirectoryName(SavedDir);
	const FString ReportsDir = FPaths::Combine(SavedDir, TEXT("UnrealClaude"), TEXT("Reports"));
	IFileManager::Get().MakeDirectory(*ReportsDir, true);
	return ReportsDir;
}

FString FUnrealClaudeReportArtifacts::GetReportSummaryGlob()
{
	return FPaths::Combine(GetReportsDirectory(), TEXT("*.summary.json"));
}

bool FUnrealClaudeReportArtifacts::ExportReport(const FUnrealClaudeReportExportRequest& Request, FUnrealClaudeReportExportResult& OutResult)
{
	OutResult = FUnrealClaudeReportExportResult();
	...
	FUnrealClaudeReportExportRequest EffectiveRequest = Request;
	...
	const FUnrealClaudeScopePolicy::FScopeCheckResult MarkdownScopeCheck = FUnrealClaudeScopePolicy::IsAbsoluteWriteAllowed(MarkdownPath);
	const FUnrealClaudeScopePolicy::FScopeCheckResult SummaryScopeCheck = FUnrealClaudeScopePolicy::IsAbsoluteWriteAllowed(SummaryPath);
...
TArray<TSharedPtr<FJsonObject>> FUnrealClaudeReportArtifacts::QueryReports(const FUnrealClaudeReportQueryOptions& Options)
{
...
FString FUnrealClaudeReportArtifacts::MakeReportSlug(const FString& InValue)
{
...
FString FUnrealClaudeReportArtifacts::MakeReportId(const FString& ReportName, const FString& ExplicitSlug)
{
...
FString FUnrealClaudeReportArtifacts::MakeMarkdownPath(const FString& ReportId)
{
	return FPaths::Combine(GetReportsDirectory(), ReportId + TEXT(".md"));
}

FString FUnrealClaudeReportArtifacts::MakeSummaryPath(const FString& ReportId)
{
	return FPaths::Combine(GetReportsDirectory(), ReportId + TEXT(".summary.json"));
}

bool FUnrealClaudeReportArtifacts::SaveUtf8MarkdownWithBom(const FString& Path, const FString& Markdown)
{
...
bool FUnrealClaudeReportArtifacts::SaveUtf8JsonWithoutBom(const FString& Path, const TSharedPtr<FJsonObject>& JsonObject)
{
	return FFileHelper::SaveStringToFile(SerializeJsonPretty(JsonObject), *Path, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM);
}

bool FUnrealClaudeReportArtifacts::LoadUtf8Markdown(const FString& Path, FString& OutMarkdown)
{
	TArray<uint8> Bytes;
...
bool FUnrealClaudeReportArtifacts::LoadJsonObject(const FString& Path, TSharedPtr<FJsonObject>& OutJsonObject)
{
	FString JsonText;
...
TSharedPtr<FJsonObject> FUnrealClaudeReportArtifacts::BuildSummaryObject(const FUnrealClaudeReportExportRequest& Request)
{
	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
	Root->SetStringField(TEXT("schema"), TEXT("unrealclaude_report_artifact"));
...
void FUnrealClaudeReportArtifacts::NormalizeTruthSummary(FUnrealClaudeReportTruthSummary& InOutTruthSummary)
{
...
void FUnrealClaudeReportArtifacts::EnrichSummaryFromRunTrace(
	const FString& RunId,
...
FString FUnrealClaudeReportArtifacts::DeriveToolFamily(const FString& ToolName)
{
	const FString Normalized = ToolName.TrimStartAndEnd().ToLower();
...
TArray<FString> FUnrealClaudeReportArtifacts::DeriveEvidenceClasses(const TArray<FString>& ToolNames, const TArray<FString>& ToolFamilies)
{
...
FString FUnrealClaudeReportArtifacts::NormalizeExecutionModeLabel(const FString& InExecutionMode)
{
...
TArray<FString> FUnrealClaudeReportArtifacts::NormalizeUniqueStrings(const TArray<FString>& Values)
{
...
void FUnrealClaudeReportArtifacts::SortSummaryObjectsNewestFirst(TArray<TSharedPtr<FJsonObject>>& Summaries)
{
