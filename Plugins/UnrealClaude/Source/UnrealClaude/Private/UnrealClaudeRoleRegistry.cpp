...
namespace
{
	...
	FUnrealClaudeRoleActionDecision MakeUnknownRoleActionDecision(
		const FString& RoleId,
		const FString& ActionId)
	{
		FUnrealClaudeRoleActionDecision Decision;
	...
	FUnrealClaudeRoleTransitionDecision MakeUnknownRoleTransitionDecision(
		const FString& SourceRoleId,
		const FString& TargetRoleId,
		const FString& MissingRoleId)
	{
		FUnrealClaudeRoleTransitionDecision Decision;
...
bool FUnrealClaudeRoleContract::AllowsNextRole(const FString& NextRoleId) const
{
	return ContainsStringCaseSensitive(AllowedNextRoles, NextRoleId);
}

bool FUnrealClaudeRoleContract::AllowsOutputOrEvidence(const FString& OutputId) const
{
	return ContainsStringCaseSensitive(AllowedOutputs, OutputId)
		|| ContainsStringCaseSensitive(RequiredOutputEvidence, OutputId);
}

bool FUnrealClaudeRoleContract::ProhibitsAction(const FString& ActionId) const
{
	return ContainsStringCaseSensitive(ProhibitedActions, ActionId);
}

TSharedPtr<FJsonObject> FUnrealClaudeRoleContract::ToJsonObject() const
{
...
TSharedPtr<FJsonObject> FUnrealClaudeRoleActionDecision::ToJsonObject() const
{
	TSharedPtr<FJsonObject> Object = MakeShared<FJsonObject>();
...
TSharedPtr<FJsonObject> FUnrealClaudeRoleTransitionDecision::ToJsonObject() const
{
	TSharedPtr<FJsonObject> Object = MakeShared<FJsonObject>();
	Object->SetBoolField(TEXT("allowed"), bAllowed);
...
namespace UnrealClaudeRoleRegistry
{
	...
	FUnrealClaudeRoleContract GetArchitectRoleContract()
	{
	...
	FUnrealClaudeRoleContract GetWorkerRoleContract()
	{
		FUnrealClaudeRoleContract Contract;
	...
	FUnrealClaudeRoleContract GetVerifierRoleContract()
	{
		FUnrealClaudeRoleContract Contract;
		...
		FUnrealClaudeRoleContract& OutContract)
	{
		...
		OutContract = FUnrealClaudeRoleContract();
		return false;
	}

	TArray<FUnrealClaudeRoleContract> GetAllRoleContracts()
	{
		return {
	...
	FUnrealClaudeRoleActionDecision EvaluateRoleAction(
		const FString& RoleId,
		const FString& ActionId)
	{
		FUnrealClaudeRoleContract Contract;
	...
	FUnrealClaudeRoleTransitionDecision EvaluateRoleTransition(
		const FString& SourceRoleId,
		const FString& TargetRoleId)
	{
		FUnrealClaudeRoleContract SourceContract;
		FUnrealClaudeRoleContract TargetContract;
