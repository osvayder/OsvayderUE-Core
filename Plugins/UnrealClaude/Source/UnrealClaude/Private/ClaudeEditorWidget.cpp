...
#include "UnrealClaudeTaskRecoveryDetector.h"   // 632 Task Recovery & Rehydration
#include "UnrealClaudeTaskRecoveryDialog.h"     // 632 Task Recovery & Rehydration
#include "UnrealClaudeTaskRecoverySummarizer.h" // 632 Task Recovery & Rehydration
#include "UnrealClaudeDeferredTasksBrowser.h"   // 634 Deferred Tasks Browser & Resume
...
namespace
{
	TWeakPtr<SClaudeEditorWidget> GLiveClaudeEditorWidget;
	...
	const FTextBlockStyle& GetSelectableCodeStyle()
	{
		...
		if (!bInit)
		{
			...
			Style.SetColorAndOpacity(FSlateColor(FLinearColor(0.8f, 0.85f, 0.75f)));
			bInit = true;
		}
		return Style;
	}
	...
	{
		return FClaudeCodeSubsystem::Get().GetConfiguredBackendStatus();
	}
	...
	FString BuildRestartSurvivalTaskRecoveryDiagnostic(const FString& InterruptionReason)
	{
		...
		const bool bHasRestartState =
			FUnrealClaudeRestartSurvivalManager::LoadState(RestartState, LoadError);
		...
		if (bHasRestartState)
		{
			const FString Phase = FString(UnrealClaudeRestartSurvivalPhaseToString(RestartState.Phase));
	...
	FString GetSessionMetadataDisplayName(const FAgentSessionMetadata& Metadata)
	{
		...
		return Metadata.Backend == EUnrealClaudeProviderBackend::CodexCli
			? TEXT("Codex CLI")
			: TEXT("Claude CLI");
	}

	FString GetDictationStateLabel(const EUnrealClaudeVoiceDictationState State)
	{
		switch (State)
		{
	...
	SessionObject->SetStringField(TEXT("backend"), UnrealClaudeProviderBackendToString(Metadata.Backend));
	...
	{
		const FUnrealClaudeBuildSyncStatus BuildSyncStatus = FUnrealClaudeModule::GetBuildSyncStatus();
	...
	FOpenEditorBuildLockCloseSafetyInputs BuildOpenEditorBuildLockCloseSafetyInputsFromEditorState()
	{
		...
		FUnrealClaudeRestartSurvivalState ExistingRestartState;
		if (FUnrealClaudeRestartSurvivalManager::DescribeCurrentState(ExistingRestartState)
			&& !FUnrealClaudeRestartSurvivalManager::CanReplaceExistingStateForFreshStart(ExistingRestartState))
		{
			Inputs.bRestartSurvivalStateActive = true;
			Inputs.RestartSurvivalPhase = FString(UnrealClaudeRestartSurvivalPhaseToString(ExistingRestartState.Phase));
		}
	...
	bool ResetRelayArtifactsForFreshRun(FString& OutError)
	{
		...
		FString Error;
		if (!FUnrealClaudeRelayAgentManager::DeleteCancelRequest(Error) && !Error.IsEmpty())
		{
			Errors.Add(Error);
		}

		Error.Reset();
		if (!FUnrealClaudeRelayAgentManager::DeleteRelayResult(Error) && !Error.IsEmpty())
		{
			Errors.Add(Error);
		}

		Error.Reset();
		if (!FUnrealClaudeRelayAgentManager::DeleteProgressLog(Error) && !Error.IsEmpty())
		{
			Errors.Add(Error);
		}

		Error.Reset();
		if (!FUnrealClaudeRelayAgentManager::DeleteHandoffContext(Error) && !Error.IsEmpty())
		{
			Errors.Add(Error);
		}
	...
	FUnrealClaudeRelayHandoffContext BuildClosedEditorRelayHandoffContext(
		const FUnrealClaudeRestartSurvivalPreparedRestoreRequest& PreparedRequest,
		const FString& ReattachToken,
		const FString& ReattachNotice)
	{
	FUnrealClaudeRelayHandoffContext Context;
	...
	Context.RelevantArtifactPaths = {
		FUnrealClaudeRestartSurvivalManager::GetStatePath(),
		FUnrealClaudeRestartSurvivalManager::GetPreparedRestoreRequestPath(),
		FUnrealClaudeRestartSurvivalManager::GetClosedEditorResultPath(),
		...
		Context.Settings = FUnrealClaudeRelayAgentManager::BuildCurrentCodexSettingsSnapshot();
		return Context;
	}
	...
	{
		OutOriginTask = FUnrealClaudeRestartSurvivalOriginTaskContext();
		OutOriginTask.OriginatingRunId = ActiveRunId;
		OutOriginTask.OriginatingUserPrompt = CurrentPrompt;
		OutOriginTask.OriginatingPromptHash = SClaudeEditorWidget::ComputeHeadlessAcceptancePromptHash(CurrentPrompt);
		...
		OutOriginTask.bOriginatingHasVisualReference =
			SClaudeEditorWidget::DoesPromptRequireVisualProof(CurrentPrompt, CurrentImagePaths);
		...
		FUnrealClaudeRestartSurvivalState RestartState;
		FString LoadError;
		if (!FUnrealClaudeRestartSurvivalManager::LoadState(RestartState, LoadError)
	...
	{
		FString Arguments = FString::Printf(TEXT("-UnrealClaudeRestartReattachToken=\"%s\""), *ReattachToken);
	...
	{
		OutToken.Reset();
		return FParse::Value(FCommandLine::Get(), TEXT("UnrealClaudeRestartReattachToken="), OutToken);
	}

	FString MakeUtcNowText();
	FString NormalizeSingleLineText(FString Value);
	bool IsVisualQaManifestRequiredForPlan(const FUnrealClaudeActivePlan& Plan);
	void ApplyVisualQaManifestEvidenceFromResponse(FUnrealClaudeActivePlan& Plan, const FString& Response);
	FString BuildPlanId();
	FUnrealClaudeActivePlan BuildFeatureWorkflowPlan(const FString& OriginalUserTask, const FAgentFeatureWorkflowState& Workflow);
	FUnrealClaudeActivePlan BuildDefaultActivePlan(const FString& OriginalUserTask);
	int32 FindMechanicIndex(const FUnrealClaudeActivePlan& Plan, const FString& MechanicId);
	...
	void MarkLeadingMechanicCompleted(FUnrealClaudeActivePlan& Plan, const FString& MechanicId);
	...
	bool IsWorkspaceFileBuildRestartOrigin(const FUnrealClaudeRestartSurvivalState& RestartState);
	...
	void RecordCompileProof(
		FUnrealClaudeCompileProofState& State,
	...
	void RecordPostCompileVerification(
		...
		const FString& ToolName,
		const FString& Outcome);
	bool IsFeatureWorkflowPlan(const FUnrealClaudeActivePlan& Plan);
	FString GetCurrentFeatureWorkflowPhaseId(const FUnrealClaudeActivePlan& Plan);
	void UpdateFeatureWorkflowForToolBoundary(FUnrealClaudeActivePlan& Plan, const FAgentCanonExecution& ActiveCanon, const FClaudeStreamEvent& Event, bool bStarting);
	...
	bool TryLoadPendingHeadlessAcceptanceReceiptContext(FUnrealClaudeHeadlessAcceptanceReceiptContext& OutContext);
	bool TryCompleteRecoveredHeadlessAcceptanceReceipt(
		...
		FString& OutError);
	bool TryLoadLatestHeadlessCloseoutDecision(FUnrealClaudeActivePlanCloseoutDecision& OutDecision, FString& OutRawJson);
	...
	FString BuildRunIsolatedCloseoutArchiveRunTag(const FUnrealClaudeRunCloseoutContext& Context);
	...
	bool TryRestoreExplicitContinuityPlanForPrompt(
		...
		FString& OutError);
	void AppendContinuationPromptToPlanIfRelevant(FUnrealClaudeActivePlan& Plan, const FString& Prompt);
	FString DescribeSettingsParity(const FUnrealClaudeRelaySettingsSnapshot& Expected, const FUnrealClaudeRelaySettingsSnapshot& Current);
	FString DescribeTaskLaneStatusInternal(const FUnrealClaudeTaskLaneState& LaneState)
	{
		const bool bBlocked =
	...
	void MirrorPlanLaneStateToFeatureWorkflow(FUnrealClaudeActivePlan& Plan)
	{
	...
	void RefreshPlanLaneStateIdentity(FUnrealClaudeActivePlan& Plan)
	{
...
void SChatMessage::Construct(const FArguments& InArgs)
{
	...
	FLinearColor BackgroundColor = bIsUser
		? FLinearColor(0.13f, 0.13f, 0.18f, 1.0f)
		: FLinearColor(0.08f, 0.08f, 0.08f, 1.0f);

	FLinearColor AccentColor = bIsUser
		? FLinearColor(0.3f, 0.5f, 0.9f, 1.0f)
		: FLinearColor(0.6f, 0.4f, 0.2f, 1.0f);

	FLinearColor TextColor = FLinearColor::White;
	FLinearColor RoleLabelColor = bIsUser
		? FLinearColor(0.4f, 0.6f, 1.0f)
		: FLinearColor(0.9f, 0.6f, 0.3f);
...
{
	VoiceDictationService = MakeShared<FUnrealClaudeVoiceDictationService>();
	VoiceDictationService->SetStatusChangedHandler([this](const FUnrealClaudeVoiceDictationStatus& Status)
	{
	...
	LastRenderedBackend = FClaudeCodeSubsystem::Get().GetConfiguredBackend();
...
void SClaudeEditorWidget::RegisterLiveWidget(const TSharedRef<SClaudeEditorWidget>& Widget)
{
	GLiveClaudeEditorWidget = Widget;
}

TSharedPtr<SClaudeEditorWidget> SClaudeEditorWidget::GetLiveWidget()
{
	return GLiveClaudeEditorWidget.Pin();
}
...
SClaudeEditorWidget::~SClaudeEditorWidget()
{
	...
	if (GLiveClaudeEditorWidget.Pin().Get() == this)
	{
		GLiveClaudeEditorWidget.Reset();
	}
}

void SClaudeEditorWidget::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	const EUnrealClaudeProviderBackend CurrentBackend = FClaudeCodeSubsystem::Get().GetConfiguredBackend();
	if (CurrentBackend != LastRenderedBackend)
	{
		const EUnrealClaudeProviderBackend PreviousBackend = LastRenderedBackend;
...
void SClaudeEditorWidget::TryRunTaskRecoveryStartupCheck()
{
	...
	FUnrealClaudeActivePlan Plan;
	FString LoadError;
	if (!FUnrealClaudeRelayAgentManager::LoadActivePlan(Plan, LoadError))
	{
		return;
	}
	...
	const FString EvidencePath = FUnrealClaudeRelayAgentManager::GetActivePlanPath();
	...
	const FUnrealClaudeTaskRecoveryDialog::FDialogResult DialogResult =
		FUnrealClaudeTaskRecoveryDialog::Show(Summary, DiagnosticNote);

	// Record choice into the plan and route side-effect per choice.
	switch (DialogResult.Choice)
	{
	case EUnrealClaudeTaskRecoveryChoice::Continue:
	{
		Plan.UserRecoveryChoice = TEXT("continue");
		FString SaveError;
		FUnrealClaudeRelayAgentManager::SaveActivePlan(Plan, SaveError);

		const FString ContextText =
			FUnrealClaudeTaskRecoveryContextBuilder::BuildContextBlockText(Summary);
		FClaudeCodeSubsystem::Get().SetPendingTaskRecoveryContext(ContextText);
	...
	case EUnrealClaudeTaskRecoveryChoice::StartFresh:
	{
	...
	case EUnrealClaudeTaskRecoveryChoice::CloseAsIrrelevant:
	{
	...
	case EUnrealClaudeTaskRecoveryChoice::None:
	default:
...
TSharedRef<SWidget> SClaudeEditorWidget::BuildToolbar()
{
	...
	return SNew(SClaudeToolbar)
		.TitleText(this, &SClaudeEditorWidget::GetToolbarTitleText)
		.ProviderSummaryText(this, &SClaudeEditorWidget::GetProviderSummaryText)
		.ProviderSummaryToolTip(this, &SClaudeEditorWidget::GetProviderSummaryToolTip)
		...
		{
			const FAgentSavedSessionIndex SavedSessions = FClaudeCodeSubsystem::Get().DescribeSavedSessions();
			const FAgentSavedSessionIndex ExpertSessions =
				FClaudeCodeSubsystem::Get().DescribeSavedSessionsForProfile(EAgentExecutionRunProfile::ExplicitExpertOptIn);
			return SavedSessions.HasAnySavedSession() || ExpertSessions.CurrentProviderSession.bHasSession;
		})
		.RestoreToolTip(this, &SClaudeEditorWidget::GetRestoreSessionToolTip)
		.bRestartSurvivalEnabled_Lambda([this]() { return CanStartRestartSurvival(); })
		.RestartSurvivalToolTip(this, &SClaudeEditorWidget::GetRestartSurvivalToolTip)
...
TSharedRef<SWidget> SClaudeEditorWidget::BuildCodexAuthDiagnosticsPanel()
{
	return SNew(SBorder)
		.BorderImage(FAppStyle::GetBrush("ToolPanel.GroupBorder"))
		.Padding(FMargin(8.0f, 4.0f))
		.Visibility(this, &SClaudeEditorWidget::GetCodexAuthDiagnosticsVisibility)
		[
			SNew(SHorizontalBox)
			+ SHorizontalBox::Slot()
			.FillWidth(1.0f)
			...
			[
				SNew(STextBlock)
				.Text(this, &SClaudeEditorWidget::GetCodexAuthDiagnosticsText)
				.ToolTipText(this, &SClaudeEditorWidget::GetCodexAuthDiagnosticsToolTip)
				.TextStyle(FAppStyle::Get(), "SmallText")
			...
			[
				...
				[
					...
					[
						...
						.IsEnabled(this, &SClaudeEditorWidget::CanProbeCodexBackendAuth)
...
int32 SClaudeEditorWidget::GetDeferredTasksCount() const
{
	return FUnrealClaudeDeferredTasksEnumerator::CountDeferredPlans();
}

void SClaudeEditorWidget::OpenDeferredTasksBrowser()
{
	FUnrealClaudeDeferredTasksBrowser::Show(
		FUnrealClaudeDeferredTasksBrowser::FOnPlanResumed::CreateLambda([this](const FString& ShortTitle)
		{
			bResumeExistingActivePlanOnNextSend = true;
			bUsePostReattachResumePolicyOnNextSend = false;
	...
	return SNew(SBorder)
		...
		[
			...
			[
				...
				.ToolTipText(this, &SClaudeEditorWidget::GetConnectionIndicatorToolTip)
				[
					SNew(SBorder)
					.BorderImage(FAppStyle::GetBrush("WhiteBrush"))
					.BorderBackgroundColor(this, &SClaudeEditorWidget::GetConnectionIndicatorColor)
					.Padding(0.0f)
				]
			]
			...
			[
				...
				.Visibility(this, &SClaudeEditorWidget::GetRetryLastTransportVisibility)
				.IsEnabled(this, &SClaudeEditorWidget::CanRetryLastTransportRequest)
				.ToolTipText(this, &SClaudeEditorWidget::GetRetryLastTransportToolTip)
				.OnClicked(this, &SClaudeEditorWidget::HandleRetryLastTransportRequest)
			]
...
void SClaudeEditorWidget::TrackVisibleChatMessage(const FString& Message)
{
	VisibleChatMessages.Add(Message);
}

void SClaudeEditorWidget::ReplaceStreamingResponseWithFinalText(const FString& Message)
{
...
static bool TryGetCurrentTypedClosedEditorResultProof(const bool bRequireTerminalSuccess, FString& OutDetail)
{
	...
	FString StateError;
	if (!FUnrealClaudeRestartSurvivalManager::LoadState(State, StateError) || State.TaskId.IsEmpty())
	{
	...
	CandidatePaths.Add(FUnrealClaudeRestartSurvivalManager::GetClosedEditorResultPath());
	CandidatePaths.Add(FUnrealClaudeRelayAgentManager::GetRelayResultPath());
...
{
	bool bVisualProofRequired = SClaudeEditorWidget::DoesPromptRequireVisualProof(Prompt, ImagePaths);
	...
	if (FUnrealClaudeRestartSurvivalManager::LoadState(RestartState, RestartStateError)
		&& RestartState.Phase == EUnrealClaudeRestartSurvivalPhase::Reattached
...
bool SClaudeEditorWidget::PrepareActivePlanForCurrentSend(const FString& Prompt, FString& OutError)
{
	FUnrealClaudeActivePlan Plan;
	if (bResumeExistingActivePlanOnNextSend)
	{
		...
		FUnrealClaudeRelayAgentManager::EActivePlanFallbackSource FallbackSource
			= FUnrealClaudeRelayAgentManager::EActivePlanFallbackSource::ActivePlanJson;
		FString FallbackError;
		const bool bFallbackLoaded = FUnrealClaudeRelayAgentManager::LoadActivePlanWithFallback(
		...
		if (FallbackSource == FUnrealClaudeRelayAgentManager::EActivePlanFallbackSource::ActivePlanJson)
		{
			if (bUsePostReattachResumePolicyOnNextSend)
			{
				if (IsFeatureWorkflowPlan(Plan))
		...
		else if (FallbackSource == FUnrealClaudeRelayAgentManager::EActivePlanFallbackSource::RestartSurvivalStateJsonFallback)
		{
		...
		{
			FUnrealClaudeActivePlan ContinuityPlan;
	...
	if (!FUnrealClaudeRelayAgentManager::SaveActivePlan(Plan, OutError))
	{
		return false;
	}
...
void SClaudeEditorWidget::UpdateActivePlanForToolBoundary(const FClaudeStreamEvent& Event, const bool bStarting)
{
	FUnrealClaudeActivePlan Plan;
	FString Error;
	if (!FUnrealClaudeRelayAgentManager::LoadActivePlan(Plan, Error))
	{
		return;
	}
	...
	if (bFeatureWorkflowPlan)
	{
		...
		if (!FUnrealClaudeAgentTraceLog::Get().TryGetActiveCanonExecutionForBackend(Event.Backend, ActiveCanon))
		{
	...
	else if (bStarting)
	{
		...
		if (FUnrealClaudeAgentTraceLog::Get().TryGetActiveCanonExecutionForBackend(Event.Backend, ActiveCanon)
			...
			&& UnrealClaudeCanonRouting::IsMutatingToolUse(Event.ToolName, Event.ToolInput)
			...
			&& ToolInputTargetsCompiledModuleSourceMutation(Event.ToolName, Event.ToolInput))
		{
			...
			Plan.CompileProof.MutationToolFamily = !ActiveCanon.PrimaryMutationToolFamily.IsEmpty()
				...
				: UnrealClaudeCanonRouting::DetermineToolFamily(ActiveCanon, Event.ToolName, Event.ToolInput);
	...
	for (FUnrealClaudePlanToolCallEntry& Entry : Plan.ToolCalls)
	{
	...
	RefreshPlanLaneStateIdentity(Plan);
	FUnrealClaudeRelayAgentManager::SaveActivePlan(Plan, Error);
}

void SClaudeEditorWidget::MarkActivePlanAwaitingClosedEditorRelay(
	const FUnrealClaudeClosedEditorBuildBlocker& Blocker,
...
void SClaudeEditorWidget::MarkActivePlanBlockedOnClosedEditorRelay(
	const FUnrealClaudeClosedEditorBuildBlocker& Blocker,
	const FString& EscalationReason,
	const FString& TechnicalDetail)
{
...
void SClaudeEditorWidget::MarkActivePlanForPostReattachVerification(const FUnrealClaudeRestartSurvivalState& RestartState)
{
	FUnrealClaudeActivePlan Plan;
	FString Error;
	if (!FUnrealClaudeRelayAgentManager::LoadActivePlan(Plan, Error))
...
FUnrealClaudeActivePlanCloseoutDecision SClaudeEditorWidget::FinalizeActivePlan(const FString& Response, const bool bSuccess, const bool bArchiveRelayArtifacts)
{
	FUnrealClaudeActivePlanCloseoutDecision FallbackDecision;
	...
	const FString ActiveRunId = FUnrealClaudeAgentTraceLog::Get().GetActiveRunIdForBackend(ActiveRequestBackend);
	FUnrealClaudeRunCloseoutContext CloseoutContext = BuildCurrentRunCloseoutContext(Plan, ActiveRunId);
	...
	const FUnrealClaudeUserFacingStatus UserFacingStatus =
		UnrealClaudeUserFacingStatus::BuildStatus(Plan, Decision, CloseoutContext);
	...
	if (!FUnrealClaudeRelayAgentManager::SaveActivePlan(Plan, Error))
	{
		return Decision;
	}
	...
	if (bArchiveRelayArtifacts)
	{
		...
		FUnrealClaudeRelayAgentManager::ArchiveTerminalArtifacts(Plan, ArchivedPaths, ArchiveError);
	...
	const FString LatestCloseoutDecisionPath =
		FPaths::Combine(FUnrealClaudeRelayAgentManager::GetRelayRootDir(), TEXT("closeout_decision.json"));
	if (!SaveCloseoutJsonObjectToFile(CloseoutDecisionJson, LatestCloseoutDecisionPath, CloseoutArtifactError))
	{
		UE_LOG(LogUnrealClaude, Warning, TEXT("Failed to persist closeout_decision artifact: %s"), *CloseoutArtifactError);
	}
...
void SClaudeEditorWidget::TrySurfacePersistedActivePlanNotice()
{
	FUnrealClaudeActivePlan Plan;
	FString Error;
	if (!FUnrealClaudeRelayAgentManager::LoadActivePlan(Plan, Error))
...
bool SClaudeEditorWidget::DispatchPreparedPrompt(
	const FString& Prompt,
	const TArray<FString>& ImagePaths,
	...
	FString& OutError)
{
	...
	ActiveRequestBackend = FClaudeCodeSubsystem::Get().GetConfiguredBackend();
	const EUnrealClaudeProviderBackend RequestBackend = ActiveRequestBackend;
	...
	TWeakPtr<SClaudeEditorWidget> SelfWeak = SharedThis(this).ToWeakPtr();
	...
	{
		TSharedPtr<SClaudeEditorWidget> Pinned = SelfWeak.Pin();
		if (!Pinned.IsValid())
		{
			return;
		}
	...
	Options.OnStreamEvent.BindLambda([SelfWeak, RequestGeneration, RequestBackend](const FClaudeStreamEvent& Event)
	{
		TSharedPtr<SClaudeEditorWidget> Pinned = SelfWeak.Pin();
		if (!Pinned.IsValid())
		{
...
void SClaudeEditorWidget::ToggleDictation()
{
	...
	const EUnrealClaudeVoiceDictationState DictationState = VoiceDictationService->GetStatus().State;
	if (DictationState == EUnrealClaudeVoiceDictationState::Recording)
	{
...
bool SClaudeEditorWidget::DebugStartDictation()
{
	if (!VoiceDictationService.IsValid() || bIsWaitingForResponse)
	{
		return false;
	...
	if (DictationState == EUnrealClaudeVoiceDictationState::PreparingRuntime ||
		DictationState == EUnrealClaudeVoiceDictationState::Transcribing)
	{
		return false;
	}
...
bool SClaudeEditorWidget::DebugStopDictation()
{
	if (!VoiceDictationService.IsValid())
	{
		return false;
	...
	if (VoiceDictationService->GetStatus().State != EUnrealClaudeVoiceDictationState::Recording)
	{
		return false;
	}
...
bool SClaudeEditorWidget::DebugToggleDictation()
{
	if (!VoiceDictationService.IsValid() || bIsWaitingForResponse)
	{
		return false;
	...
	const EUnrealClaudeVoiceDictationState BeforeState = VoiceDictationService->GetStatus().State;
	ToggleDictation();
	return VoiceDictationService->GetStatus().State != BeforeState
		|| BeforeState == EUnrealClaudeVoiceDictationState::Recording;
}

void SClaudeEditorWidget::DebugClearInputText()
{
	CurrentInputText.Empty();
	if (InputArea.IsValid())
	{
...
void SClaudeEditorWidget::DebugSetInputText(const FString& NewText)
{
...
bool SClaudeEditorWidget::DebugSendCurrentInput()
{
...
bool SClaudeEditorWidget::DebugIsWaitingForResponse() const
{
	return bIsWaitingForResponse;
}

void SClaudeEditorWidget::DebugForceBackendSnapshotRefresh(const bool bForce)
{
	RefreshBackendSnapshotIfChanged(bForce);
}

void SClaudeEditorWidget::DebugProcessBackendStateNow()
{
	const EUnrealClaudeProviderBackend CurrentBackend = FClaudeCodeSubsystem::Get().GetConfiguredBackend();
	if (CurrentBackend != LastRenderedBackend)
	{
...
int32 SClaudeEditorWidget::DebugGetVisibleMessageCount() const
{
	return VisibleChatMessages.Num();
}

bool SClaudeEditorWidget::DebugVisibleChatContains(const FString& Needle) const
{
	if (Needle.IsEmpty())
	{
		return false;
...
SClaudeEditorWidget::EBackendRuntimeConnectionState SClaudeEditorWidget::DebugGetLastRuntimeConnectionState() const
{
	return LastRuntimeConnectionState;
}

FString SClaudeEditorWidget::DebugGetLastRuntimeConnectionDetail() const
{
	return LastRuntimeConnectionDetail;
}

FString SClaudeEditorWidget::DebugGetLastResponseText() const
{
	return LastResponse;
}

bool SClaudeEditorWidget::ValidateHeadlessAcceptanceRequest(
	const FUnrealClaudeHeadlessAcceptanceRequest& Request,
	FString& OutError)
{
	OutError.Reset();
	FUnrealClaudeHeadlessAcceptanceRequest NormalizedRequest = Request;
...
FString SClaudeEditorWidget::NormalizeHeadlessAcceptancePathArgument(const FString& PathArgument)
{
...
void SClaudeEditorWidget::NormalizeHeadlessAcceptanceRequestPaths(FUnrealClaudeHeadlessAcceptanceRequest& Request)
{
...
void SClaudeEditorWidget::NormalizeMandatoryContextFlagsForNormalAssistantRun(
...
FString SClaudeEditorWidget::ComputeHeadlessAcceptancePromptHash(const FString& Prompt)
{
...
FString SClaudeEditorWidget::ResolveHeadlessAcceptanceReceiptPath(
	const FUnrealClaudeHeadlessAcceptanceRequest& Request)
{
	FString OutputDir = Request.OutputDir.IsEmpty()
		? FPaths::Combine(FUnrealClaudeRelayAgentManager::GetRelayRootDir(), TEXT("HeadlessAcceptance"))
		: Request.OutputDir;
...
namespace
{
	bool IsCurrentPrefixLogPathForHeadlessAcceptance(
		const FUnrealClaudeHeadlessAcceptanceReceiptContext& Context,
	...
	bool HasCloseoutIdentityEvidenceForHeadlessAcceptance(
		const FUnrealClaudeHeadlessAcceptanceReceiptContext& Context)
	{
		const FUnrealClaudeActivePlanCloseoutDecision& Decision = Context.CloseoutDecision;
	...
	FString GetHeadlessAcceptanceDispatchPath(const FUnrealClaudeHeadlessAcceptanceRequest& Request)
	{
	...
	FString GetHeadlessAcceptanceMode(const FUnrealClaudeHeadlessAcceptanceRequest& Request)
	{
		return Request.bVisibleManualEmulator
			? TEXT("visible_manual")
	...
	FString GetHeadlessAcceptanceVisibilityMode(const FUnrealClaudeHeadlessAcceptanceRequest& Request)
	{
...
TSharedPtr<FJsonObject> SClaudeEditorWidget::BuildHeadlessAcceptanceReceiptJson(
	const FUnrealClaudeHeadlessAcceptanceReceiptContext& Context)
{
	const FUnrealClaudeActivePlanCloseoutDecision& Decision = Context.CloseoutDecision;
	...
	{
		EffectiveStatus = (bDecisionPassed && bFreshPrefixEvidencePresent)
			? TEXT("closeout_passed")
			: TEXT("closeout_failed");
	}
...
bool SClaudeEditorWidget::SaveHeadlessAcceptanceReceiptArtifact(
	const FUnrealClaudeHeadlessAcceptanceReceiptContext& Context,
	FString& OutError)
{
...
bool SClaudeEditorWidget::StartHeadlessNewSessionAcceptanceBridge(
	const FUnrealClaudeHeadlessAcceptanceRequest& Request,
	FString& OutError)
{
	...
	{
		FUnrealClaudeHeadlessAcceptanceReceiptContext RejectedContext;
	...
	if (!FFileHelper::LoadFileToString(Prompt, *NormalizedRequest.PromptFile))
	{
		...
		FUnrealClaudeHeadlessAcceptanceReceiptContext FailedContext;
	...
	FUnrealClaudeActivePlanCloseoutDecision ExistingDecision;
	...
	{
		FUnrealClaudeHeadlessAcceptanceReceiptContext RecoveryContext;
		...
		RecoveryContext.VisibleSessionPath = FClaudeCodeSubsystem::Get().GetSessionFilePath();
		RecoveryContext.TracePath = FUnrealClaudeAgentTraceLog::Get().GetTraceLogPath();
	...
	PendingHeadlessAcceptanceBridge.Backend = FClaudeCodeSubsystem::Get().GetConfiguredBackend();
...
void SClaudeEditorWidget::TickHeadlessAcceptanceBridgeTimeout(const double InCurrentTime)
{
...
void SClaudeEditorWidget::CompletePendingHeadlessAcceptanceBridge(
...
void SClaudeEditorWidget::WriteHeadlessAcceptanceReceipt(
	const FString& Status,
	const bool bAssistantSuccess,
	const FUnrealClaudeActivePlanCloseoutDecision* CloseoutDecision,
	const FString& FailureReason,
	const FString& ResponsePreview)
{
	...
	Context.ActiveRunIdFallback =
		FUnrealClaudeAgentTraceLog::Get().GetActiveRunIdForBackend(PendingHeadlessAcceptanceBridge.Backend);
	Context.LatestCloseoutPath =
		FPaths::Combine(FUnrealClaudeRelayAgentManager::GetRelayRootDir(), TEXT("closeout_decision.json"));
	Context.VisibleSessionPath = FClaudeCodeSubsystem::Get().GetSessionFilePath();
	Context.TracePath = FUnrealClaudeAgentTraceLog::Get().GetTraceLogPath();
	...
	{
		UE_LOG(LogUnrealClaude, Warning, TEXT("Failed to persist headless acceptance receipt: %s"), *SaveError);
		return;
	}
...
void SClaudeEditorWidget::OnClaudeResponse(
	const FString& Response,
	bool bSuccess,
	const uint64 RequestGeneration,
	const EUnrealClaudeProviderBackend RequestBackend)
{
	...
	FUnrealClaudeActivePlanCloseoutDecision HeadlessCloseoutDecision;
	...
	if (bSuccess)
	{
		...
		if (!FUnrealClaudeRestartSurvivalManager::HasPendingResume(RequestBackend))
		{
			const FUnrealClaudeActivePlanCloseoutDecision CloseoutDecision =
	...
	else
	{
			...
			&& RestartState.bPostReattachCompletionDispatched)
		{
			...
			FString SaveError;
			FUnrealClaudeRestartSurvivalManager::SaveState(RestartState, SaveError);
		}

		if (RequestBackend == EUnrealClaudeProviderBackend::CodexCli
			&& FCodexCliRunner::IsPersistentTransportFailureMessage(Response))
		{
			...
			const bool bHasTransportSummary =
				FUnrealClaudeAgentTraceLog::Get().TryGetLatestTransportFailureSummary(RequestBackend, TransportSummary);
	...
	if (bHeadlessShouldComplete)
	{
		...
		else if (bHeadlessHasCloseoutDecision)
		{
			...
			{
				UE_LOG(LogUnrealClaude, Display, TEXT("Recovered headless acceptance receipt after restart-survival reattach."));
			...
			{
				UE_LOG(LogUnrealClaude, Warning, TEXT("Failed to recover headless acceptance receipt after restart-survival: %s"), *RecoveredReceiptError);
			}
		}
	}
...
{
	const EUnrealClaudeProviderBackend ActiveBackend = FClaudeCodeSubsystem::Get().GetConfiguredBackend();
	FClaudeCodeSubsystem::Get().CancelCurrentRequest();
	FUnrealClaudeRestartSurvivalState RestartState;
	if (FUnrealClaudeRestartSurvivalManager::DescribeCurrentState(RestartState)
		&& RestartState.Backend == ActiveBackend
		&& FUnrealClaudeRestartSurvivalManager::IsDetachedOwnerPhaseActive(RestartState.Phase))
	{
		FString CancelError;
		FUnrealClaudeRelayAgentManager::WriteCancelRequest(TEXT("widget_cancelled_request"), CancelError);
	}
	FUnrealClaudeAgentTraceLog::Get().MarkCancellation(
		ActiveBackend,
...
void SClaudeEditorWidget::HandleDictationStatusChanged(const FUnrealClaudeVoiceDictationStatus& Status)
{
	...
	if ((Status.State == EUnrealClaudeVoiceDictationState::Failed
		|| Status.State == EUnrealClaudeVoiceDictationState::Unavailable)
		&& InputArea.IsValid())
	{
...
void SClaudeEditorWidget::HandleDictationTranscriptReady(const FString& Transcript)
{
...
FText SClaudeEditorWidget::GetDictationStatusText() const
{
...
void SClaudeEditorWidget::ClearTransportRetryState()
{
...
void SClaudeEditorWidget::NewSession()
{
	...
	FUnrealClaudeRestartSurvivalManager::DeleteState(RestartSurvivalDeleteError);
	FString ActivePlanDeleteError;
	FUnrealClaudeRelayAgentManager::DeleteActivePlan(ActivePlanDeleteError);
	FString RelayDeleteError;
	FUnrealClaudeRelayAgentManager::DeleteHandoffContext(RelayDeleteError);
	FUnrealClaudeRelayAgentManager::DeleteProgressLog(RelayDeleteError);
	FUnrealClaudeRelayAgentManager::DeleteRelayResult(RelayDeleteError);
	FUnrealClaudeRelayAgentManager::DeleteCancelRequest(RelayDeleteError);
...
bool SClaudeEditorWidget::StartRestartSurvivalForCurrentTask(const FClaudeRestartSurvivalStartOptions& Options, FString& OutError)
{
	OutError.Reset();
	...
	FUnrealClaudeRestartSurvivalState ExistingState;
	if (FUnrealClaudeRestartSurvivalManager::DescribeCurrentState(ExistingState))
	{
		if (!FUnrealClaudeRestartSurvivalManager::CanReplaceExistingStateForFreshStart(ExistingState))
		{
			OutError = FString::Printf(
				TEXT("Restart survival is already active for this project in phase %s."),
				UnrealClaudeRestartSurvivalPhaseToString(ExistingState.Phase));
			return false;
		}

		FString DeleteError;
		if (!FUnrealClaudeRestartSurvivalManager::DeleteState(DeleteError))
		{
	...
	FUnrealClaudeRestartSurvivalRestoreIntent EffectiveRestoreIntent = Options.RestoreIntent;
	FUnrealClaudeRestartSurvivalFileWriteIntent EffectiveFileWriteIntent = Options.FileWriteIntent;
	FUnrealClaudeRestartSurvivalProofState EffectiveProof = Options.Proof;
	FString PreparedRestoreRequestId;
	FString PreparedRestoreRequestCreatedAtUtc;
	FString EffectivePostReattachCompletionText;
	FUnrealClaudeRestartSurvivalOriginTaskContext EffectiveOriginTask;
	...
	if (bHasPreparedRestoreRequestCandidate)
	{
		...
		if (!FUnrealClaudeRestartSurvivalManager::LoadPreparedRestoreRequest(PreparedRequest, RequestError))
		{
	...
	if (bHasPreparedRestoreRequest)
	{
		...
		EffectivePostReattachCompletionText =
			FUnrealClaudeRestartSurvivalManager::ResolvePreparedRequestPostReattachCompletionText(PreparedRequest);
		if (bUseRelayAgentForStart)
		{
			EffectivePostReattachCompletionText.Reset();
		}

		FUnrealClaudeRestartSurvivalPreparedStartOverride PreparedStartOverride;
		if (FUnrealClaudeRestartSurvivalManager::TryConsumePreparedRequestStartOverride(PreparedRequest, PreparedStartOverride))
		{
	...
	const FAgentProviderExecutionControlManifest Manifest =
		FClaudeCodeSubsystem::Get().GetConfiguredBackendExecutionControlManifest();
	FUnrealClaudeRestartSurvivalSupportBundle SupportBundle;
	FString SupportBundleError;
	if (!FUnrealClaudeRestartSurvivalManager::TryResolveSupportBundle(SupportBundle, SupportBundleError))
	{
	...
	if (bUseRelayAgentForStart)
	{
		...
		const FUnrealClaudeRelayHandoffContext RelayContext = BuildClosedEditorRelayHandoffContext(
			PreparedRequest,
			ReattachToken,
			ReattachNotice);
		FString RelaySaveError;
		if (!FUnrealClaudeRelayAgentManager::SaveHandoffContext(RelayContext, RelaySaveError))
		{
	...
	FString SaveError;
	if (!FUnrealClaudeRestartSurvivalManager::SaveState(RestartState, SaveError))
	{
		OutError = SaveError;
		return false;
	}
	...
	if (!SupervisorHandle.IsValid())
	{
		const FString LaunchError = FString::Printf(
			...
			*PowerShellExe);
		FString IgnoreError;
		FUnrealClaudeRestartSurvivalManager::MarkFailed(LaunchError, IgnoreError);
	...
	if (bConsumedPreparedRestoreRequest)
	{
		...
		FUnrealClaudeRestartSurvivalManager::DeletePreparedRestoreRequest(DeleteRequestError);
	}
...
void SClaudeEditorWidget::StartRestartSurvival()
{
...
bool SClaudeEditorWidget::RestoreCurrentBackendSession(const bool bAutomatic, const FString& LeadingNotice)
{
	FClaudeCodeSubsystem& Subsystem = FClaudeCodeSubsystem::Get();
	const FAgentBackendStatus Status = GetConfiguredBackendStatus();
	const EUnrealClaudeProviderBackend Backend = Status.Backend;
	...
	FUnrealClaudeAgentTraceLog::Get().AppendEvent(TEXT("restore_session"), Backend, RestoreAttemptPayload);
	...
	if (ExistingHistory.Num() > 0)
	{
		...
		FUnrealClaudeAgentTraceLog::Get().AppendEvent(TEXT("session_restored"), Backend, RestoredPayload);
		return true;
	}
	...
	if (bAutomatic && (!Settings || !Settings->ShouldAutoRestoreSessionOnOpen()))
	{
		...
		FUnrealClaudeAgentTraceLog::Get().AppendEvent(TEXT("session_restored"), Backend, RestoreOutcomePayload);
		return false;
	}
...
bool SClaudeEditorWidget::TryRestoreSessionOnOpen()
{
		...
		&& bHasReattachToken)
	{
		...
		bRestartSurvivalReattached = FUnrealClaudeRestartSurvivalManager::TryMarkReattached(
			GetConfiguredBackendStatus().Backend,
			ReattachToken,
			LeadingNotice,
			ReattachError);
	}
	else if (FUnrealClaudeRestartSurvivalManager::DescribeCurrentState(RestartState)
		&& RestartState.Backend == GetConfiguredBackendStatus().Backend
		&& !bHasReattachToken
	...
	{
		FString ReattachError;
		bRestartSurvivalReattached = FUnrealClaudeRestartSurvivalManager::TryMarkReattachedFromManualReopen(
			GetConfiguredBackendStatus().Backend,
			LeadingNotice,
			ReattachError);
	}
	...
	if (!bHasReattachToken
		&& FUnrealClaudeRestartSurvivalManager::DescribeCurrentState(RestartState)
		&& RestartState.Backend == GetConfiguredBackendStatus().Backend
		&& FUnrealClaudeRestartSurvivalManager::IsDetachedOwnerPhaseActive(RestartState.Phase)
		&& RestartState.EditorProcessId != FPlatformProcess::GetCurrentProcessId())
	{
	...
	FUnrealClaudeRestartSurvivalState PendingRestartState;
	FString PendingLoadError;
	if (FUnrealClaudeRestartSurvivalManager::LoadState(PendingRestartState, PendingLoadError)
		&& PendingRestartState.Backend == GetConfiguredBackendStatus().Backend
		&& PendingRestartState.Phase == EUnrealClaudeRestartSurvivalPhase::Reattached
...
void SClaudeEditorWidget::TryDispatchPendingRestartSurvivalCompletion()
{
	if (bIsWaitingForResponse)
	{
		return;
	...
	const FString ResumedRunId = FUnrealClaudeAgentTraceLog::Get().GetActiveRunIdForBackend(GetConfiguredBackendStatus().Backend);
...
void SClaudeEditorWidget::PollRestartSurvivalDetachedOwnerState()
{
	...
	// no new timer.
	{
		FUnrealClaudeRestartSurvivalState BannerRestartState;
		FString BannerLoadError;
		if (FUnrealClaudeRestartSurvivalManager::LoadState(BannerRestartState, BannerLoadError))
		{
		...
		&& FUnrealClaudeRestartSurvivalManager::IsDetachedOwnerPhaseActive(RestartState.Phase))
	{
		...
		{
			RestartState.Phase = EUnrealClaudeRestartSurvivalPhase::FailedTerminal;
		...
		&& RestartState.bDetachedOwnerManualReopenDetected)
	{
		FString Notice;
		FString ReattachError;
		if (FUnrealClaudeRestartSurvivalManager::TryMarkReattachedFromManualReopen(
			GetConfiguredBackendStatus().Backend,
			Notice,
			ReattachError))
		{
	...
	if (RestartState.Phase == EUnrealClaudeRestartSurvivalPhase::Reattached
...
bool SClaudeEditorWidget::ShouldShowAgent2BackgroundBanner(
	const FUnrealClaudeRestartSurvivalState& RestartState,
	const bool bDetachedOwnerProcessRunning)
{
	...
	if (RestartState.Phase == EUnrealClaudeRestartSurvivalPhase::Reattached
		|| RestartState.Phase == EUnrealClaudeRestartSurvivalPhase::FailedTerminal
		|| RestartState.Phase == EUnrealClaudeRestartSurvivalPhase::AttachedInEditor)
	{
		return false;
	}
...
FString SClaudeEditorWidget::BuildAgent2BannerCancelRequestPayload()
{
	// Same schema as the monitor-window Cancel button (schema_version=1,
	// requested_at_utc, reason) — see UnrealClaude-RestartSurvivalMonitor.ps1
...
FString SClaudeEditorWidget::DescribeTransportRetryBlockReason(const FString& RetryBlockReason)
{
...
FString SClaudeEditorWidget::DescribeTaskLaneStatus(const FUnrealClaudeTaskLaneState& LaneState)
{
	return DescribeTaskLaneStatusInternal(LaneState);
}

FString SClaudeEditorWidget::BuildTransportRetryStatusLabel(const bool bRetrySafe)
{
	return bRetrySafe
...
FString SClaudeEditorWidget::BuildTransportRetryNotice(
...
FString SClaudeEditorWidget::BuildTaskRecoveryAutoResumePrompt()
{
...
FString SClaudeEditorWidget::BuildTaskRecoveryAutoResumeVisibleMarker()
{
	return TEXT("[System-generated task recovery auto-resume]");
}

FOpenEditorBuildLockCloseSafetyDecision SClaudeEditorWidget::EvaluateOpenEditorBuildLockCloseSafety(
...
FTaskRecoveryAutoDispatchDecision SClaudeEditorWidget::EvaluateTaskRecoveryAutoDispatch(
...
void SClaudeEditorWidget::UpdateAgent2BannerStateFromRestartState(
	const FUnrealClaudeRestartSurvivalState* RestartState,
	const bool bDetachedOwnerProcessRunning)
{
	...
	Agent2BannerState.PhaseLabel = UnrealClaudeRestartSurvivalPhaseToString(RestartState->Phase);
	...
	Agent2BannerState.StateFilePath = FUnrealClaudeRestartSurvivalManager::GetStatePath();
	Agent2BannerState.MonitorScriptPath = RestartState->MonitorScriptPath.IsEmpty()
		? FUnrealClaudeRestartSurvivalManager::GetMonitorScriptPath()
		: RestartState->MonitorScriptPath;

	// The monitor computes CancelPath via Get-RelayPath(StateFilePath, "relay_cancel_request.json")
	// at UnrealClaude-RestartSurvivalMonitor.ps1:402. We mirror that here so the
...
TSharedRef<SWidget> SClaudeEditorWidget::BuildAgent2BackgroundBanner()
{
	return SNew(SBorder)
		.BorderImage(FAppStyle::GetBrush("ToolPanel.GroupBorder"))
		.Padding(FMargin(8.0f, 4.0f))
		.Visibility(this, &SClaudeEditorWidget::GetAgent2BackgroundBannerVisibility)
		[
			SNew(SHorizontalBox)
			+ SHorizontalBox::Slot()
			.FillWidth(1.0f)
			...
			[
				SNew(STextBlock)
				.Text(this, &SClaudeEditorWidget::GetAgent2BackgroundBannerText)
			]
			+ SHorizontalBox::Slot()
			.AutoWidth()
			.VAlign(VAlign_Center)
			...
			[
				...
				.OnClicked(this, &SClaudeEditorWidget::HandleAgent2BannerShowMonitor)
			]
			+ SHorizontalBox::Slot()
			.AutoWidth()
			.VAlign(VAlign_Center)
			...
			[
				...
				.OnClicked(this, &SClaudeEditorWidget::HandleAgent2BannerCancel)
			]
		];
}

EVisibility SClaudeEditorWidget::GetAgent2BackgroundBannerVisibility() const
{
	return Agent2BannerState.bVisible ? EVisibility::Visible : EVisibility::Collapsed;
}

FText SClaudeEditorWidget::GetAgent2BackgroundBannerText() const
{
...
FReply SClaudeEditorWidget::HandleAgent2BannerShowMonitor()
{
	...
	// UnrealClaude-RestartSurvival.ps1:328-340 — same -Sta -NoProfile -Bypass -File
...
FReply SClaudeEditorWidget::HandleAgent2BannerCancel()
{
...
void SClaudeEditorWidget::TryStartPendingRestartSurvivalAfterResponse(const EUnrealClaudeProviderBackend RequestBackend)
{
	if (bIsWaitingForResponse || RequestBackend != EUnrealClaudeProviderBackend::CodexCli)
	{
		return;
	}
	...
	if (!FUnrealClaudeRestartSurvivalManager::TryConsumePreparedRequestAutoStart(
		RequestBackend,
		FPaths::ConvertRelativePathToFull(FPaths::ProjectDir()),
...
void SClaudeEditorWidget::ClearPendingClosedEditorBuildBlockerIntervention()
{
	PendingClosedEditorBuildBlockerIntervention = FPendingClosedEditorBuildBlockerIntervention();
}

FClaudeRestartSurvivalStartOptions SClaudeEditorWidget::BuildArmOptionsForClosedEditorBuildBlocker()
{
	...
	// (UnrealClaude-RestartSurvival.ps1) polls editor_process_id via
	...
	// UnrealClaude.RestartSurvival.ArmEmissionPreservesEditorExitIntent can
...
bool SClaudeEditorWidget::StartRestartSurvivalForClosedEditorBuildBlocker(FString& OutError)
{
...
void SClaudeEditorWidget::TryHandleClosedEditorBuildBlocker(
	const FClaudeStreamEvent& Event,
	const EUnrealClaudeProviderBackend RequestBackend)
{
	...
	if (!FUnrealClaudeRestartSurvivalManager::TryDetectClosedEditorBuildBlocker(
		Event.ToolName,
	...
	const FString ActiveRunId = FUnrealClaudeAgentTraceLog::Get().GetActiveRunIdForBackend(RequestBackend);
	FAgentCanonExecution ActiveCanonExecution;
	FUnrealClaudeAgentTraceLog::Get().TryGetActiveCanonExecutionForBackend(RequestBackend, ActiveCanonExecution);
	FUnrealClaudeRestartSurvivalOriginTaskContext EffectiveOriginTask;
	...
	else if (RequestBackend == EUnrealClaudeProviderBackend::CodexCli
		&& FCodexCliRunner::ShouldUsePersistentConversationTransport())
	{
		...
		if (CodexRunner != nullptr && !ProviderSessionId.IsEmpty())
		{
			...
			if (FUnrealClaudeRestartSurvivalManager::TryPrepareClosedEditorBuildBlockerAutoStart(
				RequestBackend,
				FPaths::ConvertRelativePathToFull(FPaths::ProjectDir()),
				ProviderSessionId,
				Blocker,
				...
				EscalationError))
			{
				...
				if (bUseRelayAgent)
				{
					...
					if (!FUnrealClaudeRestartSurvivalManager::SavePreparedRestoreRequest(
						PreparedRequest,
	...
	if (bRestartSurvivalArmed)
	{
		...
		if (FUnrealClaudeRestartSurvivalManager::LoadPreparedRestoreRequest(PreparedRequest, PreparedRequestError))
		{
	...
	// FClaudeCodeSubsystem::Get().CancelCurrentRequest() tears the Codex
	// persistent transport down. The ordering helper in
	// FUnrealClaudeRestartSurvivalManager guarantees the cancel is skipped if
	...
	{
		FString SpawnError;
		const bool bSequenceExecuted = FUnrealClaudeRestartSurvivalManager::ExecuteSpawnBeforeTransportResetSequence(
...
bool SClaudeEditorWidget::DoesResponseClaimStatusFull(const FString& Response)
{
	if (Response.IsEmpty())
	{
		return false;
...
namespace
{
	...
	TArray<FString> FindVisualQaManifestCandidates(const FUnrealClaudeActivePlan& Plan, const FString& Response)
	{
		TArray<FString> Candidates;
	...
	void ApplyVisualQaManifestEvidenceFromResponse(FUnrealClaudeActivePlan& Plan, const FString& Response)
	{
		...
		if (SClaudeEditorWidget::DoesFinalResponseDeclareFailureOrBlocker(Response))
		{
			Plan.VisualProofStatus = TEXT("blocked");
			Plan.VisualProofBlocker = NormalizeSingleLineText(Response.Left(400));
		}
		else if (SClaudeEditorWidget::DoesFinalResponseProvideVisualProof(Response))
		{
...
bool SClaudeEditorWidget::DoesPromptRequireVisualProof(
	const FString& Prompt,
	const TArray<FString>& ImagePaths)
{
	if (ImagePaths.Num() > 0)
...
bool SClaudeEditorWidget::DoesFinalResponseProvideVisualProof(const FString& Response)
{
	const FString Lower = Response.ToLower();
...
bool SClaudeEditorWidget::DoesFinalResponseDeclareFailureOrBlocker(const FString& Response)
{
	const FString Lower = Response.ToLower();
	return Lower.Contains(TEXT("result status: fail"))
		...
		|| Lower.Contains(TEXT("не выполн"))
		|| Lower.Contains(TEXT("заблок"));
}

FUnrealClaudeActivePlanCloseoutDecision SClaudeEditorWidget::ApplyActivePlanCloseoutSafetyGates(
	const FUnrealClaudeActivePlan& Plan,
	const FString& Response,
...
namespace
{
	void AddCloseoutFactId(FUnrealClaudeCloseoutFactSnapshot& Facts, const FString& FactId)
	{
	...
	{
		return Workflow.RecipeId.Equals(UnrealClaudeRecipeRegistry::InteractionAccessRecipeId(), ESearchCase::CaseSensitive);
	}
	...
	TArray<FString> FindMissingCloseoutFactRecipeObligations(
		const FUnrealClaudeRecipeEvidenceContract& Contract,
		const FUnrealClaudeCloseoutFactSnapshot& Facts)
	{
		...
		const FUnrealClaudeCloseoutFactSnapshot& Facts)
	{
		...
		if (!UnrealClaudeRecipeRegistry::TryGetRecipeEvidenceContract(Workflow.RecipeId, Contract)
			|| !Contract.RecipeId.Equals(UnrealClaudeRecipeRegistry::InteractionAccessRecipeId(), ESearchCase::CaseSensitive))
		{
			return false;
		}
	...
	bool IsManagedUnrealClaudeStatePathMentioned(const FString& Text)
	{
		FString Normalized = Text.ToLower();
		Normalized.ReplaceInline(TEXT("\\"), TEXT("/"));
		return Normalized.Contains(TEXT("saved/unrealclaude/active_plan.json"))
			|| Normalized.Contains(TEXT("saved/unrealclaude/planarchives"))
			|| Normalized.Contains(TEXT("saved/unrealclaude/closeoutdecisions"))
			|| Normalized.Contains(TEXT("saved/unrealclaude/closeout_decision.json"))
			|| Normalized.Contains(TEXT("saved/unrealclaude/visible_session"))
			|| Normalized.Contains(TEXT("saved/unrealclaude/agent_trace"));
	}
		...
		const FString& RunId = FString())
	{
		...
		UnrealClaude::CommandClassification::FUnrealClaudeExecutionTruthInputs TruthInputs;
		...
		const UnrealClaude::CommandClassification::FUnrealClaudeExecutionTruthDecision TruthDecision =
			UnrealClaude::CommandClassification::ClassifyExecutionTruth(TruthInputs);

		if (UnrealClaude::CommandClassification::IsCommandExecutionLikeToolName(ToolName))
		{
		...
		const bool bTruthReadOnlyCommand =
			UnrealClaude::CommandClassification::IsCommandExecutionLikeToolName(ToolName)
			&& TruthDecision.Category == UnrealClaude::CommandClassification::EUnrealClaudeExecutionTruthCategory::ReadOnlyInspection;
		const bool bCommandMutation =
			!bTruthReadOnlyCommand
			&& (
			TruthDecision.Category == UnrealClaude::CommandClassification::EUnrealClaudeExecutionTruthCategory::ManagedStateWrite
			|| TruthDecision.Category == UnrealClaude::CommandClassification::EUnrealClaudeExecutionTruthCategory::ApprovedProjectMutation
		...
		if (TruthDecision.Category != UnrealClaude::CommandClassification::EUnrealClaudeExecutionTruthCategory::ManagedStateWrite
			&& !IsManagedUnrealClaudeStatePathMentioned(EvidenceText))
		{
			return;
		}
	...
	FString BuildManagedStateManualWriteDetail(const TArray<FString>& FactIds)
	{
		if (FactIds.Num() == 0)
		{
			return TEXT("agent command_execution mutated Saved/UnrealClaude managed state");
		}
		...
		FString Detail = FString::Printf(
			TEXT("agent command_execution mutated Saved/UnrealClaude managed state: %s"),
	...
	TArray<FString> ExtractPlanDeclaredExternalRuntimeProofPrefixes(const FUnrealClaudeActivePlan& Plan)
	{
	...
	bool IsExternalRuntimeProofRecoveryCandidatePlan(const FUnrealClaudeActivePlan& Plan)
	{
		...
		FUnrealClaudeRunCloseoutContext& Context)
	{
		...
		for (const FString& LogFileName : LogFileNames)
		{
			...
			FUnrealClaudeCloseoutFactSnapshot CandidateFacts;
			const FString FactPrefix =
	...
	FString GetHeadlessAcceptancePendingReceiptPath()
	{
		return FPaths::ConvertRelativePathToFull(FPaths::Combine(
			FUnrealClaudeRelayAgentManager::GetRelayRootDir(),
	...
	bool SaveHeadlessAcceptancePendingReceiptObject(const TSharedPtr<FJsonObject>& JsonObject)
	{
		...
		if (!bSaved)
		{
			UE_LOG(LogUnrealClaude, Warning, TEXT("Failed to persist pending headless acceptance receipt: %s"), *SaveError);
		}
		return bSaved;
	}
	...
	bool TryLoadPendingHeadlessAcceptanceReceiptContext(FUnrealClaudeHeadlessAcceptanceReceiptContext& OutContext)
	{
		OutContext = FUnrealClaudeHeadlessAcceptanceReceiptContext{};
		...
		FString& OutError)
	{
		...
		Context.CloseoutDecision = CloseoutDecision;
		Context.ActiveRunIdFallback =
			FUnrealClaudeAgentTraceLog::Get().GetActiveRunIdForBackend(FClaudeCodeSubsystem::Get().GetConfiguredBackend());
		Context.LatestCloseoutPath =
			FPaths::Combine(FUnrealClaudeRelayAgentManager::GetRelayRootDir(), TEXT("closeout_decision.json"));
		Context.ArchivePath = CloseoutDecision.SourceArchivePath;
		Context.VisibleSessionPath = FClaudeCodeSubsystem::Get().GetSessionFilePath();
	...
	bool TryLoadLatestHeadlessCloseoutDecision(
		FUnrealClaudeActivePlanCloseoutDecision& OutDecision,
		FString& OutRawJson)
	{
		OutDecision = FUnrealClaudeActivePlanCloseoutDecision{};
		OutRawJson.Reset();
	...
	bool IsContinuityPlanTerminal(const FUnrealClaudeActivePlan& Plan)
	{
		...
		FString& OutSourcePath)
	{
		FUnrealClaudeActivePlan ActivePlan;
		FString LoadError;
		if (FUnrealClaudeRelayAgentManager::LoadActivePlan(ActivePlan, LoadError)
			&& DoesPlanMatchExplicitContinuityRefs(ActivePlan, Refs))
		{
			OutPlan = ActivePlan;
			OutSourcePath = FUnrealClaudeRelayAgentManager::GetActivePlanPath();
			return true;
		}
		...
		{
			FUnrealClaudeActivePlan Candidate;
			FString CandidateError;
			if (!FUnrealClaudeRelayAgentManager::LoadPlanFromPath(ArchivePath, Candidate, CandidateError))
			{
				continue;
			}
	...
	bool ArchiveActivePlanContinuitySnapshotForNewSession(FString& OutArchivePath, FString& OutError)
	{
		...
		return FUnrealClaudeRelayAgentManager::SavePlanToPath(OutArchivePath, ActivePlan, OutError);
	}
...
void FUnrealClaudeCloseoutFactSnapshot::AddFactId(const FString& FactId)
{
...
void FUnrealClaudeCloseoutFactSnapshot::MergeFrom(const FUnrealClaudeCloseoutFactSnapshot& Other)
{
...
bool FUnrealClaudeCloseoutFactSnapshot::HasRuntimePrerequisiteFacts() const
{
...
bool FUnrealClaudeCloseoutFactSnapshot::HasCompleteInteractionAccessObservation() const
{
...
bool FUnrealClaudeCloseoutFactSnapshot::HasBoundedPrisonAccessAutomationProof() const
{
...
bool FUnrealClaudeCloseoutFactSnapshot::HasBoundedInteractionAccessRuntimeProof() const
{
	const FUnrealClaudeRecipeEvidenceContract Contract =
		UnrealClaudeRecipeRegistry::GetInteractionAccessRecipeEvidenceContract();
	return FindMissingCloseoutFactRecipeObligations(Contract, *this).Num() == 0;
}

TSharedPtr<FJsonObject> FUnrealClaudeCloseoutFactSnapshot::ToJsonObject() const
{
...
bool SClaudeEditorWidget::PromptHasExplicitContinuityRefsForTests(const FString& Prompt)
{
	return ExtractExplicitContinuityPromptRefs(Prompt).HasAny();
}

FString SClaudeEditorWidget::DescribeExplicitContinuityRefsForTests(const FString& Prompt)
{
	return DescribeExplicitContinuityRefs(ExtractExplicitContinuityPromptRefs(Prompt));
}
#endif

TSharedPtr<FJsonObject> FUnrealClaudeRunCloseoutContext::ToJsonObject() const
{
	TSharedPtr<FJsonObject> Object = MakeShared<FJsonObject>();
...
FUnrealClaudeCloseoutFactSnapshot SClaudeEditorWidget::ExtractCloseoutFactsFromPlan(
	const FUnrealClaudeActivePlan& Plan)
{
	FUnrealClaudeCloseoutFactSnapshot Facts;
...
FUnrealClaudeCloseoutFactSnapshot SClaudeEditorWidget::ExtractAgentTraceFactsForCloseout(
...
FUnrealClaudeCloseoutFactSnapshot SClaudeEditorWidget::ExtractCurrentAgentTraceFactsForCloseout()
{
	...
	const TArray<TSharedPtr<FJsonObject>> Records =
		FUnrealClaudeAgentTraceLog::Get().QueryEvents(Options, ResolvedRunId, TotalLoaded);
	return ExtractAgentTraceFactsForCloseout(Records);
}

FUnrealClaudeRunCloseoutContext SClaudeEditorWidget::BuildRunCloseoutContext(
	const FUnrealClaudeActivePlan& Plan,
	const TArray<TSharedPtr<FJsonObject>>& TraceRecords,
	const FString& RunId,
	const FString& ProjectRoot)
{
	...
	{
		Context.RoleId = UnrealClaudeRoleRegistry::WorkerRoleId();
	}
	FUnrealClaudeRecipeEvidenceContract ContextContract;
	if (UnrealClaudeRecipeRegistry::TryGetRecipeEvidenceContract(Context.RecipeId, ContextContract))
	{
...
FUnrealClaudeRunCloseoutContext SClaudeEditorWidget::BuildCurrentRunCloseoutContext(
	const FUnrealClaudeActivePlan& Plan,
...
FAgentFeatureWorkflowState SClaudeEditorWidget::ReduceFeatureWorkflowForCloseout(
	const FAgentFeatureWorkflowState& Workflow,
	const FUnrealClaudeCloseoutFactSnapshot& Facts)
{
...
FUnrealClaudeActivePlanCloseoutDecision SClaudeEditorWidget::EvaluateActivePlanCloseout(
	const FUnrealClaudeActivePlan& Plan,
	const bool bResponseSuccess)
{
...
FUnrealClaudeActivePlanCloseoutDecision SClaudeEditorWidget::EvaluateActivePlanCloseoutFromCurrentArtifacts(
	const FUnrealClaudeActivePlan& Plan,
	const bool bResponseSuccess)
{
...
FUnrealClaudeActivePlanCloseoutDecision SClaudeEditorWidget::EvaluateActivePlanCloseoutWithFacts(
	const FUnrealClaudeActivePlan& Plan,
	const bool bResponseSuccess,
	const FUnrealClaudeCloseoutFactSnapshot& Facts)
{
...
FUnrealClaudeActivePlanCloseoutDecision SClaudeEditorWidget::EvaluateActivePlanCloseoutWithContext(
	const FUnrealClaudeActivePlan& Plan,
	const bool bResponseSuccess,
	const FUnrealClaudeRunCloseoutContext& Context)
{
	FUnrealClaudeActivePlanCloseoutDecision Decision;
	const FUnrealClaudeCloseoutFactSnapshot& Facts = Context.Facts;
	...
	{
		Decision.SourceRoleId = UnrealClaudeRoleRegistry::WorkerRoleId();
	}
	Decision.ConsumedFactIds = Facts.ConsumedFactIds;

	FUnrealClaudeActivePlan EffectivePlan = Plan;
	...
	const bool bRecipeContractResolved = bHasRecipeId
		&& UnrealClaudeRecipeRegistry::TryGetRecipeEvidenceContract(
	...
	const bool bRoleContractResolved = bHasRoleId
		&& UnrealClaudeRoleRegistry::TryGetRoleContract(EffectiveRoleId, RoleContract);
	...
	if (bRoleContractResolved
		&& RoleContract.RoleId.Equals(UnrealClaudeRoleRegistry::ArchitectRoleId(), ESearchCase::CaseSensitive)
	...
	const FUnrealClaudeCompileProofState& CompileProof = EffectivePlan.CompileProof;
	...
	const bool bInteractionAccessRecipe =
		...
		&& RecipeContract.RecipeId.Equals(UnrealClaudeRecipeRegistry::InteractionAccessRecipeId(), ESearchCase::CaseSensitive);
...
TSharedPtr<FJsonObject> SClaudeEditorWidget::BuildCloseoutDecisionJson(
	const FUnrealClaudeActivePlan& Plan,
	...
	const FUnrealClaudeRunCloseoutContext& Context)
{
	...
	{
		SourceRoleId = UnrealClaudeRoleRegistry::WorkerRoleId();
	}
	FUnrealClaudeRecipeEvidenceContract RecipeContract;
	...
	const bool bRecipeContractResolved =
		UnrealClaudeRecipeRegistry::TryGetRecipeEvidenceContract(SourceRecipeId, RecipeContract);
	FUnrealClaudeRoleContract RoleContract;
	const bool bRoleContractResolved =
		UnrealClaudeRoleRegistry::TryGetRoleContract(SourceRoleId, RoleContract);
	...
	const FUnrealClaudeUserFacingStatus UserFacingStatus =
		UnrealClaudeUserFacingStatus::BuildStatus(Plan, Decision, Context);
...
FString SClaudeEditorWidget::BuildActivePlanCloseoutWarningText(
	const FUnrealClaudeActivePlanCloseoutDecision& Decision,
	const FString& FinalResponse)
{
	...
	else if (Decision.GateReasonCode == TEXT("managed_state_manual_write_detected"))
	{
		WarningBody = TEXT("Active-plan closeout blocked (managed-state integrity):\n"
			"- A mutating agent shell command touched UnrealClaude managed state in this run.\n"
	...
	FUnrealClaudeActivePlan EmptyPlan;
	const FUnrealClaudeUserFacingStatus UserFacingStatus =
		UnrealClaudeUserFacingStatus::BuildStatus(EmptyPlan, Decision);
...
FString SClaudeEditorWidget::RewriteResponseForTruthfulActivePlanCloseout(
	const FUnrealClaudeActivePlan& Plan,
	const FUnrealClaudeActivePlanCloseoutDecision& Decision,
	const FString& FinalResponse)
{
	...
	const FUnrealClaudeUserFacingStatus UserFacingStatus =
		UnrealClaudeUserFacingStatus::BuildStatus(Plan, Decision);
...
void SClaudeEditorWidget::CheckSelfVerificationMismatchAndWarn(
	const FString& FinalResponse,
	const EUnrealClaudeProviderBackend RequestBackend)
{
	...
	FAgentCanonExecution ActiveCanon;
	if (!FUnrealClaudeAgentTraceLog::Get().TryGetActiveCanonExecutionForBackend(RequestBackend, ActiveCanon))
	{
	...
	WarningText += TEXT("- See Docs/UnrealClaude/observed_failures.md + 631 policy in system prompt.");
	UE_LOG(LogUnrealClaude, Warning, TEXT("%s"), *WarningText);
	AddMessage(WarningText, /*bIsUser=*/false);
}

void SClaudeEditorWidget::TryHandleCompileIntentPolicy(const FClaudeStreamEvent& Event)
{
	...
	// script can surface the decision post-hoc.
	{
		...
		const EUnrealClaudeProviderBackend Backend = FClaudeCodeSubsystem::Get().GetConfiguredBackend();
		FUnrealClaudeAgentTraceLog::Get().AppendEvent(
			TEXT("policy_routing_decision"),
			Backend,
			RoutingJson,
			FUnrealClaudeAgentTraceLog::Get().GetActiveRunIdForBackend(Backend));
	}
	...
	// policy decision observable to the agent (vs log-only emission).
	{
		...
		{
			FClaudeCodeSubsystem::Get().SetPendingPolicyRoutingAdvisory(AdvisoryText);
		}
	}
}

void SClaudeEditorWidget::ResetVisibleChatState()
{
	if (ChatMessagesBox.IsValid())
	{
...
void SClaudeEditorWidget::HandleConfiguredBackendChanged(const EUnrealClaudeProviderBackend PreviousBackend)
{
	TSharedPtr<FJsonObject> SwitchPayload = MakeShared<FJsonObject>();
	SwitchPayload->SetStringField(TEXT("previous_backend"), UnrealClaudeProviderBackendToString(PreviousBackend));
	SwitchPayload->SetStringField(TEXT("current_backend"), UnrealClaudeProviderBackendToString(FClaudeCodeSubsystem::Get().GetConfiguredBackend()));
	FUnrealClaudeAgentTraceLog::Get().AppendEvent(
		TEXT("backend_switch"),
		FClaudeCodeSubsystem::Get().GetConfiguredBackend(),
		SwitchPayload,
		FUnrealClaudeAgentTraceLog::Get().GetActiveRunIdForBackend(PreviousBackend));

	InvalidateActiveRequestCallbacks();
	ResetVisibleChatState();

	const FString PreviousDisplayName = PreviousBackend == EUnrealClaudeProviderBackend::CodexCli
		? TEXT("Codex CLI")
		: TEXT("Claude CLI");
...
void SClaudeEditorWidget::RenderConversationHistory(
...
void SClaudeEditorWidget::AddInitialStatusMessage()
{
	const FAgentBackendStatus Status = GetConfiguredBackendStatus();
	...
	FUnrealClaudeAgentTraceLog::Get().AppendEvent(TEXT("backend_snapshot"), Status.Backend, TracePayload);
}

void SClaudeEditorWidget::RefreshBackendSnapshotIfChanged(bool bForce)
{
	if (!ChatMessagesBox.IsValid())
	{
		return;
	...
	FUnrealClaudeAgentTraceLog::Get().AppendEvent(TEXT("backend_snapshot"), GetConfiguredBackendStatus().Backend, TracePayload);
}

FString SClaudeEditorWidget::BuildBackendDebugSnapshot() const
{
	const FAgentBackendStatus Status = GetConfiguredBackendStatus();
	const UUnrealClaudeSettings* Settings = UUnrealClaudeSettings::Get();
	const FString DisplayName = GetBackendDisplayNameFromStatus(Status);
	...
	if (Settings)
	{
		const bool bIsCodexBackend = Status.Backend == EUnrealClaudeProviderBackend::CodexCli;
	...
	const FString RestartSummary = FUnrealClaudeRestartSurvivalManager::BuildWidgetDebugSummary();
...
FString SClaudeEditorWidget::BuildBackendSnapshotSignature() const
{
	const FAgentBackendStatus Status = GetConfiguredBackendStatus();
	const UUnrealClaudeSettings* Settings = UUnrealClaudeSettings::Get();
	...
	if (FUnrealClaudeRestartSurvivalManager::DescribeCurrentState(RestartState))
	{
		Signature += FString::Printf(TEXT("|restart_phase=%s"), UnrealClaudeRestartSurvivalPhaseToString(RestartState.Phase));
...
void SClaudeEditorWidget::LaunchBrowserVerifyLogin()
{
...
void SClaudeEditorWidget::OpenCodexAuthFolder()
{
	FString StatusMessage;
...
void SClaudeEditorWidget::LaunchCodexRelogin()
{
	FString StatusMessage;
...
void SClaudeEditorWidget::BackupAndClearCodexAuth()
{
	...
	const FString Prompt = FString::Printf(
		TEXT("Back up and clear known Codex auth artifacts from effective CODEX_HOME?\n\nCODEX_HOME: %s\nArtifact: %s\n\nOnly auth.json and credentials.json are touched. Backups are written under Project/Saved/UnrealClaude/CodexAuthDiagnostics first."),
...
void SClaudeEditorWidget::ProbeCodexBackendAuth()
{
...
bool SClaudeEditorWidget::CanSendPrompt() const
{
	return CanSendPromptForStatus(GetConfiguredBackendStatus());
}

bool SClaudeEditorWidget::CanStartRestartSurvival() const
{
	const FAgentBackendStatus Status = GetConfiguredBackendStatus();
	return !bIsWaitingForResponse
...
bool SClaudeEditorWidget::ShouldShowBrowserVerifyButton() const
{
	return GetConfiguredBackendCapabilities().bSupportsBrowserVerifyLogin;
}

EVisibility SClaudeEditorWidget::GetCodexAuthDiagnosticsVisibility() const
{
	return GetConfiguredBackendStatus().Backend == EUnrealClaudeProviderBackend::CodexCli
		? EVisibility::Visible
		: EVisibility::Collapsed;
}

FText SClaudeEditorWidget::GetCodexAuthDiagnosticsText() const
{
...
FText SClaudeEditorWidget::GetCodexAuthDiagnosticsToolTip() const
{
	FCodexCliRunner::FCodexAuthDiagnostics Diagnostics = FCodexCliRunner::GetAuthDiagnostics();
	if (!LastCodexAuthProbeStatus.IsEmpty())
	{
...
bool SClaudeEditorWidget::CanProbeCodexBackendAuth() const
{
	return !bCodexAuthProbeRunning
		&& GetConfiguredBackendStatus().Backend == EUnrealClaudeProviderBackend::CodexCli;
}

FText SClaudeEditorWidget::GetRestartSurvivalToolTip() const
{
	if (bIsWaitingForResponse)
	{
...
bool SClaudeEditorWidget::CanLaunchBrowserVerifyButton() const
{
	if (!ShouldShowBrowserVerifyButton())
	{
		return false;
...
FText SClaudeEditorWidget::GetBrowserVerifyToolTip() const
{
	if (!ShouldShowBrowserVerifyButton())
	{
...
FText SClaudeEditorWidget::GetToolbarTitleText() const
{
	return FText::FromString(FString::Printf(TEXT("Osvayder UE (%s)"), *GetConfiguredBackendDisplayName()));
}

FText SClaudeEditorWidget::GetProviderSummaryText() const
{
	return FText::FromString(BuildProviderCapabilitySummary(GetConfiguredBackendCapabilities()));
}

FText SClaudeEditorWidget::GetProviderSummaryToolTip() const
{
	const FAgentBackendStatus Status = GetConfiguredBackendStatus();
	const FString DisplayName = GetBackendDisplayNameFromStatus(Status);
	return FText::FromString(FString::Printf(
		TEXT("%s provider-specific controls: %s."),
		*DisplayName,
		*BuildProviderCapabilitySummary(Status.Capabilities)));
}

FText SClaudeEditorWidget::GetRestoreSessionToolTip() const
{
	const FAgentBackendStatus Status = GetConfiguredBackendStatus();
	const FString DisplayName = GetBackendDisplayNameFromStatus(Status);
	const FAgentSavedSessionIndex SavedSessions = FClaudeCodeSubsystem::Get().DescribeSavedSessions();
...
SClaudeEditorWidget::EBackendRuntimeConnectionState SClaudeEditorWidget::GetRuntimeConnectionState() const
{
	if (bIsWaitingForResponse)
	{
...
FSlateColor SClaudeEditorWidget::GetConnectionIndicatorColor() const
{
...
FText SClaudeEditorWidget::GetConnectionIndicatorToolTip() const
{
	const EBackendRuntimeConnectionState RuntimeState = GetRuntimeConnectionState();
	FString Prefix;
...
bool SClaudeEditorWidget::CanRetryLastTransportRequest() const
{
...
EVisibility SClaudeEditorWidget::GetRetryLastTransportVisibility() const
{
...
FText SClaudeEditorWidget::GetRetryLastTransportToolTip() const
{
...
FReply SClaudeEditorWidget::HandleRetryLastTransportRequest()
{
	if (!CanRetryLastTransportRequest())
	{
		return FReply::Handled();
	...
	FUnrealClaudeAgentTraceLog::Get().MarkTransportRetryAttempt(
		FClaudeCodeSubsystem::Get().GetConfiguredBackend(),
...
void SClaudeEditorWidget::OnClaudeProgress(
...
void SClaudeEditorWidget::OnClaudeStreamEvent(
	const FClaudeStreamEvent& Event,
	const uint64 RequestGeneration,
	const EUnrealClaudeProviderBackend RequestBackend)
{
...
void SClaudeEditorWidget::InvalidateActiveRequestCallbacks()
{
...
bool SClaudeEditorWidget::IsActiveRequestCallback(
	const uint64 RequestGeneration,
	const EUnrealClaudeProviderBackend RequestBackend,
	const TCHAR* CallbackKind) const
{
	if (RequestGeneration != ActiveRequestGeneration)
	{
		UE_LOG(
			...
			UnrealClaudeProviderBackendToString(RequestBackend),
			UnrealClaudeProviderBackendToString(FClaudeCodeSubsystem::Get().GetConfiguredBackend()));
		return false;
	}

	if (RequestBackend != FClaudeCodeSubsystem::Get().GetConfiguredBackend())
	{
		UE_LOG(
			LogUnrealClaude,
			Log,
...
void SClaudeEditorWidget::HandleToolResultEvent(
	const FClaudeStreamEvent& Event,
	const EUnrealClaudeProviderBackend RequestBackend)
{
	// Look up tool name
	...
	{
		(*StatusLabelPtr)->SetText(FText::FromString(FString::Printf(TEXT("вњ“ %s completed"), *ToolName)));
...
void SClaudeEditorWidget::UpdateToolGroupSummary()
{
	...
	if (ToolGroupCount == 1)
	{
		...
		if (ToolGroupDoneCount >= 1)
		{
			ToolGroupSummaryText->SetText(FText::FromString(
				FString::Printf(TEXT("вњ“ %s completed"), *DisplayName)));
	...
	else
	{
		...
		{
			ToolGroupSummaryText->SetText(FText::FromString(
				FString::Printf(TEXT("вњ“ %d tools completed"), ToolGroupCount)));
			ToolGroupSummaryText->SetColorAndOpacity(
				FSlateColor(FLinearColor(0.3f, 0.75f, 0.3f)));
		}
		else
...
namespace
{
	...
	FUnrealClaudeActivePlan BuildDefaultActivePlan(const FString& OriginalUserTask)
	{
		...
		const FAgentCanonExecution SeedExecution = UnrealClaudeCanonRouting::BuildInitialCanonExecution(
			OriginalUserTask,
	...
	FUnrealClaudeActivePlan BuildFeatureWorkflowPlan(const FString& OriginalUserTask, const FAgentFeatureWorkflowState& Workflow)
	{
		...
		{
			FUnrealClaudePlanMechanicEntry& Mechanic = Plan.Mechanics.AddDefaulted_GetRef();
		...
		const bool bMarkCompleted)
	{
		...
		FUnrealClaudePlanMechanicEntry& Mechanic = Plan.Mechanics[Index];
		...
		const FString& ToolName)
	{
		...
		FUnrealClaudePlanToolCallEntry Entry;
	...
	bool IsReadOnlyProjectInspectionTool(const FClaudeStreamEvent& Event)
	{
		...
		return ToolNameMatches(Event.ToolName, TEXT("read"))
			...
			|| ((ToolNameMatches(Event.ToolName, TEXT("command_execution"))
				...
				&& !UnrealClaudeCanonRouting::IsMutatingToolUse(Event.ToolName, ToolEnvelope));
	}
	...
	bool HasInteractionAccessContractEvidence(const FClaudeStreamEvent& Event)
	{
	...
	bool IsBoundedInteractionAccessAutomationWrapper(const FClaudeStreamEvent& Event)
	{
	...
	bool HasInteractionAccessAutomationLogEvidence(const FClaudeStreamEvent& Event)
	{
		if (Event.bIsError || Event.ToolResultContent.TrimStartAndEnd().IsEmpty())
		{
			return false;
	...
	FString AppendReferencedAutomationLogEvidence(const FClaudeStreamEvent& Event)
	{
	...
	bool IsInteractionAccessOptionalContractDiscoveryFailure(const FClaudeStreamEvent& Event)
	{
		...
		const bool bAllowResultless)
	{
		...
		const bool bAllowedPreflightPath =
			ContainsAnyWorkflowEvidenceToken(
				Evidence,
				{
					TEXT("saved/unrealclaude/active_plan.json"),
					TEXT("saved/unrealclaude/closeout_decision.json"),
					TEXT("saved/unrealclaude/planarchives"),
					TEXT("saved/unrealclaude"),
					TEXT("docs/unrealclaude"),
					TEXT("source/alternative/prisonaccess"),
					TEXT("saved/logs"),
	...
	bool IsInteractionAccessInputAuthoringReadOnlyContextProbe(const FClaudeStreamEvent& Event)
	{
		return IsInteractionAccessProjectLocalReadOnlyContextProbe(Event, false);
	}

	bool IsInteractionAccessRuntimeActorReadOnlyContextProbe(const FClaudeStreamEvent& Event)
	{
	...
	bool IsVerificationOnlyCompileGateReadOnlyEvidence(const FClaudeStreamEvent& Event)
	{
		if (Event.bIsError)
		{
			return false;
	...
	void EmitFeatureWorkflowTraceEvent(
		...
		const FString& ToolCallId)
	{
		const FString RunId = FUnrealClaudeAgentTraceLog::Get().GetActiveRunIdForBackend(Backend);
		if (RunId.IsEmpty())
		{
			return;
		}
		...
		FUnrealClaudeAgentTraceLog::Get().AppendEvent(EventType, Backend, Payload, RunId);
	}
		...
		const bool bStarting)
	{
		...
		FClaudeStreamEvent EvidenceEvent = Event;
		...
		const bool bMutatingToolUse = UnrealClaudeCanonRouting::IsMutatingToolUse(Event.ToolName, Event.ToolInput);
...
void SClaudeEditorWidget::ApplyFeatureWorkflowToolBoundaryForTest(
	FUnrealClaudeActivePlan& Plan,
	const FAgentCanonExecution& ActiveCanon,
	const FClaudeStreamEvent& Event,
	bool bStarting)
...
FString SClaudeEditorWidget::GenerateMCPCompactStatusSummary() const
{
...
FString SClaudeEditorWidget::GenerateMCPStatusMessage() const
{
	...
	if (!MCPServer.IsValid() || !MCPServer->IsRunning())
	{
		...
		StatusMessage += TEXT("  вЂў Check Output Log for MCP errors\n");
		StatusMessage += TEXT("  вЂў Run: npm install in Resources/mcp-bridge\n");
		StatusMessage += FString::Printf(TEXT("  вЂў Verify port %d is available\n"), UnrealClaudeConstants::MCPServer::DefaultPort);
		StatusMessage += TEXT("в”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђв”Ђ");
		return StatusMessage;
	}
