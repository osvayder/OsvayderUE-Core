		...
		[
			...
			[
				...
				[
					...
					.OnClicked(this, &SClaudeInputArea::HandleToggleDictationClicked)
					.ToolTipText_Lambda([this]() { return GetDictationToolTipText(); })
					.IsEnabled_Lambda([this]()
					{
						return !bIsWaiting.Get() && DictationState.Get() != EUnrealClaudeVoiceDictationState::PreparingRuntime
							&& DictationState.Get() != EUnrealClaudeVoiceDictationState::Transcribing;
					})
				]
...
void SClaudeInputArea::FocusTextEntry()
{
...
FReply SClaudeInputArea::HandleToggleDictationClicked()
{
...
bool SClaudeInputArea::IsDictationInteractionBusy() const
{
	const EUnrealClaudeVoiceDictationState State = DictationState.Get();
	return State == EUnrealClaudeVoiceDictationState::Recording
		|| State == EUnrealClaudeVoiceDictationState::PreparingRuntime
		|| State == EUnrealClaudeVoiceDictationState::Transcribing;
}

bool SClaudeInputArea::IsTextEntryEnabled() const
{
	return !bIsWaiting.Get() && !IsDictationInteractionBusy();
}

FText SClaudeInputArea::GetDictationButtonText() const
{
...
FText SClaudeInputArea::GetDictationToolTipText() const
{
	switch (DictationState.Get())
	{
	case EUnrealClaudeVoiceDictationState::Recording:
...
FSlateColor SClaudeInputArea::GetDictationStatusColor() const
{
	switch (DictationState.Get())
	{
	case EUnrealClaudeVoiceDictationState::Recording:
		return FSlateColor(FLinearColor(0.9f, 0.35f, 0.2f));

	case EUnrealClaudeVoiceDictationState::PreparingRuntime:
	case EUnrealClaudeVoiceDictationState::Transcribing:
		return FSlateColor(FLinearColor(0.8f, 0.7f, 0.2f));
