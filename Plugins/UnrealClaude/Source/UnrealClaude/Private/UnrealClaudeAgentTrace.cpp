...
namespace
{
	...
	FString BackendFilterLabel(const EUnrealClaudeProviderBackend Backend)
	{
...
FUnrealClaudeAgentTraceLog& FUnrealClaudeAgentTraceLog::Get()
{
	static FUnrealClaudeAgentTraceLog Instance;
	return Instance;
}

FString FUnrealClaudeAgentTraceLog::BeginRun(
...
void FUnrealClaudeAgentTraceLog::AppendObservedEvent(const FString& RunId, const FAgentRunEvent& Event)
{
	...
	case EAgentRunEventType::ToolUse:
		{
			...
			TArray<FString> PartBLines;
			{
				...
				if (FAgentTraceActiveRunState* ActiveRun = ActiveRuns.Find(RunId))
				{
					...
					ClassifiedToolFamily = UnrealClaudeCanonRouting::DetermineToolFamily(
						ActiveRun->CanonExecution,
						Event.ToolName,
						Event.ToolInput);
					bMutatingTool = UnrealClaudeCanonRouting::IsMutatingToolUse(Event.ToolName, Event.ToolInput);
					UnrealClaudeCanonRouting::UpdateFromToolUse(
						ActiveRun->CanonExecution,
						Event.ToolName,
						Event.ToolInput,
			...
			}
			{
				...
				Payload->SetObjectField(
					...
					UnrealClaude::CommandClassification::ClassifyExecutionTruth(TruthInputs).ToJsonObject());
			}
...
void FUnrealClaudeAgentTraceLog::AppendEvent(
	...
	const FString& RunId)
{
	...
	if (EventType == TEXT("policy_denied"))
	{
		...
		if (FAgentTraceActiveRunState* ActiveRun = ActiveRuns.Find(RunId))
		{
			UnrealClaudeCanonRouting::MarkPolicyDeny(ActiveRun->CanonExecution);
		}
	}
...
void FUnrealClaudeAgentTraceLog::LogBackendFailure(
	const FString& RunId,
	const EUnrealClaudeProviderBackend Backend,
	...
	const FString& Stage)
{
	const FString TransportFailureFamily =
		Backend == EUnrealClaudeProviderBackend::CodexCli
...
void FUnrealClaudeAgentTraceLog::MarkCancellation(const EUnrealClaudeProviderBackend Backend, const FString& Detail)
{
	FString RunId;
	FString ProviderSessionId;
	{
...
void FUnrealClaudeAgentTraceLog::CompleteRun(
	const FString& RunId,
	const EUnrealClaudeProviderBackend Backend,
	...
	const bool bSuccess)
{
	...
	UnrealClaudeCanonRouting::Finalize(ActiveRunState.CanonExecution, bSuccess, bTimedOut);
	const TSharedPtr<FJsonObject> CanonExecutionObject = UnrealClaudeCanonRouting::MakeCanonExecutionJson(ActiveRunState.CanonExecution);
...
FString FUnrealClaudeAgentTraceLog::GetTraceLogPath() const
{
	return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealClaude"), TEXT("agent_trace.jsonl"));
}

FString FUnrealClaudeAgentTraceLog::GetActiveRunIdForBackend(const EUnrealClaudeProviderBackend Backend) const
{
	FScopeLock Lock(&Mutex);
	if (const FString* ActiveRunId = ActiveRunIdsByBackend.Find(static_cast<uint8>(Backend)))
	{
...
bool FUnrealClaudeAgentTraceLog::TryGetActiveCanonExecutionForBackend(
	const EUnrealClaudeProviderBackend Backend,
...
bool FUnrealClaudeAgentTraceLog::IsRunMarkedCancelled(const FString& RunId) const
{
	FScopeLock Lock(&Mutex);
	if (const FAgentTraceActiveRunState* ActiveRun = ActiveRuns.Find(RunId))
	{
...
bool FUnrealClaudeAgentTraceLog::TryGetLatestTransportFailureSummary(
	const EUnrealClaudeProviderBackend Backend,
...
bool FUnrealClaudeAgentTraceLog::MarkTransportRetryAttempt(
	const EUnrealClaudeProviderBackend Backend,
...
TArray<TSharedPtr<FJsonObject>> FUnrealClaudeAgentTraceLog::QueryEvents(
...
TSharedPtr<FJsonObject> FUnrealClaudeAgentTraceLog::BuildRunMetadataObject(
	const FAgentBackendStatus& Status,
	const FAgentRequestConfig& Config,
	const bool bIncludeEngineContext,
	const bool bIncludeProjectContext) const
{
	...
	Payload->SetStringField(TEXT("auth_effective_path"),
		Status.Backend == EUnrealClaudeProviderBackend::CodexCli ? FCodexCliRunner::GetEffectiveAuthEntryPath() : FString());
	Payload->SetStringField(TEXT("auth_ownership_model"),
		Status.Backend == EUnrealClaudeProviderBackend::CodexCli ? FCodexCliRunner::GetEffectiveAuthOwnershipModel() : FString());
	...
	Payload->SetStringField(TEXT("speed_mode_effective"),
		Status.Backend == EUnrealClaudeProviderBackend::CodexCli ? FCodexCliRunner::GetEffectiveSpeedModeName() : FString());
	Payload->SetStringField(TEXT("speed_mode_support"),
		Status.Backend == EUnrealClaudeProviderBackend::CodexCli ? FCodexCliRunner::GetSpeedModeSupportLabel() : FString());
	Payload->SetStringField(TEXT("work_mode"),
		(Status.Backend == EUnrealClaudeProviderBackend::CodexCli && Settings)
	...
	const TSharedPtr<FJsonObject> CanonObject = UnrealClaudeCanonRouting::MakeCanonExecutionJson(Config.CanonExecution);
...
TSharedPtr<FJsonObject> FUnrealClaudeAgentTraceLog::MakeTraceRecordObject(const FAgentTraceRecord& Record) const
{
...
void FUnrealClaudeAgentTraceLog::StoreAndPersistRecord(const FAgentTraceRecord& Record)
{
	{
		FScopeLock Lock(&Mutex);
...
void FUnrealClaudeAgentTraceLog::PersistRecord(const FAgentTraceRecord& Record) const
{
...
TArray<TSharedPtr<FJsonObject>> FUnrealClaudeAgentTraceLog::LoadPersistedRecords() const
{
	TArray<TSharedPtr<FJsonObject>> Records;
	const FString TraceLogFile = GetTraceLogPath();
	if (!IFileManager::Get().FileExists(*TraceLogFile))
	{
		return Records;
	}

	TArray<FString> Lines;
	if (!FFileHelper::LoadFileToStringArray(Lines, *TraceLogFile))
	{
		return Records;
	}
...
FString FUnrealClaudeAgentTraceLog::MakeRunId() const
{
	return FString::Printf(TEXT("run_%s"), *FGuid::NewGuid().ToString(EGuidFormats::Digits));
}

FString FUnrealClaudeAgentTraceLog::MakeEventId() const
{
	return FString::Printf(TEXT("evt_%s"), *FGuid::NewGuid().ToString(EGuidFormats::Digits));
}

bool FUnrealClaudeAgentTraceLog::IsTimeoutMessage(const FString& Message)
{
