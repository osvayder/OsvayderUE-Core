...
namespace
{
	...
	FString BuildPhaseStatusLine(const FUnrealClaudeActivePlan& Plan)
	{
		...
		{
			const FUnrealClaudePlanMechanicEntry& Entry = Plan.Mechanics[CurrentIndex];
			Line = FString::Printf(
		...
		for (int32 i = 0; i < Plan.Mechanics.Num(); ++i)
		{
			...
			const FUnrealClaudePlanMechanicEntry& M = Plan.Mechanics[i];
	...
	FString BuildProgressSummary(const FUnrealClaudeActivePlan& Plan)
	{
		...
		while (i < Plan.ToolCalls.Num())
		{
			...
			const FUnrealClaudePlanToolCallEntry& Rep = Plan.ToolCalls[j - 1];
		...
		for (const FUnrealClaudePlanToolCallEntry& TC : Plan.ToolCalls)
		{
	...
	FString BuildStallIndicator(const FUnrealClaudeActivePlan& Plan)
	{
		...
		const FUnrealClaudePlanToolCallEntry& A = Plan.ToolCalls[N - 3];
		const FUnrealClaudePlanToolCallEntry& B = Plan.ToolCalls[N - 2];
		const FUnrealClaudePlanToolCallEntry& C = Plan.ToolCalls[N - 1];
	...
	FString PickLastIntent(const FUnrealClaudeActivePlan& Plan)
	{
...
FTaskRecoverySummary FUnrealClaudeTaskRecoverySummarizer::BuildRecoverySummary(
...
FString FUnrealClaudeTaskRecoverySummarizer::FormatElapsed(int64 TotalSeconds)
{
	if (TotalSeconds < 30)
	{
