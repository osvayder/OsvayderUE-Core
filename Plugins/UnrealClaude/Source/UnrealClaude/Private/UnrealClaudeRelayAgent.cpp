...
namespace
{
	...
	constexpr TCHAR RelayAgentScriptName[] = TEXT("UnrealClaude-RelayAgent.ps1");
	...
	TSharedRef<FJsonObject> MakeSettingsJson(const FUnrealClaudeRelaySettingsSnapshot& Snapshot)
	{
	...
	FUnrealClaudeRelaySettingsSnapshot ParseSettingsSnapshot(const TSharedPtr<FJsonObject>& Object)
	{
		FUnrealClaudeRelaySettingsSnapshot Snapshot;
		if (!Object.IsValid())
		{
			return Snapshot;
		}
	...
	TSharedRef<FJsonObject> MakeCompileProofJson(const FUnrealClaudeCompileProofState& State)
	{
		TSharedRef<FJsonObject> Root = MakeShared<FJsonObject>();
	...
	FUnrealClaudeCompileProofState ParseCompileProofState(const TSharedPtr<FJsonObject>& Object)
	{
	...
	TSharedRef<FJsonObject> MakeHandoffJson(const FUnrealClaudeRelayHandoffContext& Context)
	{
		TSharedRef<FJsonObject> Root = MakeShared<FJsonObject>();
	...
	FUnrealClaudeRelayHandoffContext ParseHandoffContext(const TSharedPtr<FJsonObject>& Object)
	{
	...
	TSharedRef<FJsonObject> MakeProgressJson(const FUnrealClaudeRelayProgressEntry& Entry)
	{
		TSharedRef<FJsonObject> Root = MakeShared<FJsonObject>();
		...
		Root->SetStringField(TEXT("terminal_outcome"), UnrealClaudeRelayTerminalOutcomeToString(Entry.TerminalOutcome));
		return Root;
	}

	FUnrealClaudeRelayProgressEntry ParseProgressEntry(const TSharedPtr<FJsonObject>& Object)
	{
		FUnrealClaudeRelayProgressEntry Entry;
		if (!Object.IsValid())
		{
			return Entry;
		}
		...
		ParseUnrealClaudeRelayTerminalOutcome(GetStringFieldOrEmpty(Object, TEXT("terminal_outcome")), Entry.TerminalOutcome);
		return Entry;
	}

	TSharedRef<FJsonObject> MakeResultJson(const FUnrealClaudeRelayResult& Result)
	{
		TSharedRef<FJsonObject> Root = MakeShared<FJsonObject>();
		...
		Root->SetStringField(TEXT("terminal_outcome"), UnrealClaudeRelayTerminalOutcomeToString(Result.TerminalOutcome));
	...
	FUnrealClaudeRelayResult ParseRelayResult(const TSharedPtr<FJsonObject>& Object)
	{
		...
		ParseUnrealClaudeRelayTerminalOutcome(GetStringFieldOrEmpty(Object, TEXT("terminal_outcome")), Result.TerminalOutcome);
	...
	TSharedRef<FJsonObject> MakeMechanicJson(const FUnrealClaudePlanMechanicEntry& Mechanic)
	{
		TSharedRef<FJsonObject> Root = MakeShared<FJsonObject>();
	...
	FUnrealClaudePlanMechanicEntry ParseMechanicEntry(const TSharedPtr<FJsonObject>& Object)
	{
		FUnrealClaudePlanMechanicEntry Entry;
		if (!Object.IsValid())
		{
			return Entry;
		}
	...
	TSharedRef<FJsonObject> MakeToolCallJson(const FUnrealClaudePlanToolCallEntry& ToolCall)
	{
		TSharedRef<FJsonObject> Root = MakeShared<FJsonObject>();
	...
	FUnrealClaudePlanToolCallEntry ParseToolCallEntry(const TSharedPtr<FJsonObject>& Object)
	{
	...
	TSharedRef<FJsonObject> MakeActivePlanJson(const FUnrealClaudeActivePlan& Plan)
	{
		TSharedRef<FJsonObject> Root = MakeShared<FJsonObject>();
		...
		for (const FUnrealClaudePlanMechanicEntry& Entry : Plan.Mechanics)
		{
		...
		for (const FUnrealClaudePlanToolCallEntry& Entry : Plan.ToolCalls)
		{
	...
	FUnrealClaudeActivePlan ParseActivePlan(const TSharedPtr<FJsonObject>& Object)
	{
		...
		{
			Plan.LaneState = FUnrealClaudeTaskLaneState::FromJsonObject(*LaneStateObject);
		}
...
const TCHAR* UnrealClaudeRelayTerminalOutcomeToString(EUnrealClaudeRelayTerminalOutcome Outcome)
{
	switch (Outcome)
	{
	case EUnrealClaudeRelayTerminalOutcome::Success:
		return TEXT("success");
	case EUnrealClaudeRelayTerminalOutcome::TerminalFail:
		return TEXT("terminal_fail");
	case EUnrealClaudeRelayTerminalOutcome::Cancelled:
...
bool ParseUnrealClaudeRelayTerminalOutcome(const FString& Value, EUnrealClaudeRelayTerminalOutcome& OutOutcome)
{
	if (Value.Equals(TEXT("success"), ESearchCase::IgnoreCase))
	{
		OutOutcome = EUnrealClaudeRelayTerminalOutcome::Success;
		return true;
	}

	if (Value.Equals(TEXT("terminal_fail"), ESearchCase::IgnoreCase))
	{
		OutOutcome = EUnrealClaudeRelayTerminalOutcome::TerminalFail;
		return true;
	}

	if (Value.Equals(TEXT("cancelled"), ESearchCase::IgnoreCase))
	{
		OutOutcome = EUnrealClaudeRelayTerminalOutcome::Cancelled;
		return true;
	}

	if (Value.Equals(TEXT("none"), ESearchCase::IgnoreCase) || Value.IsEmpty())
	{
		OutOutcome = EUnrealClaudeRelayTerminalOutcome::None;
		return true;
	}
...
bool FUnrealClaudeRelayAgentManager::LoadLatestProgressEntryForTask(
	const FString& TaskId,
	FUnrealClaudeRelayProgressEntry& OutEntry,
	FString& OutError)
{
	OutError.Reset();
	if (TaskId.IsEmpty())
	...
	FString Jsonl;
	if (!FFileHelper::LoadFileToString(Jsonl, *GetRelayProgressPath()))
	{
		OutError = FString::Printf(TEXT("Could not read relay progress log: %s"), *GetRelayProgressPath());
		return false;
	}
	...
	for (int32 Index = Lines.Num() - 1; Index >= 0; --Index)
	{
		...
		const FUnrealClaudeRelayProgressEntry Entry = ParseProgressEntry(Object);
...
FString FUnrealClaudeRelayAgentManager::GetRelayRootDir()
{
	return FUnrealClaudeRestartSurvivalManager::GetStateRootDir();
}

FString FUnrealClaudeRelayAgentManager::GetActivePlanPath()
{
	return FPaths::Combine(GetRelayRootDir(), ActivePlanFileName);
}

FString FUnrealClaudeRelayAgentManager::GetPlanArchiveDir()
{
	return FPaths::Combine(GetRelayRootDir(), PlanArchiveDirName);
}

FString FUnrealClaudeRelayAgentManager::GetRelayArchiveDir()
{
	return FPaths::Combine(GetRelayRootDir(), RelayArchiveDirName);
}

FString FUnrealClaudeRelayAgentManager::GetHandoffContextPath()
{
	return FPaths::Combine(GetRelayRootDir(), TEXT("handoff_context.json"));
}

FString FUnrealClaudeRelayAgentManager::GetRelayProgressPath()
{
	return FPaths::Combine(GetRelayRootDir(), TEXT("relay_progress.jsonl"));
}

FString FUnrealClaudeRelayAgentManager::GetRelayResultPath()
{
	return FPaths::Combine(GetRelayRootDir(), TEXT("relay_result.json"));
}

FString FUnrealClaudeRelayAgentManager::GetRelayCancelRequestPath()
{
	return FPaths::Combine(GetRelayRootDir(), TEXT("relay_cancel_request.json"));
}

FString FUnrealClaudeRelayAgentManager::GetRelayAgentScriptPath()
{
	return NormalizeFilePath(FPaths::Combine(GetUnrealClaudePluginBaseDir(), TEXT("Script"), RelayAgentScriptName));
}

bool FUnrealClaudeRelayAgentManager::LoadActivePlan(FUnrealClaudeActivePlan& OutPlan, FString& OutError)
{
	return LoadPlanFromPath(GetActivePlanPath(), OutPlan, OutError);
}
...
namespace
{
	...
	FUnrealClaudeActivePlan BuildFallbackPlan(const FString& OriginalUserTask, const FString& ResumeHintOverride)
	{
		FUnrealClaudeActivePlan Plan;
...
bool FUnrealClaudeRelayAgentManager::LoadActivePlanWithFallback(
	...
	FString& OutError)
{
	...
	const bool bStateLoaded = FUnrealClaudeRestartSurvivalManager::LoadState(State, StateLoadError);
	if (bStateLoaded && !State.PostReattachCompletionText.IsEmpty())
	{
		...
		UE_LOG(LogUnrealClaude, Display,
			TEXT("UnrealClaudeRelayAgent: active_plan.json absent; using state.json.post_reattach_completion_text fallback (expected for certain supervisor branches)."));
		return true;
	}
	...
	UE_LOG(LogUnrealClaude, Warning,
		TEXT("UnrealClaudeRelayAgent: No continuation plan recoverable; reverting to fresh session. (active_plan.json missing; state.json %s)"),
		bStateLoaded ? TEXT("loaded but has no post_reattach_completion_text") : TEXT("also missing or unreadable"));
	return true;
}

bool FUnrealClaudeRelayAgentManager::SaveActivePlan(const FUnrealClaudeActivePlan& Plan, FString& OutError)
{
	return SavePlanToPath(GetActivePlanPath(), Plan, OutError);
}

bool FUnrealClaudeRelayAgentManager::DeleteActivePlan(FString& OutError)
{
	if (!IFileManager::Get().FileExists(*GetActivePlanPath()))
	{
		return true;
	}

	if (IFileManager::Get().Delete(*GetActivePlanPath(), false, true, true))
	{
		return true;
	}

	OutError = FString::Printf(TEXT("Could not delete active plan: %s"), *GetActivePlanPath());
	return false;
}

bool FUnrealClaudeRelayAgentManager::LoadPlanFromPath(
...
bool FUnrealClaudeRelayAgentManager::SavePlanToPath(
	const FString& PlanPath,
...
bool FUnrealClaudeRelayAgentManager::LoadProgressEntriesForTask(
	const FString& TaskId,
	TArray<FUnrealClaudeRelayProgressEntry>& OutEntries,
	FString& OutError)
{
	OutEntries.Reset();
	if (TaskId.IsEmpty())
	...
	if (OutEntries.Num() == 0)
	{
		OutError = FString::Printf(TEXT("Relay progress log has no parseable entries for task %s: %s"), *TaskId, *GetRelayProgressPath());
		return false;
	}
...
bool FUnrealClaudeRelayAgentManager::ArchiveTerminalArtifacts(
	const FUnrealClaudeActivePlan& Plan,
	...
	FString& OutError)
{
	...
	DeleteProgressLog(OutError);
	DeleteRelayResult(OutError);
	DeleteHandoffContext(OutError);
	DeleteCancelRequest(OutError);
	return PruneArchivedArtifacts(DefaultArchiveRetention, DefaultArchiveRetention, OutError);
}

bool FUnrealClaudeRelayAgentManager::PruneArchivedArtifacts(
...
bool FUnrealClaudeRelayAgentManager::LoadHandoffContext(FUnrealClaudeRelayHandoffContext& OutContext, FString& OutError)
{
	TSharedPtr<FJsonObject> Root;
	if (!LoadJsonFile(GetHandoffContextPath(), Root, OutError))
	{
		return false;
	}
...
bool FUnrealClaudeRelayAgentManager::SaveHandoffContext(const FUnrealClaudeRelayHandoffContext& Context, FString& OutError)
{
	return SaveJsonFile(GetHandoffContextPath(), MakeHandoffJson(Context), OutError);
}

bool FUnrealClaudeRelayAgentManager::DeleteHandoffContext(FString& OutError)
{
	if (!IFileManager::Get().FileExists(*GetHandoffContextPath()))
	{
		return true;
	}

	if (IFileManager::Get().Delete(*GetHandoffContextPath(), false, true, true))
	{
		return true;
	}

	OutError = FString::Printf(TEXT("Could not delete relay handoff context: %s"), *GetHandoffContextPath());
	return false;
}

bool FUnrealClaudeRelayAgentManager::AppendProgressEntry(const FUnrealClaudeRelayProgressEntry& Entry, FString& OutError)
{
...
bool FUnrealClaudeRelayAgentManager::LoadLatestProgressEntry(FUnrealClaudeRelayProgressEntry& OutEntry, FString& OutError)
{
	FString Jsonl;
	if (!FFileHelper::LoadFileToString(Jsonl, *GetRelayProgressPath()))
	{
	...
	OutError = FString::Printf(TEXT("Relay progress log has no parseable entries: %s"), *GetRelayProgressPath());
	return false;
}

bool FUnrealClaudeRelayAgentManager::DeleteProgressLog(FString& OutError)
{
	if (!IFileManager::Get().FileExists(*GetRelayProgressPath()))
	{
		return true;
	}

	if (IFileManager::Get().Delete(*GetRelayProgressPath(), false, true, true))
	{
		return true;
	}

	OutError = FString::Printf(TEXT("Could not delete relay progress log: %s"), *GetRelayProgressPath());
	return false;
}

bool FUnrealClaudeRelayAgentManager::LoadRelayResult(FUnrealClaudeRelayResult& OutResult, FString& OutError)
{
	TSharedPtr<FJsonObject> Root;
	if (!LoadJsonFile(GetRelayResultPath(), Root, OutError))
	{
		return false;
	}
...
bool FUnrealClaudeRelayAgentManager::LoadRelayResultForTask(
	const FString& TaskId,
	FUnrealClaudeRelayResult& OutResult,
	FString& OutError)
{
...
bool FUnrealClaudeRelayAgentManager::SaveRelayResult(const FUnrealClaudeRelayResult& Result, FString& OutError)
{
	return SaveJsonFile(GetRelayResultPath(), MakeResultJson(Result), OutError);
}

bool FUnrealClaudeRelayAgentManager::DeleteRelayResult(FString& OutError)
{
	if (!IFileManager::Get().FileExists(*GetRelayResultPath()))
	{
		return true;
	}

	if (IFileManager::Get().Delete(*GetRelayResultPath(), false, true, true))
	{
		return true;
	}

	OutError = FString::Printf(TEXT("Could not delete relay result: %s"), *GetRelayResultPath());
	return false;
}

bool FUnrealClaudeRelayAgentManager::HasCancelRequest()
{
	return IFileManager::Get().FileExists(*GetRelayCancelRequestPath());
}

bool FUnrealClaudeRelayAgentManager::WriteCancelRequest(const FString& Reason, FString& OutError)
{
	TSharedRef<FJsonObject> Root = MakeShared<FJsonObject>();
...
bool FUnrealClaudeRelayAgentManager::DeleteCancelRequest(FString& OutError)
{
	if (!HasCancelRequest())
	{
		return true;
	...
	if (IFileManager::Get().Delete(*GetRelayCancelRequestPath(), false, true, true))
	{
		return true;
	}

	OutError = FString::Printf(TEXT("Could not delete relay cancel request: %s"), *GetRelayCancelRequestPath());
	return false;
}

FUnrealClaudeRelaySettingsSnapshot FUnrealClaudeRelayAgentManager::BuildCurrentCodexSettingsSnapshot()
{
	FUnrealClaudeRelaySettingsSnapshot Snapshot;
