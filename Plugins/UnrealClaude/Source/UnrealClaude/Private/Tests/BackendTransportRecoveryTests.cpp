...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.PersistentTransport.ClassifierTruth",
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.AgentTrace.TransportRecovery.SafeRetryTruth",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FBackendTransportRecovery_TraceSafeRetryTruth::RunTest(const FString& Parameters)
{
	...
	FUnrealClaudeExecutionLog::Get().Clear();
	...
	const FString RunId = FUnrealClaudeAgentTraceLog::Get().BeginRun(Status, Config, Config.Prompt, true, true);
	FUnrealClaudeAgentTraceLog::Get().LogBackendFailure(RunId, Status.Backend, TimeoutMessage, TEXT("backend_completion"));
	FUnrealClaudeAgentTraceLog::Get().CompleteRun(RunId, Status.Backend, TimeoutMessage, false);

	FAgentTransportFailureSummary Summary;
	TestTrue(
		TEXT("latest transport failure summary should be available for Codex"),
		FUnrealClaudeAgentTraceLog::Get().TryGetLatestTransportFailureSummary(Status.Backend, Summary));
	...
	const TArray<FExecutionReceipt> Receipts = FUnrealClaudeExecutionLog::Get().GetRecent(4);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.AgentTrace.TransportRecovery.UnsafeRetryTruth",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FBackendTransportRecovery_TraceUnsafeRetryTruth::RunTest(const FString& Parameters)
{
	...
	ToolUse.ToolInput = TEXT("{\"path\":\"Plugins/UnrealClaude/Source/UnrealClaude/Private/CodexCliRunner.cpp\"}");
	ToolUse.ToolCallId = TEXT("tool_001");
	FUnrealClaudeAgentTraceLog::Get().AppendObservedEvent(RunId, ToolUse);

	FUnrealClaudeAgentTraceLog::Get().LogBackendFailure(RunId, Status.Backend, TransportMessage, TEXT("backend_completion"));
	FUnrealClaudeAgentTraceLog::Get().CompleteRun(RunId, Status.Backend, TransportMessage, false);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.AgentTrace.TransportRecovery.RetryAttemptAndReplayMetadata",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FBackendTransportRecovery_RetryAttemptAndReplayMetadata::RunTest(const FString& Parameters)
{
	...
	const FString FailedRunId = FUnrealClaudeAgentTraceLog::Get().BeginRun(Status, Config, Config.Prompt, true, true);
	FUnrealClaudeAgentTraceLog::Get().LogBackendFailure(FailedRunId, Status.Backend, TimeoutMessage, TEXT("backend_completion"));
	FUnrealClaudeAgentTraceLog::Get().CompleteRun(FailedRunId, Status.Backend, TimeoutMessage, false);
	TestTrue(
		TEXT("retry attempt should be recorded for the latest transport failure"),
		FUnrealClaudeAgentTraceLog::Get().MarkTransportRetryAttempt(Status.Backend, TEXT("widget_retry_last")));
	...
	const FString ReplayRunId = FUnrealClaudeAgentTraceLog::Get().BeginRun(Status, ReplayConfig, ReplayConfig.Prompt, true, true);
	FUnrealClaudeAgentTraceLog::Get().CompleteRun(ReplayRunId, Status.Backend, TEXT("retry succeeded"), true);
	...
	for (const FExecutionReceipt& Receipt : FUnrealClaudeExecutionLog::Get().GetRecent(8))
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.Widget.TransportRecovery.GuidanceTruth",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FBackendTransportRecovery_WidgetGuidanceTruth::RunTest(const FString& Parameters)
{
	const FString SafeStatus = SClaudeEditorWidget::BuildTransportRetryStatusLabel(true);
	const FString UnsafeStatus = SClaudeEditorWidget::BuildTransportRetryStatusLabel(false);
	const FString UnsafeNotice =
		SClaudeEditorWidget::BuildTransportRetryNotice(
			TEXT("Codex CLI"),
			false,
	...
	TestTrue(
		...
		SClaudeEditorWidget::DescribeTransportRetryBlockReason(TEXT("tool_activity_observed")).Contains(TEXT("tool activity"), ESearchCase::IgnoreCase));
