...
/**
 * 626 P1 crash-safety tests for SClaudeEditorWidget.
 *
 ...
 *
 * Reference: `AgentBridge/CODEX_TO_CLAUDE.md` 2026-04-19 13:00 dispatch for
...
namespace UnrealClaudeWidgetCrashSafetyTestsDetail
{
	/**
...
bool FWidget_CrashSafety_MalformedCodeFences_DoesNotCrash::RunTest(const FString& /*Parameters*/)
{
	// Classic well-formed case as a baseline sanity check.
	{
		...
		SClaudeEditorWidget::ParseCodeFences(TEXT("before\n```cpp\ncode;\n```\nafter"), Sections);
	...
	{
		TArray<TPair<FString, bool>> Sections;
		SClaudeEditorWidget::ParseCodeFences(TEXT("just text\n```\nmore text"), Sections);
	...
	// these should be safe but test the pathway explicitly).
	{
		...
		TArray<TPair<FString, bool>> Sections;
		SClaudeEditorWidget::ParseCodeFences(InputWithNulls, Sections);
	...
	{
		TArray<TPair<FString, bool>> Sections;
		SClaudeEditorWidget::ParseCodeFences(TEXT("``"), Sections);
	...
	{
		TArray<TPair<FString, bool>> Sections;
		SClaudeEditorWidget::ParseCodeFences(FString(), Sections);
	...
	// Adversarial: many consecutive opening fences without closes.
	{
		...
		TArray<TPair<FString, bool>> Sections;
		SClaudeEditorWidget::ParseCodeFences(PathologicalInput, Sections);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.Widget.CrashSafety.WeakPtrInvariant_PinAfterDestructionIsNoOp",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FWidget_CrashSafety_WeakPtrInvariant_PinAfterDestructionIsNoOp::RunTest(const FString& /*Parameters*/)
{
	...
	// relationship SClaudeEditorWidget uses. This verifies the exact
	// capture + pin sequence the P1 refactor relies on without needing
	// to construct a full SClaudeEditorWidget (which requires editor
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.Widget.CrashSafety.ArraySnapshot_SurvivesConcurrentClear",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

bool FWidget_CrashSafety_ArraySnapshot_SurvivesConcurrentClear::RunTest(const FString& /*Parameters*/)
{
	using UnrealClaudeWidgetCrashSafetyTestsDetail::FCountedResource;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.Widget.CrashSafety.OneShotGuard_SecondCallIsNoOp",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FWidget_CrashSafety_OneShotGuard_SecondCallIsNoOp::RunTest(const FString& /*Parameters*/)
{
	...
	auto GuardedFinalize = [&]()
	{
		...
		// SClaudeEditorWidget::FinalizeStreamingResponse.
		if (bHasCompleted)
		{
