...
namespace
{
		...
		UPackage*& OutPackage)
	{
		...
		const FString PackageName = FString::Printf(
			TEXT("/Game/UnrealClaudeP6SaveAssetTest_%s_%s"), *ShortName, *UniqueSuffix);
		...
		UPackage*& OutPackage)
	{
		const FString UniqueSuffix = FGuid::NewGuid().ToString(EGuidFormats::Digits);
		const FString PackageName = FString::Printf(
			TEXT("/Game/UnrealClaudeP6SaveAssetBPTest_%s_%s"), *ShortName, *UniqueSuffix);
		OutPackage = CreatePackage(*PackageName);
		if (!OutPackage) return nullptr;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.MCP.Tools.Asset.SaveAsset_Regression_SameBehaviorAsPrePipeline",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FMCPToolAsset_SaveAsset_Regression_SameBehaviorAsPrePipeline::RunTest(const FString& /*Parameters*/)
{
	...
	UnrealClaude::SavePipeline::Testing::SetAssetCreatedOverride([](UObject* /*A*/) {});
	ON_SCOPE_EXIT
	{
		UnrealClaude::SavePipeline::Testing::ClearSavePackageOverride();
		UnrealClaude::SavePipeline::Testing::ClearAssetCreatedOverride();
	};
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.MCP.Tools.Asset.SaveAsset_NowCompilesBlueprint",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.MCP.Tools.Asset.SaveAsset_NowNotifiesAssetRegistry",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
