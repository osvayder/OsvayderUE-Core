...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.RelayAgent.HandoffRoundTripPreservesPromptAndBudgets",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRelayAgent_ProgressJsonlAppendsAndLoadsLatest,
	"UnrealClaude.RelayAgent.ProgressJsonlAppendsAndLoadsLatest",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRelayAgent_ProgressTaskFilterIgnoresStaleOtherTaskEntries,
	"UnrealClaude.RelayAgent.ProgressTaskFilterIgnoresStaleOtherTaskEntries",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRelayAgent_ResultTaskFilterRejectsMismatchedTask,
	"UnrealClaude.RelayAgent.ResultTaskFilterRejectsMismatchedTask",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRelayAgent_CancelRequestRoundTrip,
	"UnrealClaude.RelayAgent.CancelRequestRoundTrip",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRelayAgent_SettingsSnapshotParityMatchesCurrentCodexResolution,
	"UnrealClaude.RelayAgent.SettingsSnapshotParityMatchesCurrentCodexResolution",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRelayAgent_ActivePlanRoundTripPreservesOriginalTaskAndLocalizedState,
	"UnrealClaude.RelayAgent.ActivePlanRoundTripPreservesOriginalTaskAndLocalizedState",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRelayAgent_SaveActivePlanIsAtomicAndLeavesNoTempFile,
	"UnrealClaude.RelayAgent.SaveActivePlanIsAtomicAndLeavesNoTempFile",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRelayAgent_ArchiveTerminalArtifactsArchivesAndPrunesBoundedRetention,
	"UnrealClaude.RelayAgent.ArchiveTerminalArtifactsArchivesAndPrunesBoundedRetention",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRelayAgent_ScriptCopiesStayInSyncAndAvoidDetachedActivePlanWrites,
	"UnrealClaude.RelayAgent.ScriptCopiesStayInSyncAndAvoidDetachedActivePlanWrites",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRelayAgent_ScriptWritesDirectRelayArtifactSchema,
	"UnrealClaude.RelayAgent.ScriptWritesDirectRelayArtifactSchema",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRelayAgent_HandoffRoundTripPreservesPromptAndBudgets::RunTest(const FString& Parameters)
{
	...
	FString Error;
	TestTrue(TEXT("handoff context should save"), FUnrealClaudeRelayAgentManager::SaveHandoffContext(Context, Error));
	if (!Error.IsEmpty())
	{
		AddInfo(Error);
	}

	FUnrealClaudeRelayHandoffContext Loaded;
	TestTrue(TEXT("handoff context should load"), FUnrealClaudeRelayAgentManager::LoadHandoffContext(Loaded, Error));
...
bool FRelayAgent_ProgressJsonlAppendsAndLoadsLatest::RunTest(const FString& Parameters)
{
	...
	FUnrealClaudeRelayProgressEntry First;
	...
	FUnrealClaudeRelayProgressEntry Second;
	...
	Second.TerminalOutcome = EUnrealClaudeRelayTerminalOutcome::TerminalFail;

	FString Error;
	TestTrue(TEXT("first progress entry should append"), FUnrealClaudeRelayAgentManager::AppendProgressEntry(First, Error));
	TestTrue(TEXT("second progress entry should append"), FUnrealClaudeRelayAgentManager::AppendProgressEntry(Second, Error));

	FUnrealClaudeRelayProgressEntry Latest;
	TestTrue(TEXT("latest progress entry should load"), FUnrealClaudeRelayAgentManager::LoadLatestProgressEntry(Latest, Error));
	...
	TestEqual(TEXT("terminal outcome should survive"), static_cast<uint8>(Latest.TerminalOutcome), static_cast<uint8>(EUnrealClaudeRelayTerminalOutcome::TerminalFail));
	return true;
}
...
bool FRelayAgent_ProgressTaskFilterIgnoresStaleOtherTaskEntries::RunTest(const FString& Parameters)
{
	...
	FUnrealClaudeRelayProgressEntry StaleEntry;
	...
	StaleEntry.TerminalOutcome = EUnrealClaudeRelayTerminalOutcome::TerminalFail;
	TestTrue(TEXT("stale progress entry should append"), FUnrealClaudeRelayAgentManager::AppendProgressEntry(StaleEntry, Error));

	FUnrealClaudeRelayProgressEntry CurrentEntry;
	...
	TestTrue(TEXT("current progress entry should append"), FUnrealClaudeRelayAgentManager::AppendProgressEntry(CurrentEntry, Error));

	FUnrealClaudeRelayProgressEntry FilteredEntry;
	TestTrue(
		TEXT("task-filtered progress load should find the current task entry"),
		FUnrealClaudeRelayAgentManager::LoadLatestProgressEntryForTask(TEXT("task_current"), FilteredEntry, Error));
	...
	TestFalse(
		...
		FUnrealClaudeRelayAgentManager::LoadLatestProgressEntryForTask(TEXT("task_missing"), FilteredEntry, Error));
	return true;
}
...
bool FRelayAgent_ResultTaskFilterRejectsMismatchedTask::RunTest(const FString& Parameters)
{
	...
	Result.TerminalOutcome = EUnrealClaudeRelayTerminalOutcome::TerminalFail;
	Result.Summary = TEXT("stale relay fail");

	FString Error;
	TestTrue(TEXT("mismatched relay result should save"), FUnrealClaudeRelayAgentManager::SaveRelayResult(Result, Error));

	FUnrealClaudeRelayResult LoadedResult;
	TestFalse(
		TEXT("task-filtered relay result load should reject a mismatched task"),
		FUnrealClaudeRelayAgentManager::LoadRelayResultForTask(TEXT("task_current"), LoadedResult, Error));
	...
	TestTrue(TEXT("matching relay result should overwrite"), FUnrealClaudeRelayAgentManager::SaveRelayResult(Result, Error));
	TestTrue(
...
bool FRelayAgent_CancelRequestRoundTrip::RunTest(const FString& Parameters)
{
	...
	TestFalse(TEXT("cancel request should not exist initially"), FUnrealClaudeRelayAgentManager::HasCancelRequest());

	FString Error;
	TestTrue(TEXT("cancel request should write"), FUnrealClaudeRelayAgentManager::WriteCancelRequest(TEXT("user_cancelled"), Error));
	TestTrue(TEXT("cancel request should exist after write"), FUnrealClaudeRelayAgentManager::HasCancelRequest());
	TestTrue(TEXT("cancel request should delete"), FUnrealClaudeRelayAgentManager::DeleteCancelRequest(Error));
	TestFalse(TEXT("cancel request should be gone after delete"), FUnrealClaudeRelayAgentManager::HasCancelRequest());
	return true;
}
...
bool FRelayAgent_SettingsSnapshotParityMatchesCurrentCodexResolution::RunTest(const FString& Parameters)
{
	...
	const FUnrealClaudeRelaySettingsSnapshot Snapshot = FUnrealClaudeRelayAgentManager::BuildCurrentCodexSettingsSnapshot();
...
bool FRelayAgent_ActivePlanRoundTripPreservesOriginalTaskAndLocalizedState::RunTest(const FString& Parameters)
{
	...
	Plan.VerificationChecklist = { TEXT("Open Claude Assistant"), TEXT("Review build result"), TEXT("Reply with truthful final status") };

	FUnrealClaudePlanMechanicEntry Inspect;
	...
	FUnrealClaudePlanMechanicEntry Verify;
	...
	FUnrealClaudePlanToolCallEntry ToolCall;
	...
	FUnrealClaudeActivePlan Loaded;
	TestTrue(TEXT("active plan should load"), FUnrealClaudeRelayAgentManager::LoadActivePlan(Loaded, Error));
...
bool FRelayAgent_SaveActivePlanIsAtomicAndLeavesNoTempFile::RunTest(const FString& Parameters)
{
	...
	FString Error;
	TestTrue(TEXT("active plan should save atomically"), FUnrealClaudeRelayAgentManager::SaveActivePlan(Plan, Error));
	if (!Error.IsEmpty())
	{
		AddInfo(Error);
	}
...
bool FRelayAgent_ArchiveTerminalArtifactsArchivesAndPrunesBoundedRetention::RunTest(const FString& Parameters)
{
	...
	FString Error;
	TestTrue(TEXT("active plan should save before archive"), FUnrealClaudeRelayAgentManager::SaveActivePlan(Plan, Error));
	...
	TestTrue(TEXT("handoff should save before archive"), FUnrealClaudeRelayAgentManager::SaveHandoffContext(Handoff, Error));
	...
	TestTrue(TEXT("progress should append before archive"), FUnrealClaudeRelayAgentManager::AppendProgressEntry(Progress, Error));
	...
	TestTrue(TEXT("relay result should save before archive"), FUnrealClaudeRelayAgentManager::SaveRelayResult(Result, Error));

	TestTrue(TEXT("cancel request should save before archive"), FUnrealClaudeRelayAgentManager::WriteCancelRequest(TEXT("archive test"), Error));

	TArray<FString> ArchivedPaths;
	TestTrue(TEXT("terminal archive should succeed"), FUnrealClaudeRelayAgentManager::ArchiveTerminalArtifacts(Plan, ArchivedPaths, Error));
	if (!Error.IsEmpty())
	{
		AddInfo(Error);
	}
	...
	TestFalse(TEXT("active plan should be cleared after archive"), IFileManager::Get().FileExists(*FUnrealClaudeRelayAgentManager::GetActivePlanPath()));
	TestFalse(TEXT("progress log should be cleared after archive"), IFileManager::Get().FileExists(*FUnrealClaudeRelayAgentManager::GetRelayProgressPath()));
	TestFalse(TEXT("relay result should be cleared after archive"), IFileManager::Get().FileExists(*FUnrealClaudeRelayAgentManager::GetRelayResultPath()));
	TestFalse(TEXT("handoff should be cleared after archive"), IFileManager::Get().FileExists(*FUnrealClaudeRelayAgentManager::GetHandoffContextPath()));
	TestFalse(TEXT("cancel request should be cleared after archive"), IFileManager::Get().FileExists(*FUnrealClaudeRelayAgentManager::GetRelayCancelRequestPath()));

	const FString PlanArchiveDir = FUnrealClaudeRelayAgentManager::GetPlanArchiveDir();
	const FString RelayArchiveDir = FUnrealClaudeRelayAgentManager::GetRelayArchiveDir();
	...
	TestTrue(TEXT("archive pruning should succeed"), FUnrealClaudeRelayAgentManager::PruneArchivedArtifacts(2, 2, Error));
	if (!Error.IsEmpty())
	{
		AddInfo(Error);
	}
...
{
	const FString PluginScriptPath = FUnrealClaudeRelayAgentManager::GetRelayAgentScriptPath();
	const FString ProjectPluginScriptPath = NormalizePathForRelayTest(
		FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("UnrealClaude"), TEXT("Script"), TEXT("UnrealClaude-RelayAgent.ps1")));
	const FString OptionalProjectMirrorScriptPath = NormalizePathForRelayTest(
		FPaths::Combine(FPaths::ProjectDir(), TEXT("Script"), TEXT("UnrealClaude-RelayAgent.ps1")));
	...
	{
		AddInfo(TEXT("Project-root Script/UnrealClaude-RelayAgent.ps1 mirror is absent in this host project; project-plugin script remains the active source of truth."));
	}
