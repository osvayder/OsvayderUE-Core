...
namespace
{
	...
	{
		FString Root = FPaths::Combine(FPlatformProcess::UserTempDir(), TEXT("UnrealClaudeAutomation"), TEXT("CodexHomeResolution"), TestName);
	...
	{
		UUnrealClaudeSettings* Settings = nullptr;
		EUnrealClaudeCodexAuthMode OriginalValue = EUnrealClaudeCodexAuthMode::Auto;

		explicit FScopedCodexAuthModeOverride(const EUnrealClaudeCodexAuthMode NewValue)
		{
			Settings = UUnrealClaudeSettings::GetMutable();
			if (Settings)
			{
	...
	FString WriteFakeCodexAppServerScript(const FString& TestRoot)
	{
		...
		const FString ScriptText =
			...
			TEXT("const scenario = process.env.UNREALCLAUDE_FAKE_APP_SERVER_SCENARIO || 'always_success';\n")
			TEXT("const markerPath = process.env.UNREALCLAUDE_FAKE_APP_SERVER_MARKER || '';\n")
			TEXT("const counterPath = process.env.UNREALCLAUDE_FAKE_APP_SERVER_COUNTER || '';\n")
			TEXT("const allowedToolsLogPath = process.env.UNREALCLAUDE_FAKE_APP_SERVER_ALLOWED_TOOLS_LOG || '';\n")
			TEXT("const codexHomeLogPath = process.env.UNREALCLAUDE_FAKE_APP_SERVER_CODEX_HOME_LOG || '';\n")
			...
			TEXT("    const match = configArg.match(/mcp_servers\\.unrealclaude\\.env\\.UNREAL_MCP_ALLOWED_TOOLS='([^']*)'/);\n")
			...
			TEXT("      return;\n")
			TEXT("    }\n")
			TEXT("    send({ method: 'item/agentMessage/delta', params: { delta: `launch=${launchIndex} turn=${turnCounter} tools=${allowedTools}` } });\n")
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.HomeResolution.MachineStandardCandidatePrecedence",
...
bool FCodexCliRunner_MachineStandardCandidateResolutionPrecedence::RunTest(const FString& Parameters)
{
	...
	FScopedEnvironmentVariable MachineEnvOverride(TEXT("UNREALCLAUDE_CODEX_HOME"), FString());
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.HomeResolution.ExplicitOverrideWins",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCodexCliRunner_ExplicitOverrideWinsHomeResolution::RunTest(const FString& Parameters)
{
	...
	FScopedEnvironmentVariable UserProfileOverride(TEXT("USERPROFILE"), UserHome);
	FScopedEnvironmentVariable HomeOverride(TEXT("HOME"), UserHome);
	FScopedEnvironmentVariable MachineEnvOverride(TEXT("UNREALCLAUDE_CODEX_HOME"), EnvHome);
	FScopedEnvironmentVariable OpenAiKeyOverride(TEXT("OPENAI_API_KEY"), FString());
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.HomeResolution.MachineEnvOverrideWins",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.HomeResolution.PluginSettingsReadbackTruth",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.HomeResolution.ProjectSessionIsolationRemainsProjectLocal",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCodexCliRunner_ProjectSessionIsolationRemainsProjectLocal::RunTest(const FString& Parameters)
{
	...
	const FString ProjectSavedRoot = NormalizeDirectoryPathForTest(FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealClaude")));
	const FString NormalizedCodexHomePath = NormalizeDirectoryPathForTest(CodexHomePath);
	TestTrue(TEXT("shared machine auth home should stay outside the project-local Saved/UnrealClaude root"),
	...
	TestTrue(TEXT("legacy shared session path should stay project-local"),
		NormalizeDirectoryPathForTest(LegacySharedPath).StartsWith(ProjectSavedRoot, ESearchCase::IgnoreCase));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.AuthDiagnostics.HomeResolutionDisplay",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCodexCliRunner_AuthDiagnostics_HomeResolutionDisplay::RunTest(const FString& Parameters)
{
	...
	FScopedCodexAuthModeOverride AuthModeOverride(EUnrealClaudeCodexAuthMode::Auto);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.AuthDiagnostics.MissingArtifactStatus",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.AuthDiagnostics.RefreshFailureClassifier",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.AuthDiagnostics.BackupBeforeClear",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.AuthDiagnostics.ProbeLaunchUsesEffectiveHome",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCodexCliRunner_AuthDiagnostics_ProbeLaunchUsesEffectiveHome::RunTest(const FString& Parameters)
{
	...
	FScopedEnvironmentVariable MachineEnvOverride(TEXT("UNREALCLAUDE_CODEX_HOME"), FString());
	FScopedEnvironmentVariable OpenAiKeyOverride(TEXT("OPENAI_API_KEY"), FString());
	FScopedEnvironmentVariable ScenarioOverride(TEXT("UNREALCLAUDE_FAKE_APP_SERVER_SCENARIO"), TEXT("always_success"));
	FScopedEnvironmentVariable CodexHomeLogOverride(TEXT("UNREALCLAUDE_FAKE_APP_SERVER_CODEX_HOME_LOG"), CodexHomeLogPath);
	FScopedCodexHomeOverride CodexHomeOverride(CodexHome);
	FScopedCodexAuthModeOverride AuthModeOverride(EUnrealClaudeCodexAuthMode::Auto);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.AuthDiagnostics.StatusTextMapping",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.PersistentTransport.WorkspaceWriteSandboxPolicyTruth",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.PersistentTransport.ExplicitExpertSandboxPolicyTruth",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.PersistentTransport.TimeoutRecoveryStartsFreshTransport",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FCodexCliRunner_PersistentTransport_ExportRestartSurvivalStateFallsBackToPersistedSnapshot,
	"UnrealClaude.CodexCliRunner.PersistentTransport.ExportRestartSurvivalStateFallsBackToPersistedSnapshot",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCodexCliRunner_PersistentTransport_TimeoutRecoveryStartsFreshTransport::RunTest(const FString& Parameters)
{
	...
	FScopedEnvironmentVariable MachineEnvOverride(TEXT("UNREALCLAUDE_CODEX_HOME"), FString());
	FScopedEnvironmentVariable OpenAiKeyOverride(TEXT("OPENAI_API_KEY"), FString());
	FScopedEnvironmentVariable ScenarioOverride(TEXT("UNREALCLAUDE_FAKE_APP_SERVER_SCENARIO"), TEXT("turn_timeout_once_then_success"));
	FScopedEnvironmentVariable MarkerOverride(TEXT("UNREALCLAUDE_FAKE_APP_SERVER_MARKER"), MarkerPath);
	FScopedEnvironmentVariable CounterOverride(TEXT("UNREALCLAUDE_FAKE_APP_SERVER_COUNTER"), CounterPath);
	FScopedCodexHomeOverride CodexHomeOverride(CodexHome);
	FScopedCodexAuthModeOverride AuthModeOverride(EUnrealClaudeCodexAuthMode::Auto);
	FScopedPersistentAppServerOverride PersistentTransportOverride(true);
	FScopedCodexLaunchOverride LaunchOverride(NodeExecutablePath, ScriptPath);
...
bool FCodexCliRunner_PersistentTransport_ExportRestartSurvivalStateFallsBackToPersistedSnapshot::RunTest(const FString& Parameters)
{
	...
	FScopedEnvironmentVariable OpenAiKeyOverride(TEXT("OPENAI_API_KEY"), FString());
	FScopedEnvironmentVariable ScenarioOverride(TEXT("UNREALCLAUDE_FAKE_APP_SERVER_SCENARIO"), TEXT("always_success"));
	FScopedEnvironmentVariable MarkerOverride(TEXT("UNREALCLAUDE_FAKE_APP_SERVER_MARKER"), FString());
	FScopedEnvironmentVariable CounterOverride(TEXT("UNREALCLAUDE_FAKE_APP_SERVER_COUNTER"), FString());
	FScopedCodexHomeOverride CodexHomeOverride(CodexHome);
	FScopedCodexAuthModeOverride AuthModeOverride(EUnrealClaudeCodexAuthMode::Auto);
	FScopedPersistentAppServerOverride PersistentTransportOverride(true);
	FScopedCodexLaunchOverride LaunchOverride(NodeExecutablePath, ScriptPath);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"UnrealClaude.CodexCliRunner.PersistentTransport.ResetConversationRestartsProcess",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FCodexCliRunner_PersistentTransport_ScopedMcpFilterChangeRestartsProcess,
	"UnrealClaude.CodexCliRunner.PersistentTransport.ScopedMcpFilterChangeRestartsProcess",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FCodexCliRunner_PersistentTransport_ResetConversationRestartsProcess::RunTest(const FString& Parameters)
{
	...
	FScopedEnvironmentVariable MarkerOverride(TEXT("UNREALCLAUDE_FAKE_APP_SERVER_MARKER"), FString());
	FScopedEnvironmentVariable CounterOverride(TEXT("UNREALCLAUDE_FAKE_APP_SERVER_COUNTER"), CounterPath);
	FScopedEnvironmentVariable AllowedToolsLogOverride(TEXT("UNREALCLAUDE_FAKE_APP_SERVER_ALLOWED_TOOLS_LOG"), AllowedToolsLogPath);
	FScopedCodexHomeOverride CodexHomeOverride(CodexHome);
	FScopedCodexAuthModeOverride AuthModeOverride(EUnrealClaudeCodexAuthMode::Auto);
	FScopedPersistentAppServerOverride PersistentTransportOverride(true);
	FScopedCodexLaunchOverride LaunchOverride(NodeExecutablePath, ScriptPath);
...
bool FCodexCliRunner_PersistentTransport_ScopedMcpFilterChangeRestartsProcess::RunTest(const FString& Parameters)
{
	...
	InputConfig.AllowedTools.Add(TEXT("mcp__unrealclaude__enhanced_input"));
	...
	AnimationConfig.AllowedTools.Add(TEXT("mcp__unrealclaude__anim_blueprint_modify"));
