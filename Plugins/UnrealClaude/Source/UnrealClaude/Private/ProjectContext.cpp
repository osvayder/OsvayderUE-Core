...
void FProjectContextManager::RefreshContext()
{
	...
	UE_LOG(LogUnrealClaude, Log, TEXT("  - Memory docs: %d"), CachedContext.MemoryDocs.Num());
}
...
void FProjectContextManager::LoadProjectMemoryDocs()
{
	...
	const UUnrealClaudeSettings* MemSettings = UUnrealClaudeSettings::Get();
	FString MemoryRelPath = (MemSettings && !MemSettings->ProjectMemoryPath.IsEmpty()) ? MemSettings->ProjectMemoryPath : TEXT("Docs/UnrealClaude");
	...
	const int32 MaxDocs = FMath::Min(
		OrderedFiles.Num(),
		UnrealClaudeConstants::Context::MaxMemoryDocsToInclude
	);
	...
	for (int32 Index = 0; Index < MaxDocs; ++Index)
	{
		...
		if (MemoryDoc.Content.Len() > UnrealClaudeConstants::Context::MaxMemoryDocCharacters)
		{
			MemoryDoc.Content = MemoryDoc.Content.Left(UnrealClaudeConstants::Context::MaxMemoryDocCharacters);
