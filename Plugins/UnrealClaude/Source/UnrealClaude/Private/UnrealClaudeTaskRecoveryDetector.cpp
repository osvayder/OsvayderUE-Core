...
namespace
{
	...
	bool IsRestartSurvivalPhaseActiveForSuppression(const EUnrealClaudeRestartSurvivalPhase Phase)
	{
		return Phase == EUnrealClaudeRestartSurvivalPhase::Detaching
			...
			|| Phase == EUnrealClaudeRestartSurvivalPhase::AwaitingReattach;
	}
	...
	bool HasRestartSurvivalFailure(const FUnrealClaudeRestartSurvivalState& State)
	{
		return State.Phase == EUnrealClaudeRestartSurvivalPhase::FailedTerminal
			|| State.DetachedTerminalOutcome.Equals(TEXT("failed"), ESearchCase::IgnoreCase)
			|| !State.FailureReason.IsEmpty();
	}
}

FTaskRecoveryDetectionResult FUnrealClaudeTaskRecoveryDetector::ClassifyPlan(
	const FUnrealClaudeActivePlan& Plan,
	const FDateTime& NowUtc,
	const TArray<FDateTime>& CrashReportMtimes,
	const TOptional<FUnrealClaudeRestartSurvivalState>& RestartSurvivalState,
	int64 InterruptionThresholdSeconds)
{
	...
	{
		const FUnrealClaudeRestartSurvivalState& RestartState = RestartSurvivalState.GetValue();
...
TArray<FDateTime> FUnrealClaudeTaskRecoveryDetector::CollectCrashReportMtimes()
{
...
void FUnrealClaudeTaskRecoveryDetector::RunStartupProbe()
{
	...
	TOptional<FUnrealClaudeRestartSurvivalState> RestartState;
	const FString RestartStatePath = FUnrealClaudeRestartSurvivalManager::GetStatePath();
	if (IFileManager::Get().FileExists(*RestartStatePath))
	{
		FUnrealClaudeRestartSurvivalState LoadedRestartState;
		FString RestartLoadError;
		if (FUnrealClaudeRestartSurvivalManager::LoadState(LoadedRestartState, RestartLoadError))
		{
	...
	const FTaskRecoveryDetectionResult Result =
		FUnrealClaudeTaskRecoveryDetector::ClassifyPlan(Plan, NowUtc, CrashMtimes, RestartState);
	...
	if (Result.bDialogSuppressed)
	{
		...
		if (bHadInterruptionMetadata)
		{
			...
			FString SaveError;
			if (!FUnrealClaudeRelayAgentManager::SaveActivePlan(Plan, SaveError))
			{
				UE_LOG(LogUnrealClaude, Warning,
		...
		{
			const FUnrealClaudeRestartSurvivalState& State = RestartState.GetValue();
			Payload->SetStringField(TEXT("restart_phase"), UnrealClaudeRestartSurvivalPhaseToString(State.Phase));
