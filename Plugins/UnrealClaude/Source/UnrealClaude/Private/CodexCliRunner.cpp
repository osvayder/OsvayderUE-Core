...
namespace
{
	...
	constexpr TCHAR CodexPersistentServiceName[] = TEXT("unrealclaude_editor");
	...
	FString GetMachineEnvOverrideCodexHomePath()
	{
		return NormalizeConfiguredCodexHomePath(FPlatformMisc::GetEnvironmentVariable(TEXT("UNREALCLAUDE_CODEX_HOME")));
	}
...
#if PLATFORM_WINDOWS
		AddUniqueCodexHomePath(Candidates, TEXT("C:/Users/user/.codex-cli"));
#endif
	...
	FCodexHomeResolution ResolveConfiguredSharedCodexHome()
	{
		...
		const FString ExplicitOverride = Settings
			...
			: FString();
		const FString MachineEnvOverride = GetMachineEnvOverrideCodexHomePath();
	...
	FString BuildSharedCodexHomeResolutionNote(const FCodexHomeResolution& Resolution)
	{
		...
		{
			return FString::Printf(
				TEXT("Shared Codex home is pinned by UNREALCLAUDE_CODEX_HOME to %s."),
				*Resolution.ConfiguredHomePath);
		}
		...
		if (Resolution.ResolutionSource.Equals(TEXT("machine_standard_candidate"), ESearchCase::IgnoreCase))
		{
			...
			{
				return FString::Printf(
					TEXT("The default Codex home at %s did not contain a known file-backed credential artifact, so UnrealClaude selected the detected machine-standard Codex home at %s."),
					*Resolution.DefaultHomePath,
					*Resolution.ConfiguredHomePath);
			}

			return FString::Printf(
				TEXT("UnrealClaude selected the detected machine-standard Codex home at %s."),
				*Resolution.ConfiguredHomePath);
		}
	...
	{
		return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealClaude"), TEXT("codex_api_key_env_home"));
	}

	FString GetIsolatedBrowserVerifyCodexHome()
	{
		return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealClaude"), TEXT("codex_browser_verify_home"));
	}

	FString GetPersistentCodexThreadStatePath()
	{
		return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealClaude"), TEXT("codex_persistent_thread.json"));
	}
	...
	FString GetPreferredRipgrepExecutablePath()
	{
		...
		const FString ExplicitOverride = FPlatformMisc::GetEnvironmentVariable(TEXT("UNREALCLAUDE_RG_PATH")).TrimStartAndEnd();
		if (!ExplicitOverride.IsEmpty())
		{
...
#if PLATFORM_WINDOWS
		CandidateExecutablePaths.Add(TEXT("C:/Users/user/AppData/Local/Programs/cursor/resources/app/node_modules/@vscode/ripgrep/bin/rg.exe"));
		CandidateExecutablePaths.Add(TEXT("C:/Users/user/AppData/Local/Programs/cursor/resources/app/extensions/cursor-agent/dist/claude-agent-sdk/vendor/ripgrep/rg.exe"));
		CandidateExecutablePaths.Add(TEXT("C:/Program Files/Microsoft VS Code/resources/app/node_modules/@vscode/ripgrep/bin/rg.exe"));
		CandidateExecutablePaths.Add(TEXT("C:/Users/user/AppData/Local/Programs/Microsoft VS Code/resources/app/node_modules/@vscode/ripgrep/bin/rg.exe"));
#endif
	...
	bool EnsureRipgrepShim(FString& OutShimDirectory, FString& OutRealExecutablePath)
	{
		...
		OutShimDirectory = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealClaude"), TEXT("LocalTools"), TEXT("bin"));
		...
		const FString WrapperScript =
			TEXT("$realRg = $env:UNREALCLAUDE_REAL_RG\n")
			TEXT("if ([string]::IsNullOrWhiteSpace($realRg) -or -not (Test-Path $realRg)) {\n")
			TEXT("    Write-Error 'UNREALCLAUDE_REAL_RG is not set to a valid executable path.'\n")
			TEXT("    exit 9009\n")
			TEXT("}\n")
	...
	FString BuildStandardCodexEnvSetup(const FCodexEnvSetupOptions& Options = FCodexEnvSetupOptions())
	{
		...
		if (!PreferredRipgrepPath.IsEmpty())
		{
			...
			EnvSetup += QuoteCmdEnv(TEXT("UNREALCLAUDE_REAL_RG"), PreferredRipgrepPath);
		...
		FString& OutDiagnostic)
	{
		...
		while (FPlatformTime::Seconds() < Deadline)
		{
			...
			if (TryExtractJsonlMessage(InOutBuffer, CompleteLine))
			{
				...
				{
					UE_LOG(LogUnrealClaude, Warning, TEXT("Ignoring non-JSON Codex app-server output during Browser Verify: %s"), *CompleteLine);
					continue;
				}
			...
			for (;;)
			{
				...
				if (!OutputChunk.IsEmpty())
				{
					...
					while (TryExtractJsonlMessage(PendingBuffer, CompleteLine))
					{
						...
						if (!FJsonSerializer::Deserialize(Reader, JsonMessage) || !JsonMessage.IsValid())
						{
							UE_LOG(LogUnrealClaude, Warning, TEXT("Ignoring non-JSON Codex Browser Verify monitor output: %s"), *CompleteLine);
							continue;
						}
						...
						if (bSuccess)
						{
							UE_LOG(LogUnrealClaude, Log, TEXT("Codex Browser Verify completed for loginId %s."), *LoginId);
						}
						else
						{
							UE_LOG(LogUnrealClaude, Warning, TEXT("Codex Browser Verify failed for loginId %s: %s"), *LoginId, *ErrorText);
						}
				...
				else if (!FPlatformProcess::IsProcRunning(ProcessHandle))
				{
					...
					{
						UE_LOG(LogUnrealClaude, Log, TEXT("Codex Browser Verify app-server exited. Credential artifact is now present at %s."), *CredentialArtifactPath);
					}
					else
					{
						UE_LOG(LogUnrealClaude, Log, TEXT("Codex Browser Verify app-server exited before any credential artifact was detected."));
					}
	...
	{
		EUnrealClaudeCodexAuthMode Mode = EUnrealClaudeCodexAuthMode::Auto;
		FString ModeName = TEXT("auto");
		FString ApiKeyEnvVarName = UUnrealClaudeSettings::GetDefaultCodexApiKeyEnvVarName();
	...
	FCodexEnvSetupOptions BuildEnvSetupOptionsForAuthContext(const FCodexAuthContext& AuthContext)
	{
		...
		else if (AuthContext.Mode == EUnrealClaudeCodexAuthMode::BrowserVerify)
		{
	...
	FCodexAuthContext ResolveCodexAuthContext()
	{
		...
		if (Context.Mode == EUnrealClaudeCodexAuthMode::ApiKeyEnvVar)
		{
		...
		if (Context.Mode == EUnrealClaudeCodexAuthMode::CliTerminal)
		{
		...
		if (Context.Mode == EUnrealClaudeCodexAuthMode::BrowserVerify)
		{
			Context.bUseIsolatedCodexHome = true;
		...
		FString& OutDiagnostic)
	{
		...
		if (AuthContext.Mode == EUnrealClaudeCodexAuthMode::ApiKeyEnvVar)
		{
		...
		if (AuthContext.Mode == EUnrealClaudeCodexAuthMode::BrowserVerify)
		{
	...
	FString BuildCodexAuthDetailForDiagnostics(const FCodexAuthContext& AuthContext, const FString& EffectiveHomePath, const FString& CredentialArtifactPath)
	{
		...
		if (AuthContext.bHasInheritedOpenAiApiKey && AuthContext.Mode == EUnrealClaudeCodexAuthMode::Auto)
		{
	...
	bool TryBuildCodexMcpOverrideArgs(const FAgentRequestConfig& Config, FString& OutArgs, FString& OutDiagnostic)
	{
		...
		{
			OutDiagnostic += TEXT("Could not resolve UnrealClaude plugin directory.\n");
			return false;
		}
		...
		const UUnrealClaudeSettings* Settings = UUnrealClaudeSettings::Get();
		const int32 McpPort = Settings ? Settings->MCPServerPort : UnrealClaudeConstants::MCPServer::DefaultPort;
		const FString McpUrl = FString::Printf(TEXT("http://localhost:%d"), McpPort);

		if (!TryAppendLiteralConfigOverride(OutArgs, TEXT("mcp_servers.unrealclaude.command"), TEXT("node"), OutDiagnostic))
		{
			return false;
		}
		...
		OutArgs += FString::Printf(TEXT("-c \"mcp_servers.unrealclaude.args=['%s']\" "), *BridgePath);

		if (!TryAppendLiteralConfigOverride(OutArgs, TEXT("mcp_servers.unrealclaude.env.UNREAL_MCP_URL"), McpUrl, OutDiagnostic))
		{
			return false;
		}
		...
		if (!Config.bEnableUnrealMcpBridge)
		{
			...
			if (ScopedToolNames.Num() > 0)
			{
				if (!TryAppendLiteralConfigOverride(
						OutArgs,
						TEXT("mcp_servers.unrealclaude.env.UNREAL_MCP_ALLOWED_TOOLS"),
		...
		if (Settings && Settings->bEnableOsvayderEye)
		{
			if (!Settings->OsvayderEyeServerPath.FilePath.IsEmpty())
			{
				...
				if (!TryAppendLiteralConfigOverride(OutArgs, TEXT("mcp_servers.unrealclaude.env.EYE_SERVER_PATH"), EyeServerPath, OutDiagnostic))
				{
					return false;
				}
			}
			...
			if (!Settings->OsvayderEyePythonPath.FilePath.IsEmpty())
			{
				...
				if (!TryAppendLiteralConfigOverride(OutArgs, TEXT("mcp_servers.unrealclaude.env.EYE_PYTHON_PATH"), EyePythonPath, OutDiagnostic))
				{
					return false;
				}
			}
	...
	{
		FString RootDir = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealClaude"), TEXT("CodexAuthDiagnostics"));
...
FAgentBackendCapabilities FCodexCliRunner::GetCapabilities() const
{
	...
	Capabilities.Backend = EUnrealClaudeProviderBackend::CodexCli;
...
FAgentBackendStatus FCodexCliRunner::GetStatus() const
{
	...
	if (AuthContext.Mode == EUnrealClaudeCodexAuthMode::CliTerminal)
	{
	...
	if (AuthContext.Mode == EUnrealClaudeCodexAuthMode::BrowserVerify)
	{
		...
		else
		{
			...
			Status.Detail = FString::Printf(
				TEXT("Codex CLI detected at %s using %s, but Browser Verify login has not completed yet. Use the Browser Verify button in the UnrealClaude widget or run `UnrealClaude.LaunchCodexBrowserVerify`."),
				*Status.ExecutablePath,
				*BuildCodexSelectionSummary());
		}
...
FCodexCliRunner::FCodexAuthDiagnostics FCodexCliRunner::GetAuthDiagnostics()
{
	...
	else if (AuthContext.Mode == EUnrealClaudeCodexAuthMode::Auto && AuthContext.bHasInheritedOpenAiApiKey)
	{
...
bool FCodexCliRunner::CanLaunchBrowserVerify(FString& OutReason)
{
	...
	if (!Settings)
	{
		OutReason = TEXT("UnrealClaude settings are not available.");
		return false;
	}

	if (Settings->GetCodexAuthMode() != EUnrealClaudeCodexAuthMode::BrowserVerify)
	{
	...
	OutReason = TEXT("Launch Codex Browser Verify by requesting a browser login URL from `codex app-server` and opening it from the UnrealClaude UI.");
	return true;
}
...
bool FCodexCliRunner::LaunchCodexRelogin(FString& OutStatusMessage)
{
	...
	if (!TryBuildCodexAppServerLaunchSpec(
		EffectiveHomePath,
		UUnrealClaudeSettings::Get() && UUnrealClaudeSettings::Get()->ShouldClearProxyEnvForBrowserVerify(),
		Executable,
		Args,
		LaunchDiagnostic))
	{
	...
	ClientInfo->SetStringField(TEXT("name"), TEXT("unrealclaude_relogin"));
	...
	OutStatusMessage = FString::Printf(
		...
		*EffectiveHomePath);
	UE_LOG(LogUnrealClaude, Log, TEXT("%s"), *OutStatusMessage);
	return true;
}
...
bool FCodexCliRunner::ProbeBackendAuth(FString& OutStatusMessage)
{
	...
	FString LaunchDiagnostic;
	{
		...
		if (AuthContext.Mode == EUnrealClaudeCodexAuthMode::ApiKeyEnvVar)
		{
			...
			AuthContext.Mode == EUnrealClaudeCodexAuthMode::CliTerminal)
		{
			EnvSetup += QuoteCmdEnv(TEXT("OPENAI_API_KEY"), FString());
		}
	...
	TSharedRef<FJsonObject> InitializeParams = MakeShared<FJsonObject>();
	TSharedRef<FJsonObject> ClientInfo = MakeShared<FJsonObject>();
	ClientInfo->SetStringField(TEXT("name"), TEXT("unrealclaude_auth_probe"));
	ClientInfo->SetStringField(TEXT("version"), TEXT("1.0"));
	InitializeParams->SetObjectField(TEXT("clientInfo"), ClientInfo);
	...
	ThreadParams->SetStringField(TEXT("serviceName"), TEXT("unrealclaude_auth_probe"));
...
bool FCodexCliRunner::LaunchBrowserVerifyLogin(FString& OutStatusMessage)
{
	...
	TSharedRef<FJsonObject> InitializeParams = MakeShared<FJsonObject>();
	TSharedRef<FJsonObject> ClientInfo = MakeShared<FJsonObject>();
	ClientInfo->SetStringField(TEXT("name"), TEXT("unrealclaude"));
	ClientInfo->SetStringField(TEXT("version"), TEXT("1.0"));
	InitializeParams->SetObjectField(TEXT("clientInfo"), ClientInfo);
	...
	FOnAgentProgress OnProgress)
{
	...
	{
		UE_LOG(LogUnrealClaude, Warning, TEXT("Codex CLI backend is already executing a request"));
		return false;
	}
...
bool FCodexCliRunner::CreateProcessPipes()
{
	...
	{
		UE_LOG(LogUnrealClaude, Error, TEXT("Failed to create Codex stdout pipe"));
		return false;
	}

	if (!FPlatformProcess::CreatePipe(StdInReadPipe, StdInWritePipe, true))
	{
		UE_LOG(LogUnrealClaude, Error, TEXT("Failed to create Codex stdin pipe"));
		FPlatformProcess::ClosePipe(ReadPipe, WritePipe);
		ReadPipe = nullptr;
		WritePipe = nullptr;
		return false;
...
bool FCodexCliRunner::TryLoadPersistedThreadState(FString& OutThreadId) const
{
	...
	const FString StatePath = GetPersistentThreadStatePath();
...
{
	IFileManager::Get().Delete(*GetPersistentThreadStatePath(), false, true, true);
}
...
bool FCodexCliRunner::ReadNextPersistentAppServerMessage(const int32 TimeoutMs, TSharedPtr<FJsonObject>& OutMessage, FString& OutDiagnostic)
{
	...
	while (FPlatformTime::Seconds() < Deadline)
	{
		...
		if (TryExtractJsonlMessage(PersistentSession.ReadBuffer, CompleteLine))
		{
			...
			if (!FJsonSerializer::Deserialize(Reader, JsonMessage) || !JsonMessage.IsValid())
			{
				UE_LOG(LogUnrealClaude, Warning, TEXT("Ignoring non-JSON Codex persistent app-server output: %s"), *CompleteLine);
				continue;
			}
...
bool FCodexCliRunner::EnsurePersistentAppServerReady(FString& OutDiagnostic)
{
	...
	if (const UUnrealClaudeSettings* Settings = UUnrealClaudeSettings::Get(); Settings && Settings->HasExplicitCodexProfile())
	{
		const FString ExplicitProfile = Settings->GetExplicitCodexProfile();
		if (ExplicitProfile.Contains(TEXT("\"")))
		{
	...
	ClientInfo->SetStringField(TEXT("title"), TEXT("UnrealClaude Editor"));
...
bool FCodexCliRunner::LaunchProcess(const FString& FullCommand, const FString& WorkingDir)
{
	...
	{
		UE_LOG(LogUnrealClaude, Error, TEXT("%s"), *EffectiveHomeDiagnostic);
		return false;
	}
	FString EnvSetup = BuildStandardCodexEnvSetup(EnvOptions);
	if (AuthContext.Mode == EUnrealClaudeCodexAuthMode::ApiKeyEnvVar)
	...
	else if (AuthContext.Mode == EUnrealClaudeCodexAuthMode::CliTerminal)
	{
	...
	if (!ProcessHandle.IsValid())
	{
		UE_LOG(LogUnrealClaude, Error, TEXT("Failed to launch Codex CLI process"));
		UE_LOG(LogUnrealClaude, Error, TEXT("Exe: %s"), *ActualExe);
