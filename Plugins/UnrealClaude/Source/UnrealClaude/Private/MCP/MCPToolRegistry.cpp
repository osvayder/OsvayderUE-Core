...
namespace
{
	...
	TSet<FString> PackageNamesFromLifecycleResults(const TArray<UnrealClaude::SavePipeline::FLifecyclePhaseResult>& Results)
	{
		TSet<FString> Names;
		...
		const FLifecyclePackageContext* Context = nullptr)
	{
		...
		for (const UnrealClaude::SavePipeline::FLifecyclePhaseResult& R : Outcome.Saved)
		{
		...
		for (const UnrealClaude::SavePipeline::FLifecyclePhaseResult& R : Outcome.Failed)
		{
		...
		if (Outcome.Compiled.Num() > 0 || Outcome.Failed.ContainsByPredicate(
			[](const UnrealClaude::SavePipeline::FLifecyclePhaseResult& R)
			{
				return R.Phase.Equals(TEXT("compile"), ESearchCase::IgnoreCase);
			}))
		{
			TArray<TSharedPtr<FJsonValue>> CompileJson;
			for (const UnrealClaude::SavePipeline::FLifecyclePhaseResult& R : Outcome.Compiled)
			{
				TSharedPtr<FJsonObject> Entry = MakeShared<FJsonObject>();
				Entry->SetStringField(TEXT("asset"), R.PackageName);
		...
		if (Outcome.Deferred.Num() > 0)
		{
			...
			for (const UnrealClaude::SavePipeline::FLifecyclePhaseResult& R : Outcome.Deferred)
			{
				TSharedPtr<FJsonObject> Entry = MakeShared<FJsonObject>();
				Entry->SetStringField(TEXT("asset"), R.PackageName);
		...
		// "SC inactive" from "SC absent from report".
		{
			...
			for (const UnrealClaude::SavePipeline::FLifecyclePhaseResult& R : Outcome.SourceControlWarnings)
			{
				TSharedPtr<FJsonObject> Entry = MakeShared<FJsonObject>();
				Entry->SetStringField(TEXT("asset"), R.PackageName);
		...
		const FString& ToolName)
	{
		...
		if (!FUnrealClaudeAgentTraceLog::Get().TryGetActiveCanonExecutionForBackend(BackendStatus.Backend, ActiveExecution))
		{
			return false;
		}
...
void FMCPToolRegistry::RegisterBuiltinTools()
{
	...
	UE_LOG(LogUnrealClaude, Log, TEXT("Registered %d built-in MCP tools"), Tools.Num());
			...
			&& IFileManager::Get().FileExists(*PythonPath))
		{
			...
			if (EyeCount < 0)
			{
				...
				if (IFileManager::Get().FileExists(*HttpServerPath))
				{
					...
					if (Handle.IsValid())
					{
						UE_LOG(LogUnrealClaude, Log, TEXT("OsvayderEye sidecar auto-started: %s %s"), *PythonPath, *Args);
					...
					{
						UE_LOG(LogUnrealClaude, Warning, TEXT("Failed to auto-start OsvayderEye sidecar"));
					}
				}
				else
				{
					UE_LOG(LogUnrealClaude, Log, TEXT("OsvayderEye http_server.py not found at: %s"), *HttpServerPath);
		}
	}
		else
		{
			UE_LOG(LogUnrealClaude, Log, TEXT("OsvayderEye paths not configured or files missing"));
		}
	}
	}
	else
	{
		UE_LOG(LogUnrealClaude, Log, TEXT("OsvayderEye disabled in settings"));
	}
	if (EyeCount > 0)
	{
		UE_LOG(LogUnrealClaude, Log, TEXT("Total MCP tools: %d (built-in) + %d (eye) = %d"), Tools.Num() - EyeCount, EyeCount, Tools.Num());
	}
}
	...
	FMCPToolResult& OutResult) const
{
	...
	const FAgentProviderExecutionControlManifest Manifest =
		FClaudeCodeSubsystem::Get().GetConfiguredBackendExecutionControlManifestForProfile(Profile);
...
FMCPToolResult FMCPToolRegistry::ExecuteTool(const FString& ToolName, const TSharedRef<FJsonObject>& Params)
{
	...
	{
		UE_LOG(LogUnrealClaude, Log, TEXT("Tool '%s' blocked by execution policy: %s"), *ToolName, *Result.Message);
	}
	else
	{
		...
		auto WorkOnGameThread = [ToolShared, Params](FMCPToolResult& Out)
		{
			...
			const UnrealClaude::SavePipeline::FLifecycleOutcome Outcome =
				UnrealClaude::SavePipeline::Run(SaveSpec);
		...
		if (Result.Data.IsValid())
		{
				...
				&& LifecycleObj && LifecycleObj->IsValid())
			{
				...
				if (bAutoSave)
				{
					...
					FUnrealClaudeExecutionLog::Get().AddReceipt(LifecycleReceipt);
				}
			}
		}
	}
