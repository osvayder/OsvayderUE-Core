...
struct FMCPAsyncTask
{
	...
	/** Flag indicating cancellation was requested by timeout rather than user cancel */
	FThreadSafeBool bTimeoutRequested;
	...
	TSharedPtr<FJsonObject> ToJson(bool bIncludeResult = false) const
	{
		...
		const bool bHasStarted = StartedTime > FDateTime::MinValue();
		if (bHasStarted)
		{
		...
		if (IsComplete())
		{
			...
			// Calculate duration
			FTimespan Duration = bHasStarted ? (CompletedTime - StartedTime) : FTimespan::Zero();
