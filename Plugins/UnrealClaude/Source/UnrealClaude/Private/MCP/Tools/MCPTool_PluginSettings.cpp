...
FMCPToolResult FMCPTool_PluginSettings::Execute(const TSharedRef<FJsonObject>& Params)
{
	...
	ScopeObj->SetStringField(TEXT("mode"), Settings->ScopeMode == EUnrealClaudeScopeMode::PluginOnly ? TEXT("PluginOnly") : TEXT("PluginAndProject"));
	Data->SetObjectField(TEXT("scope"), ScopeObj);

	// Verification
	TSharedPtr<FJsonObject> VerifObj = MakeShared<FJsonObject>();
	VerifObj->SetStringField(TEXT("mode"), Settings->VerificationMode == EUnrealClaudeVerificationMode::PracticalEditorFirst ? TEXT("PracticalEditorFirst") : TEXT("Mixed"));
	...
	BackendObj->SetStringField(TEXT("preferred"), Settings->PreferredBackend == EUnrealClaudeProviderBackend::CodexCli ? TEXT("CodexCli") : TEXT("ClaudeCli"));
	BackendObj->SetStringField(TEXT("current"), BackendStatus.DisplayName);
	BackendObj->SetStringField(TEXT("backend"), BackendStatus.Backend == EUnrealClaudeProviderBackend::CodexCli ? TEXT("CodexCli") : TEXT("ClaudeCli"));
	...
	{
		FClaudeSessionManager TempSessionManager;
		BackendObj->SetStringField(TEXT("claude_persistent_session_id"),
			TempSessionManager.ReadPersistentSessionId(EUnrealClaudeProviderBackend::ClaudeCli));
	}
	...
	BackendObj->SetStringField(TEXT("plugin_preflight_launcher"), FUnrealClaudeModule::GetPreflightLauncherPath());

	const FAgentProviderExecutionControlManifest ExecutionControlManifest = FClaudeCodeSubsystem::Get().GetConfiguredBackendExecutionControlManifest();
	const FAgentProviderExecutionControlManifest BoundedExecutionControlManifest =
		FClaudeCodeSubsystem::Get().GetConfiguredBackendExecutionControlManifestForProfile(EAgentExecutionRunProfile::BoundedPluginMutation);
	const FAgentProviderExecutionControlManifest ReadOnlyExecutionControlManifest =
		FClaudeCodeSubsystem::Get().GetConfiguredBackendExecutionControlManifestForProfile(EAgentExecutionRunProfile::ReadOnlyDiagnostic);
	...
	auto BuildSessionMetadataObject = [](const FAgentSessionMetadata& Metadata)
	{
		...
		SessionMetadataObj->SetStringField(TEXT("backend"), UnrealClaudeProviderBackendToString(Metadata.Backend));
	...
	SessionObj->SetObjectField(
		...
		BuildSessionMetadataObject(
			FClaudeCodeSubsystem::Get().DescribeSavedSessionsForProfile(EAgentExecutionRunProfile::ExplicitExpertOptIn).CurrentProviderSession));
	...
	BackendObj->SetObjectField(TEXT("restart_survival"), FUnrealClaudeRestartSurvivalManager::BuildReadbackJson());
	...
	PathsObj->SetStringField(TEXT("report_artifacts_dir"), FUnrealClaudeReportArtifacts::GetReportsDirectory());
	PathsObj->SetStringField(TEXT("report_artifact_summary_glob"), FUnrealClaudeReportArtifacts::GetReportSummaryGlob());
	Data->SetObjectField(TEXT("paths"), PathsObj);

	// Logging
	Data->SetBoolField(TEXT("verbose_logging"), Settings->bVerboseToolLogging);
	Data->SetStringField(TEXT("execution_log_path"), FUnrealClaudeExecutionLog::Get().GetSessionLogPath());
	Data->SetStringField(TEXT("agent_trace_log_path"), FUnrealClaudeAgentTraceLog::Get().GetTraceLogPath());

	return FMCPToolResult::Success(TEXT("Current UnrealClaude settings"), Data);
}
