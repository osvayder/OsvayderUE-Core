...
public:
	...
	virtual FMCPToolInfo GetInfo() const override
	{
		...
		Info.Parameters = {
			...
			FMCPToolParameter(TEXT("execution_profile"), TEXT("string"),
				TEXT("Optional execution profile for governed surfaces: configured_default_runtime, read_only_diagnostic, bounded_plugin_mutation, or explicit_expert_opt_in. Propagates to the submitted tool when nested params do not set one."), false),
	...
	virtual FMCPToolResult Execute(const TSharedRef<FJsonObject>& Params) override
	{
		...
		// Propagate the selected execution profile into the submitted tool call when needed.
		FString ExecutionProfile;
		if (Params->TryGetStringField(TEXT("execution_profile"), ExecutionProfile)
			&& !ExecutionProfile.IsEmpty()
			&& !ToolParams->HasField(TEXT("execution_profile")))
		{
			ToolParams->SetStringField(TEXT("execution_profile"), ExecutionProfile);
		}
		...
		for (const TCHAR* InternalBoolField : InternalBoolFields)
		{
			...
			if (Params->TryGetBoolField(InternalBoolField, bInternalValue)
				&& !ToolParams->HasField(InternalBoolField))
			{
				ToolParams->SetBoolField(InternalBoolField, bInternalValue);
			}
		}

		FMCPToolResult PolicyDenyResult;
		if (TaskQueue->TryBuildSubmissionPolicyDenyResult(ToolName, ToolParams.ToSharedRef(), PolicyDenyResult))
		{
