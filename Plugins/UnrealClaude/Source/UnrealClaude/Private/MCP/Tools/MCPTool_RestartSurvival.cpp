...
FMCPToolResult FMCPTool_RestartSurvival::Execute(const TSharedRef<FJsonObject>& Params)
{
	...
	if (FUnrealClaudeRestartSurvivalManager::DescribeCurrentState(ExistingState))
	{
		const bool bCanReplaceExistingState =
			ExistingState.Phase == EUnrealClaudeRestartSurvivalPhase::FailedTerminal
			|| (ExistingState.Phase == EUnrealClaudeRestartSurvivalPhase::Reattached && !ExistingState.bProviderThreadResumePending);
	...
	FString SaveError;
	if (!FUnrealClaudeRestartSurvivalManager::SavePreparedRestoreRequest(Request, SaveError))
	{
	...
	Data->SetStringField(TEXT("backend"), UnrealClaudeProviderBackendToString(Request.Backend));
	...
	Data->SetStringField(TEXT("prepared_request_path"), FUnrealClaudeRestartSurvivalManager::GetPreparedRestoreRequestPath());
