...
UENUM()
enum class EUnrealClaudeScopeMode : uint8
{
	/** Plugin files only (Plugins/UnrealClaude, Docs/UnrealClaude, AgentBridge) */
...
UENUM()
enum class EUnrealClaudeVerificationMode : uint8
{
...
UENUM()
enum class EUnrealClaudeCodexAuthMode : uint8
{
...
UENUM()
enum class EUnrealClaudeCodexWorkMode : uint8
{
...
UENUM()
enum class EUnrealClaudeCodexReasoningEffort : uint8
{
...
UENUM()
enum class EUnrealClaudeCodexVerbosity : uint8
{
	ModelDefault UMETA(DisplayName="Model Default"),
	Low UMETA(DisplayName="Low"),
	Medium UMETA(DisplayName="Medium"),
...
UENUM()
enum class EUnrealClaudeCodexSpeedMode : uint8
{
	Standard UMETA(DisplayName="Standard"),
	Fast UMETA(DisplayName="Fast"),
};

/** Requested Claude CLI effort level вЂ” mirrors Codex Speed/Work-Mode pattern (plan 623). */
UENUM()
enum class EUnrealClaudeClaudeEffortLevel : uint8
{
...
UENUM()
enum class EUnrealClaudeDictationLanguage : uint8
{
...
/**
 * UnrealClaude Plugin Settings
 * Configurable in Editor: Project Settings в†’ Plugins в†’ UnrealClaude
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="UnrealClaude"))
...
public:
	...
	// в”Ђв”Ђв”Ђ Scope / Safety в”Ђв”Ђв”Ђ

	/** Plugin write scope policy */
	UPROPERTY(config, EditAnywhere, Category="General", meta=(DisplayName="Write Scope", ToolTip="How far UnrealClaude is allowed to write: only plugin-owned files, or plugin plus project files."))
	EUnrealClaudeScopeMode ScopeMode = EUnrealClaudeScopeMode::PluginOnly;

	// в”Ђв”Ђв”Ђ Verification Policy в”Ђв”Ђв”Ђ

	/** How the plugin verifies its operations */
	UPROPERTY(config, EditAnywhere, Category="Verification", meta=(DisplayName="Verification Mode", ToolTip="Choose whether UnrealClaude should prefer short editor checks or a mixed verification style."))
	EUnrealClaudeVerificationMode VerificationMode = EUnrealClaudeVerificationMode::PracticalEditorFirst;
	...
	// в”Ђв”Ђв”Ђ Architecture Defaults в”Ђв”Ђв”Ђ
	...
	UPROPERTY(config, EditAnywhere, Category="Paths", meta=(DisplayName="Project Memory Path", ToolTip="Relative path to the UnrealClaude memory/docs folder inside the project."))
	FString ProjectMemoryPath = TEXT("Docs/UnrealClaude");
	...
	// в”Ђв”Ђв”Ђ MCP Server в”Ђв”Ђв”Ђ

	/** Port for UnrealClaude MCP HTTP server */
	UPROPERTY(config, EditAnywhere, Category="MCP", meta=(DisplayName="MCP Server Port", ClampMin=1024, ClampMax=65535, ToolTip="HTTP port used by the built-in UnrealClaude MCP server. Editor restart required after changes."))
	int32 MCPServerPort = 3000;

	// в”Ђв”Ђв”Ђ OsvayderEye Integration в”Ђв”Ђв”Ђ
	...
	/** Preferred backend for editor prompts */
	UPROPERTY(config, EditAnywhere, Category="General", meta=(DisplayName="Assistant Backend", ToolTip="Choose which assistant UnrealClaude should use for normal editor prompts. Backend-specific settings stay grouped below."))
	EUnrealClaudeProviderBackend PreferredBackend = EUnrealClaudeProviderBackend::ClaudeCli;

	// в”Ђв”Ђв”Ђ Claude Assistant Widget в”Ђв”Ђв”Ђ

	/** Default model to use in Claude Assistant */
	UPROPERTY(config, EditAnywhere, Category="Claude", meta=(DisplayName="Claude Model", ToolTip="Default Claude model used when the preferred backend is Claude."))
	...
	EUnrealClaudeCodexSpeedMode DefaultCodexSpeedMode = EUnrealClaudeCodexSpeedMode::Standard;
	...
	EUnrealClaudeCodexWorkMode DefaultCodexWorkMode = EUnrealClaudeCodexWorkMode::Balanced;

	/** Manual reasoning effort used only when Codex work mode is Custom. */
	UPROPERTY(config, EditAnywhere, Category="Codex", meta=(DisplayName="Reasoning (Custom)", EditCondition="DefaultCodexWorkMode == EUnrealClaudeCodexWorkMode::Custom", ToolTip="Manual reasoning setting used only when Work Mode is Custom."))
	EUnrealClaudeCodexReasoningEffort DefaultCodexReasoningEffort = EUnrealClaudeCodexReasoningEffort::Medium;

	/** Manual response detail used only when Codex work mode is Custom. */
	UPROPERTY(config, EditAnywhere, Category="Codex", meta=(DisplayName="Response Detail (Custom)", EditCondition="DefaultCodexWorkMode == EUnrealClaudeCodexWorkMode::Custom", ToolTip="Manual response-detail setting used only when Work Mode is Custom."))
	EUnrealClaudeCodexVerbosity DefaultCodexVerbosity = EUnrealClaudeCodexVerbosity::Medium;

	/** Managed shared Codex home override. Empty means the runtime resolution ladder chooses the shared home (UNREALCLAUDE_CODEX_HOME, then a known machine-standard artifact home, then ~/.codex). This is intended for machine bring-up when terminal Codex login intentionally lives in a different home such as C:/Users/<user>/.codex-cli, not for normal day-to-day UI editing. */
	UPROPERTY(config, VisibleAnywhere, Category="Codex", meta=(DisplayName="Home Override (Bring-Up Only)", ToolTip="Advanced bring-up field. Empty means UnrealClaude uses the accepted Codex home resolution ladder. This field stays read-only in Project Settings."))
	...
	UPROPERTY(config, EditAnywhere, Category="Codex", meta=(DisplayName="Authentication", ToolTip="Choose how UnrealClaude should authenticate Codex: Auto, API env var, external CLI Terminal login, or Browser Verify. The widget diagnostics panel shows the exact effective CODEX_HOME/auth artifact and provides relogin/probe/backup-clear actions."))
	EUnrealClaudeCodexAuthMode DefaultCodexAuthMode = EUnrealClaudeCodexAuthMode::Auto;

	/** Named editor environment variable used when Codex auth mode is ApiKeyEnvVar. */
	UPROPERTY(config, EditAnywhere, Category="Codex", meta=(DisplayName="Editor Env Var For API Key", EditCondition="DefaultCodexAuthMode == EUnrealClaudeCodexAuthMode::ApiKeyEnvVar", ToolTip="Used only when Authentication is API. UnrealClaude reads this editor env var and forwards it to Codex as OPENAI_API_KEY."))
	FString DefaultCodexApiKeyEnvVar = TEXT("UNREALCLAUDE_OPENAI_API_KEY");
	...
	UPROPERTY(config, EditAnywhere, Category="Codex", meta=(DisplayName="Browser Verify: Clear Proxy Env", EditCondition="DefaultCodexAuthMode == EUnrealClaudeCodexAuthMode::BrowserVerify", ToolTip="Browser Verify only. Clears inherited proxy variables before UnrealClaude requests a browser login from Codex app-server."))
	...
	UPROPERTY(config, EditAnywhere, Category="General", meta=(DisplayName="Restore Last Session On Open", ToolTip="Automatically restore the latest saved chat/session when the UnrealClaude panel is reopened."))
	...
	EUnrealClaudeDictationLanguage DefaultDictationLanguage = EUnrealClaudeDictationLanguage::Auto;

	/** Maximum response tokens */
	UPROPERTY(config, EditAnywhere, Category="Claude", meta=(DisplayName="Claude Max Response Tokens", ClampMin=256, ClampMax=128000, ToolTip="Maximum response length requested from Claude."))
	...
	UPROPERTY(config, EditAnywhere, Category="Claude", meta=(DisplayName="Effort Level", ToolTip="Reasoning depth forwarded to Claude CLI via --effort. 'Default' skips the flag. Mirrors the Codex Speed/Work-Mode pattern."))
	EUnrealClaudeClaudeEffortLevel DefaultClaudeEffortLevel = EUnrealClaudeClaudeEffortLevel::Default;
	...
	UPROPERTY(config, EditAnywhere, Category="Claude", meta=(DisplayName="Persistent Session", ToolTip="Keep one Claude CLI conversation alive across prompts via --session-id/--resume, instead of starting a fresh conversation every time. Required for multi-turn memory."))
	bool bClaudeUsePersistentSession = true;

	/** If true, UnrealClaude appends a one-line language directive to the Claude system prompt so the assistant replies in the user's configured dictation language. Spec 621 v3 В§2c: default is OFF вЂ” Claude CLI natively honors the input language (Step 1a evidence: `claude -p "Р—Р°РїРѕРјРЅРё С‡РёСЃР»Рѕ 42."` replied in Russian without any directive), so this setting is now opt-in for users who explicitly want to force a target language distinct from input language. */
	UPROPERTY(config, EditAnywhere, Category="Claude", meta=(DisplayName="Forward Dictation Language", ToolTip="Opt-in. Claude CLI already replies in the user's input language without help. Enable this only if you want to force a target language that differs from the input language."))
	bool bClaudeForwardLanguageToSystemPrompt = false;

	// в”Ђв”Ђв”Ђ Mutation Lifecycle в”Ђв”Ђв”Ђ

	/** When enabled, mutation tools automatically save and (for Blueprints) compile affected assets after each successful mutation. Default is on because UnrealClaude is designed for autonomous agent use. Per-call override: pass auto_save=false in the tool params for one specific call. Disable globally only if you want every mutation to leave the asset dirty in memory for manual save. */
	UPROPERTY(config, EditAnywhere, Category="Mutation Lifecycle", meta=(DisplayName="Autonomous Mutation Mode", ToolTip="When enabled, mutation tools automatically save and (for Blueprints) compile affected assets after each successful mutation. Default is on because UnrealClaude is designed for autonomous agent use. Per-call override: pass auto_save=false in the tool params for one specific call. Disable globally only if you want every mutation to leave the asset dirty in memory for manual save."))
	...
	// в”Ђв”Ђв”Ђ Accessors в”Ђв”Ђв”Ђ

	/** Get singleton settings */
	static const UUnrealClaudeSettings* Get()
	{
		return GetDefault<UUnrealClaudeSettings>();
	}

	/** Get mutable settings */
	static UUnrealClaudeSettings* GetMutable()
	{
		return GetMutableDefault<UUnrealClaudeSettings>();
	}
	...
	EUnrealClaudeCodexAuthMode GetCodexAuthMode() const
	{
	...
	FString GetConfiguredCodexAuthModeName() const
	{
		...
		{
		case EUnrealClaudeCodexAuthMode::ApiKeyEnvVar:
			return TEXT("api");

		case EUnrealClaudeCodexAuthMode::CliTerminal:
			return TEXT("cli_terminal");

		case EUnrealClaudeCodexAuthMode::BrowserVerify:
			return TEXT("browser_verify");

		case EUnrealClaudeCodexAuthMode::Auto:
		default:
			return TEXT("auto");
		}
	}
	...
	{
		return TEXT("UNREALCLAUDE_OPENAI_API_KEY");
	}
	...
	FString GetConfiguredClaudeEffortLevelName() const
	{
		...
		{
		case EUnrealClaudeClaudeEffortLevel::Low:    return TEXT("low");
		case EUnrealClaudeClaudeEffortLevel::Medium: return TEXT("medium");
		case EUnrealClaudeClaudeEffortLevel::High:   return TEXT("high");
		case EUnrealClaudeClaudeEffortLevel::XHigh:  return TEXT("xhigh");
		case EUnrealClaudeClaudeEffortLevel::Max:    return TEXT("max");
		case EUnrealClaudeClaudeEffortLevel::Default:
		default:
			return FString();
		}
	}
	...
	EUnrealClaudeDictationLanguage GetConfiguredDictationLanguage() const
	{
	...
	FString GetConfiguredCodexWorkModeName() const
	{
		...
		{
		case EUnrealClaudeCodexWorkMode::Fast:
			return TEXT("fast");

		case EUnrealClaudeCodexWorkMode::Deep:
			return TEXT("deep");

		case EUnrealClaudeCodexWorkMode::Max:
			return TEXT("max");

		case EUnrealClaudeCodexWorkMode::Custom:
			return TEXT("custom");

		case EUnrealClaudeCodexWorkMode::Balanced:
		default:
	...
	FString GetConfiguredCodexSpeedModeName() const
	{
		...
		{
		case EUnrealClaudeCodexSpeedMode::Fast:
			return TEXT("fast");

		case EUnrealClaudeCodexSpeedMode::Standard:
		default:
			return TEXT("standard");
		}
	}

	static FString GetCodexReasoningEffortName(const EUnrealClaudeCodexReasoningEffort Effort)
	{
		switch (Effort)
		{
		case EUnrealClaudeCodexReasoningEffort::Low:
			return TEXT("low");

		case EUnrealClaudeCodexReasoningEffort::Medium:
			return TEXT("medium");

		case EUnrealClaudeCodexReasoningEffort::High:
			return TEXT("high");

		case EUnrealClaudeCodexReasoningEffort::VeryHigh:
			return TEXT("xhigh");

		case EUnrealClaudeCodexReasoningEffort::ModelDefault:
		default:
			return TEXT("model_default");
		}
	}

	static FString GetCodexVerbosityName(const EUnrealClaudeCodexVerbosity Verbosity)
	{
		switch (Verbosity)
		{
		case EUnrealClaudeCodexVerbosity::Low:
			return TEXT("low");

		case EUnrealClaudeCodexVerbosity::Medium:
			return TEXT("medium");

		case EUnrealClaudeCodexVerbosity::High:
			return TEXT("high");

		case EUnrealClaudeCodexVerbosity::ModelDefault:
		default:
			return TEXT("model_default");
		}
	}

	EUnrealClaudeCodexReasoningEffort GetEffectiveCodexReasoningEffort() const
	{
		switch (DefaultCodexWorkMode)
		{
		case EUnrealClaudeCodexWorkMode::Fast:
			return EUnrealClaudeCodexReasoningEffort::Low;

		case EUnrealClaudeCodexWorkMode::Deep:
			return EUnrealClaudeCodexReasoningEffort::High;

		case EUnrealClaudeCodexWorkMode::Max:
			return EUnrealClaudeCodexReasoningEffort::VeryHigh;
		...
		case EUnrealClaudeCodexWorkMode::Balanced:
		default:
			return EUnrealClaudeCodexReasoningEffort::Medium;
		}
	}

	EUnrealClaudeCodexVerbosity GetEffectiveCodexVerbosity() const
	{
		switch (DefaultCodexWorkMode)
		{
		case EUnrealClaudeCodexWorkMode::Fast:
			return EUnrealClaudeCodexVerbosity::Low;

		case EUnrealClaudeCodexWorkMode::Deep:
			return EUnrealClaudeCodexVerbosity::High;
		...
		case EUnrealClaudeCodexWorkMode::Balanced:
		default:
			return EUnrealClaudeCodexVerbosity::Medium;
		}
	}
	...
	{
		return GetEffectiveCodexReasoningEffort() != EUnrealClaudeCodexReasoningEffort::ModelDefault;
	}

	bool HasExplicitCodexVerbosityOverride() const
	{
		return GetEffectiveCodexVerbosity() != EUnrealClaudeCodexVerbosity::ModelDefault;
	}
	...
	FString GetConfiguredModelForBackend(const EUnrealClaudeProviderBackend Backend) const
	{
	...
	FString GetConfiguredProfileLabelForBackend(const EUnrealClaudeProviderBackend Backend) const
	{
		return Backend == EUnrealClaudeProviderBackend::CodexCli
	...
	FString GetConfiguredSpeedModeLabelForBackend(const EUnrealClaudeProviderBackend Backend) const
	{
		return Backend == EUnrealClaudeProviderBackend::CodexCli
	...
	FString GetConfiguredAuthModeLabelForBackend(const EUnrealClaudeProviderBackend Backend) const
	{
		return Backend == EUnrealClaudeProviderBackend::CodexCli
	...
	{
		FString Summary;
		Summary += FString::Printf(TEXT("Scope: %s\n"), ScopeMode == EUnrealClaudeScopeMode::PluginOnly ? TEXT("PluginOnly") : TEXT("PluginAndProject"));
		Summary += FString::Printf(TEXT("Verification: %s, ShortChecks=%s, OsvayderFallback=%s\n"),
			VerificationMode == EUnrealClaudeVerificationMode::PracticalEditorFirst ? TEXT("PracticalEditorFirst") : TEXT("Mixed"),
		...
		Summary += FString::Printf(TEXT("MCP: port=%d, OsvayderEye=%s (%s)\n"),
			...
			*OsvayderEyeUrl);
		const TCHAR* BackendName = PreferredBackend == EUnrealClaudeProviderBackend::CodexCli
			? TEXT("CodexCli")
			: TEXT("ClaudeCli");
	...
	virtual FName GetSectionName() const override { return TEXT("UnrealClaude"); }

#if WITH_EDITOR
	virtual FText GetSectionText() const override { return NSLOCTEXT("UnrealClaude", "SettingsSection", "UnrealClaude"); }
	virtual FText GetSectionDescription() const override { return NSLOCTEXT("UnrealClaude", "SettingsDesc", "Configure UnrealClaude for normal editor use. Advanced bring-up, verification, and diagnostics stay clearly labeled."); }
#endif
};
