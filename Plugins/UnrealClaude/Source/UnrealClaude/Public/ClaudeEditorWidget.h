...
class STextBlock;
class FUnrealClaudeVoiceDictationService;
...
struct UNREALCLAUDE_API FUnrealClaudeActivePlanCloseoutDecision
{
	FString PlanStatus;
...
struct UNREALCLAUDE_API FUnrealClaudeCloseoutFactSnapshot
{
	...
	void MergeFrom(const FUnrealClaudeCloseoutFactSnapshot& Other);
...
struct UNREALCLAUDE_API FUnrealClaudeRunCloseoutContext
{
	FString RunId;
	FString PlanId;
...
struct UNREALCLAUDE_API FUnrealClaudeHeadlessAcceptanceRequest
{
	FString PromptFile;
	FString Prefix;
...
struct UNREALCLAUDE_API FUnrealClaudeHeadlessAcceptanceReceiptContext
{
	...
	FUnrealClaudeActivePlanCloseoutDecision CloseoutDecision;
};

struct UNREALCLAUDE_API FTaskRecoveryAutoDispatchDecision
{
...
struct UNREALCLAUDE_API FOpenEditorBuildLockCloseSafetyInputs
{
...
struct UNREALCLAUDE_API FOpenEditorBuildLockCloseSafetyDecision
{
...
public:
	...
	static void RegisterLiveWidget(const TSharedRef<SClaudeEditorWidget>& Widget);
	static TSharedPtr<SClaudeEditorWidget> GetLiveWidget();
	...
	bool StartHeadlessNewSessionAcceptanceBridge(const FUnrealClaudeHeadlessAcceptanceRequest& Request, FString& OutError);
	static FString NormalizeHeadlessAcceptancePathArgument(const FString& PathArgument);
	static void NormalizeHeadlessAcceptanceRequestPaths(FUnrealClaudeHeadlessAcceptanceRequest& Request);
	static bool ValidateHeadlessAcceptanceRequest(const FUnrealClaudeHeadlessAcceptanceRequest& Request, FString& OutError);
	static FString ComputeHeadlessAcceptancePromptHash(const FString& Prompt);
	static FString ResolveHeadlessAcceptanceReceiptPath(const FUnrealClaudeHeadlessAcceptanceRequest& Request);
	 ...
	 * so the regression test UnrealClaude.RestartSurvival.ArmEmissionPreservesEditorExitIntent
	 ...
	 * UnrealClaude.Widget.Agent2BackgroundBannerVisibilityDecision can exercise
	...
	static FString DescribeTaskLaneStatus(const FUnrealClaudeTaskLaneState& LaneState);
...
private:
	...
	void HandleConfiguredBackendChanged(EUnrealClaudeProviderBackend PreviousBackend);
	...
	void TryStartPendingRestartSurvivalAfterResponse(EUnrealClaudeProviderBackend RequestBackend);
	...
	void TryHandleClosedEditorBuildBlocker(const FClaudeStreamEvent& Event, EUnrealClaudeProviderBackend RequestBackend);
	 ...
	 * flag (set by `FUnrealClaudeAgentTraceLog::AppendObservedEvent` on the
	 ...
	 */
	void CheckSelfVerificationMismatchAndWarn(const FString& FinalResponse, EUnrealClaudeProviderBackend RequestBackend);
...
public:
	...
	static FUnrealClaudeActivePlanCloseoutDecision EvaluateActivePlanCloseout(
		const FUnrealClaudeActivePlan& Plan,
		bool bResponseSuccess);

	static FUnrealClaudeCloseoutFactSnapshot ExtractCloseoutFactsFromPlan(
		const FUnrealClaudeActivePlan& Plan);

	static FUnrealClaudeCloseoutFactSnapshot ExtractAgentTraceFactsForCloseout(
		const TArray<TSharedPtr<FJsonObject>>& TraceRecords);

	static FUnrealClaudeCloseoutFactSnapshot ExtractCurrentAgentTraceFactsForCloseout();

	static FUnrealClaudeRunCloseoutContext BuildRunCloseoutContext(
		const FUnrealClaudeActivePlan& Plan,
		const TArray<TSharedPtr<FJsonObject>>& TraceRecords,
		const FString& RunId,
		const FString& ProjectRoot);

	static FUnrealClaudeRunCloseoutContext BuildCurrentRunCloseoutContext(
		const FUnrealClaudeActivePlan& Plan,
	...
	static FUnrealClaudeActivePlanCloseoutDecision EvaluateActivePlanCloseoutWithContext(
		const FUnrealClaudeActivePlan& Plan,
		bool bResponseSuccess,
		const FUnrealClaudeRunCloseoutContext& Context);

	static FUnrealClaudeActivePlanCloseoutDecision EvaluateActivePlanCloseoutWithFacts(
		const FUnrealClaudeActivePlan& Plan,
		bool bResponseSuccess,
		const FUnrealClaudeCloseoutFactSnapshot& Facts);

	static FUnrealClaudeActivePlanCloseoutDecision EvaluateActivePlanCloseoutFromCurrentArtifacts(
		const FUnrealClaudeActivePlan& Plan,
		bool bResponseSuccess);
	...
	static FUnrealClaudeActivePlanCloseoutDecision ApplyActivePlanCloseoutSafetyGates(
		const FUnrealClaudeActivePlan& Plan,
...
private:
	 ...
	 */
	void TryHandleCompileIntentPolicy(const FClaudeStreamEvent& Event);
	...
	void UpdateActivePlanForToolBoundary(const FClaudeStreamEvent& Event, bool bStarting);
	...
	void MarkActivePlanForPostReattachVerification(const FUnrealClaudeRestartSurvivalState& RestartState);

	/** Finalize, archive, and clear the active plan once the current task reaches a terminal user answer. */
	FUnrealClaudeActivePlanCloseoutDecision FinalizeActivePlan(const FString& Response, bool bSuccess, bool bArchiveRelayArtifacts);
	...
	void OnClaudeResponse(const FString& Response, bool bSuccess, uint64 RequestGeneration, EUnrealClaudeProviderBackend RequestBackend);

	/** Handle streamed text only if it belongs to the still-active request generation */
	void OnClaudeProgress(const FString& PartialOutput, uint64 RequestGeneration, EUnrealClaudeProviderBackend RequestBackend);

	/** Handle structured stream events only if they belong to the still-active request generation */
	void OnClaudeStreamEvent(const FClaudeStreamEvent& Event, uint64 RequestGeneration, EUnrealClaudeProviderBackend RequestBackend);
	...
	void HandleDictationStatusChanged(const FUnrealClaudeVoiceDictationStatus& Status);
	...
	bool IsActiveRequestCallback(uint64 RequestGeneration, EUnrealClaudeProviderBackend RequestBackend, const TCHAR* CallbackKind) const;
...
private:
	...
	TSharedPtr<FUnrealClaudeVoiceDictationService> VoiceDictationService;
	...
	EUnrealClaudeProviderBackend LastRenderedBackend = EUnrealClaudeProviderBackend::ClaudeCli;
	...
	/**
	 * 632 wiring: check once on first Tick whether Saved/UnrealClaude/active_plan.json
	...
	EUnrealClaudeProviderBackend ActiveRequestBackend = EUnrealClaudeProviderBackend::ClaudeCli;
	...
	void HandleToolResultEvent(const FClaudeStreamEvent& Event, EUnrealClaudeProviderBackend RequestBackend);
...
public:
	 ...
	 * (UnrealClaude.Widget.CrashSafety.MalformedCodeFences_DoesNotCrash).
