...
namespace
{
	...
	TArray<FString> CollectCurrentToolInventory(FString& InventorySource)
	{
		...
		{
			const TSharedPtr<FOsvayderUEMCPServer> MCPServer = FOsvayderUEModule::Get().GetMCPServer();
		...
		if (ToolNames.Num() == 0)
		{
			ToolNames = OsvayderUEConstants::MCPServer::ExpectedTools;
		}
	...
	FAgentCppScopeDefinitionManifest BuildCppScopeDefinitionManifest()
	{
		...
		AddCategory(
			...
			},
			{
				...
				TEXT("practical proof is currently anchored to plugin module OsvayderUE"),
		...
		AddCategory(
			...
			TEXT("not_applicable"),
			TEXT("not_applicable"),
			TEXT("stop_and_escalate_if packet scope would leave Plugins/OsvayderUE or imply host-project native editing"),
			false,
			false,
			false,
			{
	...
	const FAgentProviderExecutionControlManifest& ExplicitExpertManifest)
{
...

	{
		...
		Family.MajorControlDependencies = {
			TEXT("Saved/OsvayderUE/Reports"),
			TEXT("Saved/OsvayderUE/agent_trace.jsonl"),
			TEXT("Saved/OsvayderUE/diagnostic_trace.jsonl"),
