...
FString FClipboardImageUtils::GetScreenshotDirectory()
{
	return FPaths::Combine(
		...
		TEXT("OsvayderUE"),
		OsvayderUEConstants::ClipboardImage::ScreenshotSubdirectory);
}
...
bool FClipboardImageUtils::SaveClipboardImageToFile(FString& OutFilePath, const FString& SaveDirectory)
{
	...
	{
		UE_LOG(LogOsvayderUE, Warning, TEXT("Failed to open clipboard"));
		return false;
	}
	...
	{
		::CloseClipboard();
		UE_LOG(LogOsvayderUE, Warning, TEXT("No DIB data in clipboard"));
		return false;
	}
	...
	{
		::CloseClipboard();
		UE_LOG(LogOsvayderUE, Warning, TEXT("Failed to lock clipboard data"));
		return false;
	}
	...
	{
		UE_LOG(LogOsvayderUE, Warning, TEXT("Unsupported clipboard image format: %dx%d, %d bpp"), Width, Height, BitCount);
		::GlobalUnlock(hData);
		::CloseClipboard();
		return false;
	}
	...
	{
		UE_LOG(LogOsvayderUE, Warning, TEXT("Clipboard image dimensions too large: %dx%d (max %d)"), Width, Height, MaxReasonableDimension);
		::GlobalUnlock(hData);
		::CloseClipboard();
		return false;
	}
	...
	{
		UE_LOG(LogOsvayderUE, Warning, TEXT("Unsupported clipboard DIB compression type: %d"), Header->biCompression);
		::GlobalUnlock(hData);
		::CloseClipboard();
		return false;
	}
	...
	{
		UE_LOG(LogOsvayderUE, Error, TEXT("Failed to create PNG image wrapper"));
		return false;
	}

	if (!ImageWrapper->SetRaw(Pixels.GetData(), Pixels.Num() * sizeof(FColor), Width, Height, ERGBFormat::BGRA, 8))
	{
		UE_LOG(LogOsvayderUE, Error, TEXT("Failed to set raw pixel data for PNG compression"));
		return false;
	}
	...
	{
		UE_LOG(LogOsvayderUE, Error, TEXT("PNG compression returned empty data"));
		return false;
	}
	...
	{
		UE_LOG(LogOsvayderUE, Error, TEXT("Failed to save clipboard image to: %s"), *OutFilePath);
		FExecutionReceipt FailReceipt;
		FailReceipt.Tool = TEXT("clipboard_image:save");
		FailReceipt.bSuccess = false;
		FailReceipt.TargetType = TEXT("file");
		FailReceipt.Targets.Add(OutFilePath);
		FailReceipt.Classification = TEXT("internal_state");
		FailReceipt.Status = TEXT("failed");
		FailReceipt.ErrorOrDenialReason = TEXT("SaveArrayToFile failed");
		FOsvayderUEExecutionLog::Get().AddReceipt(FailReceipt);
		return false;
	}

	UE_LOG(LogOsvayderUE, Log, TEXT("Saved clipboard image: %s (%dx%d, %lld bytes)"),
		*OutFilePath, Width, Height, CompressedData.Num());

	FExecutionReceipt Receipt;
	Receipt.Tool = TEXT("clipboard_image:save");
	Receipt.bSuccess = true;
	Receipt.TargetType = TEXT("file");
	Receipt.Targets.Add(OutFilePath);
	Receipt.Created.Add(OutFilePath);
	Receipt.Classification = TEXT("internal_state");
	Receipt.Status = TEXT("success");
	Receipt.Summary = FString::Printf(TEXT("Clipboard image saved: %s (%dx%d)"), *OutFilePath, Width, Height);
		...
		&ReturnCode, &StdOut, &StdErr) && ReturnCode == 0)
	{
		...
		{
			UE_LOG(LogOsvayderUE, Log, TEXT("Saved clipboard image via wl-paste: %s (%lld bytes)"),
				*OutFilePath, IFileManager::Get().FileSize(*OutFilePath));
			FExecutionReceipt WlReceipt;
			WlReceipt.Tool = TEXT("clipboard_image:save");
			WlReceipt.bSuccess = true;
			WlReceipt.TargetType = TEXT("file");
			WlReceipt.Targets.Add(OutFilePath);
			WlReceipt.Created.Add(OutFilePath);
			WlReceipt.Classification = TEXT("internal_state");
			WlReceipt.Status = TEXT("success");
			WlReceipt.Summary = FString::Printf(TEXT("Clipboard image saved via wl-paste: %s"), *OutFilePath);
			FOsvayderUEExecutionLog::Get().AddReceipt(WlReceipt);
			return true;
		}
	}
		...
		&ReturnCode, &StdOut, &StdErr) && ReturnCode == 0)
	{
		if (IFileManager::Get().FileExists(*OutFilePath) && IFileManager::Get().FileSize(*OutFilePath) > 0)
		{
			UE_LOG(LogOsvayderUE, Log, TEXT("Saved clipboard image via xclip: %s (%lld bytes)"),
				*OutFilePath, IFileManager::Get().FileSize(*OutFilePath));
			FExecutionReceipt XcReceipt;
			XcReceipt.Tool = TEXT("clipboard_image:save");
			XcReceipt.bSuccess = true;
			XcReceipt.TargetType = TEXT("file");
			XcReceipt.Targets.Add(OutFilePath);
			XcReceipt.Created.Add(OutFilePath);
			XcReceipt.Classification = TEXT("internal_state");
			XcReceipt.Status = TEXT("success");
			XcReceipt.Summary = FString::Printf(TEXT("Clipboard image saved via xclip: %s"), *OutFilePath);
			FOsvayderUEExecutionLog::Get().AddReceipt(XcReceipt);
			return true;
		}
	}
	...
	UE_LOG(LogOsvayderUE, Warning, TEXT("Failed to get clipboard image. Install wl-paste (wl-clipboard) or xclip."));
	{
		FExecutionReceipt FailReceipt;
		FailReceipt.Tool = TEXT("clipboard_image:save");
		FailReceipt.bSuccess = false;
		...
		FailReceipt.Classification = TEXT("internal_state");
		FailReceipt.Status = TEXT("failed");
		FailReceipt.ErrorOrDenialReason = TEXT("wl-paste and xclip both failed or unavailable");
		FOsvayderUEExecutionLog::Get().AddReceipt(FailReceipt);
	}
	return false;
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("SaveClipboardImage: saving to %s"), *OutFilePath);
	...
	{
		// Receipt for helper script creation
		{
			FExecutionReceipt HelperReceipt;
			HelperReceipt.Tool = TEXT("clipboard_image:helper_script");
			HelperReceipt.bSuccess = true;
			HelperReceipt.TargetType = TEXT("file");
			HelperReceipt.Targets.Add(ScriptPath);
			HelperReceipt.Created.Add(ScriptPath);
			HelperReceipt.Classification = TEXT("internal_state");
			HelperReceipt.Status = TEXT("success");
			HelperReceipt.Summary = FString::Printf(TEXT("Created AppleScript helper: %s"), *ScriptPath);
			FOsvayderUEExecutionLog::Get().AddReceipt(HelperReceipt);
		}
		...
		if (FPlatformProcess::ExecProcess(TEXT("/usr/bin/osascript"), *ScriptPath, &ReturnCode, &StdOut, &StdErr) && ReturnCode == 0)
		{
			...
			if (IFileManager::Get().FileExists(*OutFilePath) && IFileManager::Get().FileSize(*OutFilePath) > 0)
			{
				UE_LOG(LogOsvayderUE, Log, TEXT("Saved clipboard image via osascript: %s (%lld bytes)"),
					*OutFilePath, IFileManager::Get().FileSize(*OutFilePath));
				FExecutionReceipt MacReceipt;
				MacReceipt.Tool = TEXT("clipboard_image:save");
				MacReceipt.bSuccess = true;
				MacReceipt.TargetType = TEXT("file");
				MacReceipt.Targets.Add(OutFilePath);
				MacReceipt.Created.Add(OutFilePath);
				MacReceipt.Classification = TEXT("internal_state");
				MacReceipt.Status = TEXT("success");
				MacReceipt.Summary = FString::Printf(TEXT("Clipboard image saved via osascript: %s"), *OutFilePath);
				FOsvayderUEExecutionLog::Get().AddReceipt(MacReceipt);
				return true;
			}
		}
		UE_LOG(LogOsvayderUE, Warning, TEXT("SaveClipboardImage: osascript failed (ReturnCode=%d, StdErr='%s')"),
			ReturnCode, *StdErr);
		IFileManager::Get().Delete(*ScriptPath);
	}
	...
	UE_LOG(LogOsvayderUE, Warning, TEXT("Failed to get clipboard image on macOS"));
	{
		FExecutionReceipt FailReceipt;
		FailReceipt.Tool = TEXT("clipboard_image:save");
		FailReceipt.bSuccess = false;
		...
		FailReceipt.Classification = TEXT("internal_state");
		FailReceipt.Status = TEXT("failed");
		FailReceipt.ErrorOrDenialReason = TEXT("osascript clipboard extraction failed on macOS");
		FOsvayderUEExecutionLog::Get().AddReceipt(FailReceipt);
	}
	return false;

#else
	UE_LOG(LogOsvayderUE, Warning, TEXT("Clipboard image paste is not supported on this platform"));
	return false;
#endif
}
...
void FClipboardImageUtils::CleanupOldScreenshots(const FString& Directory, double MaxAgeSeconds)
{
	...
	if (DeletedCount > 0)
	{
		UE_LOG(LogOsvayderUE, Log, TEXT("Cleaned up %d old clipboard screenshots from %s"), DeletedCount, *Directory);

		FExecutionReceipt Receipt;
		Receipt.Tool = TEXT("clipboard_image:cleanup");
		Receipt.bSuccess = true;
		Receipt.TargetType = TEXT("file");
		Receipt.Targets.Add(Directory);
		Receipt.Deleted = DeletedPaths;
		Receipt.Classification = TEXT("internal_state");
		Receipt.Status = TEXT("success");
		Receipt.Summary = FString::Printf(TEXT("Cleaned up %d old clipboard screenshots from %s"), DeletedCount, *Directory);
		FOsvayderUEExecutionLog::Get().AddReceipt(Receipt);
	}
}
