...
namespace
{
	...
	FString TransportForReadOnlyLane(const EOsvayderUEProviderBackend Backend)
	{
	...
	FString TransportForWorkspaceWriteLane(const EOsvayderUEProviderBackend Backend)
	{
		return Backend == EOsvayderUEProviderBackend::CodexCli
			? TEXT("exec_per_message")
			: TEXT("cli_process");
	...
	FString ApprovalPolicyForReadOnlyLane(const EOsvayderUEProviderBackend Backend)
	{
		return Backend == EOsvayderUEProviderBackend::CodexCli
	...
	FString ApprovalPolicyForWorkspaceWriteLane(const EOsvayderUEProviderBackend Backend)
	{
		return Backend == EOsvayderUEProviderBackend::CodexCli
	...
	FString SandboxPolicyForReadOnlyLane(const EOsvayderUEProviderBackend)
	{
		return TEXT("provider_managed_or_unspecified");
	}

	FString SandboxPolicyForWorkspaceWriteLane(const EOsvayderUEProviderBackend Backend)
	{
		return Backend == EOsvayderUEProviderBackend::CodexCli
	...
	FString BackendLabelForMatrix(const EOsvayderUEProviderBackend Backend)
	{
	...
	const FAgentRequestConfig& Config)
{
...

	{
		...
		Row.Transport = Config.ExecutionProfile == EAgentExecutionRunProfile::ExplicitExpertOptIn
			...
			: (Status.Backend == EOsvayderUEProviderBackend::CodexCli ? TEXT("persistent_app_server_or_exec_per_message") : TEXT("cli_process"));
		Row.ApprovalPolicy = Status.Backend == EOsvayderUEProviderBackend::CodexCli ? TEXT("ask_for_approval_never") : TEXT("dangerously_skip_permissions");
		Row.SandboxPolicy = Status.Backend == EOsvayderUEProviderBackend::CodexCli ? TEXT("danger-full-access") : TEXT("provider_managed_or_unspecified");
...
TSharedPtr<FJsonObject> MakeAgentProviderExecutionControlJson(const FAgentProviderExecutionControlManifest& Manifest)
{
	...
	Object->SetStringField(TEXT("backend"), OsvayderUEProviderBackendToString(Manifest.Backend));
