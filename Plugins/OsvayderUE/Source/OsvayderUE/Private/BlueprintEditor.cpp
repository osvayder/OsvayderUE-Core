	...
	FString& OutError)
{
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("Added variable '%s' to Blueprint '%s'"),
	...
	FString& OutError)
{
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("Removed variable '%s' from Blueprint '%s'"),
		*VariableName, *Blueprint->GetName());
	return true;
}
	...
	FString& OutError)
{
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("Added function '%s' to Blueprint '%s'"),
	...
	FString& OutError)
{
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("Removed function '%s' from Blueprint '%s'"),
		*FunctionName, *Blueprint->GetName());
	return true;
}
	...
	FString& OutError)
{
	...
	// Class references (TSubclassOf<ClassName>)
	if (CleanType.StartsWith(TEXT("TSubclassOf<")) || CleanType.StartsWith(TEXT("SubclassOf<")) || CleanType.StartsWith(TEXT("Class<")))
	{
		FString InnerType = CleanType;
		InnerType.RemoveFromStart(TEXT("TSubclassOf<")); InnerType.RemoveFromStart(TEXT("SubclassOf<")); InnerType.RemoveFromStart(TEXT("Class<"));
		InnerType.RemoveFromEnd(TEXT(">"));
		InnerType.TrimStartAndEndInline();
		InnerType.RemoveFromEnd(TEXT("*"));

		// Resolve the inner class
		UClass* InnerClass = FindObject<UClass>(nullptr, *InnerType);
		if (!InnerClass)
		{
			TArray<FString> Paths = { TEXT("/Script/Engine."), TEXT("/Script/CoreUObject.") };
			for (const FString& P : Paths) { InnerClass = LoadClass<UObject>(nullptr, *(P + InnerType)); if (InnerClass) break; }
		}
		if (InnerClass)
		{
			OutPinType.PinCategory = UEdGraphSchema_K2::PC_Class;
			OutPinType.PinSubCategoryObject = InnerClass;
			return true;
		}
		OutError = FString::Printf(TEXT("Cannot resolve class for TSubclassOf: %s"), *InnerType);
		return false;
	}

	// Soft object references (TSoftObjectPtr<ClassName> or SoftObject<ClassName>)
	if (CleanType.StartsWith(TEXT("TSoftObjectPtr<")) || CleanType.StartsWith(TEXT("SoftObject<")))
	{
		FString InnerType = CleanType;
		InnerType.RemoveFromStart(TEXT("TSoftObjectPtr<")); InnerType.RemoveFromStart(TEXT("SoftObject<"));
		InnerType.RemoveFromEnd(TEXT(">"));
		InnerType.TrimStartAndEndInline();
		InnerType.RemoveFromEnd(TEXT("*"));
		...
		OutError = FString::Printf(TEXT("Cannot resolve class for TSoftObjectPtr: %s"), *InnerType);
		return false;
	}

	// Soft class references (TSoftClassPtr<ClassName> or SoftClass<ClassName>)
	if (CleanType.StartsWith(TEXT("TSoftClassPtr<")) || CleanType.StartsWith(TEXT("SoftClass<")))
	{
		FString InnerType = CleanType;
		InnerType.RemoveFromStart(TEXT("TSoftClassPtr<")); InnerType.RemoveFromStart(TEXT("SoftClass<"));
		InnerType.RemoveFromEnd(TEXT(">"));
		InnerType.TrimStartAndEndInline();
		InnerType.RemoveFromEnd(TEXT("*"));
		...
		OutError = FString::Printf(TEXT("Cannot resolve class for TSoftClassPtr: %s"), *InnerType);
		return false;
	}

	// Enum types (enum:EnumName or EEnumName)
	if (CleanType.StartsWith(TEXT("enum:")) || CleanType.StartsWith(TEXT("E")))
	{
		FString EnumName = CleanType;
		EnumName.RemoveFromStart(TEXT("enum:"));

		UEnum* FoundEnum = FindObject<UEnum>(nullptr, *EnumName);
		if (!FoundEnum)
		{
			// Search common modules
			TArray<FString> Paths = { TEXT("/Script/Engine."), TEXT("/Script/CoreUObject."), TEXT("/Script/PhysicsCore."), TEXT("/Script/InputCore.") };
			for (const FString& P : Paths)
			{
				FoundEnum = FindObject<UEnum>(nullptr, *(P + EnumName));
				if (FoundEnum) break;
			}
		}
		if (FoundEnum)
		...
		// Only fail for explicit enum: prefix — E-prefixed names could be object types
		if (CleanType.StartsWith(TEXT("enum:")))
		{
			OutError = FString::Printf(TEXT("Enum not found: %s"), *EnumName);
			return false;
		}
		// Fall through to object type resolution for E-prefixed names
	}
	...
	if (CleanType.EndsWith(TEXT("*")))
	{
		...
		// Strip common prefixes (A for actors, U for objects)
		FString CleanClassName = ClassName;

		UClass* Class = FindObject<UClass>(nullptr, *ClassName);

		// Try common module paths in order
		if (!Class)
		{
			...
			for (const FString& ModPath : ModulePaths)
			{
				Class = LoadClass<UObject>(nullptr, *(ModPath + ClassName));
				if (Class) break;
			}
		}

		// Try without A/U prefix
		if (!Class && (ClassName.StartsWith(TEXT("A")) || ClassName.StartsWith(TEXT("U"))))
		{
			FString Stripped = ClassName.RightChop(1);
			TArray<FString> ModulePaths = { TEXT("/Script/Engine."), TEXT("/Script/CoreUObject.") };
			for (const FString& ModPath : ModulePaths)
			{
				Class = LoadClass<UObject>(nullptr, *(ModPath + Stripped));
				if (Class) break;
			}
		}

		// Try project-generated classes (Blueprint-generated)
		if (!Class)
		{
			FString GamePath = FString::Printf(TEXT("/Game/%s.%s_C"), *ClassName, *ClassName);
			Class = LoadClass<UObject>(nullptr, *GamePath);
		}
		...
		OutError = FString::Printf(TEXT("Unknown class: %s. Searched Engine, CoreUObject, UMG, AIModule, NavigationSystem, PhysicsCore, EnhancedInput, GameplayTags/Tasks/Abilities modules, and /Game/ project classes."), *ClassName);
		return false;
	}
...
FString FBlueprintEditor::PinTypeToString(const FEdGraphPinType& PinType)
{
	...
	else if (PinType.PinCategory == UEdGraphSchema_K2::PC_Byte)
	{
		...
		{
			TypeName = FString::Printf(TEXT("enum:%s"), *Enum->GetName());
		}
		else
		{
			TypeName = TEXT("byte");
	...
	else if (PinType.PinCategory == UEdGraphSchema_K2::PC_Class)
	{
		if (UClass* Class = Cast<UClass>(PinType.PinSubCategoryObject.Get()))
		{
			TypeName = FString::Printf(TEXT("TSubclassOf<%s>"), *Class->GetName());
		}
	}
	else if (PinType.PinCategory == UEdGraphSchema_K2::PC_SoftObject)
	{
		if (UClass* Class = Cast<UClass>(PinType.PinSubCategoryObject.Get()))
		{
			TypeName = FString::Printf(TEXT("TSoftObjectPtr<%s>"), *Class->GetName());
		}
	}
	else if (PinType.PinCategory == UEdGraphSchema_K2::PC_SoftClass)
	{
		if (UClass* Class = Cast<UClass>(PinType.PinSubCategoryObject.Get()))
		{
			TypeName = FString::Printf(TEXT("TSoftClassPtr<%s>"), *Class->GetName());
		}
	}
	else if (PinType.PinCategory == UEdGraphSchema_K2::PC_MCDelegate)
	{
		TypeName = TEXT("mcdelegate");
	}
	else
	{
