...
void FProjectContextManager::RefreshContext()
{
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("Refreshing project context..."));
	...
	CachedContext.MemoryDocs.Empty();
	CachedContext.AssetCount = 0;
	CachedContext.BlueprintCount = 0;
	CachedContext.CppClassCount = 0;
	CachedContext.CurrentLevelName.Empty();
	...
	GatherLevelActors();
	CountAssets();
	LoadProjectMemoryDocs();

	bHasContext = true;

	UE_LOG(LogOsvayderUE, Log, TEXT("Project context gathered:"));
	UE_LOG(LogOsvayderUE, Log, TEXT("  - Source files: %d"), CachedContext.SourceFiles.Num());
	UE_LOG(LogOsvayderUE, Log, TEXT("  - UCLASS types: %d"), CachedContext.UClasses.Num());
	UE_LOG(LogOsvayderUE, Log, TEXT("  - Level actors: %d"), CachedContext.LevelActors.Num());
	UE_LOG(LogOsvayderUE, Log, TEXT("  - Total assets: %d"), CachedContext.AssetCount);
	UE_LOG(LogOsvayderUE, Log, TEXT("  - Memory docs: %d"), CachedContext.MemoryDocs.Num());
}
...
void FProjectContextManager::ScanSourceFiles()
{
	...
	{
		UE_LOG(LogOsvayderUE, Warning, TEXT("Source directory not found: %s"), *SourceDir);
		return;
	}
...
bool FProjectContextManager::ParseSingleUClass(const FString& FileContent, const FString& RelativePath, int32 UClassPos, int32& OutNextSearchPos)
{
	...
	if (ClassPos == INDEX_NONE || ClassPos > UClassPos + OsvayderUEConstants::Context::MaxUClassToClassKeywordDistance)
	{
	...
	if (InheritPos != INDEX_NONE && InheritPos < ClassNameEnd + OsvayderUEConstants::Context::MaxClassNameToInheritanceDistance)
	{
		int32 ParentEnd;
...
void FProjectContextManager::LoadProjectMemoryDocs()
{
	// Use settings-driven path, fallback to default
	const UOsvayderUESettings* MemSettings = UOsvayderUESettings::Get();
	FString MemoryRelPath = (MemSettings && !MemSettings->ProjectMemoryPath.IsEmpty()) ? MemSettings->ProjectMemoryPath : TEXT("Docs/OsvayderUE");
	const FString MemoryDocsDir = FPaths::Combine(CachedContext.ProjectPath, *MemoryRelPath);
	if (!FPaths::DirectoryExists(MemoryDocsDir))
	{
		return;
	}
	...
	// Priority docs — always included first regardless of cap
	static const TArray<FString> PriorityDocs = {
		TEXT("00_ProjectMemory.md"),
		TEXT("80_ExecutionJournal.md"),
		TEXT("90_CurrentState.md"),
	};

	// Partition: priority first, then remaining alphabetically
	TArray<FString> OrderedFiles;
	TArray<FString> RemainingFiles;

	for (const FString& PriorityName : PriorityDocs)
	{
		for (const FString& FullPath : FoundFiles)
		{
			if (FullPath.EndsWith(PriorityName))
			{
	...
	for (const FString& FullPath : FoundFiles)
	{
		if (!OrderedFiles.Contains(FullPath))
		{
			RemainingFiles.Add(FullPath);
		}
	}
	OrderedFiles.Append(RemainingFiles);

	const int32 MaxDocs = FMath::Min(
		OrderedFiles.Num(),
		OsvayderUEConstants::Context::MaxMemoryDocsToInclude
	);

	for (int32 Index = 0; Index < MaxDocs; ++Index)
	{
		const FString& FullPath = OrderedFiles[Index];
		...
		FProjectMemoryDoc MemoryDoc;
		MemoryDoc.FilePath = FullPath;
		FPaths::MakePathRelativeTo(MemoryDoc.FilePath, *CachedContext.ProjectPath);
		MemoryDoc.Content = Content;

		if (MemoryDoc.Content.Len() > OsvayderUEConstants::Context::MaxMemoryDocCharacters)
		{
			MemoryDoc.Content = MemoryDoc.Content.Left(OsvayderUEConstants::Context::MaxMemoryDocCharacters);
			MemoryDoc.Content += TEXT("\n\n[TRUNCATED FOR PROMPT CONTEXT]");
			MemoryDoc.bTruncated = true;
		}

		CachedContext.MemoryDocs.Add(MemoryDoc);
	}
}
...
FString FProjectContextManager::FormatContextForPrompt() const
{
	...
	if (CachedContext.UClasses.Num() > 0)
	{
		...
		int32 MaxClasses = FMath::Min(CachedContext.UClasses.Num(), OsvayderUEConstants::Context::MaxClassesToFormat);
	...
	if (CachedContext.SourceFiles.Num() > 0)
	{
		...
		{
			if (DirCount++ >= OsvayderUEConstants::Context::MaxDirectoriesToShow)
			{
				Context += FString::Printf(TEXT("  ... and %d more directories\n"), FilesByDir.Num() - OsvayderUEConstants::Context::MaxDirectoriesToShow);
				break;
			}
	...
	if (CachedContext.MemoryDocs.Num() > 0)
	{
		Context += TEXT("Project Memory Docs:\n");
		for (const FProjectMemoryDoc& MemoryDoc : CachedContext.MemoryDocs)
		{
			Context += FString::Printf(TEXT("\n--- %s ---\n"), *MemoryDoc.FilePath);
			Context += MemoryDoc.Content;
			Context += TEXT("\n");
		}
		Context += TEXT("\n");
	}
