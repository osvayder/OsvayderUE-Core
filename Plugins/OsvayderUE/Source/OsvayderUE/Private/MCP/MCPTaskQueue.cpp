	...
	, LastCleanupTime(FDateTime::UtcNow())
{
	...
	if (!WakeUpEvent)
	{
		UE_LOG(LogOsvayderUE, Error, TEXT("Failed to get sync event from pool for task queue"));
	}
}
...
void FMCPTaskQueue::Start()
{
	...
	if (WorkerThread)
	{
		UE_LOG(LogOsvayderUE, Log, TEXT("MCP Task Queue started"));
	}
	else
	{
		UE_LOG(LogOsvayderUE, Error, TEXT("Failed to create MCP Task Queue thread"));
	}
}
...
void FMCPTaskQueue::Shutdown()
{
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("MCP Task Queue shutting down..."));
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("MCP Task Queue stopped"));
}
...
FGuid FMCPTaskQueue::SubmitTask(const FString& ToolName, TSharedPtr<FJsonObject> Parameters, uint32 TimeoutMs)
{
	...
	{
		UE_LOG(LogOsvayderUE, Warning, TEXT("Cannot submit task: Tool '%s' not found"), *ToolName);
		return FGuid();
	}
	...
	// Add to task map and queue
	{
		...
		{
			UE_LOG(LogOsvayderUE, Warning, TEXT("Task queue at capacity (%d tasks), rejecting new task"), Config.MaxHistorySize);
			return FGuid();
		}
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("Task submitted: %s (tool: %s)"), *Task->TaskId.ToString(), *ToolName);
...
bool FMCPTaskQueue::TryBuildSubmissionPolicyDenyResult(
	const FString& ToolName,
	const TSharedRef<FJsonObject>& Parameters,
	FMCPToolResult& OutResult) const
{
	return ToolRegistry && ToolRegistry->TryBuildGovernanceDenyResult(ToolName, Parameters, OutResult);
}
...
bool FMCPTaskQueue::CancelTask(const FGuid& TaskId)
{
	...
	if (CurrentStatus == EMCPTaskStatus::Pending)
	{
		...
		UE_LOG(LogOsvayderUE, Log, TEXT("Task cancelled (pending): %s"), *TaskId.ToString());
		return true;
	}
	else if (CurrentStatus == EMCPTaskStatus::Running)
	{
		// Running tools are not pre-empted. They stay Running until ExecuteTask
		// unwinds, so clients cannot mistake the cancel request for a terminal state.
		Task->bCancellationRequested = true;
		UE_LOG(LogOsvayderUE, Log, TEXT("Task cancellation requested (running): %s"), *TaskId.ToString());
		return true;
	}
...
void FMCPTaskQueue::ExecuteTask(TSharedPtr<FMCPAsyncTask> Task)
{
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("Task started: %s (tool: %s)"), *Task->TaskId.ToString(), *Task->ToolName);
	...
	bool bExecutionTimedOut = false;
	bool bExecutionCompletedAfterTimeoutWait = false;

#if WITH_DEV_AUTOMATION_TESTS
	double GuardedPreExecuteDelayMs = 0.0;
	if (Task->ToolName == TEXT("async_guardrail_fixture")
		&& Params->TryGetNumberField(TEXT("pre_execute_delay_ms"), GuardedPreExecuteDelayMs)
		&& GuardedPreExecuteDelayMs > 0.0)
	{
		const FDateTime DelayStart = FDateTime::UtcNow();
		while ((FDateTime::UtcNow() - DelayStart).GetTotalMilliseconds() < GuardedPreExecuteDelayMs)
		{
			if (Task->bCancellationRequested)
			{
				Task->Status.Store(Task->bTimeoutRequested ? EMCPTaskStatus::TimedOut : EMCPTaskStatus::Cancelled);
				Task->Result = FMCPToolResult::Error(Task->bTimeoutRequested
					? TEXT("Task timed out before guarded fixture execution")
					: TEXT("Task cancelled before guarded fixture execution"));
				Task->CompletedTime = FDateTime::UtcNow();
				Task->Progress.Store(100);
				UE_LOG(LogOsvayderUE, Log, TEXT("Guarded fixture task stopped before execution: %s (status: %s)"),
					*Task->TaskId.ToString(),
					*FMCPAsyncTask::StatusToString(Task->Status.Load()));
				return;
			}

			if ((FDateTime::UtcNow() - Task->StartedTime).GetTotalMilliseconds() > Task->TimeoutMs)
			{
				Task->bCancellationRequested = true;
				Task->bTimeoutRequested = true;
			}
	...
	if (ToolRegistry->TryBuildGovernanceDenyResult(Task->ToolName, Params, Result))
	{
		Task->Status.Store(EMCPTaskStatus::Failed);
		Task->Result = Result;
		Task->CompletedTime = FDateTime::UtcNow();
		Task->Progress.Store(100);
		UE_LOG(LogOsvayderUE, Log, TEXT("Task denied by execution policy: %s (tool: %s)"), *Task->TaskId.ToString(), *Task->ToolName);
		return;
	}
	...
	else if (Task->TimeoutMs > 30000)
	{
		...
		FTSTicker::GetCoreTicker().AddTicker(TEXT("MCPTask_Execute"), 0.0f,
			[SharedResult, Tool, Params, CompletionEvent, bCompleted, DispatchStateLock, DispatchState, Task](float) -> bool
		{
			bool bCanExecute = false;
			{
				FScopeLock Lock(DispatchStateLock.Get());
				if (*DispatchState == 0 && !Task->bCancellationRequested && !Task->bTimeoutRequested)
				{
					*DispatchState = 1;
					bCanExecute = true;
				}
				else if (*DispatchState == 0)
				{
					*DispatchState = 2;
				}
			}
			...
			if (!bCanExecute)
			{
				*SharedResult = FMCPToolResult::Error(Task->bTimeoutRequested
					? TEXT("Task timed out before game-thread dispatch; execution skipped to prevent late mutation")
					: TEXT("Task cancelled before game-thread dispatch; execution skipped to prevent late mutation"));
				*bCompleted = true;
		...
		{
			UE_LOG(LogOsvayderUE, Error, TEXT("Task '%s' timed out after %d ms on game thread"),
			...
			if (bStartedOnGameThread)
			{
				UE_LOG(LogOsvayderUE, Warning,
					TEXT("Task '%s' exceeded %d ms after game-thread execution started; waiting for truthful completion instead of returning timeout while mutation may still land"),
					*Task->ToolName, Task->TimeoutMs);
				CompletionEvent->Wait();
				Result = *SharedResult;
				bExecutionCompletedAfterTimeoutWait = true;
			}
			else
			{
				bExecutionTimedOut = true;
	...
	if (bExecutionCompletedAfterTimeoutWait)
	{
		if (!Result.Data.IsValid())
		{
			Result.Data = MakeShared<FJsonObject>();
		}
		TSharedPtr<FJsonObject> AsyncLifecycle = MakeShared<FJsonObject>();
		AsyncLifecycle->SetBoolField(TEXT("cancel_requested"), Task->bCancellationRequested);
		AsyncLifecycle->SetBoolField(TEXT("timeout_requested"), Task->bTimeoutRequested);
		AsyncLifecycle->SetStringField(TEXT("terminal_truth"), TEXT("actual_result_after_timeout_wait"));
		Result.Data->SetObjectField(TEXT("async_lifecycle"), AsyncLifecycle);
		Task->Status.Store(Result.bSuccess ? EMCPTaskStatus::Completed : EMCPTaskStatus::Failed);
		Task->Result = Result;
	}
	else if ((bExecutionTimedOut || Task->bTimeoutRequested) && Result.bSuccess)
	{
		if (!Result.Data.IsValid())
		{
			Result.Data = MakeShared<FJsonObject>();
		...
		AsyncLifecycle->SetBoolField(TEXT("cancel_requested"), Task->bCancellationRequested);
		AsyncLifecycle->SetBoolField(TEXT("timeout_requested"), Task->bTimeoutRequested);
		AsyncLifecycle->SetStringField(TEXT("terminal_truth"), TEXT("completed_after_request"));
		Result.Data->SetObjectField(TEXT("async_lifecycle"), AsyncLifecycle);
		Task->Status.Store(EMCPTaskStatus::Completed);
		Task->Result = Result;
	}
	else if (Task->bCancellationRequested && Result.bSuccess)
	{
		if (!Result.Data.IsValid())
		{
			Result.Data = MakeShared<FJsonObject>();
		...
		TSharedPtr<FJsonObject> AsyncLifecycle = MakeShared<FJsonObject>();
		AsyncLifecycle->SetBoolField(TEXT("cancel_requested"), true);
		AsyncLifecycle->SetBoolField(TEXT("timeout_requested"), Task->bTimeoutRequested);
		AsyncLifecycle->SetStringField(TEXT("terminal_truth"), TEXT("completed_after_request"));
		Result.Data->SetObjectField(TEXT("async_lifecycle"), AsyncLifecycle);
		Task->Status.Store(EMCPTaskStatus::Completed);
	...
	else if (bExecutionTimedOut || Task->bTimeoutRequested)
	{
		Task->Status.Store(EMCPTaskStatus::TimedOut);
		Task->Result = FMCPToolResult::Error(FString::Printf(TEXT("Task timed out after %d ms"), Task->TimeoutMs));
	}
	else if (Task->bCancellationRequested)
	{
		Task->Status.Store(EMCPTaskStatus::Cancelled);
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("Task completed: %s (status: %s, duration: %.2fs)"),
		*Task->TaskId.ToString(),
		*FMCPAsyncTask::StatusToString(Task->Status.Load()),
...
void FMCPTaskQueue::CleanupOldTasks()
{
	...
	{
		UE_LOG(LogOsvayderUE, Log, TEXT("Cleaned up %d old tasks"), TasksToRemove.Num());
	}
}
	...
	for (auto& Pair : Tasks)
	{
		...
		if (Task->Status.Load() == EMCPTaskStatus::Running)
		{
			...
			if (Elapsed.GetTotalMilliseconds() > Task->TimeoutMs)
			{
				Task->bCancellationRequested = true;
				Task->bTimeoutRequested = true;
				UE_LOG(LogOsvayderUE, Warning, TEXT("Task timeout requested; waiting for execution to unwind: %s"), *Task->TaskId.ToString());
			}
		}
	}
}
