...
namespace
{
	const TCHAR* GMCPAuthHeaderName = TEXT("X-OsvayderUE-MCP-Token");
	const TCHAR* GMCPAuthTokenSchema = TEXT("osvayderue_mcp_auth_token.v1");
	const TCHAR* GMCPAuthDeniedSchema = TEXT("osvayderue_mcp_auth_denied.v1");
...
FOsvayderUEMCPServer::FOsvayderUEMCPServer()
	: bIsRunning(false)
	, ServerPort(UOsvayderUESettings::Get() ? UOsvayderUESettings::Get()->MCPServerPort : OsvayderUEConstants::MCPServer::DefaultPort)
{
...
void FOsvayderUEMCPServer::InitializeSecurityState()
{
...
FString FOsvayderUEMCPServer::BuildAuthTokenFilePath() const
{
	return FPaths::ConvertRelativePathToFull(FPaths::ProjectSavedDir() / TEXT("OsvayderUE/mcp_auth_token.json"));
}

FString FOsvayderUEMCPServer::BuildDeniedAuditPath() const
{
	return FPaths::ConvertRelativePathToFull(FPaths::ProjectSavedDir() / TEXT("OsvayderUE/mcp_denied_audit.jsonl"));
}

bool FOsvayderUEMCPServer::PersistAuthTokenFile() const
{
...
bool FOsvayderUEMCPServer::Start(uint32 Port)
{
	if (bIsRunning)
	{
		UE_LOG(LogOsvayderUE, Warning, TEXT("MCP Server is already running on port %d"), ServerPort);
		return true;
	}
	...
	{
		UE_LOG(LogOsvayderUE, Error, TEXT("Failed to persist MCP auth token handoff file at %s"), *AuthTokenFilePath);
		return false;
	}
	...
	{
		UE_LOG(LogOsvayderUE, Error, TEXT("Failed to get HTTP router for port %d"), ServerPort);
		return false;
	}
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("MCP Server started on http://localhost:%d"), ServerPort);
	UE_LOG(LogOsvayderUE, Log, TEXT("  GET  /mcp/tools      - List available tools (token required)"));
	UE_LOG(LogOsvayderUE, Log, TEXT("  POST /mcp/tool/{name} - Execute a tool (token required)"));
	UE_LOG(LogOsvayderUE, Log, TEXT("  GET  /mcp/status     - Server status (local read-only discovery)"));
	UE_LOG(LogOsvayderUE, Log, TEXT("MCP auth token handoff file written at %s"), *AuthTokenFilePath);

	return true;
}

void FOsvayderUEMCPServer::Stop()
{
	if (!bIsRunning)
	{
		return;
	...
	bIsRunning = false;
	UE_LOG(LogOsvayderUE, Log, TEXT("MCP Server stopped"));
}

bool FOsvayderUEMCPServer::ConstantTimeTokenEquals(const FString& A, const FString& B) const
{
...
FString FOsvayderUEMCPServer::ExtractRequestToken(const FHttpServerRequest& Request, bool& bOutHeaderPresent) const
{
...
bool FOsvayderUEMCPServer::IsRequestFromLocalPeer(const FHttpServerRequest& Request, FString& OutRemoteAddress) const
{
...
FMCPHttpAuthValidationResult FOsvayderUEMCPServer::ValidateRequestAuth(
...
void FOsvayderUEMCPServer::AttachAuthenticatedRequestMetadata(const TSharedRef<FJsonObject>& Params) const
{
...
void FOsvayderUEMCPServer::AppendDeniedAudit(
	const FString& Endpoint,
	const FString& ToolName,
...
void FOsvayderUEMCPServer::AuditToolDenyResult(
	const FString& Endpoint,
	const FString& ToolName,
...
void FOsvayderUEMCPServer::SetSessionTokenForTests(const FString& Token)
{
	SessionToken = Token;
...
void FOsvayderUEMCPServer::SetDeniedAuditPathForTests(const FString& AuditPath)
{
	DeniedAuditPath = AuditPath;
}

FMCPHttpAuthValidationResult FOsvayderUEMCPServer::ValidateRequestAuthForTests(
	const FHttpServerRequest& Request,
	const FString& Endpoint,
	const FString& ToolName)
{
...
void FOsvayderUEMCPServer::AppendDeniedAuditForTests(
	const FString& Endpoint,
	const FString& ToolName,
	const FString& Reason,
	const FHttpServerRequest& Request)
...
void FOsvayderUEMCPServer::SetupRoutes()
{
	if (!HttpRouter.IsValid())
	{
		return;
	...
	ListToolsHandle = HttpRouter->BindRoute(
		...
		FHttpRequestHandler::CreateRaw(this, &FOsvayderUEMCPServer::HandleListTools)
	);
	...
	ExecuteToolHandle = HttpRouter->BindRoute(
		...
		FHttpRequestHandler::CreateRaw(this, &FOsvayderUEMCPServer::HandleExecuteTool)
	);
	...
	StatusHandle = HttpRouter->BindRoute(
		...
		EHttpServerRequestVerbs::VERB_GET,
		FHttpRequestHandler::CreateRaw(this, &FOsvayderUEMCPServer::HandleStatus)
	);
}

bool FOsvayderUEMCPServer::HandleListTools(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
...
bool FOsvayderUEMCPServer::HandleExecuteTool(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	...
	if (Request.Body.Num() > OsvayderUEConstants::MCPServer::MaxRequestBodySize)
	{
		UE_LOG(LogOsvayderUE, Warning, TEXT("Request body too large: %d bytes (max %d)"), Request.Body.Num(), OsvayderUEConstants::MCPServer::MaxRequestBodySize);
	...
	if (Request.Body.Num() > 0)
	{
		...
		{
			UE_LOG(LogOsvayderUE, Warning, TEXT("Failed to parse JSON body: %s"), *BodyString);
...
bool FOsvayderUEMCPServer::HandleStatus(const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
{
	TSharedPtr<FJsonObject> ResponseJson = MakeShared<FJsonObject>();
...
TUniquePtr<FHttpServerResponse> FOsvayderUEMCPServer::CreateJsonResponse(const FString& JsonContent, EHttpServerResponseCodes Code)
{
...
TUniquePtr<FHttpServerResponse> FOsvayderUEMCPServer::CreateAuthDeniedResponse(const FMCPHttpAuthValidationResult& AuthResult)
{
...
TUniquePtr<FHttpServerResponse> FOsvayderUEMCPServer::CreateErrorResponse(const FString& Message, EHttpServerResponseCodes Code)
{
	TSharedPtr<FJsonObject> ErrorJson = MakeShared<FJsonObject>();
	ErrorJson->SetBoolField(TEXT("success"), false);
