...
FMCPToolResult FMCPTool_MoveActor::Execute(const TSharedRef<FJsonObject>& Params)
{
	...
	ResultData->SetObjectField(TEXT("location"), OsvayderUEJsonUtils::VectorToJson(Actor->GetActorLocation()));
	ResultData->SetObjectField(TEXT("rotation"), OsvayderUEJsonUtils::RotatorToJson(Actor->GetActorRotation()));
	ResultData->SetObjectField(TEXT("scale"), OsvayderUEJsonUtils::VectorToJson(Actor->GetActorScale3D()));
