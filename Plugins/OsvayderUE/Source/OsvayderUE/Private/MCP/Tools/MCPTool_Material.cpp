...
FMCPToolResult FMCPTool_Material::ExecuteCreateMaterialInstance(const TSharedRef<FJsonObject>& Params)
{
	...
	if (Params->HasField(TEXT("parameters")))
	{
		...
		if (Params->TryGetObjectField(TEXT("parameters"), ParamsObj))
		{
			...
			if (!ApplyParametersFromJson(MatInst, *ParamsObj, ParamError))
			{
				...
				UE_LOG(LogOsvayderUE, Warning, TEXT("Material instance created but some parameters failed: %s"), *ParamError);
			}
		}
	}
	...
	// Emit execution receipt
	{
		...
		Receipt.Tool = TEXT("material:create_material_instance");
		Receipt.TargetType = TEXT("asset");
		Receipt.Targets.Add(FullPackagePath);
		Receipt.Classification = TEXT("user_mutation");
		if (SaveResult.IsSuccessful())
		{
			Receipt.bSuccess = true;
			Receipt.Status = TEXT("success");
			Receipt.Summary = FString::Printf(TEXT("Created material instance: %s"), *FullPackagePath);
			Receipt.Created.Add(FullPackagePath);
		}
		else
		{
			Receipt.bSuccess = false;
			Receipt.Status = TEXT("failed");
			Receipt.ErrorOrDenialReason = FString::Printf(TEXT("Save failed: %s"), *FullPackagePath);
		}
