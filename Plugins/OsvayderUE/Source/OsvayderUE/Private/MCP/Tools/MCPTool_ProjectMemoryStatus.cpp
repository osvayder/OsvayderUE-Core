...
FMCPToolResult FMCPTool_ProjectMemoryStatus::Execute(const TSharedRef<FJsonObject>& Params)
{
	...
	FString MemoryPath = (Settings && !Settings->ProjectMemoryPath.IsEmpty())
		? Settings->ProjectMemoryPath : TEXT("Docs/OsvayderUE");
	...
	const FString WorkerToReviewer = FPaths::Combine(FullBridgePath, TEXT("WORKER_TO_REVIEWER.md"));
	const FString ReviewerToWorker = FPaths::Combine(FullBridgePath, TEXT("REVIEWER_TO_WORKER.md"));

	BridgeObj->SetStringField(TEXT("worker_to_reviewer_latest"), ExtractLatestHandoffMeta(WorkerToReviewer));
	BridgeObj->SetStringField(TEXT("reviewer_to_worker_latest"), ExtractLatestHandoffMeta(ReviewerToWorker));
	...
	if (Settings)
	{
		...
		Caps->SetStringField(TEXT("scope_mode"),
			Settings->ScopeMode == EOsvayderUEScopeMode::PluginOnly ? TEXT("PluginOnly") : TEXT("PluginAndProject"));
