...
public:
	virtual FMCPToolInfo GetInfo() const override
	{
		...
		Info.Parameters = {
			...
			FMCPToolParameter(TEXT("package_path"), TEXT("string"),
				TEXT("Required for mutate_marker. Must be under /Game/__OsvayderUETestSandbox/."), false),
	...
	virtual FMCPToolResult Execute(const TSharedRef<FJsonObject>& Params) override
	{
		...
		if (!PackagePath.StartsWith(TEXT("/Game/__OsvayderUETestSandbox/"))
			|| PackagePath.Contains(TEXT(".."))
			|| PackagePath.Contains(TEXT("\\"))
			|| PackagePath.Contains(TEXT(".")))
		{
			return FMCPToolResult::Error(TEXT("Invalid package path: async guardrail fixture only writes under /Game/__OsvayderUETestSandbox/"));
		}
