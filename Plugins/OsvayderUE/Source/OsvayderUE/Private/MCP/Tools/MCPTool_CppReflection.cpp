...
namespace
{
	...
	TSharedPtr<FJsonObject> BuildBuildSyncObject(const FOsvayderUEBuildSyncStatus& Status)
	{
	...
	FOsvayderUEReportTruthSummary BuildDeclarationEvidenceBundleTruthSummary(const TSharedPtr<FJsonObject>& BundleObject)
	{
	...
	{
		const FString ReceiptDirectory = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("OsvayderUE"), TEXT("cpp_reflection_mutations"));
	...
	TSharedPtr<FJsonObject> BuildDeclarationFutureApplyExpectationsObject(
		const FOsvayderUEBuildSyncStatus& BuildSyncStatus)
	{
		...
		ExpectationsObject->SetStringField(TEXT("preflight_launcher_path"), FOsvayderUEModule::GetPreflightLauncherPath());
	...
	TSharedPtr<FJsonObject> BuildDeclarationCompileHandshakeObject(
		...
		const FOsvayderUEBuildSyncStatus& BuildSyncBefore,
		const FOsvayderUEBuildSyncStatus& BuildSyncAfter)
	{
		...
		HandshakeObject->SetStringField(TEXT("preflight_launcher_path"), FOsvayderUEModule::GetPreflightLauncherPath());
...
FMCPToolInfo FMCPTool_CppReflection::GetInfo() const
{
	...
	Info.Parameters = {
		...
		FMCPToolParameter(TEXT("export_report"), TEXT("boolean"), TEXT("For build_reflected_property_declaration_evidence_bundle, if true, persist a Markdown report and normalized sidecar under Saved/OsvayderUE/Reports."), false, TEXT("false")),
...
FMCPToolResult FMCPTool_CppReflection::ExecutePreviewReflectedPropertyDeclaration(const TSharedRef<FJsonObject>& Params)
{
	...
	Data->SetObjectField(TEXT("future_apply_expectations"), BuildDeclarationFutureApplyExpectationsObject(FOsvayderUEModule::GetBuildSyncStatus()));
...
{
	const FOsvayderUEBuildSyncStatus BuildSyncBefore = FOsvayderUEModule::GetBuildSyncStatus();
	...
	const FOsvayderUEBuildSyncStatus BuildSyncAfter = FOsvayderUEModule::GetBuildSyncStatus();
...
{
	const FOsvayderUEBuildSyncStatus BuildSyncCurrent = FOsvayderUEModule::GetBuildSyncStatus();
...
FMCPToolResult FMCPTool_CppReflection::ExecutePreviewPropertyMetadataMutation(const TSharedRef<FJsonObject>& Params)
{
	...
	PreviewHandshake->SetStringField(TEXT("preflight_launcher_path"), FOsvayderUEModule::GetPreflightLauncherPath());
	PreviewHandshake->SetObjectField(TEXT("build_status_current"), BuildBuildSyncObject(FOsvayderUEModule::GetBuildSyncStatus()));
...
FMCPToolResult FMCPTool_CppReflection::ExecuteApplyPropertyMetadataMutation(const TSharedRef<FJsonObject>& Params)
{
	...
	if (bSafeForLiveCoding && MutationApply.bApplied)
	{
		...
		// calls).
		const OsvayderUE::CppReflectionCompile::Testing::FMockTriggerLiveCodingCompileResult LcResult =
			OsvayderUE::CppReflectionCompile::InvokeTriggerLiveCodingCompile();
