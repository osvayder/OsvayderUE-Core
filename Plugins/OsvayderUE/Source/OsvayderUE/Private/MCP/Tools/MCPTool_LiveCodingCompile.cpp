...
namespace
{
	...
	TMap<FString, FDateTime> ScanPluginDllMtimes()
	{
		...
		// Resolve the plugin's Binaries/Win64 directory.
...
FMCPToolInfo FMCPTool_LiveCodingCompile::GetInfo() const
{
	...
	Info.Parameters = {
		...
		FMCPToolParameter(TEXT("enable_for_session_if_needed"), TEXT("boolean"),
			TEXT("If true and Live Coding is currently disabled for the session, call ILiveCodingModule::EnableForSession(true) before compiling. Respects UOsvayderUESettings::bAllowAutoEnableLiveCoding -- when that setting is false, the tool returns a structured error instead of auto-enabling. Default: true."), false, TEXT("true")),
...
{
	using namespace OsvayderUE::LiveCodingCompile;
