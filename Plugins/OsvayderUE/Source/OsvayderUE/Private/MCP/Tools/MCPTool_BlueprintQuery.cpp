...
FMCPToolResult FMCPTool_BlueprintQuery::Execute(const TSharedRef<FJsonObject>& Params)
{
	...
	else if (Operation == TEXT("animation_preflight"))
	{
		return ExecuteAnimationPreflight(Params);
	}
...
FMCPToolResult FMCPTool_BlueprintQuery::ExecuteGetAnimBlueprintInfo(const TSharedRef<FJsonObject>& Params)
{
	...
	TSharedPtr<FJsonObject> ResultData = FAnimationBlueprintUtils::SerializeAnimBlueprintInfo(AnimBP);
	ResultData->SetBoolField(TEXT("is_custom_anim_instance"),
		AnimBP->ParentClass && !AnimBP->ParentClass->GetName().Equals(TEXT("AnimInstance")));
...
FMCPToolResult FMCPTool_BlueprintQuery::ExecuteAnimationPreflight(const TSharedRef<FJsonObject>& Params)
{
	FString BlueprintPath;
	TOptional<FMCPToolResult> Error;
	if (!ExtractAndValidate(Params, TEXT("blueprint_path"), FMCPParamValidator::ValidateBlueprintPath, BlueprintPath, Error))
	...
	const FString ActorBlueprintPath = ExtractOptionalString(Params, TEXT("actor_blueprint_path"));
	if (!ActorBlueprintPath.IsEmpty())
	{
		FString ActorValidationError;
		if (!FMCPParamValidator::ValidateBlueprintPath(ActorBlueprintPath, ActorValidationError))
		{
			return FMCPToolResult::Error(ActorValidationError);
		}

		FString ActorLoadError;
		ActorBlueprint = FBlueprintUtils::LoadBlueprint(ActorBlueprintPath, ActorLoadError);
		if (!ActorBlueprint)
		{
			return FMCPToolResult::Error(ActorLoadError);
		}
	}

	TArray<FString> RequiredRoles;
	const TArray<TSharedPtr<FJsonValue>>* RolesArray = nullptr;
	if (Params->TryGetArrayField(TEXT("required_animation_roles"), RolesArray) && RolesArray)
	{
		for (const TSharedPtr<FJsonValue>& RoleValue : *RolesArray)
		{
			FString Role;
			if (RoleValue.IsValid() && RoleValue->TryGetString(Role) && !Role.IsEmpty())
			{
	...
	const int32 MatchLimit = ExtractOptionalNumber<int32>(Params, TEXT("match_limit"), 5);
	FString ImportedAssetRoot = ExtractOptionalString(Params, TEXT("asset_root"));
	if (ImportedAssetRoot.IsEmpty())
	{
		ImportedAssetRoot = ExtractOptionalString(Params, TEXT("imported_asset_root"));
	}
	FString PackIdentifier = ExtractOptionalString(Params, TEXT("pack_identifier"));
	if (PackIdentifier.IsEmpty())
	{
		PackIdentifier = ExtractOptionalString(Params, TEXT("imported_pack_identifier"));
	}

	FString PreflightError;
	TSharedPtr<FJsonObject> ResultData = FAnimationBlueprintUtils::BuildAnimationPreflightReport(
		AnimBP,
		RequiredRoles,
		ActorBlueprint,
		ComponentName,
	...
	if (!ResultData.IsValid() || !ResultData->GetBoolField(TEXT("success")))
	{
		return FMCPToolResult::Error(PreflightError.IsEmpty() ? TEXT("Failed to build animation preflight report") : PreflightError);
	}

	const TArray<TSharedPtr<FJsonValue>>* MissingRoles = nullptr;
	const TArray<TSharedPtr<FJsonValue>>* UnsatisfiedRoles = nullptr;
	int32 MissingCount = 0;
	int32 UnsatisfiedCount = 0;
	if (ResultData->TryGetArrayField(TEXT("missing_roles"), MissingRoles) && MissingRoles)
	{
		MissingCount = MissingRoles->Num();
	}
	if (ResultData->TryGetArrayField(TEXT("unsatisfied_roles"), UnsatisfiedRoles) && UnsatisfiedRoles)
	{
		UnsatisfiedCount = UnsatisfiedRoles->Num();
	}

	const FString ProofClassification = ResultData->GetStringField(TEXT("proof_classification"));
	return FMCPToolResult::Success(
		FString::Printf(
			TEXT("Animation preflight for '%s': %d missing roles, %d unsatisfied roles, proof=%s"),
			*AnimBP->GetName(),
			MissingCount,
			UnsatisfiedCount,
			*ProofClassification),
