...
{
	/**
	 * P6 helper: run the MCPSavePipeline for an explicit `save_asset` tool
	 * call on a single package. Returns true when the package landed in
	 * `Outcome.Saved`; fills OutError + OutLifecycle with details from the
	 * outcome so the caller can surface them in the tool result. Shared
	 * between ExecuteSaveAsset and the 3 P6 regression tests.
	 */
	bool RunExplicitSavePipelineForPackage(
		UPackage* Package,
		FString& OutError,
		OsvayderUE::SavePipeline::FLifecycleOutcome& OutOutcome)
	{
		OsvayderUE::SavePipeline::FSaveSpec Spec;
		Spec.Packages.Add(Package);
		Spec.bIsExplicitToolCall = true;
		OutOutcome = OsvayderUE::SavePipeline::Run(Spec);
		if (OutOutcome.Failed.Num() > 0)
		{
			OutError = OutOutcome.Failed[0].Error;
			return false;
		}
		return OutOutcome.Saved.Num() > 0;
	}
}
...
FMCPToolInfo FMCPTool_Asset::GetInfo() const
{
	...
	// Distinct from `save` / `mark_dirty` — `auto_save` controls the outer wrapper's
	// mutation-lifecycle behavior; `save` / `mark_dirty` control save_asset's inner logic.
...
{
	UE_LOG(LogTemp, Warning, TEXT("[ExecuteSaveAsset] diagnostic ping"));
	// P6 refactor: `save_asset` now delegates to MCPSavePipeline::Run via the
	// `bIsExplicitToolCall=true` branch. External contract preserved
	// (`asset_path` param, `{saved, marked_dirty, asset_path}` result, same
	// success/failure messages). New coverage gained: Blueprint packages now
	// compile-before-save, silent source-control checkout runs, and newly-
	// created assets get FAssetRegistryModule::AssetCreated — all via the
	// shared P1-P5 pipeline. The explicit `FExecutionReceipt` emission that
	// lived here pre-P6 is removed: MCPToolRegistry::ExecuteTool emits a
	// per-tool receipt + an `asset`-mutation_lifecycle receipt for every
	// dispatch, covering observability without this tool's duplicate one.
	FString AssetPath;
	TOptional<FMCPToolResult> Error;
	if (!ExtractRequiredString(Params, TEXT("asset_path"), AssetPath, Error)) return Error.GetValue();
	if (!ValidateBlueprintPathParam(AssetPath, Error)) return Error.GetValue();

	bool bSave = true;                                           Params->TryGetBoolField(TEXT("save"), bSave);
	bool bMarkDirty = !bSave;                                    Params->TryGetBoolField(TEXT("mark_dirty"), bMarkDirty);
	...
	if (bMarkDirty)                                              { Asset->MarkPackageDirty(); bWasMarkedDirty = true; }

	bool bWasSaved = false;
	OsvayderUE::SavePipeline::FLifecycleOutcome Outcome;
	FString SaveError;
	if (bSave)                                                   bWasSaved = RunExplicitSavePipelineForPackage(Asset->GetOutermost(), SaveError, Outcome);
	if (bSave && !bWasSaved) return FMCPToolResult::Error(FString::Printf(TEXT("Failed to save asset: %s%s%s"), *AssetPath, SaveError.IsEmpty() ? TEXT("") : TEXT(" - "), *SaveError));

	TSharedPtr<FJsonObject> ResultData = MakeShared<FJsonObject>();
	ResultData->SetStringField(TEXT("asset_path"), AssetPath);
	ResultData->SetBoolField(TEXT("saved"), bWasSaved);
	ResultData->SetBoolField(TEXT("marked_dirty"), bWasMarkedDirty);
	return FMCPToolResult::Success(bWasSaved ? FString::Printf(TEXT("Saved asset: %s"), *AssetPath) : FString::Printf(TEXT("Marked asset dirty: %s"), *AssetPath), ResultData);
}
