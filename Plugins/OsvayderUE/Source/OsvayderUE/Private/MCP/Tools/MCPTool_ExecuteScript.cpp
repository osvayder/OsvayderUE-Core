...
FMCPToolResult FMCPTool_ExecuteScript::Execute(const TSharedRef<FJsonObject>& Params)
{
	...
	if (!TaskQueue.IsValid())
	{
		...
		UE_LOG(LogOsvayderUE, Warning, TEXT("execute_script: No task queue available, falling back to sync execution"));
		return ExecuteSync(Params);
	}
...
FMCPToolResult FMCPTool_ExecuteScript::ExecuteSync(const TSharedRef<FJsonObject>& Params)
{
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("Executing %s script: %s"), *ScriptTypeStr, *Description);
	...
	if (ScriptTypeStr.ToLower() == TEXT("cpp"))
	{
		...
		// Surface script file path for repair loop reference
		if (!Result.ScriptFilePath.IsEmpty())
			ResultData->SetStringField(TEXT("script_file"), Result.ScriptFilePath);

		// Surface structured compile diagnostics (file/line/severity/code)
		if (Result.Diagnostics.IsValid())
			ResultData->SetObjectField(TEXT("diagnostics"), Result.Diagnostics);

		// Post-compile visibility contract
		if (Result.bSuccess)
		{
			ResultData->SetStringField(TEXT("refresh_status"), TEXT("live_coding_patched"));
			ResultData->SetStringField(TEXT("refresh_note"), TEXT("Code changes are hot-patched via Live Coding. New classes/structs require editor restart. Modified function bodies are immediately active."));
		}
	}
