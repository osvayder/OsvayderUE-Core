...
namespace
{
	...
	TArray<FString> PackageNamesFromResults(const TArray<OsvayderUE::SavePipeline::FLifecyclePhaseResult>& Results)
	{
		TArray<FString> Names;
	...
	TArray<TSharedPtr<FJsonValue>> PhaseResultsToJson(const TArray<OsvayderUE::SavePipeline::FLifecyclePhaseResult>& Results)
	{
		TArray<TSharedPtr<FJsonValue>> JsonValues;
	...
	FMCPToolResult MakeSandboxPlacementError(const FString& Message, const FString& Reason, const FString& MapPath)
	{
		...
		Result.Data->SetStringField(TEXT("schema_version"), TEXT("osvayderue_sandbox_map_placement.v1"));
		...
		const OsvayderUE::SavePipeline::FLifecycleOutcome& Outcome)
	{
		...
		Lifecycle->SetStringField(TEXT("schema_version"), TEXT("osvayderue_sandbox_map_lifecycle.v1"));
...
{
	return TEXT("/Game/__OsvayderUETestSandbox/");
}
...
FMCPToolResult FMCPTool_SandboxMapPlacement::Execute(const TSharedRef<FJsonObject>& Params)
{
	...
	const OsvayderUE::SavePipeline::FLifecycleOutcome SaveOutcome = OsvayderUE::SavePipeline::Run(SaveSpec);
	...
	ResultData->SetStringField(TEXT("schema_version"), TEXT("osvayderue_sandbox_map_placement.v1"));
