...
using namespace OsvayderUEConstants::MCPValidation;
...
bool FMCPParamValidator::ValidateBlueprintPath(const FString& BlueprintPath, FString& OutError)
{
	...
	if (BlueprintPath.Contains(TEXT("\\")) || BlueprintPath.Contains(TEXT(":")))
	{
		OutError = TEXT("Invalid package path: Blueprint paths must use Unreal package syntax like /Game/Folder/BP_Name. Raw disk paths are invalid.");
		return false;
	}

	if (!BlueprintPath.StartsWith(TEXT("/")))
	{
		OutError = TEXT("Invalid package path: Blueprint paths must start with a mounted Unreal package root such as /Game/.");
		return false;
	}
	...
	const FString PackagePath = FPackageName::ObjectPathToPackageName(BlueprintPath);
	FText PackagePathError;
	if (!FPackageName::IsValidLongPackageName(PackagePath, false, &PackagePathError))
	{
		OutError = FString::Printf(TEXT("Invalid package path: %s"), *PackagePathError.ToString());
		return false;
	}
