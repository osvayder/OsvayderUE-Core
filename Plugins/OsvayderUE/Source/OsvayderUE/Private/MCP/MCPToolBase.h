...
protected:
		...
		const FVector& DefaultValue = FVector::ZeroVector) const
	{
		...
		{
			return OsvayderUEJsonUtils::ExtractVector(*VectorObj, DefaultValue);
		}
		return DefaultValue;
	}
		...
		const FRotator& DefaultValue = FRotator::ZeroRotator) const
	{
		...
		{
			return OsvayderUEJsonUtils::ExtractRotator(*RotatorObj, DefaultValue);
		}
		return DefaultValue;
	}
	...
	TSharedPtr<FJsonObject> BuildActorInfoWithTransformJson(AActor* Actor) const
	{
		...
		if (Actor)
		{
			ActorJson->SetObjectField(TEXT("location"), OsvayderUEJsonUtils::VectorToJson(Actor->GetActorLocation()));
			ActorJson->SetObjectField(TEXT("rotation"), OsvayderUEJsonUtils::RotatorToJson(Actor->GetActorRotation()));
			ActorJson->SetObjectField(TEXT("scale"), OsvayderUEJsonUtils::VectorToJson(Actor->GetActorScale3D()));
		}
		return ActorJson;
	}
