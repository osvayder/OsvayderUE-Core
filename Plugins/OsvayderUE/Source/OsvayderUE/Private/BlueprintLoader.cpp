...
UBlueprint* FBlueprintLoader::LoadBlueprint(const FString& BlueprintPath, FString& OutError)
{
	...
	// Special path: resolve current level blueprint
	if (BlueprintPath.Equals(TEXT("level_blueprint"), ESearchCase::IgnoreCase))
	{
		if (!GEditor)
		{
		...
		if (!World)
		{
			OutError = TEXT("No level open in editor");
			return nullptr;
		}
		UBlueprint* LevelBP = World->PersistentLevel->GetLevelScriptBlueprint(false);
		if (!LevelBP)
		{
			OutError = TEXT("Level Blueprint not available for current level");
			return nullptr;
		}
		return LevelBP;
	}
...
{
	// Special alias: level_blueprint is always valid
	if (BlueprintPath.Equals(TEXT("level_blueprint"), ESearchCase::IgnoreCase))
	{
		return true;
	}
...
FBlueprintCompileResult FBlueprintLoader::CompileBlueprintWithResult(UBlueprint* Blueprint)
{
	...
	// Compile the Blueprint (skip MarkBlueprintAsStructurallyModified — it can crash
	// on Blueprints with MC_Delegate variables that have NAME_None subcategory)
	...
	auto ProcessGraph = [&](UEdGraph* Graph)
	{
		...
		for (UEdGraphNode* Node : Graph->Nodes)
		{
			...
			if (Node->bHasCompilerMessage)
			{
				...
				Msg.GraphName = CurrentGraphName;

				// Generate stable node ID for repair-loop use
				Msg.NodeId = FBlueprintGraphEditor::GetNodeId(Node);
				...
				VerboseBuilder.Appendf(TEXT("[%s] Graph '%s', Node '%s' (%s): %s\n"),
					*Msg.Severity, *CurrentGraphName, *Msg.NodeName, *Msg.NodeId, *Msg.Message);
			}
		}
	};
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("Blueprint '%s' compiled: %s (Errors: %d, Warnings: %d)"),
	...
	FString& OutError)
{
	...
	UE_LOG(LogOsvayderUE, Log, TEXT("Created Blueprint: %s (Parent: %s)"),
