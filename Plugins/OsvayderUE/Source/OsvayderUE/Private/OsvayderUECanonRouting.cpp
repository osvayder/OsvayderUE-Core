...
namespace
{
	...
	bool LooksLocalAnimationPackPathPrompt(const FString& Prompt)
	{
	...
	FString ResolveLedgerPath()
	{
		...
		return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("OsvayderUE"), TEXT("canon_ledger.json"));
	}
	...
	FString ResolveProvenRunReceiptPath(const FString& InPathOrRunTag)
	{
		...
		{
			return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("OsvayderUE"), TEXT("backend_runs"), Trimmed + TEXT(".json"));
		}
	...
	FAgentFeatureWorkflowState MakeInventoryBasicFeatureWorkflow(const FString& Prompt)
	{
		...
		Workflow.EvidenceSchemaVersion =
			OsvayderUERecipeRegistry::GetInventoryBasicUiRecipeEvidenceContract().EvidenceSchemaVersion;
	...
	FAgentFeatureWorkflowState MakeInteractionAccessFeatureWorkflow(const FString& Prompt)
	{
		...
		Workflow.RoleId = OsvayderUERoleRegistry::WorkerRoleId();
		Workflow.EvidenceSchemaVersion =
			OsvayderUERecipeRegistry::GetInteractionAccessRecipeEvidenceContract().EvidenceSchemaVersion;
	...
	{
		FString Context = FString::Printf(
			TEXT("task_mode = %s\ndetected_subsystem = %s\napproved_pattern_found = %s\nrequested_tool_family = %s\nsource_priority = project_local -> official_unreal_docs -> official_first_party_samples -> community_support_only\nacceptance_checklist_rule = extract gameplay/mechanic checklist before first mutation\nmechanic_input_conflict_rule = run mechanic_preflight inspect and audit requested controls against existing input bindings before implementation / before first mutating tool call\nmechanic_preflight_artifact_required = true\nmechanic_input_conflict_blocker_code = mechanic_input_conflict_unresolved\nexternal_asset_blocker_code = manual_asset_dependency_blocker"),
			*Execution.TaskMode,
			*Execution.DetectedSubsystem,
			Execution.bApprovedPatternFound ? TEXT("true") : TEXT("false"),
			*Execution.RequestedToolFamily);
		if (Execution.MandatoryAnimationWorkflowSteps.Num() > 0
			|| Execution.ConditionalMandatoryAnimationWorkflowSteps.Num() > 0)
		{
			Context += FString::Printf(
				TEXT("\nmandatory_animation_workflow_steps = %s\nconditional_mandatory_animation_workflow_steps = %s\nanimation_closeout_gate_reason_codes = %s"),
				*FString::Join(Execution.MandatoryAnimationWorkflowSteps, TEXT(", ")),
				*FString::Join(Execution.ConditionalMandatoryAnimationWorkflowSteps, TEXT(", ")),
				*FString::Join(Execution.CloseoutGateReasonCodes, TEXT(", ")));
		}
		return Context;
	}
	...
	FString NormalizeObservedToolName(const FString& ToolName)
	{
		...
		if (CanonicalName.StartsWith(TEXT("mcp__osvayderue__")))
		{
			CanonicalName.RightChopInline(18, EAllowShrinking::No);
		}
		if (CanonicalName.StartsWith(TEXT("osvayderue/")))
		{
	...
	FString EncodeAllowedToolName(const FString& ToolName)
	{
		...
		return FString::Printf(TEXT("mcp__osvayderue__%s"), *NormalizedToolName);
	}
	...
	TArray<FString> ConcreteToolsForFamily(const FString& FamilyId)
	{
		...
		{
			return { TEXT("local_animation_pack_intake"), TEXT("animation_retarget_fixup"), TEXT("anim_blueprint_modify"), TEXT("blueprint_query") };
		}
	...
	FString BuildToolExposureContext(const FAgentCanonExecution& Execution)
	{
		...
		FString Context = FString::Printf(
			...
			*Execution.RequestedToolFamily,
			*EnabledFamilies);
		if (Execution.RequestedToolFamily == TEXT("unreal_animation"))
		{
			Context += TEXT(
				"\nlocal_animation_pack_route = if the prompt contains an explicit local filesystem animation pack path, call local_animation_pack_intake before anim_blueprint_modify/find_animations"
				"\nskeleton_mismatch_route = if animation_preflight reports skeleton_mismatch or retarget_required_not_implemented, call animation_retarget_fixup before declaring a blocker or hooking animations"
				"\nalready_imported_animation_route = if the prompt names an existing /Game path only, keep using anim_blueprint_modify/find_animations or animation_preflight");
			if (Execution.MandatoryAnimationWorkflowSteps.Num() > 0
				|| Execution.ConditionalMandatoryAnimationWorkflowSteps.Num() > 0)
			{
				Context += FString::Printf(
					TEXT("\nmandatory_animation_workflow_steps = %s\nconditional_mandatory_animation_workflow_steps = %s\ncloseout_enforced_gate_reason_codes = %s"),
					*FString::Join(Execution.MandatoryAnimationWorkflowSteps, TEXT(", ")),
					*FString::Join(Execution.ConditionalMandatoryAnimationWorkflowSteps, TEXT(", ")),
					*FString::Join(Execution.CloseoutGateReasonCodes, TEXT(", ")));
			}
	...
	int32 ScorePatternMatch(
		const FOsvayderUECanonLedgerEntry& Entry,
	...
	bool TryBuildLedgerEntryFromProvenRun(
		...
		FOsvayderUECanonLedgerEntry& OutEntry,
		FString& OutError)
	{
		OutEntry = FOsvayderUECanonLedgerEntry();
		OutError.Reset();
	...
	TSharedPtr<FJsonObject> MakeLedgerEntryJson(const FOsvayderUECanonLedgerEntry& Entry)
	{
	...
	bool TryParseLedgerEntry(const TSharedPtr<FJsonObject>& Object, FOsvayderUECanonLedgerEntry& OutEntry)
	{
		if (!Object.IsValid())
		{
			return false;
...
FString FOsvayderUECanonLedger::GetLedgerPath()
{
	return ResolveLedgerPath();
}

bool FOsvayderUECanonLedger::LoadEntries(TArray<FOsvayderUECanonLedgerEntry>& OutEntries, FString& OutError)
{
	OutEntries.Reset();
	OutError.Reset();
...
bool FOsvayderUECanonLedger::SaveEntries(const TArray<FOsvayderUECanonLedgerEntry>& Entries, FString& OutError)
{
	OutError.Reset();
	...
	for (const FOsvayderUECanonLedgerEntry& Entry : Entries)
	{
		if (!Entry.IsValid())
		{
			continue;
...
bool FOsvayderUECanonLedger::PromoteEntry(const FOsvayderUECanonLedgerEntry& Entry, FString& OutError)
{
	OutError.Reset();
	...
	TArray<FOsvayderUECanonLedgerEntry> Entries;
	if (!LoadEntries(Entries, OutError))
	{
		return false;
	}

	const int32 ExistingIndex = Entries.IndexOfByPredicate([&Entry](const FOsvayderUECanonLedgerEntry& Existing)
	{
...
bool FOsvayderUECanonLedger::PromoteFromProvenRun(
	const FOsvayderUECanonPromotionRequest& Request,
	FString& OutError,
	FOsvayderUECanonLedgerEntry* OutPromotedEntry)
{
	FOsvayderUECanonLedgerEntry Entry;
...
FString FOsvayderUECanonLedger::BuildPromptContext(
	const FString& Prompt,
	const FString& DetectedSubsystem,
	...
	FString& OutApprovedPatternKey)
{
	...
	int32 BestScore = 0;
	const FOsvayderUECanonLedgerEntry* BestEntry = nullptr;
...
void FOsvayderUECanonLedger::SetTestLedgerPathOverride(const FString& InPath)
{
	GCanonLedgerPathOverride = InPath;
}

void FOsvayderUECanonLedger::ClearTestLedgerPathOverride()
{
...
namespace OsvayderUECanonRouting
{
		...
		TArray<FAgentPromptContextBlock>& InOutContextBlocks)
	{
		...
		Execution.LedgerPath = FOsvayderUECanonLedger::GetLedgerPath();

		FString ApprovedPatternKey;
		const FString LedgerContext = FOsvayderUECanonLedger::BuildPromptContext(
			Prompt,
			Execution.DetectedSubsystem,
		...
		Execution.RequestedToolFamily = DetermineRequestedToolFamily(
			...
			Execution.DetectedSubsystem,
			FeatureRecipeId);
		if (LooksLocalAnimationPackPathPrompt(Prompt) && Execution.RequestedToolFamily == TEXT("unreal_animation"))
		{
			Execution.MandatoryAnimationWorkflowSteps = {
				TEXT("local_animation_pack_intake"),
				TEXT("animation_preflight")
			};
			Execution.ConditionalMandatoryAnimationWorkflowSteps = {
				TEXT("animation_retarget_fixup_if_skeleton_mismatch")
			};
			Execution.CloseoutGateReasonCodes = {
				TEXT("local_animation_pack_intake_required_missing"),
				TEXT("animation_preflight_required_missing"),
				TEXT("animation_retarget_required_missing"),
				TEXT("animation_compatibility_gate_failed")
			};
		}
	...
	TSharedPtr<FJsonObject> MakeCanonExecutionJson(const FAgentCanonExecution& Execution)
	{
		...
		if (Execution.MandatoryAnimationWorkflowSteps.Num() > 0)
		{
			...
			for (const FString& Step : Execution.MandatoryAnimationWorkflowSteps)
			{
				StepValues.Add(MakeShared<FJsonValueString>(Step));
			}
			Object->SetArrayField(TEXT("mandatory_animation_workflow_steps"), StepValues);
		}
		if (Execution.ConditionalMandatoryAnimationWorkflowSteps.Num() > 0)
		{
			TArray<TSharedPtr<FJsonValue>> StepValues;
			for (const FString& Step : Execution.ConditionalMandatoryAnimationWorkflowSteps)
			{
				StepValues.Add(MakeShared<FJsonValueString>(Step));
			}
			Object->SetArrayField(TEXT("conditional_mandatory_animation_workflow_steps"), StepValues);
		}
		if (Execution.CloseoutGateReasonCodes.Num() > 0)
		{
			TArray<TSharedPtr<FJsonValue>> ReasonValues;
			for (const FString& ReasonCode : Execution.CloseoutGateReasonCodes)
			{
				ReasonValues.Add(MakeShared<FJsonValueString>(ReasonCode));
			}
			Object->SetArrayField(TEXT("closeout_gate_reason_codes"), ReasonValues);
		}
