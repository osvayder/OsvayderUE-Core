...
- When launching `UnrealEditor-Cmd.exe` or `UnrealEditor.exe` for headless automation, `Automation RunTests`, PIE/runtime probes, or editor preflight from an acceptance/emulator run, include `-DDC-ForceMemoryCache` unless you first create and validate a writable configured local DDC path
- If an Unreal run fails with Derived Data Cache / `InstalledDerivedDataBackendGraph` / `no writable nodes`, treat it as an environment runner blocker and rerun with the DDC fallback; do not report it as a manual visual/gameplay blocker

RESTART-SURVIVAL BOUNDARY:
- On the ordinary Codex workspace-write runtime, `mcp__osvayderue__restart_survival` is the one allowed structured closed-editor escalation tool
...
- Run `mechanic_preflight` with operation=`inspect` before the first mutating tool call or file/content edit
- Inspect the current map/character/input context/source for existing action owners before touching files or assets; do not rely on stale docs alone
- Emit a current-run machine-readable mechanic preflight artifact covering requested mechanic, target map/class, requested inputs, existing actions, movement/camera/replication/animation conflicts, selected resolution, and whether implementation may continue
...
- If a conflict is resolved, mention the preflight artifact and selected resolution in final closeout evidence
- A final full/partial closeout must not ignore unresolved high/critical `mechanic_preflight` conflicts; report the exact conflict blocker instead
...
- Treat Docs/OsvayderUE/*.md as persistent working memory for this project
...
- While Unreal is closed, do not pretend editor-only MCP surfaces remain available)");
	...
	const FString LocalAnimationPackIntakeGuardText = TEXT(R"(LOCAL ANIMATION PACK INTAKE GUARD:
- If the user provides an explicit local animation pack path and asks to find/use animations, treat it as a bounded local_animation_pack_intake workflow
- First use `local_animation_pack_intake` for the local path; reserve `anim_blueprint_modify find_animations` for assets already under `/Game/...`
- Constrain discovery to the provided pack path and the assigned Unreal project only; never expand to parent directories, drive roots, or unrelated engine installs
- Do not run global `rg`, `grep`, `findstr`, or recursive `Get-ChildItem` against `D:\`, `C:\`, `C:\Program Files`, or similar broad roots unless the user provided one exact bounded engine source/header path for a read-only API lookup
- When copying `.uasset` content packs, preserve the native Unreal package root: `/Game/ParkourAnimations` must land at `<Project>/Content/ParkourAnimations`
- Copy only missing files; if an existing destination file does not byte-match, stop and report the mismatch instead of overwriting, moving, renaming, or deleting anything
- Close/reopen the assigned editor only when file locks or build/preflight actually require it
- After the assets exist under `/Game/...`, run `animation_preflight` with the imported `asset_root` / `pack_identifier` before claiming the pack is usable
- If `animation_preflight` finds candidate animations but reports `skeleton_mismatch` / `retarget_required_not_implemented`, call `animation_retarget_fixup` with the target AnimBP/skeleton, candidate paths or imported root, and a target-owned destination root before declaring a blocker
- Do not blind-hook mismatched animations; use only already-compatible assets or verified target-skeleton-compatible outputs from `animation_retarget_fixup`
- If retarget tooling/assets are absent, report the exact structured blocker such as `retarget_tooling_unavailable` instead of a generic closeout failure
- Unless the user explicitly requests mechanic/input changes, keep movement/input unchanged and treat animation wiring as presentation-only
- If the remaining proof gap is visual quality, montage feel, or PIE observation, emit a truthful manual blocker/checklist instead of claiming full completion)");
...
- When the user reports "didn't work" / "не работает" / "broken" / equivalent for a task you previously reported complete, your next turn MUST write a new entry to `Docs/OsvayderUE/observed_failures.md` BEFORE attempting any fix.
	...
	const FString& CustomSystemPrompt)
{
	...
	if (bIncludeEngineContext)
	{
		...
		Contract.MutationPolicy = MutationPolicyText + TEXT("\n\n") + LocalAnimationPackIntakeGuardText;
	...
	FString* OutBlockerDetail)
{
	...
	if (!OsvayderUERoleRegistry::TryGetRoleContract(RoleId, RoleContract))
	{
		...
		UpsertContextBlock(
			...
			TEXT("ROLE CONTRACT"),
			OsvayderUERoleRegistry::BuildRoleContractPromptContext(RoleId, RecipeId, EvidenceSchemaVersion));
		return false;
	}
	...
	if (ResolvedEvidenceSchemaVersion <= 0 && !RecipeId.IsEmpty())
	{
		...
		if (OsvayderUERecipeRegistry::TryGetRecipeEvidenceContract(RecipeId, RecipeContract))
		{
	...
	UpsertContextBlock(
		Contract.ContextBlocks,
		TEXT("ROLE CONTRACT"),
		OsvayderUERoleRegistry::BuildRoleContractPromptContext(
			RoleContract.RoleId,
			RecipeId,
