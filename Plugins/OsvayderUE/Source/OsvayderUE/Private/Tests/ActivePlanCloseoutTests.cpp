...
namespace
{
	FOsvayderUEActivePlan MakeCppMutationPlan()
	{
	...
	FOsvayderUEActivePlan MakeFeatureWorkflowPlan()
	{
		FOsvayderUEActivePlan Plan = MakeCppMutationPlan();
	...
	FOsvayderUEActivePlan MakeInteractionAccessFeatureWorkflowPlan()
	{
		FOsvayderUEActivePlan Plan = MakeCppMutationPlan();
	...
	FOsvayderUEActivePlan MakeVerificationOnlyInteractionAccessFeatureWorkflowPlan()
	{
		FOsvayderUEActivePlan Plan = MakeInteractionAccessFeatureWorkflowPlan();
		Plan.CompileProof = FOsvayderUECompileProofState();
		Plan.FeatureWorkflow.bCompileProofRequired = false;
		Plan.FeatureWorkflow.CompileProofState = TEXT("not_required");
		return Plan;
	}

	void PrimeCompileSuccess(FOsvayderUEActivePlan& Plan)
	{
	...
	void PrimeBoundedInteractionAccessCloseoutFacts(FOsvayderUECloseoutFactSnapshot& Facts)
	{
	...
	bool SaveCloseoutJsonArtifactForTest(const TSharedPtr<FJsonObject>& Object, const FString& Path)
	{
		FString JsonText;
	...
	TSharedPtr<FJsonObject> MakeCloseoutTraceUserPrompt(
		const FString& EventId,
	...
	{
		TSharedPtr<FJsonObject> Payload = MakeShared<FJsonObject>();
		Payload->SetStringField(TEXT("prompt_text"), PromptText);

		TSharedPtr<FJsonObject> Record = MakeShared<FJsonObject>();
		Record->SetStringField(TEXT("event_id"), EventId);
		Record->SetStringField(TEXT("event_type"), TEXT("user_prompt_submitted"));
		if (!RunId.IsEmpty())
		{
			Record->SetStringField(TEXT("run_id"), RunId);
		}
		...
		const FString& TimestampPrefix)
	{
		Records.Add(MakeCloseoutTraceToolResult(
			...
			TEXT("osvayderue/enhanced_input"),
		...
		Records.Add(MakeCloseoutTraceToolResult(
			...
			TEXT("osvayderue/get_level_actors"),
			TEXT("{}"),
	...
	FOsvayderUEActivePlan MakePacket662ExternalRuntimeProofPlan(const FString& Suffix)
	{
		FOsvayderUEActivePlan Plan = MakeVerificationOnlyInteractionAccessFeatureWorkflowPlan();
	...
	bool HasExternalRuntimeProofLogFact(const FOsvayderUECloseoutFactSnapshot& Facts)
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.ActivePlanCloseout.CppMutationWithoutCompileProofNotAchieved",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_CompileFailureAfterLatestMutationNotAchieved,
	"OsvayderUE.ActivePlanCloseout.CompileFailureAfterLatestMutationNotAchieved",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_CompileSuccessWithoutVerificationStaysPartial,
	"OsvayderUE.ActivePlanCloseout.CompileSuccessWithoutVerificationStaysPartial",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_CompileSuccessWithVerificationCanBeFull,
	"OsvayderUE.ActivePlanCloseout.CompileSuccessWithVerificationCanBeFull",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_NonCppTasksRemainFullOnSuccess,
	"OsvayderUE.ActivePlanCloseout.NonCppTasksRemainFullOnSuccess",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_StaleCompileProofAfterLaterMutationNotAchieved,
	"OsvayderUE.ActivePlanCloseout.StaleCompileProofAfterLaterMutationNotAchieved",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_MissingCompileWarningShownEvenForPartialResponse,
	"OsvayderUE.ActivePlanCloseout.MissingCompileWarningShownEvenForPartialResponse",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_VerificationFailureAfterCompileNotAchieved,
	"OsvayderUE.ActivePlanCloseout.VerificationFailureAfterCompileNotAchieved",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_FeatureWorkflowStopLossNotAchieved,
	"OsvayderUE.ActivePlanCloseout.FeatureWorkflowStopLossNotAchieved",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_FeatureWorkflowRuntimeProofMissingStaysPartial,
	"OsvayderUE.ActivePlanCloseout.FeatureWorkflowRuntimeProofMissingStaysPartial",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ManualAssetDependencyBlockerIsPrecise,
	"OsvayderUE.ActivePlanCloseout.ManualAssetDependencyBlockerIsPrecise",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_BlockedOnToolSurfaceIsPrecise,
	"OsvayderUE.ActivePlanCloseout.BlockedOnToolSurfaceIsPrecise",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_MechanicInputConflictIsPrecise,
	"OsvayderUE.ActivePlanCloseout.MechanicInputConflictIsPrecise",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_FeatureWorkflowCompileAndRuntimeCanBeFull,
	"OsvayderUE.ActivePlanCloseout.FeatureWorkflowCompileAndRuntimeCanBeFull",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_FeatureWorkflowAuthoringLaneDeniedNotAchieved,
	"OsvayderUE.ActivePlanCloseout.FeatureWorkflowAuthoringLaneDeniedNotAchieved",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_FeatureWorkflowAutomationDiscoveryZeroNotAchieved,
	"OsvayderUE.ActivePlanCloseout.FeatureWorkflowAutomationDiscoveryZeroNotAchieved",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_FeatureWorkflowAutomationExecutionSatisfiedCanBeFull,
	"OsvayderUE.ActivePlanCloseout.FeatureWorkflowAutomationExecutionSatisfiedCanBeFull",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_FeatureWorkflowVerificationOnlyAutomationAndSmokeCanBeFull,
	"OsvayderUE.ActivePlanCloseout.FeatureWorkflowVerificationOnlyAutomationAndSmokeCanBeFull",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_InteractionAccessRuntimeProofRequiresAttemptResolverObservation,
	"OsvayderUE.ActivePlanCloseout.InteractionAccessRuntimeProofRequiresAttemptResolverObservation",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_Review10PluginManagedArchiveRequiresAllEvidence,
	"OsvayderUE.ActivePlanCloseout.Review10PluginManagedArchiveRequiresAllEvidence",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_FeatureWorkflowProofPrerequisitesMissingNotAchieved,
	"OsvayderUE.ActivePlanCloseout.FeatureWorkflowProofPrerequisitesMissingNotAchieved",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_FeatureWorkflowStopLossRewritesFalseFullCloseout,
	"OsvayderUE.ActivePlanCloseout.FeatureWorkflowStopLossRewritesFalseFullCloseout",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_FeatureWorkflowStopLossRewritesBulletStatusFullCloseout,
	"OsvayderUE.ActivePlanCloseout.FeatureWorkflowStopLossRewritesBulletStatusFullCloseout",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_FeatureWorkflowRuntimeProofMissingRewritesFalseFullCloseout,
	"OsvayderUE.ActivePlanCloseout.FeatureWorkflowRuntimeProofMissingRewritesFalseFullCloseout",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicJudgeFactsOverrideStaleProofPrerequisiteStopLoss,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.FactsOverrideStaleProofPrerequisiteStopLoss",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicJudgeDocsProseAloneDoNotCompleteRuntimeProof,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.DocsProseAloneDoNotCompleteRuntimeProof",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicJudgeMissingAutomationRuntimeProofStaysNotFull,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.MissingAutomationRuntimeProofStaysNotFull",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicJudgeArbitraryShellDriftStillStopLosses,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.ArbitraryShellDriftStillStopLosses",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicAgentTraceFactExtractorConsumesTypedProofFacts,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.AgentTraceFactExtractorConsumesTypedProofFacts",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicManagedStateManualWriteRejectsAcceptance,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.ManagedStateManualWriteRejectsAcceptance",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ExecutionTruthManagedStateCommandRejectsWithoutLegacyMutationFlag,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.ExecutionTruthManagedStateCommandRejectsWithoutLegacyMutationFlag",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicReadOnlyTraceAuditDoesNotTriggerManagedStateWrite,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.ReadOnlyTraceAuditDoesNotTriggerManagedStateWrite",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ExternalRuntimeProofLogCurrentPrefixRecoversStopLoss,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.ExternalRuntimeProofLogCurrentPrefixRecoversStopLoss",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ExternalRuntimeProofLogStaleOrWrongPrefixRejected,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.ExternalRuntimeProofLogStaleOrWrongPrefixRejected",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ExternalRuntimeProofLogMalformedRejected,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.ExternalRuntimeProofLogMalformedRejected",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ExternalRuntimeProofRetryRecoversSameIdentity,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.ExternalRuntimeProofRetryRecoversSameIdentity",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ExternalRuntimeProofLaterPlanRejectsOlderValidLog,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.ExternalRuntimeProofLaterPlanRejectsOlderValidLog",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ExternalRuntimeProofStopLossPlanCanResumeSameWorkflow,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.ExternalRuntimeProofStopLossPlanCanResumeSameWorkflow",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ExternalRuntimeProofAdHocStopLossRecoversCurrentPrefix,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.ExternalRuntimeProofAdHocStopLossRecoversCurrentPrefix",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ExternalRuntimeProofLogReadsProjectSourceForCurrentPrefix,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.ExternalRuntimeProofLogReadsProjectSourceForCurrentPrefix",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_AttemptResolverStopLossRecoversFromCurrentPrefixProof,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.AttemptResolverStopLossRecoversFromCurrentPrefixProof",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ContinuitySchemaLabelsDoNotBecomeRefs,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.ContinuitySchemaLabelsDoNotBecomeRefs",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicCleanPluginOwnedCloseoutAccepted,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.CleanPluginOwnedCloseoutAccepted",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_RecipeRegistryUnknownRecipeFailsClosed,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.RecipeRegistryUnknownRecipeFailsClosed",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_RecipeRegistryMissingObligationNamesMissingFact,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.RecipeRegistryMissingObligationNamesMissingFact",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_RecipeRegistryDecisionJsonSerializesContract,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.RecipeRegistryDecisionJsonSerializesContract",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_RecipeRegistryOldRunEvidenceDoesNotSatisfyCurrentRecipe,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.RecipeRegistryOldRunEvidenceDoesNotSatisfyCurrentRecipe",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_RoleContractsUnknownRoleFailsClosed,
	"OsvayderUE.ActivePlanCloseout.RoleContracts.UnknownRoleFailsClosed",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_RoleContractsArchitectCannotCloseRuntimeProof,
	"OsvayderUE.ActivePlanCloseout.RoleContracts.ArchitectCannotCloseRuntimeProof",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_RoleContractsWorkerManagedStateWriteStillRejectsAcceptance,
	"OsvayderUE.ActivePlanCloseout.RoleContracts.WorkerManagedStateWriteStillRejectsAcceptance",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_RoleContractsVerifierDecisionJsonCitesCloseoutContract,
	"OsvayderUE.ActivePlanCloseout.RoleContracts.VerifierDecisionJsonCitesCloseoutContract",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicRunScopedOldTraceManualWriteDoesNotPoisonCleanRun,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.RunScopedOldTraceManualWriteDoesNotPoisonCleanRun",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicRunScopedCurrentRunManualWriteRejectsAcceptance,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.RunScopedCurrentRunManualWriteRejectsAcceptance",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicRunScopedDecisionJsonCarriesRunPlanIdentity,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.RunScopedDecisionJsonCarriesRunPlanIdentity",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicCleanPostProofMemoryUpdateCommandDriftLatchesAchieved,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.CleanPostProofMemoryUpdateCommandDriftLatchesAchieved",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicPostProofMemoryUpdateCommandDriftWithoutRuntimeProofStillFails,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.PostProofMemoryUpdateCommandDriftWithoutRuntimeProofStillFails",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicJudgeMissingProofInputMappingRejectsRuntimeProof,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.MissingProofInputMappingRejectsRuntimeProof",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicJudgePayloadJsonEvidenceConsumesProofInputMapping,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.PayloadJsonEvidenceConsumesProofInputMapping",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_SystemicCloseoutDecisionIdentityIsNonContradictoryAcrossRepeatedBuilds,
	"OsvayderUE.ActivePlanCloseout.SystemicVerifierJudge.CloseoutDecisionIdentityIsNonContradictoryAcrossRepeatedBuilds",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FActivePlanCloseout_CppMutationWithoutCompileProofNotAchieved::RunTest(const FString& Parameters)
{
	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseout(MakeCppMutationPlan(), true);
...
bool FActivePlanCloseout_CompileFailureAfterLatestMutationNotAchieved::RunTest(const FString& Parameters)
{
	...
	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseout(Plan, true);
...
{
	FOsvayderUEActivePlan Plan = MakeFeatureWorkflowPlan();
	Plan.FeatureWorkflow.bStopLossTriggered = true;
...
bool FActivePlanCloseout_Review10PluginManagedArchiveRequiresAllEvidence::RunTest(const FString& Parameters)
{
	...
	const FOsvayderUEActivePlanCloseoutDecision MissingSourceDecision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseout(Plan, true);
	...
	Plan.FeatureWorkflow.InteractionAccessAttemptResolverObservation.bAttemptResolverSourceObserved = true;
	Plan.FeatureWorkflow.InteractionAccessAttemptResolverObservation.bEventSubsystemSourceObserved = true;
	const FOsvayderUEActivePlanCloseoutDecision CompleteEvidenceDecision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseout(Plan, true);
...
bool FActivePlanCloseout_FeatureWorkflowStopLossRewritesFalseFullCloseout::RunTest(const FString& Parameters)
{
	...
	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseout(Plan, true);
	const FString Rewritten = SOsvayderEditorWidget::RewriteResponseForTruthfulActivePlanCloseout(
		Plan,
		Decision,
...
bool FActivePlanCloseout_SystemicJudgeFactsOverrideStaleProofPrerequisiteStopLoss::RunTest(const FString& Parameters)
{
	...
	const FAgentFeatureWorkflowState ReducedWorkflow =
		SOsvayderEditorWidget::ReduceFeatureWorkflowForCloseout(Plan.FeatureWorkflow, Facts);
	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseoutWithFacts(Plan, true, Facts);
...
bool FActivePlanCloseout_SystemicAgentTraceFactExtractorConsumesTypedProofFacts::RunTest(const FString& Parameters)
{
	...
	const FOsvayderUECloseoutFactSnapshot Facts =
		SOsvayderEditorWidget::ExtractAgentTraceFactsForCloseout(Records);
	FOsvayderUEActivePlan Plan = MakeVerificationOnlyInteractionAccessFeatureWorkflowPlan();
	Plan.FeatureWorkflow.CurrentPhase = TEXT("runtime_proof");
	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseoutWithFacts(Plan, true, Facts);
	FOsvayderUEActivePlanCloseoutDecision DecisionWithArchive = Decision;
	DecisionWithArchive.SourceArchivePath =
		TEXT("X:/PublicExample/Unreal/SampleProject/Saved/OsvayderUE/PlanArchives/test.active_plan.json");
	const TSharedPtr<FJsonObject> DecisionJson =
		SOsvayderEditorWidget::BuildCloseoutDecisionJson(Plan, DecisionWithArchive, Facts);
...
bool FActivePlanCloseout_SystemicManagedStateManualWriteRejectsAcceptance::RunTest(const FString& Parameters)
{
	...
	Records.Add(MakeCloseoutTraceToolUse(
		...
		TEXT("command_execution"),
		TEXT("Set-Content Saved/OsvayderUE/active_plan.json; Copy-Item Saved/OsvayderUE/active_plan.json Saved/OsvayderUE/PlanArchives/manual.active_plan.json; Set-Content Saved/OsvayderUE/CloseoutDecisions/manual.closeout_decision.json; Set-Content Saved/OsvayderUE/visible_session_codex_cli.json"),
		true,
	...
	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseoutWithFacts(Plan, true, Facts);
	const TSharedPtr<FJsonObject> DecisionJson =
		SOsvayderEditorWidget::BuildCloseoutDecisionJson(Plan, Decision, Facts);
	...
	const FString& Parameters)
{
	...
	Records.Add(MakeCloseoutTraceToolUse(
		...
		TEXT("command_execution"),
		TEXT("Set-Content Saved/OsvayderUE/active_plan.json '{}'; Out-File Saved/OsvayderUE/closeout_decision.json"),
		false,
	...
	const FOsvayderUERunCloseoutContext Context =
		SOsvayderEditorWidget::BuildRunCloseoutContext(
			Plan,
			Records,
			CurrentRunId,
			TEXT("X:/PublicExample/Unreal/SampleProject"));
	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseoutWithContext(Plan, true, Context);
	...
	const FString& Parameters)
{
	...
	const FString ReadOnlyAuditCommand =
		...
		TEXT("$manualManagedWrites = $cmds | Where-Object { ($_ -match 'Saved[\\\\/]+OsvayderUE') -and ($_ -match $manualManagedWritePattern) }");
	...
	Records.Add(MakeCloseoutTraceToolResult(
		...
		TEXT("command_execution"),
		ReadOnlyAuditCommand,
		TEXT("{\"CommandExecutionManagedStateWriteFlagCount\":0,\"ManualShellSavedOsvayderUEWritePatternCount\":0}"),
		CurrentRunId,
	...
	const FString& Parameters)
{
	...
	FOsvayderUEActivePlan Plan = MakePacket662ExternalRuntimeProofPlan(TEXT("current"));
	...
	const FOsvayderUERunCloseoutContext Context =
		SOsvayderEditorWidget::BuildRunCloseoutContext(Plan, Records, RunId, ProjectRoot);
	const FAgentFeatureWorkflowState ReducedWorkflow =
		SOsvayderEditorWidget::ReduceFeatureWorkflowForCloseout(Plan.FeatureWorkflow, Context.Facts);
	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseoutWithContext(Plan, true, Context);
	...
	const FString& Parameters)
{
	...
	FOsvayderUEActivePlan Plan = MakePacket662ExternalRuntimeProofPlan(TEXT("stale"));
	...
	const FString ArchiveDir =
		FPaths::Combine(ProjectRoot, TEXT("Saved"), TEXT("OsvayderUE"), TEXT("PlanArchives"));
	...
	const FString& Parameters)
{
	...
	FOsvayderUEActivePlan Plan = MakePacket662ExternalRuntimeProofPlan(TEXT("malformed"));
	...
	const FString& Parameters)
{
	...
	FOsvayderUEActivePlan Plan = MakePacket662ExternalRuntimeProofPlan(TEXT("retry_identity"));
	...
	const FString& Parameters)
{
	...
	FOsvayderUEActivePlan Plan = MakePacket662ExternalRuntimeProofPlan(TEXT("later_plan"));
	Plan.OriginalUserTask =
...
bool FActivePlanCloseout_ExternalRuntimeProofStopLossPlanCanResumeSameWorkflow::RunTest(
	const FString& Parameters)
{
	FOsvayderUEActivePlan Plan = MakePacket662ExternalRuntimeProofPlan(TEXT("resume_same_workflow"));
	...
	const FString& Parameters)
{
	...
	FOsvayderUEActivePlan Plan = MakePacket662ExternalRuntimeProofPlan(TEXT("ad_hoc_stop_loss"));
	...
	const FString& Parameters)
{
	...
	FOsvayderUEActivePlan Plan = MakePacket662ExternalRuntimeProofPlan(TEXT("project_source_ingestion"));
	...
	const FString& Parameters)
{
	...
	FOsvayderUEActivePlan Plan = MakePacket662ExternalRuntimeProofPlan(TEXT("attempt_resolver_stop_loss"));
...
bool FActivePlanCloseout_ContinuitySchemaLabelsDoNotBecomeRefs::RunTest(const FString& Parameters)
{
	...
	TestFalse(TEXT("schema-only final answer labels must not be treated as continuity references"),
		SOsvayderEditorWidget::PromptHasExplicitContinuityRefsForTests(SchemaOnlyPrompt));
	TestEqual(TEXT("schema-only final answer labels should describe no refs"),
		SOsvayderEditorWidget::DescribeExplicitContinuityRefsForTests(SchemaOnlyPrompt),
		FString());
	...
	const FString RealRefs = SOsvayderEditorWidget::DescribeExplicitContinuityRefsForTests(RealIdPrompt);
	...
	TestEqual(TEXT("reserved labels should be skipped so a later real plan id remains bindable"),
		SOsvayderEditorWidget::DescribeExplicitContinuityRefsForTests(MixedPrompt),
...
bool FActivePlanCloseout_SystemicCleanPluginOwnedCloseoutAccepted::RunTest(const FString& Parameters)
{
	...
	FOsvayderUEActivePlanCloseoutDecision DecisionWithArchive = Decision;
	DecisionWithArchive.SourceArchivePath =
		TEXT("Saved/OsvayderUE/PlanArchives/plugin_owned.active_plan.json");
	const TSharedPtr<FJsonObject> DecisionJson =
		SOsvayderEditorWidget::BuildCloseoutDecisionJson(Plan, DecisionWithArchive, Facts);
...
bool FActivePlanCloseout_RecipeRegistryDecisionJsonSerializesContract::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("decision should carry recipe id"),
		...
		OsvayderUERecipeRegistry::InteractionAccessRecipeId());
...
bool FActivePlanCloseout_SystemicRunScopedOldTraceManualWriteDoesNotPoisonCleanRun::RunTest(const FString& Parameters)
{
	...
	Records.Add(MakeCloseoutTraceToolUse(
		...
		TEXT("command_execution"),
		TEXT("Set-Content Saved/OsvayderUE/active_plan.json; Set-Content Saved/OsvayderUE/CloseoutDecisions/old.closeout_decision.json"),
		true,
		TEXT("workspace_file_build"),
		OldRunId,
...
bool FActivePlanCloseout_SystemicRunScopedCurrentRunManualWriteRejectsAcceptance::RunTest(const FString& Parameters)
{
	...
	Records.Add(MakeCloseoutTraceToolUse(
		...
		TEXT("command_execution"),
		TEXT("Copy-Item Saved/OsvayderUE/active_plan.json Saved/OsvayderUE/PlanArchives/current.active_plan.json; Set-Content Saved/OsvayderUE/visible_session_codex_cli.json"),
		true,
		TEXT("workspace_file_build"),
		CurrentRunId,
...
bool FActivePlanCloseout_SystemicRunScopedDecisionJsonCarriesRunPlanIdentity::RunTest(const FString& Parameters)
{
	...
	const FString ProjectRoot = TEXT("X:/PublicExample/Unreal/SampleProject");
	FOsvayderUEActivePlan Plan = MakeVerificationOnlyInteractionAccessFeatureWorkflowPlan();
	...
	FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseoutWithContext(Plan, true, Context);
	Decision.SourceArchivePath = TEXT("Saved/OsvayderUE/PlanArchives/run_scoped.active_plan.json");
	const TSharedPtr<FJsonObject> DecisionJson =
		SOsvayderEditorWidget::BuildCloseoutDecisionJson(Plan, Decision, Context);
...
{
	TSharedPtr<FJsonObject> Payload = MakeShared<FJsonObject>();
	Payload->SetStringField(TEXT("tool_name"), TEXT("osvayderue/enhanced_input"));
	...
	const FString& Parameters)
{
	...
	const FString ArchivePath =
		TEXT("Saved/OsvayderUE/PlanArchives/20260427-run_packet654_idempotent_clean-plan_packet654_idempotent.active_plan.json");
	FOsvayderUEActivePlan Plan = MakeVerificationOnlyInteractionAccessFeatureWorkflowPlan();
...
{
	FOsvayderUEActivePlan Plan = MakeVerificationOnlyInteractionAccessFeatureWorkflowPlan();
	Plan.FeatureWorkflow.RoleId = OsvayderUERoleRegistry::ArchitectRoleId();
	Plan.FeatureWorkflow.RuntimeProofState = TEXT("passed");
	PrimeProofInputMappingObservation(Plan.FeatureWorkflow);
...
{
	FOsvayderUEActivePlan Plan = MakeVerificationOnlyInteractionAccessFeatureWorkflowPlan();
	Plan.FeatureWorkflow.RoleId = OsvayderUERoleRegistry::VerifierRoleId();
	Plan.FeatureWorkflow.RuntimeProofState = TEXT("passed");
	PrimeProofInputMappingObservation(Plan.FeatureWorkflow);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.FallbackProviderSuccessRequiresCurrentProof",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FActivePlanCloseout_FallbackProviderSuccessRequiresCurrentProof::RunTest(const FString& Parameters)
{
	...
	const FOsvayderUEActivePlanCloseoutDecision GatedDecision =
		SOsvayderEditorWidget::ApplyActivePlanCloseoutSafetyGates(
			Plan,
			TEXT("status: full"),
			true,
			false,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.ExplicitFailureTextRejectsProviderSuccess",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.ExplicitManualAssetBlockerTextStaysPrecise",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ExplicitMechanicInputConflictTextStaysPrecise,
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.ExplicitMechanicInputConflictTextStaysPrecise",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.VisualReferenceRequiresProof",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FActivePlanCloseout_VisualReferenceRequiresProof::RunTest(const FString& Parameters)
{
	TestTrue(
		...
		SOsvayderEditorWidget::DoesPromptRequireVisualProof(TEXT("Создай combat inventory как на картинке"), TArray<FString>()));
	...
	const FOsvayderUEActivePlanCloseoutDecision GatedDecision = SOsvayderEditorWidget::EvaluateActivePlanCloseout(Plan, true);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.VisualReferenceRejectsBareScreenshotPath",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FActivePlanCloseout_VisualReferenceRejectsBareScreenshotPath::RunTest(const FString& Parameters)
{
	...
	const FOsvayderUEActivePlanCloseoutDecision GatedDecision =
		SOsvayderEditorWidget::ApplyActivePlanCloseoutSafetyGates(
			Plan,
			TEXT("status: full. Visual proof artifact path: Saved/OsvayderUE/Screenshots/combat_inventory.png"),
			true,
			true,
			Decision);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.VisualReferenceAllowsPassedManifestWithScreenshotPath",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FActivePlanCloseout_VisualReferenceAllowsPassedManifestWithScreenshotPath::RunTest(const FString& Parameters)
{
	...
	const FOsvayderUEActivePlanCloseoutDecision Decision = SOsvayderEditorWidget::EvaluateActivePlanCloseout(Plan, true);
	...
	FOsvayderUEActivePlanCloseoutDecision TransportDecision;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.VisualReferenceAllowsManualMechanicsChecklist",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.VisualReferenceFailedManifestBlocksCloseout",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.VisualReferenceRejectsStaleManifestPlanId",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.NonVisualTaskIgnoresVisualQaGate",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ClosedEditorSuccessManualGameplayProofGapIsPartial,
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.ClosedEditorSuccessManualGameplayProofGapIsPartial",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ManualVerificationPassedDoesNotBecomeManualRequired,
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.ManualVerificationPassedDoesNotBecomeManualRequired",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_ManualGameplayProofGapWithoutCurrentProofDoesNotPass,
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.ManualGameplayProofGapWithoutCurrentProofDoesNotPass",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_Packet702ManualGameplayProofGapWithoutCurrentProofIsPartial,
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.Packet702ManualGameplayProofGapWithoutCurrentProofIsPartial",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_CompileFailureTextStillFailsWithClosedEditorProof,
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.CompileFailureTextStillFailsWithClosedEditorProof",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_VisualManifestGateBeatsManualGameplayGap,
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.VisualManifestGateBeatsManualGameplayGap",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_NotCompletedRuntimeUntestedDoesNotBecomeManualPartial,
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.NotCompletedRuntimeUntestedDoesNotBecomeManualPartial",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_RussianSuccessfulManualGameplayProofGapIsPartial,
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.RussianSuccessfulManualGameplayProofGapIsPartial",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_RussianHardFailureTextStillFailsWithClosedEditorProof,
	"OsvayderUE.ActivePlanCloseout.RestartSurvival.RussianHardFailureTextStillFailsWithClosedEditorProof",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FActivePlanCloseout_ManualVerificationPassedDoesNotBecomeManualRequired::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_manual_verification_passed_full_success");

	FOsvayderUEActivePlanCloseoutDecision Decision;
	Decision.PlanStatus = TEXT("done");
	Decision.ResultStatus = TEXT("achieved_fully");

	const FOsvayderUEActivePlanCloseoutDecision PassedDecision =
		SOsvayderEditorWidget::ApplyActivePlanCloseoutSafetyGates(
			Plan,
			TEXT("Status: complete\nImplementation succeeded and build succeeded. Manual verification passed."),
			true,
			true,
			Decision);
	TestEqual(TEXT("manual verification passed should remain done"), PassedDecision.PlanStatus, FString(TEXT("done")));
	TestEqual(TEXT("manual verification passed should remain fully achieved"), PassedDecision.ResultStatus, FString(TEXT("achieved_fully")));
	TestNotEqual(
		TEXT("manual verification passed must not become manual-required"),
		PassedDecision.GateReasonCode,
		FString(TEXT("gameplay_runtime_visual_proof_manual_required")));

	const FOsvayderUEActivePlanCloseoutDecision CompletedDecision =
		SOsvayderEditorWidget::ApplyActivePlanCloseoutSafetyGates(
			Plan,
			TEXT("Status: complete\nImplementation succeeded and build succeeded. Manual verification completed."),
			true,
			true,
			Decision);
	TestEqual(TEXT("manual verification completed should remain done"), CompletedDecision.PlanStatus, FString(TEXT("done")));
	TestEqual(TEXT("manual verification completed should remain fully achieved"), CompletedDecision.ResultStatus, FString(TEXT("achieved_fully")));
	TestNotEqual(
		TEXT("manual verification completed must not become manual-required"),
		CompletedDecision.GateReasonCode,
		FString(TEXT("gameplay_runtime_visual_proof_manual_required")));
	return true;
}

bool FActivePlanCloseout_ClosedEditorSuccessManualGameplayProofGapIsPartial::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_telekinesis_closed_editor_manual_gap");
	Plan.ReviewerPlanReference = TEXT("reviewer_plan_current");
	...
	const FString FinalResponse =
		TEXT("Status: partial\n")
		TEXT("Implementation succeeded and build succeeded.\n")
		TEXT("PIE/runtime/visual gameplay verification was not performed; manual verification is required.");
	const FOsvayderUEActivePlanCloseoutDecision GatedDecision =
		SOsvayderEditorWidget::ApplyActivePlanCloseoutSafetyGates(
			Plan,
			FinalResponse,
			...
			Decision);
	const TSharedPtr<FJsonObject> DecisionJson =
		SOsvayderEditorWidget::BuildCloseoutDecisionJson(Plan, GatedDecision, FOsvayderUECloseoutFactSnapshot());
	const TSharedPtr<FJsonObject>* DecisionObject = nullptr;
	DecisionJson->TryGetObjectField(TEXT("decision"), DecisionObject);

	const FOsvayderUEUserFacingStatus UserStatus =
		OsvayderUEUserFacingStatus::BuildStatus(Plan, GatedDecision);
	bool bClosedEditorProofObserved = false;
	if (DecisionObject && (*DecisionObject).IsValid())
	{
		(*DecisionObject)->TryGetBoolField(TEXT("current_closed_editor_proof_observed"), bClosedEditorProofObserved);
	}

	TestEqual(TEXT("manual runtime/visual gap should still close the plan"), GatedDecision.PlanStatus, FString(TEXT("done")));
	TestEqual(TEXT("manual runtime/visual gap should be partial"), GatedDecision.ResultStatus, FString(TEXT("achieved_partially")));
	TestEqual(
		TEXT("manual runtime/visual gap should use exact gate"),
		GatedDecision.GateReasonCode,
		FString(TEXT("gameplay_runtime_visual_proof_manual_required")));
	TestTrue(TEXT("closed-editor proof should be recorded on the decision"), GatedDecision.bCurrentClosedEditorProofObserved);
	TestTrue(TEXT("closeout JSON should expose current closed-editor proof"), bClosedEditorProofObserved);
	TestNotEqual(
		TEXT("manual runtime/visual gap must not surface as closeout_failed"),
		UserStatus.StatusId,
		OsvayderUEUserFacingStatus::CloseoutFailedStatusId());
	TestEqual(
		TEXT("manual runtime/visual gap should be user-facing manual required"),
		UserStatus.StatusId,
		OsvayderUEUserFacingStatus::ManualVerificationRequiredStatusId());
	return true;
}

bool FActivePlanCloseout_Packet702ManualGameplayProofGapWithoutCurrentProofIsPartial::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_packet702_manual_gap_without_current_proof");
	Plan.ReviewerPlanReference = TEXT("reviewer_plan_current");
	...
	const FOsvayderUEUserFacingStatus UserStatus =
		OsvayderUEUserFacingStatus::BuildStatus(Plan, GatedDecision);
	bool bClosedEditorProofObserved = true;
	if (DecisionObject && (*DecisionObject).IsValid())
	{
		(*DecisionObject)->TryGetBoolField(TEXT("current_closed_editor_proof_observed"), bClosedEditorProofObserved);
	}

	TestEqual(TEXT("Packet702 manual gap without current proof should still close the plan"), GatedDecision.PlanStatus, FString(TEXT("done")));
	TestEqual(TEXT("Packet702 manual gap without current proof should be partial"), GatedDecision.ResultStatus, FString(TEXT("achieved_partially")));
	TestNotEqual(
		TEXT("Packet702 manual gap without current proof must not stay full"),
		GatedDecision.ResultStatus,
		FString(TEXT("achieved_fully")));
	TestEqual(
		TEXT("Packet702 manual gap without current proof should use exact gate"),
		GatedDecision.GateReasonCode,
		FString(TEXT("gameplay_runtime_visual_proof_manual_required")));
	TestEqual(
		TEXT("Packet702 manual gap without current proof should use manual blocker family"),
		GatedDecision.BlockerFamily,
		FString(TEXT("manual_verification_required")));
	TestFalse(TEXT("current closed-editor proof should remain false"), GatedDecision.bCurrentClosedEditorProofObserved);
	TestFalse(TEXT("closeout JSON should expose absent current closed-editor proof"), bClosedEditorProofObserved);
	TestNotEqual(
		TEXT("Packet702 manual gap must not surface as closeout_failed"),
		UserStatus.StatusId,
		OsvayderUEUserFacingStatus::CloseoutFailedStatusId());
	TestEqual(
		TEXT("Packet702 manual gap should be user-facing manual required"),
		UserStatus.StatusId,
		OsvayderUEUserFacingStatus::ManualVerificationRequiredStatusId());
	return true;
}

bool FActivePlanCloseout_ManualGameplayProofGapWithoutCurrentProofDoesNotPass::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_fallback_telekinesis_manual_gap_without_proof");
	Plan.ReviewerPlanReference = TEXT("post_reattach_state_json_fallback_plan_v1");
	...
	const FOsvayderUEActivePlanCloseoutDecision GatedDecision =
		SOsvayderEditorWidget::ApplyActivePlanCloseoutSafetyGates(
			Plan,
			TEXT("Implementation succeeded. Status: partial. PIE/runtime gameplay verification was not performed; manual verification is required."),
			true,
			false,
			Decision);

	TestEqual(TEXT("fallback/manual gap without current proof should fail"), GatedDecision.PlanStatus, FString(TEXT("failed")));
	TestEqual(TEXT("fallback/manual gap without current proof should not achieve"), GatedDecision.ResultStatus, FString(TEXT("not_achieved")));
	TestEqual(
		TEXT("missing typed proof should remain the blocker"),
		GatedDecision.GateReasonCode,
		FString(TEXT("fallback_closeout_proof_missing")));
	return true;
}

bool FActivePlanCloseout_CompileFailureTextStillFailsWithClosedEditorProof::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_compile_failure_text_with_closed_editor_proof");
	...
	const FOsvayderUEActivePlanCloseoutDecision GatedDecision =
		SOsvayderEditorWidget::ApplyActivePlanCloseoutSafetyGates(
			Plan,
			TEXT("Implementation attempted, but compile failed."),
			true,
			true,
			Decision);

	TestEqual(TEXT("compile failure text should fail"), GatedDecision.PlanStatus, FString(TEXT("failed")));
	TestEqual(TEXT("compile failure text should not achieve"), GatedDecision.ResultStatus, FString(TEXT("not_achieved")));
	TestEqual(
		TEXT("compile failure text should remain a final-response failure"),
		GatedDecision.GateReasonCode,
		FString(TEXT("final_response_declares_failure")));
	return true;
}

bool FActivePlanCloseout_VisualManifestGateBeatsManualGameplayGap::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_visual_manifest_still_required_manual_gap");
	Plan.bVisualProofRequired = true;
	Plan.bVisualQaManifestRequired = true;
	...
	const FOsvayderUEActivePlanCloseoutDecision GatedDecision =
		SOsvayderEditorWidget::ApplyActivePlanCloseoutSafetyGates(
			Plan,
			TEXT("Implementation and build succeeded. Status: partial. Visual/PIE verification was not performed; manual verification is required."),
			true,
			true,
			Decision);

	TestEqual(TEXT("visual reference should still require manifest proof"), GatedDecision.PlanStatus, FString(TEXT("failed")));
	TestEqual(TEXT("visual reference without manifest should not achieve"), GatedDecision.ResultStatus, FString(TEXT("not_achieved")));
	TestEqual(
		TEXT("visual manifest gate should beat manual gameplay gap"),
		GatedDecision.GateReasonCode,
		FString(TEXT("visual_qa_manifest_required_missing")));
	return true;
}

bool FActivePlanCloseout_NotCompletedRuntimeUntestedDoesNotBecomeManualPartial::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_not_completed_runtime_untested");
	...
	const FOsvayderUEActivePlanCloseoutDecision GatedDecision =
		SOsvayderEditorWidget::ApplyActivePlanCloseoutSafetyGates(
			Plan,
			TEXT("Implementation not completed. Runtime was not tested."),
			true,
			true,
			Decision);

	TestEqual(TEXT("not completed text should fail"), GatedDecision.PlanStatus, FString(TEXT("failed")));
	TestEqual(TEXT("not completed text should not achieve"), GatedDecision.ResultStatus, FString(TEXT("not_achieved")));
	TestEqual(
		TEXT("not completed text should remain generic final-response failure"),
		GatedDecision.GateReasonCode,
		FString(TEXT("final_response_declares_failure")));
	TestNotEqual(
		TEXT("not completed text must not become manual proof gap"),
		GatedDecision.GateReasonCode,
		FString(TEXT("gameplay_runtime_visual_proof_manual_required")));
	return true;
}

bool FActivePlanCloseout_RussianSuccessfulManualGameplayProofGapIsPartial::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_russian_success_manual_gap");
	...
	const FOsvayderUEActivePlanCloseoutDecision GatedDecision =
		SOsvayderEditorWidget::ApplyActivePlanCloseoutSafetyGates(
			Plan,
			TEXT("Реализация выполнена, сборка прошла. Статус: частично. PIE/runtime/визуальная проверка не выполнялась; нужна ручная проверка."),
			true,
			true,
			Decision);
	const FOsvayderUEUserFacingStatus UserStatus =
	...
	TestEqual(TEXT("Russian manual runtime/visual gap should close the plan"), GatedDecision.PlanStatus, FString(TEXT("done")));
	TestEqual(TEXT("Russian manual runtime/visual gap should be partial"), GatedDecision.ResultStatus, FString(TEXT("achieved_partially")));
	TestEqual(
		TEXT("Russian manual runtime/visual gap should use exact gate"),
		GatedDecision.GateReasonCode,
		FString(TEXT("gameplay_runtime_visual_proof_manual_required")));
	TestEqual(
		TEXT("Russian manual runtime/visual gap should be user-facing manual required"),
		UserStatus.StatusId,
		OsvayderUEUserFacingStatus::ManualVerificationRequiredStatusId());
	return true;
}

bool FActivePlanCloseout_RussianHardFailureTextStillFailsWithClosedEditorProof::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_russian_hard_failure");
	...
	const FOsvayderUEActivePlanCloseoutDecision GatedDecision =
		SOsvayderEditorWidget::ApplyActivePlanCloseoutSafetyGates(
			Plan,
			TEXT("Сборка не прошла, механика не реализована."),
			true,
			true,
			Decision);

	TestEqual(TEXT("Russian hard failure should fail"), GatedDecision.PlanStatus, FString(TEXT("failed")));
	TestEqual(TEXT("Russian hard failure should not achieve"), GatedDecision.ResultStatus, FString(TEXT("not_achieved")));
	TestEqual(
		TEXT("Russian hard failure should remain final-response failure"),
		GatedDecision.GateReasonCode,
		FString(TEXT("final_response_declares_failure")));
	return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_LocalAnimationPackIntakeMissingBlocksSuccess,
	"OsvayderUE.ActivePlanCloseout.AnimationWorkflow.LocalPackIntakeMissingBlocksSuccess",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FActivePlanCloseout_LocalAnimationPackIntakeMissingBlocksSuccess::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_animation_local_pack_missing_intake");

	TArray<TSharedPtr<FJsonObject>> Records;
	Records.Add(MakeCloseoutTraceUserPrompt(
		TEXT("prompt_local_pack"),
		TEXT("Create parkour wall climb and use animations from X:/PublicExample/AnimationPacks/7 -Parkour Animation/ParkourAnimations/Animations/UE5_Skelet."),
		TEXT("run_animation_local_pack_missing_intake"),
		TEXT("2026-05-05T00:00:00Z")));

	const FOsvayderUERunCloseoutContext Context = SOsvayderEditorWidget::BuildRunCloseoutContext(
		Plan,
		Records,
		TEXT("run_animation_local_pack_missing_intake"),
		TEXT("X:/PublicExample/Unreal/SampleProject"));
	...
	TestEqual(TEXT("local pack prompt without intake should fail"), Decision.PlanStatus, FString(TEXT("failed")));
	TestEqual(
		TEXT("local pack prompt without intake should use stable gate reason"),
		Decision.GateReasonCode,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_LocalAnimationPackPreflightMissingBlocksSuccess,
	"OsvayderUE.ActivePlanCloseout.AnimationWorkflow.LocalPackPreflightMissingBlocksSuccess",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FActivePlanCloseout_LocalAnimationPackPreflightMissingBlocksSuccess::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_animation_local_pack_missing_preflight");

	TArray<TSharedPtr<FJsonObject>> Records;
	Records.Add(MakeCloseoutTraceUserPrompt(
		TEXT("prompt_local_pack_preflight"),
		TEXT("Use local animation pack X:/PublicExample/AnimationPacks/Parkour for wallrun animation wiring."),
		TEXT("run_animation_local_pack_missing_preflight"),
		TEXT("2026-05-05T00:00:00Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("intake_success"),
		TEXT("osvayderue/local_animation_pack_intake"),
		TEXT("{\"source_root\":\"X:/PublicExample/AnimationPacks/Parkour\"}"),
		TEXT("{\"result_type\":\"local_animation_pack_intake\",\"success\":true,\"status\":\"completed\",\"imported_root\":\"/Game/ParkourAnimations\"}"),
		TEXT("run_animation_local_pack_missing_preflight"),
		TEXT("2026-05-05T00:00:01Z")));
	...
	TestEqual(TEXT("local pack intake without preflight should fail"), Decision.PlanStatus, FString(TEXT("failed")));
	TestEqual(
		TEXT("local pack intake without preflight should use stable gate reason"),
		Decision.GateReasonCode,
		FString(TEXT("animation_preflight_required_missing")));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_AnimationMismatchRequiresRetargetFixup,
	"OsvayderUE.ActivePlanCloseout.AnimationWorkflow.MismatchRequiresRetargetFixup",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FActivePlanCloseout_AnimationMismatchRequiresRetargetFixup::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_animation_mismatch_requires_retarget");

	TArray<TSharedPtr<FJsonObject>> Records;
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("preflight_mismatch"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\",\"asset_root\":\"/Game/ParkourAnimations\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"blocker_codes\":[\"skeleton_mismatch\",\"retarget_required_not_implemented\"],\"recommendation_codes\":[\"run_animation_retarget_fixup\"],\"proof_classification\":\"manual_blocker\"}"),
		TEXT("run_animation_mismatch_requires_retarget"),
		TEXT("2026-05-05T00:00:00Z")));
	...
	TestEqual(TEXT("mismatch without retarget should fail"), Decision.PlanStatus, FString(TEXT("failed")));
	TestEqual(
		TEXT("mismatch without retarget should use stable gate reason"),
		Decision.GateReasonCode,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_AnimationRetargetNeedsCompatiblePostPreflight,
	"OsvayderUE.ActivePlanCloseout.AnimationWorkflow.RetargetNeedsCompatiblePostPreflight",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FActivePlanCloseout_AnimationRetargetNeedsCompatiblePostPreflight::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_animation_retarget_needs_post_preflight");

	TArray<TSharedPtr<FJsonObject>> Records;
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("preflight_mismatch_before_retarget"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\",\"asset_root\":\"/Game/ParkourAnimations\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"blocker_codes\":[\"skeleton_mismatch\"],\"proof_classification\":\"manual_blocker\"}"),
		TEXT("run_animation_retarget_needs_post_preflight"),
		TEXT("2026-05-05T00:00:00Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("retarget_success_without_post_preflight"),
		TEXT("osvayderue/animation_retarget_fixup"),
		TEXT("{\"operation\":\"animation_retarget_fixup\",\"asset_root\":\"/Game/ParkourAnimations\"}"),
		TEXT("{\"result_type\":\"animation_retarget_fixup\",\"success\":true,\"status\":\"completed\",\"ready_to_claim_retarget_success\":true,\"generated_assets\":[\"/Game/ParkourRetargeted/WallClimb\"]}"),
		TEXT("run_animation_retarget_needs_post_preflight"),
		TEXT("2026-05-05T00:00:01Z")));
	...
	TestEqual(TEXT("retarget without compatible post-preflight should fail"), Decision.PlanStatus, FString(TEXT("failed")));
	TestEqual(
		TEXT("retarget without compatible post-preflight should use stable gate reason"),
		Decision.GateReasonCode,
		FString(TEXT("animation_compatibility_gate_failed")));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_AnimationRetargetAndCompatiblePostPreflightPassesCompatibilityGate,
	"OsvayderUE.ActivePlanCloseout.AnimationWorkflow.RetargetAndCompatiblePostPreflightPassesCompatibilityGate",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FActivePlanCloseout_AnimationRetargetAndCompatiblePostPreflightPassesCompatibilityGate::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_animation_retarget_post_preflight_passes");

	TArray<TSharedPtr<FJsonObject>> Records;
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("preflight_mismatch_before_retarget_pass"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\",\"asset_root\":\"/Game/ParkourAnimations\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"blocker_codes\":[\"skeleton_mismatch\",\"retarget_required_not_implemented\"],\"proof_classification\":\"manual_blocker\"}"),
		TEXT("run_animation_retarget_post_preflight_passes"),
		TEXT("2026-05-05T00:00:00Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("retarget_success_before_post_preflight_pass"),
		TEXT("osvayderue/animation_retarget_fixup"),
		TEXT("{\"operation\":\"animation_retarget_fixup\",\"asset_root\":\"/Game/ParkourAnimations\"}"),
		TEXT("{\"result_type\":\"animation_retarget_fixup\",\"success\":true,\"status\":\"completed\",\"ready_to_claim_retarget_success\":true,\"generated_assets\":[\"/Game/ParkourRetargeted/WallClimb\"]}"),
		TEXT("run_animation_retarget_post_preflight_passes"),
		TEXT("2026-05-05T00:00:01Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("post_retarget_preflight_compatible"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\",\"asset_root\":\"/Game/ParkourRetargeted\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"preflight_ready_for_manual_verification\":true,\"blocker_codes\":[],\"compatible_roles\":[\"wall_climb\",\"ledge_attach\",\"mantle\"],\"proof_classification\":\"gameplay_code_proof\"}"),
		TEXT("run_animation_retarget_post_preflight_passes"),
		TEXT("2026-05-05T00:00:02Z")));
	...
	TestEqual(TEXT("compatible post-retarget preflight should not fail closeout"), Decision.PlanStatus, FString(TEXT("done")));
	TestEqual(TEXT("compatible post-retarget preflight should fully achieve when no other gates apply"), Decision.ResultStatus, FString(TEXT("achieved_fully")));
	TestTrue(TEXT("compatible post-retarget preflight should leave gate reason empty"), Decision.GateReasonCode.IsEmpty());
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_RealPlanOriginalTaskLocalPackMissingIntakeBlocksSuccess,
	"OsvayderUE.ActivePlanCloseout.AnimationWorkflow.RealPlanOriginalTaskLocalPackMissingIntakeBlocksSuccess",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FActivePlanCloseout_RealPlanOriginalTaskLocalPackMissingIntakeBlocksSuccess::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_animation_real_origin_task_missing_intake");
	Plan.OriginalUserTask =
		TEXT("Создай parkour wall climb. В папке X:\\PublicExample\\AnimationPacks\\7 -Parkour Animation\\ParkourAnimations\\Animations\\UE5_Skelet посмотри подходящие анимации и примени их.");

	const FOsvayderUECloseoutFactSnapshot Facts = SOsvayderEditorWidget::ExtractCloseoutFactsFromPlan(Plan);
	TestTrue(
		TEXT("original_user_task local Windows animation path should require local pack intake"),
		Facts.bLocalAnimationPackIntakeRequired);

	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseout(Plan, true);
	TestEqual(TEXT("origin-task local pack without intake should fail"), Decision.PlanStatus, FString(TEXT("failed")));
	TestEqual(
		TEXT("origin-task local pack without intake should use precise gate"),
		Decision.GateReasonCode,
		FString(TEXT("local_animation_pack_intake_required_missing")));
	return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_CommandExecutionMcpAnimationPreflightMismatchBlocksSuccess,
	"OsvayderUE.ActivePlanCloseout.AnimationWorkflow.CommandExecutionMcpAnimationPreflightMismatchBlocksSuccess",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FActivePlanCloseout_CommandExecutionMcpAnimationPreflightMismatchBlocksSuccess::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_animation_command_execution_preflight_mismatch");

	TArray<TSharedPtr<FJsonObject>> Records;
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("mcp_http_preflight_mismatch"),
		TEXT("command_execution"),
		TEXT("Invoke-WebRequest -UseBasicParsing -Uri http://localhost:3000/mcp/tools -Headers @{ Authorization = 'Bearer token' } -Method Post -Body '{\"tool_name\":\"blueprint_query\",\"operation\":\"animation_preflight\"}'"),
		TEXT("{\"success\":true,\"data\":{\"result_type\":\"animation_preflight\",\"operation\":\"animation_preflight\",\"blocker_codes\":[\"skeleton_mismatch\",\"retarget_required_not_implemented\"],\"recommendation_codes\":[\"run_animation_retarget_fixup\"],\"proof_classification\":\"manual_blocker\"}}"),
		TEXT("run_animation_command_execution_preflight_mismatch"),
		TEXT("2026-05-05T00:00:00Z")));
	...
	const FOsvayderUERunCloseoutContext Context = SOsvayderEditorWidget::BuildRunCloseoutContext(
		...
		TEXT("run_animation_command_execution_preflight_mismatch"),
		TEXT("X:/PublicExample/Unreal/SampleProject"));
	TestTrue(TEXT("command_execution MCP preflight should be observed"), Context.Facts.bAnimationPreflightObserved);
	TestTrue(
		TEXT("command_execution MCP preflight should preserve skeleton mismatch"),
		Context.Facts.bAnimationPreflightSkeletonMismatchObserved);

	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseoutWithContext(Plan, true, Context);
	TestEqual(TEXT("MCP preflight mismatch without retarget should fail"), Decision.PlanStatus, FString(TEXT("failed")));
	TestEqual(
		TEXT("MCP preflight mismatch without retarget should use precise gate"),
		Decision.GateReasonCode,
		FString(TEXT("animation_retarget_required_missing")));
	return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_CompatiblePreflightBeforeLatestMismatchDoesNotSatisfyGate,
	"OsvayderUE.ActivePlanCloseout.AnimationWorkflow.CompatiblePreflightBeforeLatestMismatchDoesNotSatisfyGate",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FActivePlanCloseout_CompatiblePreflightBeforeLatestMismatchDoesNotSatisfyGate::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_animation_compatible_before_latest_mismatch");

	TArray<TSharedPtr<FJsonObject>> Records;
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("preflight_mismatch_early"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"blocker_codes\":[\"skeleton_mismatch\"],\"proof_classification\":\"manual_blocker\"}"),
		TEXT("run_animation_compatible_before_latest_mismatch"),
		TEXT("2026-05-05T00:00:00Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("retarget_success_early"),
		TEXT("osvayderue/animation_retarget_fixup"),
		TEXT("{\"operation\":\"animation_retarget_fixup\"}"),
		TEXT("{\"result_type\":\"animation_retarget_fixup\",\"success\":true,\"status\":\"completed\",\"ready_to_claim_retarget_success\":true,\"generated_assets\":[\"/Game/ParkourRetargeted/WallClimb\"]}"),
		TEXT("run_animation_compatible_before_latest_mismatch"),
		TEXT("2026-05-05T00:00:01Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("compatible_preflight_early"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"preflight_ready_for_manual_verification\":true,\"blocker_codes\":[],\"compatible_roles\":[\"wall_climb\"]}"),
		TEXT("run_animation_compatible_before_latest_mismatch"),
		TEXT("2026-05-05T00:00:02Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("preflight_mismatch_latest"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"blocker_codes\":[\"skeleton_mismatch\"],\"proof_classification\":\"manual_blocker\"}"),
		TEXT("run_animation_compatible_before_latest_mismatch"),
		TEXT("2026-05-05T00:00:03Z")));
	...
	const FOsvayderUERunCloseoutContext Context = SOsvayderEditorWidget::BuildRunCloseoutContext(
		...
		TEXT("X:/PublicExample/Unreal/SampleProject"));
	TestFalse(
		TEXT("compatible preflight before latest mismatch should be reset"),
		Context.Facts.bPostRetargetCompatibleAnimationPreflightObserved);

	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseoutWithContext(Plan, true, Context);
	TestEqual(TEXT("latest mismatch after compatible preflight should fail"), Decision.PlanStatus, FString(TEXT("failed")));
	TestEqual(
		TEXT("latest mismatch should require a later retarget"),
		Decision.GateReasonCode,
		FString(TEXT("animation_retarget_required_missing")));
	return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_AnimationRoleLineageMissingRoleBlocksFalseGreen,
	"OsvayderUE.ActivePlanCloseout.AnimationWorkflow.RoleLineageMissingRoleBlocksFalseGreen",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FActivePlanCloseout_AnimationRoleLineageMissingRoleBlocksFalseGreen::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_animation_role_lineage_missing_role");

	TArray<TSharedPtr<FJsonObject>> Records;
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_missing_role_mismatch"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"blocker_codes\":[\"skeleton_mismatch\"],\"unsatisfied_roles\":[\"wall_climb\",\"ledge_attach\"],\"role_inventory\":[{\"role\":\"wall_climb\",\"status\":\"skeleton_mismatch\",\"matches\":[{\"path\":\"/Game/ParkourAnimations/A_WallClimb\"}]},{\"role\":\"ledge_attach\",\"status\":\"skeleton_mismatch\",\"matches\":[{\"path\":\"/Game/ParkourAnimations/A_LedgeAttach\"}]}],\"proof_classification\":\"manual_blocker\"}"),
		TEXT("run_animation_role_lineage_missing_role"),
		TEXT("2026-05-05T00:00:00Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_missing_role_retarget"),
		TEXT("osvayderue/animation_retarget_fixup"),
		TEXT("{\"operation\":\"animation_retarget_fixup\"}"),
		TEXT("{\"result_type\":\"animation_retarget_fixup\",\"success\":true,\"status\":\"completed\",\"ready_to_claim_retarget_success\":true,\"generated_assets\":[{\"role\":\"wall_climb\",\"source_animation_path\":\"/Game/ParkourAnimations/A_WallClimb\",\"destination_object_path\":\"/Game/Variant_Platforming/Anims/A_WallClimb_Retargeted\"}]}"),
		TEXT("run_animation_role_lineage_missing_role"),
		TEXT("2026-05-05T00:00:01Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_missing_role_compatible"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"preflight_ready_for_manual_verification\":true,\"blocker_codes\":[],\"compatible_roles\":[\"wall_climb\"],\"role_inventory\":[{\"role\":\"wall_climb\",\"status\":\"compatible\",\"matches\":[{\"path\":\"/Game/Variant_Platforming/Anims/A_WallClimb_Retargeted\"}]}],\"proof_classification\":\"gameplay_code_proof\"}"),
		TEXT("run_animation_role_lineage_missing_role"),
		TEXT("2026-05-05T00:00:02Z")));
	...
	const FOsvayderUERunCloseoutContext Context = SOsvayderEditorWidget::BuildRunCloseoutContext(
		...
		TEXT("run_animation_role_lineage_missing_role"),
		TEXT("X:/PublicExample/Unreal/SampleProject"));
	TestTrue(TEXT("structured required roles should activate lineage gate"), Context.Facts.bAnimationLineageRoleEvidencePresent);
	TestTrue(TEXT("ledge_attach should be missing from compatible proof"), Context.Facts.MissingAnimationLineageRoles.Contains(TEXT("ledge_attach")));

	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseoutWithContext(Plan, true, Context);
	TestEqual(TEXT("missing required role should fail"), Decision.PlanStatus, FString(TEXT("failed")));
	TestEqual(
		TEXT("missing required role should use precise lineage gate"),
		Decision.GateReasonCode,
		FString(TEXT("animation_required_roles_not_compatible")));

	const FString ArtifactDir = FPaths::Combine(
		TEXT("X:/PublicExample/Unreal/SampleProject/Saved/Logs"),
		TEXT("packet723_lineage_replay"));
	const FString CloseoutPath = FPaths::Combine(ArtifactDir, TEXT("packet723_lineage_missing_role.closeout_decision.json"));
	const FString ReceiptPath = FPaths::Combine(ArtifactDir, TEXT("packet723_lineage_missing_role.headless_acceptance_receipt.json"));
	TestTrue(
		TEXT("lineage false-green replay closeout artifact should save"),
		SaveCloseoutJsonArtifactForTest(SOsvayderEditorWidget::BuildCloseoutDecisionJson(Plan, Decision, Context), CloseoutPath));
	FOsvayderUEHeadlessAcceptanceReceiptContext ReceiptContext;
	ReceiptContext.Request.Prefix = TEXT("packet723_lineage_missing_role");
	ReceiptContext.Request.OutputDir = ArtifactDir;
	ReceiptContext.ReceiptPath = ReceiptPath;
	ReceiptContext.StartedAtUtc = TEXT("2026-05-05T00:00:00Z");
	ReceiptContext.CompletedAtUtc = TEXT("2026-05-05T00:00:03Z");
	ReceiptContext.Status = TEXT("closeout_failed");
	ReceiptContext.FailureReason = Decision.GateReasonCode;
	ReceiptContext.ActiveRunIdFallback = TEXT("run_animation_role_lineage_missing_role");
	ReceiptContext.LatestCloseoutPath = CloseoutPath;
	ReceiptContext.bAssistantSuccess = true;
	ReceiptContext.bHasCloseoutDecision = true;
	ReceiptContext.CloseoutDecision = Decision;
	TestTrue(
		TEXT("lineage false-green replay receipt artifact should save"),
		SaveCloseoutJsonArtifactForTest(SOsvayderEditorWidget::BuildHeadlessAcceptanceReceiptJson(ReceiptContext), ReceiptPath));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_AnimationRoleLineageOriginalSourcePathBlocksFalseGreen,
	"OsvayderUE.ActivePlanCloseout.AnimationWorkflow.RoleLineageOriginalSourcePathBlocksFalseGreen",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FActivePlanCloseout_AnimationRoleLineageOriginalSourcePathBlocksFalseGreen::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_animation_role_lineage_source_path");

	TArray<TSharedPtr<FJsonObject>> Records;
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_source_path_mismatch"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"blocker_codes\":[\"skeleton_mismatch\"],\"unsatisfied_roles\":[\"wall_climb\"],\"role_inventory\":[{\"role\":\"wall_climb\",\"status\":\"skeleton_mismatch\",\"matches\":[{\"path\":\"/Game/ParkourAnimations/A_WallClimb\"}]}],\"proof_classification\":\"manual_blocker\"}"),
		TEXT("run_animation_role_lineage_source_path"),
		TEXT("2026-05-05T00:00:00Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_source_path_retarget"),
		TEXT("osvayderue/animation_retarget_fixup"),
		TEXT("{\"operation\":\"animation_retarget_fixup\"}"),
		TEXT("{\"result_type\":\"animation_retarget_fixup\",\"success\":true,\"status\":\"completed\",\"ready_to_claim_retarget_success\":true,\"generated_assets\":[{\"role\":\"wall_climb\",\"source_animation_path\":\"/Game/ParkourAnimations/A_WallClimb\",\"destination_object_path\":\"/Game/Variant_Platforming/Anims/A_WallClimb_Retargeted\"}]}"),
		TEXT("run_animation_role_lineage_source_path"),
		TEXT("2026-05-05T00:00:01Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_source_path_compatible"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"preflight_ready_for_manual_verification\":true,\"blocker_codes\":[],\"compatible_roles\":[\"wall_climb\"],\"role_inventory\":[{\"role\":\"wall_climb\",\"status\":\"compatible\",\"matches\":[{\"path\":\"/Game/ParkourAnimations/A_WallClimb\"}]}],\"proof_classification\":\"gameplay_code_proof\"}"),
		TEXT("run_animation_role_lineage_source_path"),
		TEXT("2026-05-05T00:00:02Z")));
	...
	const FOsvayderUERunCloseoutContext Context = SOsvayderEditorWidget::BuildRunCloseoutContext(
		...
		TEXT("run_animation_role_lineage_source_path"),
		TEXT("X:/PublicExample/Unreal/SampleProject"));
	TestTrue(TEXT("path lineage check should be applied"), Context.Facts.bAnimationLineagePathCheckApplied);
	TestTrue(TEXT("wall_climb should be path-mismatched"), Context.Facts.AnimationLineagePathMismatchRoles.Contains(TEXT("wall_climb")));

	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseoutWithContext(Plan, true, Context);
	TestEqual(TEXT("compatible role pointing at source path should fail"), Decision.PlanStatus, FString(TEXT("failed")));
	TestEqual(
		TEXT("source path should use path lineage gate"),
		Decision.GateReasonCode,
		FString(TEXT("animation_role_lineage_gate_failed")));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_AnimationRoleLineageCompleteCoveragePasses,
	"OsvayderUE.ActivePlanCloseout.AnimationWorkflow.RoleLineageCompleteCoveragePasses",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FActivePlanCloseout_AnimationRoleLineageCompleteCoveragePasses::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_animation_role_lineage_complete");

	TArray<TSharedPtr<FJsonObject>> Records;
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_complete_mismatch"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"blocker_codes\":[\"skeleton_mismatch\"],\"unsatisfied_roles\":[\"wall_climb\",\"ledge_attach\"],\"role_inventory\":[{\"role\":\"wall_climb\",\"status\":\"skeleton_mismatch\",\"matches\":[{\"path\":\"/Game/ParkourAnimations/A_WallClimb\"}]},{\"role\":\"ledge_attach\",\"status\":\"skeleton_mismatch\",\"matches\":[{\"path\":\"/Game/ParkourAnimations/A_LedgeAttach\"}]}],\"proof_classification\":\"manual_blocker\"}"),
		TEXT("run_animation_role_lineage_complete"),
		TEXT("2026-05-05T00:00:00Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_complete_retarget"),
		TEXT("osvayderue/animation_retarget_fixup"),
		TEXT("{\"operation\":\"animation_retarget_fixup\"}"),
		TEXT("{\"result_type\":\"animation_retarget_fixup\",\"success\":true,\"status\":\"completed\",\"ready_to_claim_retarget_success\":true,\"generated_assets\":[{\"role\":\"wall_climb\",\"source_animation_path\":\"/Game/ParkourAnimations/A_WallClimb\",\"destination_object_path\":\"/Game/Variant_Platforming/Anims/A_WallClimb_Retargeted\"},{\"role\":\"ledge_attach\",\"source_animation_path\":\"/Game/ParkourAnimations/A_LedgeAttach\",\"destination_object_path\":\"/Game/Variant_Platforming/Anims/A_LedgeAttach_Retargeted\"}]}"),
		TEXT("run_animation_role_lineage_complete"),
		TEXT("2026-05-05T00:00:01Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_complete_compatible"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"preflight_ready_for_manual_verification\":true,\"blocker_codes\":[],\"compatible_roles\":[\"wall_climb\",\"ledge_attach\"],\"role_inventory\":[{\"role\":\"wall_climb\",\"status\":\"compatible\",\"matches\":[{\"path\":\"/Game/Variant_Platforming/Anims/A_WallClimb_Retargeted\"}]},{\"role\":\"ledge_attach\",\"status\":\"compatible\",\"matches\":[{\"path\":\"/Game/Variant_Platforming/Anims/A_LedgeAttach_Retargeted\"}]}],\"proof_classification\":\"gameplay_code_proof\"}"),
		TEXT("run_animation_role_lineage_complete"),
		TEXT("2026-05-05T00:00:02Z")));
	...
	const FOsvayderUERunCloseoutContext Context = SOsvayderEditorWidget::BuildRunCloseoutContext(
		...
		TEXT("run_animation_role_lineage_complete"),
		TEXT("X:/PublicExample/Unreal/SampleProject"));
	TestTrue(TEXT("complete lineage should apply path check"), Context.Facts.bAnimationLineagePathCheckApplied);
	TestEqual(TEXT("complete lineage should have no missing roles"), Context.Facts.MissingAnimationLineageRoles.Num(), 0);
	TestEqual(TEXT("complete lineage should have no path mismatches"), Context.Facts.AnimationLineagePathMismatchRoles.Num(), 0);

	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseoutWithContext(Plan, true, Context);
	TestEqual(TEXT("complete role/path lineage should pass"), Decision.PlanStatus, FString(TEXT("done")));
	TestTrue(TEXT("complete role/path lineage should not set a gate"), Decision.GateReasonCode.IsEmpty());
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_AnimationRoleLineageLegacyEvidenceStillPasses,
	"OsvayderUE.ActivePlanCloseout.AnimationWorkflow.RoleLineageLegacyEvidenceStillPasses",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FActivePlanCloseout_AnimationRoleLineageLegacyEvidenceStillPasses::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_animation_role_lineage_legacy_pass");

	TArray<TSharedPtr<FJsonObject>> Records;
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_legacy_mismatch"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"blocker_codes\":[\"skeleton_mismatch\"],\"proof_classification\":\"manual_blocker\"}"),
		TEXT("run_animation_role_lineage_legacy"),
		TEXT("2026-05-05T00:00:00Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_legacy_retarget"),
		TEXT("osvayderue/animation_retarget_fixup"),
		TEXT("{\"operation\":\"animation_retarget_fixup\"}"),
		TEXT("{\"result_type\":\"animation_retarget_fixup\",\"success\":true,\"status\":\"completed\",\"ready_to_claim_retarget_success\":true,\"generated_assets\":[\"/Game/ParkourRetargeted/WallClimb\"]}"),
		TEXT("run_animation_role_lineage_legacy"),
		TEXT("2026-05-05T00:00:01Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_legacy_compatible"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"preflight_ready_for_manual_verification\":true,\"blocker_codes\":[],\"compatible_roles\":[\"wall_climb\"],\"proof_classification\":\"gameplay_code_proof\"}"),
		TEXT("run_animation_role_lineage_legacy"),
		TEXT("2026-05-05T00:00:02Z")));
	...
	const FOsvayderUERunCloseoutContext Context = SOsvayderEditorWidget::BuildRunCloseoutContext(
		...
		TEXT("run_animation_role_lineage_legacy"),
		TEXT("X:/PublicExample/Unreal/SampleProject"));
	TestFalse(TEXT("legacy evidence without required roles should not activate lineage gate"), Context.Facts.bAnimationLineageRoleEvidencePresent);

	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseoutWithContext(Plan, true, Context);
	TestEqual(TEXT("legacy Packet 722 compatibility evidence should still pass"), Decision.PlanStatus, FString(TEXT("done")));
	TestTrue(TEXT("legacy Packet 722 evidence should not set a gate"), Decision.GateReasonCode.IsEmpty());
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FActivePlanCloseout_AnimationRoleLineageLatestMismatchResetsCoverage,
	"OsvayderUE.ActivePlanCloseout.AnimationWorkflow.RoleLineageLatestMismatchResetsCoverage",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FActivePlanCloseout_AnimationRoleLineageLatestMismatchResetsCoverage::RunTest(const FString& Parameters)
{
	FOsvayderUEActivePlan Plan;
	Plan.PlanId = TEXT("plan_animation_role_lineage_latest_mismatch_reset");

	TArray<TSharedPtr<FJsonObject>> Records;
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_reset_mismatch_early"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"blocker_codes\":[\"skeleton_mismatch\"],\"unsatisfied_roles\":[\"wall_climb\"],\"role_inventory\":[{\"role\":\"wall_climb\",\"status\":\"skeleton_mismatch\",\"matches\":[{\"path\":\"/Game/ParkourAnimations/A_WallClimb\"}]}],\"proof_classification\":\"manual_blocker\"}"),
		TEXT("run_animation_role_lineage_reset"),
		TEXT("2026-05-05T00:00:00Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_reset_retarget_early"),
		TEXT("osvayderue/animation_retarget_fixup"),
		TEXT("{\"operation\":\"animation_retarget_fixup\"}"),
		TEXT("{\"result_type\":\"animation_retarget_fixup\",\"success\":true,\"status\":\"completed\",\"ready_to_claim_retarget_success\":true,\"generated_assets\":[{\"role\":\"wall_climb\",\"source_animation_path\":\"/Game/ParkourAnimations/A_WallClimb\",\"destination_object_path\":\"/Game/Variant_Platforming/Anims/A_WallClimb_Retargeted\"}]}"),
		TEXT("run_animation_role_lineage_reset"),
		TEXT("2026-05-05T00:00:01Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_reset_compatible_early"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"preflight_ready_for_manual_verification\":true,\"blocker_codes\":[],\"compatible_roles\":[\"wall_climb\"],\"role_inventory\":[{\"role\":\"wall_climb\",\"status\":\"compatible\",\"matches\":[{\"path\":\"/Game/Variant_Platforming/Anims/A_WallClimb_Retargeted\"}]}],\"proof_classification\":\"gameplay_code_proof\"}"),
		TEXT("run_animation_role_lineage_reset"),
		TEXT("2026-05-05T00:00:02Z")));
	Records.Add(MakeCloseoutTraceToolResult(
		TEXT("lineage_reset_mismatch_latest"),
		TEXT("osvayderue/anim_blueprint_modify"),
		TEXT("{\"operation\":\"animation_preflight\"}"),
		TEXT("{\"result_type\":\"animation_preflight\",\"blocker_codes\":[\"skeleton_mismatch\"],\"unsatisfied_roles\":[\"ledge_attach\"],\"role_inventory\":[{\"role\":\"ledge_attach\",\"status\":\"skeleton_mismatch\",\"matches\":[{\"path\":\"/Game/ParkourAnimations/A_LedgeAttach\"}]}],\"proof_classification\":\"manual_blocker\"}"),
		TEXT("run_animation_role_lineage_reset"),
		TEXT("2026-05-05T00:00:03Z")));
	...
	const FOsvayderUERunCloseoutContext Context = SOsvayderEditorWidget::BuildRunCloseoutContext(
		...
		TEXT("X:/PublicExample/Unreal/SampleProject"));
	TestFalse(
		TEXT("post-compatible proof before latest mismatch should be reset"),
		Context.Facts.bPostRetargetCompatibleAnimationPreflightObserved);
	TestFalse(TEXT("latest mismatch should reset compatible role lineage"), Context.Facts.CompatiblePostRetargetAnimationRoles.Contains(TEXT("wall_climb")));
	TestTrue(TEXT("latest mismatch should track new required role"), Context.Facts.RequiredAnimationRoles.Contains(TEXT("ledge_attach")));

	const FOsvayderUEActivePlanCloseoutDecision Decision =
		SOsvayderEditorWidget::EvaluateActivePlanCloseoutWithContext(Plan, true, Context);
	TestEqual(TEXT("later mismatch should fail closeout"), Decision.PlanStatus, FString(TEXT("failed")));
	TestEqual(
		TEXT("later mismatch should require later retarget"),
		Decision.GateReasonCode,
		FString(TEXT("animation_retarget_required_missing")));
	return true;
}
