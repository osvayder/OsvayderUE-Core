...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.RestartSurvival.ProjectLocalIsolationAcrossRoots",
...
bool FRestartSurvival_ProjectLocalIsolationAcrossRoots::RunTest(const FString& Parameters)
{
...

	{
		...
		FString Error;
		TestTrue(TEXT("project A state should save"), FOsvayderUERestartSurvivalManager::SaveState(State, Error));
		TestTrue(TEXT("project A state path should exist"), IFileManager::Get().FileExists(*FOsvayderUERestartSurvivalManager::GetStatePath()));
	}

	{
		...
		State.Phase = EOsvayderUERestartSurvivalPhase::DetachedRunning;
		State.ProviderSessionId = TEXT("thread_b");
		FString Error;
		TestTrue(TEXT("project B state should save"), FOsvayderUERestartSurvivalManager::SaveState(State, Error));
		TestTrue(TEXT("project B state path should exist"), IFileManager::Get().FileExists(*FOsvayderUERestartSurvivalManager::GetStatePath()));
	}

	{
		FScopedRestartSurvivalRootOverride Override(ProjectARoot);
		FOsvayderUERestartSurvivalState State;
		TestTrue(TEXT("project A state should load"), FOsvayderUERestartSurvivalManager::DescribeCurrentState(State));
...

	{
		FScopedRestartSurvivalRootOverride Override(ProjectBRoot);
		FOsvayderUERestartSurvivalState State;
		TestTrue(TEXT("project B state should load"), FOsvayderUERestartSurvivalManager::DescribeCurrentState(State));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.RestartSurvival.PendingResumeMatchesBackend",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_LaneStateRoundTripReflectsReattachContract,
	"OsvayderUE.RestartSurvival.LaneStateRoundTripReflectsReattachContract",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRestartSurvival_PendingResumeMatchesBackend::RunTest(const FString& Parameters)
{
	...
	FString Error;
	TestTrue(TEXT("state should save"), FOsvayderUERestartSurvivalManager::SaveState(State, Error));
	TestTrue(TEXT("codex backend should have pending resume"), FOsvayderUERestartSurvivalManager::HasPendingResume(EOsvayderUEProviderBackend::CodexCli));
	TestFalse(TEXT("claude backend should not reuse codex resume"), FOsvayderUERestartSurvivalManager::HasPendingResume(EOsvayderUEProviderBackend::ClaudeCli));
	return true;
}
...
bool FRestartSurvival_LaneStateRoundTripReflectsReattachContract::RunTest(const FString& Parameters)
{
	...
	FString Error;
	TestTrue(TEXT("active plan should save for continuity identity seeding"), FOsvayderUERelayAgentManager::SaveActivePlan(Plan, Error));
	...
	TestTrue(TEXT("restart-survival state should save"), FOsvayderUERestartSurvivalManager::SaveState(State, Error));

	FOsvayderUERestartSurvivalState Loaded;
	TestTrue(TEXT("restart-survival state should load"), FOsvayderUERestartSurvivalManager::LoadState(Loaded, Error));
	...
	TestEqual(TEXT("lane status helper should surface reattach wording"), SOsvayderEditorWidget::DescribeTaskLaneStatus(Loaded.LaneState), FString(TEXT("Reopening editor and reattaching")));
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_PluginSettingsReadbackTruth,
	"OsvayderUE.RestartSurvival.PluginSettingsReadbackTruth",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_PortableSupportBundlePrefersPluginOwnedPath,
	"OsvayderUE.RestartSurvival.PortableSupportBundlePrefersPluginOwnedPath",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_RestartSurvivalToolRegistered,
	"OsvayderUE.RestartSurvival.RestartSurvivalToolRegistered",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_AutonomousClosedEditorToolContractTruth,
	"OsvayderUE.RestartSurvival.AutonomousClosedEditorToolContractTruth",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_AutonomousClosedEditorPromptContractTruth,
	"OsvayderUE.RestartSurvival.AutonomousClosedEditorPromptContractTruth",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_AutonomousClosedEditorStartOverrideAcceptsDerivedIds,
	"OsvayderUE.RestartSurvival.AutonomousClosedEditorStartOverrideAcceptsDerivedIds",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_AutonomousClosedEditorStartOverrideRejectsWrongExplicitIds,
	"OsvayderUE.RestartSurvival.AutonomousClosedEditorStartOverrideRejectsWrongExplicitIds",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRestartSurvival_PortableSupportBundlePrefersPluginOwnedPath::RunTest(const FString& Parameters)
{
	...
	FString Error;
	TestTrue(TEXT("support bundle should resolve"), FOsvayderUERestartSurvivalManager::TryResolveSupportBundle(Bundle, Error));
	if (!Error.IsEmpty())
	{
		AddInfo(FString::Printf(TEXT("support bundle detail: %s"), *Error));
	}

	const FString ExpectedPluginRoot = NormalizePathForTest(FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("OsvayderUE")));
	...
	TestEqual(TEXT("resolved supervisor path should match the manager helper"),
		NormalizePathForTest(FOsvayderUERestartSurvivalManager::GetSupervisorScriptPath()),
		NormalizePathForTest(Bundle.SupervisorScriptPath));
	TestEqual(TEXT("resolved preflight path should match the manager helper"),
		NormalizePathForTest(FOsvayderUERestartSurvivalManager::GetPreflightScriptPath()),
...
bool FRestartSurvival_AutonomousClosedEditorToolContractTruth::RunTest(const FString& Parameters)
{
	...
	// (OsvayderUE.RestartSurvival.Description.*).
	TestTrue(
...
bool FRestartSurvival_AutonomousClosedEditorStartOverrideAcceptsDerivedIds::RunTest(const FString& Parameters)
{
	...
	FOsvayderUERestartSurvivalPreparedStartOverride Override;
	...
	Override.AdditionalRelaunchArguments = TEXT("-ExecCmds=\"OsvayderUE.Test\"");

	FOsvayderUERestartSurvivalManager::ClearPreparedRequestStartOverride();
	FOsvayderUERestartSurvivalManager::ArmPreparedRequestStartOverride(Override);

	FOsvayderUERestartSurvivalPreparedStartOverride ConsumedOverride;
	TestTrue(
		TEXT("empty task/session override should attach to derived autonomous ids"),
		FOsvayderUERestartSurvivalManager::TryConsumePreparedRequestStartOverride(Request, ConsumedOverride));
	TestEqual(
	...
	FOsvayderUERestartSurvivalPreparedStartOverride SecondAttempt;
	TestFalse(
		TEXT("autonomous override should be consumed exactly once"),
		FOsvayderUERestartSurvivalManager::TryConsumePreparedRequestStartOverride(Request, SecondAttempt));
	return true;
}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.RestartSurvival.PreparedRestoreRequestRoundTrip",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRestartSurvival_PreparedRestoreRequestRoundTrip::RunTest(const FString& Parameters)
{
	...
	const FString BackupPath = FPaths::Combine(ProjectRoot, TEXT("Saved"), TEXT("OsvayderUE"), TEXT("restore_target.backup.txt"));
	...
	FString Error;
	TestTrue(TEXT("prepared request should save"), FOsvayderUERestartSurvivalManager::SavePreparedRestoreRequest(Request, Error));
	TestTrue(TEXT("prepared request file should exist"), FOsvayderUERestartSurvivalManager::HasPreparedRestoreRequest());

	FOsvayderUERestartSurvivalPreparedRestoreRequest LoadedRequest;
	TestTrue(TEXT("prepared request should load"), FOsvayderUERestartSurvivalManager::LoadPreparedRestoreRequest(LoadedRequest, Error));
	...
	TestEqual(
		...
		FOsvayderUERestartSurvivalManager::ResolvePreparedRequestPostReattachCompletionText(LoadedRequest),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.RestartSurvival.PreparedRestoreRequestRejectsWrongProviderSession",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_TaskDrivenContinuationWithoutRestoreValidates,
	"OsvayderUE.RestartSurvival.TaskDrivenContinuationWithoutRestoreValidates",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_PreparedFileWriteRequestRoundTrip,
	"OsvayderUE.RestartSurvival.PreparedFileWriteRequestRoundTrip",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_TaskDrivenContinuationWithFileWriteValidates,
	"OsvayderUE.RestartSurvival.TaskDrivenContinuationWithFileWriteValidates",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRestartSurvival_TaskDrivenContinuationWithoutRestoreValidates::RunTest(const FString& Parameters)
{
	...
	TestTrue(
		...
		FOsvayderUERestartSurvivalManager::ValidatePreparedRestoreRequestForStart(
			Request,
...
bool FRestartSurvival_PreparedFileWriteRequestRoundTrip::RunTest(const FString& Parameters)
{
	...
	const FString SourcePath = NormalizePathForTest(FPaths::Combine(ProjectRoot, TEXT("Saved"), TEXT("OsvayderUE"), TEXT("file_write_source.txt")));
	...
	const FString BackupPath = NormalizePathForTest(FPaths::Combine(ProjectRoot, TEXT("Saved"), TEXT("OsvayderUE"), TEXT("file_write_target.backup.txt")));
	IFileManager::Get().MakeDirectory(*FPaths::GetPath(SourcePath), true);
	IFileManager::Get().MakeDirectory(*FPaths::GetPath(TargetPath), true);
	TestTrue(
	...
	FString Error;
	TestTrue(TEXT("prepared file-write request should save"), FOsvayderUERestartSurvivalManager::SavePreparedRestoreRequest(Request, Error));

	FOsvayderUERestartSurvivalPreparedRestoreRequest LoadedRequest;
	TestTrue(TEXT("prepared file-write request should load"), FOsvayderUERestartSurvivalManager::LoadPreparedRestoreRequest(LoadedRequest, Error));
...
bool FRestartSurvival_TaskDrivenContinuationWithFileWriteValidates::RunTest(const FString& Parameters)
{
	...
	FString Error;
	const bool bValidated = FOsvayderUERestartSurvivalManager::ValidatePreparedRestoreRequestForStart(
		Request,
		EOsvayderUEProviderBackend::CodexCli,
		ProjectRoot,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.RestartSurvival.PreparedRequestAutoStartArmConsumesMatchingRequest",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_PreparedRequestAutoStartArmSurvivesMissingCurrentProviderSession,
	"OsvayderUE.RestartSurvival.PreparedRequestAutoStartArmSurvivesMissingCurrentProviderSession",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_PreparedRequestAutoStartArmRejectsDifferentCurrentProviderSession,
	"OsvayderUE.RestartSurvival.PreparedRequestAutoStartArmRejectsDifferentCurrentProviderSession",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_ClosedEditorBuildBlockerDetectionTruth,
	"OsvayderUE.RestartSurvival.ClosedEditorBuildBlockerDetectionTruth",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_ClosedEditorBuildBlockerNegativeCompileFailureDoesNotMatch,
	"OsvayderUE.RestartSurvival.ClosedEditorBuildBlockerNegativeCompileFailureDoesNotMatch",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_ClosedEditorBuildBlockerAutoStartArmsPreparedRequest,
	"OsvayderUE.RestartSurvival.ClosedEditorBuildBlockerAutoStartArmsPreparedRequest",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_ClosedEditorBuildBlockerAutoStartPreservesOriginalTaskIntent,
	"OsvayderUE.RestartSurvival.ClosedEditorBuildBlockerAutoStartPreservesOriginalTaskIntent",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_ClosedEditorBuildBlockerAutoStartReadOnlyIntentStaysReadOnly,
	"OsvayderUE.RestartSurvival.ClosedEditorBuildBlockerAutoStartReadOnlyIntentStaysReadOnly",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_ClosedEditorBuildBlockerAutoStartRejectsMissingOriginPrompt,
	"OsvayderUE.RestartSurvival.ClosedEditorBuildBlockerAutoStartRejectsMissingOriginPrompt",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_ClosedEditorBuildBlockerAutoStartCarriesVisualProofRequirement,
	"OsvayderUE.RestartSurvival.ClosedEditorBuildBlockerAutoStartCarriesVisualProofRequirement",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_MarkReattachValidatedDoesNotClearPendingDispatch,
	"OsvayderUE.RestartSurvival.MarkReattachValidatedDoesNotClearPendingDispatch",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_OpenEditorBuildLockCloseSafetyDecisionTruth,
	"OsvayderUE.RestartSurvival.OpenEditorBuildLockCloseSafetyDecisionTruth",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRestartSurvival_PreparedRequestAutoStartArmConsumesMatchingRequest::RunTest(const FString& Parameters)
{
	...
	FOsvayderUERestartSurvivalPreparedRestoreRequest ConsumedRequest;
	TestTrue(
		TEXT("matching auto-start arm should consume prepared request"),
		FOsvayderUERestartSurvivalManager::TryConsumePreparedRequestAutoStart(
			EOsvayderUEProviderBackend::CodexCli,
			ProjectRoot,
	...
	TestFalse(TEXT("auto-start arm should be one-shot"), FOsvayderUERestartSurvivalManager::HasPreparedRequestAutoStartArm());
	return true;
}
...
bool FRestartSurvival_PreparedRequestAutoStartArmSurvivesMissingCurrentProviderSession::RunTest(const FString& Parameters)
{
	...
	FOsvayderUERestartSurvivalPreparedRestoreRequest ConsumedRequest;
	const bool bConsumed = FOsvayderUERestartSurvivalManager::TryConsumePreparedRequestAutoStart(
		EOsvayderUEProviderBackend::CodexCli,
		ProjectRoot,
		FString(),
		ConsumedRequest,
	...
	TestFalse(TEXT("auto-start arm should still be one-shot after fallback consumption"), FOsvayderUERestartSurvivalManager::HasPreparedRequestAutoStartArm());
	return true;
}
...
bool FRestartSurvival_PreparedRequestAutoStartArmRejectsDifferentCurrentProviderSession::RunTest(const FString& Parameters)
{
	...
	TestFalse(TEXT("auto-start arm should be one-shot even after explicit rejection"), FOsvayderUERestartSurvivalManager::HasPreparedRequestAutoStartArm());
	return true;
}
...
bool FRestartSurvival_ClosedEditorBuildBlockerDetectionTruth::RunTest(const FString& Parameters)
{
	...
	Blocker = FOsvayderUEClosedEditorBuildBlocker();
	...
	const FString LinkerLockFixture =
		TEXT("link : fatal error LNK1104: cannot open file 'D:/Project/Binaries/Win64/UnrealEditor-OsvayderUE.dll'\n");
	TestTrue(
...
bool FRestartSurvival_ClosedEditorBuildBlockerAutoStartArmsPreparedRequest::RunTest(const FString& Parameters)
{
	...
	Blocker.Family = EOsvayderUEClosedEditorBuildBlockerFamily::UbtLogBackupAccessDenied;
	Blocker.FamilyLabel = OsvayderUEClosedEditorBuildBlockerFamilyToString(Blocker.Family);
	...
	TestTrue(
		...
		FOsvayderUERestartSurvivalManager::TryPrepareClosedEditorBuildBlockerAutoStart(
			EOsvayderUEProviderBackend::CodexCli,
			...
			TransitionNotice,
			Error));
	TestTrue(TEXT("prepared request file should exist after auto-start preparation"), FOsvayderUERestartSurvivalManager::HasPreparedRestoreRequest());
	TestTrue(TEXT("prepared request auto-start should be armed"), FOsvayderUERestartSurvivalManager::HasPreparedRequestAutoStartArm());
	...
	FOsvayderUERestartSurvivalPreparedRestoreRequest LoadedRequest;
	TestTrue(TEXT("prepared request should load after auto-start preparation"), FOsvayderUERestartSurvivalManager::LoadPreparedRestoreRequest(LoadedRequest, Error));
	TestTrue(
	...
	TestTrue(
		...
		FOsvayderUERestartSurvivalManager::ResolvePreparedRequestPostReattachCompletionText(LoadedRequest).Contains(
			OriginTask.OriginatingUserPrompt,
			ESearchCase::CaseSensitive));
	FOsvayderUERestartSurvivalManager::ClearPreparedRequestAutoStartArm();
	return true;
}
...
bool FRestartSurvival_ClosedEditorBuildBlockerAutoStartPreservesOriginalTaskIntent::RunTest(const FString& Parameters)
{
	...
	FOsvayderUEClosedEditorBuildBlocker Blocker;
	Blocker.bDetected = true;
	Blocker.Family = EOsvayderUEClosedEditorBuildBlockerFamily::UhtGeneratedRenameLock;
	Blocker.FamilyLabel = OsvayderUEClosedEditorBuildBlockerFamilyToString(Blocker.Family);
	...
	const FString ResolvedPrompt = FOsvayderUERestartSurvivalManager::ResolvePreparedRequestPostReattachCompletionText(PreparedRequest);
...
bool FRestartSurvival_ClosedEditorBuildBlockerAutoStartReadOnlyIntentStaysReadOnly::RunTest(const FString& Parameters)
{
	...
	FOsvayderUEClosedEditorBuildBlocker Blocker;
	Blocker.bDetected = true;
	Blocker.Family = EOsvayderUEClosedEditorBuildBlockerFamily::LiveCodingActive;
	Blocker.FamilyLabel = OsvayderUEClosedEditorBuildBlockerFamilyToString(Blocker.Family);
...
bool FRestartSurvival_ClosedEditorBuildBlockerAutoStartRejectsMissingOriginPrompt::RunTest(const FString& Parameters)
{
	...
	TestFalse(TEXT("auto-start should not be armed without origin prompt"), FOsvayderUERestartSurvivalManager::HasPreparedRequestAutoStartArm());
	return true;
}
...
bool FRestartSurvival_MarkReattachValidatedDoesNotClearPendingDispatch::RunTest(const FString& Parameters)
{
	...
	TestFalse(
		...
		FOsvayderUERestartSurvivalManager::MarkReattachValidated(Error));

	FOsvayderUERestartSurvivalState ReloadedState;
	TestTrue(TEXT("state should reload after blocked validation"), FOsvayderUERestartSurvivalManager::DescribeCurrentState(ReloadedState));
...
bool FRestartSurvival_OpenEditorBuildLockCloseSafetyDecisionTruth::RunTest(const FString& Parameters)
{
	...
	FOpenEditorBuildLockCloseSafetyDecision Decision =
		SOsvayderEditorWidget::EvaluateOpenEditorBuildLockCloseSafety(Inputs);
	...
	Decision = SOsvayderEditorWidget::EvaluateOpenEditorBuildLockCloseSafety(Inputs);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.RestartSurvival.ReattachRequiresMatchingToken",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_FailedDetachedBuildRelaunchesEditor,
	"OsvayderUE.RestartSurvival.FailedDetachedBuildRelaunchesEditor",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_RelayAgentSuccessRelaunchesEditorAndInjectsContinuation,
	"OsvayderUE.RestartSurvival.RelayAgentSuccessRelaunchesEditorAndInjectsContinuation",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_RelayAgentAdoptsExternalTerminalResultAndRelaunchesEditor,
	"OsvayderUE.RestartSurvival.RelayAgentAdoptsExternalTerminalResultAndRelaunchesEditor",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRestartSurvival_ReattachRequiresMatchingToken::RunTest(const FString& Parameters)
{
	...
	TestFalse(
		...
		FOsvayderUERestartSurvivalManager::TryMarkReattached(
			EOsvayderUEProviderBackend::CodexCli,
			TEXT("token_wrong"),
			Notice,
			ReattachError));

	FOsvayderUERestartSurvivalState StillAwaitingState;
	TestTrue(TEXT("state should still load after mismatched token"), FOsvayderUERestartSurvivalManager::DescribeCurrentState(StillAwaitingState));
	TestEqual(TEXT("mismatched token must not advance phase"), StillAwaitingState.Phase, EOsvayderUERestartSurvivalPhase::AwaitingReattach);
	...
	FOsvayderUERestartSurvivalState ReattachedState;
	TestTrue(TEXT("state should load after matching token"), FOsvayderUERestartSurvivalManager::DescribeCurrentState(ReattachedState));
	TestEqual(TEXT("matching token should advance phase"), ReattachedState.Phase, EOsvayderUERestartSurvivalPhase::Reattached);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.RestartSurvival.PrematureReopenDoesNotMarkReattached",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_ReattachedDispatchedStateCanBeReplacedForFreshStart,
	"OsvayderUE.RestartSurvival.ReattachedDispatchedStateCanBeReplacedForFreshStart",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_DetachedOwnerStateRoundTripPersistsObjective,
	"OsvayderUE.RestartSurvival.DetachedOwnerStateRoundTripPersistsObjective",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_PreparedBuildBlockerContinuationDoesNotRewrapExistingContinuationPrompt,
	"OsvayderUE.RestartSurvival.PreparedBuildBlockerContinuationDoesNotRewrapExistingContinuationPrompt",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_ManualReopenAwaitingReattachCanAdoptSameState,
	"OsvayderUE.RestartSurvival.ManualReopenAwaitingReattachCanAdoptSameState",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_ProjectEditorAlreadyReopenedBeforePreflightReturnsAwaitingReattach,
	"OsvayderUE.RestartSurvival.ProjectEditorAlreadyReopenedBeforePreflightReturnsAwaitingReattach",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_RepeatedNormalizedBlockerSignatureReachesTruthfulTerminalFail,
	"OsvayderUE.RestartSurvival.RepeatedNormalizedBlockerSignatureReachesTruthfulTerminalFail",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FRestartSurvival_SupervisorPreservesUtf8NoBomPayloadBytes,
	"OsvayderUE.RestartSurvival.SupervisorPreservesUtf8NoBomPayloadBytes",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRestartSurvival_PrematureReopenDoesNotMarkReattached::RunTest(const FString& Parameters)
{
	...
	State.Backend = EOsvayderUEProviderBackend::CodexCli;
	State.bProviderThreadResumePending = true;
	State.Phase = EOsvayderUERestartSurvivalPhase::Relaunching;
	...
	FOsvayderUERestartSurvivalState ReloadedState;
	TestTrue(TEXT("state should still load after premature reopen"), FOsvayderUERestartSurvivalManager::DescribeCurrentState(ReloadedState));
	TestEqual(TEXT("premature reopen must keep phase unchanged"), ReloadedState.Phase, EOsvayderUERestartSurvivalPhase::Relaunching);
...
{
	FOsvayderUERestartSurvivalState ReattachedPendingState;
	ReattachedPendingState.Phase = EOsvayderUERestartSurvivalPhase::Reattached;
	...
	TestFalse(
		...
		FOsvayderUERestartSurvivalManager::CanReplaceExistingStateForFreshStart(ReattachedPendingState));
	...
	FOsvayderUERestartSurvivalState FailedState;
	FailedState.Phase = EOsvayderUERestartSurvivalPhase::FailedTerminal;
	TestTrue(
		TEXT("failed terminal state should remain replaceable"),
		FOsvayderUERestartSurvivalManager::CanReplaceExistingStateForFreshStart(FailedState));
...
bool FRestartSurvival_DetachedOwnerStateRoundTripPersistsObjective::RunTest(const FString& Parameters)
{
	...
	FOsvayderUERestartSurvivalState ReloadedState;
	TestTrue(TEXT("state should load"), FOsvayderUERestartSurvivalManager::LoadState(ReloadedState, Error));
...
bool FRestartSurvival_ManualReopenAwaitingReattachCanAdoptSameState::RunTest(const FString& Parameters)
{
	...
	TestTrue(
		...
		FOsvayderUERestartSurvivalManager::TryMarkReattachedFromManualReopen(
			EOsvayderUEProviderBackend::CodexCli,
			Notice,
			Error));
	...
	FOsvayderUERestartSurvivalState ReloadedState;
	TestTrue(TEXT("state should reload after manual reopen adoption"), FOsvayderUERestartSurvivalManager::LoadState(ReloadedState, Error));
	TestEqual(TEXT("manual reopen should advance phase to reattached"), ReloadedState.Phase, EOsvayderUERestartSurvivalPhase::Reattached);
...
bool FRestartSurvival_ProjectEditorAlreadyReopenedBeforePreflightReturnsAwaitingReattach::RunTest(const FString& Parameters)
{
	...
	State.bProviderThreadResumePending = true;
	State.Phase = EOsvayderUERestartSurvivalPhase::Detaching;
	...
	const FString SupervisorScriptPath = FOsvayderUERestartSurvivalManager::GetSupervisorScriptPath();
	...
	TestTrue(TEXT("updated state should load"), FOsvayderUERestartSurvivalManager::LoadState(UpdatedState, Error));
	...
	TestEqual(TEXT("manual reopen preflight should return to awaiting reattach"), UpdatedState.Phase, EOsvayderUERestartSurvivalPhase::AwaitingReattach);
...
bool FRestartSurvival_RepeatedNormalizedBlockerSignatureReachesTruthfulTerminalFail::RunTest(const FString& Parameters)
{
	...
	const FString BuildRoot = FPaths::Combine(SavedRoot, TEXT("UnrealBuildTool"));
	const FString BuildLogPath = FPaths::Combine(BuildRoot, TEXT("Poligon1Editor-current-build.log"));
	const FString RelaunchMarkerPath = FPaths::Combine(SavedRoot, TEXT("OsvayderUE"), TEXT("Automation"), TEXT("repeated_blocker_relaunch_marker.txt"));
	...
	FOsvayderUERestartSurvivalState UpdatedState;
	TestTrue(TEXT("updated state should load"), FOsvayderUERestartSurvivalManager::LoadState(UpdatedState, Error));
	TestEqual(TEXT("repeated signature should reach truthful terminal fail"), UpdatedState.Phase, EOsvayderUERestartSurvivalPhase::AwaitingReattach);
...
bool FRestartSurvival_SupervisorPreservesUtf8NoBomPayloadBytes::RunTest(const FString& Parameters)
{
	...
	const FString SavedRoot = FPaths::Combine(ProjectRoot, TEXT("Saved"), TEXT("OsvayderUE"));
	...
	FOsvayderUERestartSurvivalState UpdatedState;
	TestTrue(TEXT("updated UTF-8 state should load"), FOsvayderUERestartSurvivalManager::LoadState(UpdatedState, Error));
	if (!Error.IsEmpty())
	{
	...
	TestEqual(TEXT("UTF-8 payload run should return to awaiting reattach"),
		UpdatedState.Phase,
		EOsvayderUERestartSurvivalPhase::AwaitingReattach);
...
bool FRestartSurvival_PluginSettingsReadbackTruth::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("support bundle should resolve for readback truth"), FOsvayderUERestartSurvivalManager::TryResolveSupportBundle(SupportBundle, SupportBundleError));
	...
	State.ProviderThreadStatePath = TEXT("D:/ReadbackProject/Saved/OsvayderUE/codex_persistent_thread.json");
	...
	TestEqual(TEXT("restart_survival state path should match the project-local override"),
		...
		NormalizePathForTest(FOsvayderUERestartSurvivalManager::GetStatePath()));
	return true;
}
...
bool FRestartSurvival_FailedDetachedBuildRelaunchesEditor::RunTest(const FString& Parameters)
{
	...
	const FString MarkerDir = FPaths::Combine(SavedRoot, TEXT("OsvayderUE"), TEXT("Automation"));
...
bool FRestartSurvival_RelayAgentSuccessRelaunchesEditorAndInjectsContinuation::RunTest(const FString& Parameters)
{
	...
	const FString ProjectRoot = FPaths::ConvertRelativePathToFull(FPaths::Combine(
		...
		TEXT("RelayProject")));
	const FString RelayRoot = FPaths::Combine(ProjectRoot, TEXT("Saved"), TEXT("OsvayderUE"));
	...
	const FString RelayResultPath = FOsvayderUERelayAgentManager::GetRelayResultPath();
	...
	FString Error;
	TestTrue(TEXT("relay restart-survival state should save"), FOsvayderUERestartSurvivalManager::SaveState(State, Error));
	if (!Error.IsEmpty())
	{
	...
	Handoff.Settings = FOsvayderUERelayAgentManager::BuildCurrentCodexSettingsSnapshot();

	TestTrue(TEXT("relay handoff context should save"), FOsvayderUERelayAgentManager::SaveHandoffContext(Handoff, Error));
	if (!Error.IsEmpty())
	{
	...
	const FString PreviousRelayExecutable = FPlatformMisc::GetEnvironmentVariable(TEXT("OSVAYDERUE_RELAY_CODEX_EXECUTABLE"));
	FPlatformMisc::SetEnvironmentVar(TEXT("OSVAYDERUE_RELAY_CODEX_EXECUTABLE"), *FakeCodexPath);
	...
	FPlatformMisc::SetEnvironmentVar(TEXT("OSVAYDERUE_RELAY_CODEX_EXECUTABLE"), *PreviousRelayExecutable);
	...
	FOsvayderUERestartSurvivalState UpdatedState;
	TestTrue(TEXT("relay updated state should load"), FOsvayderUERestartSurvivalManager::LoadState(UpdatedState, Error));
	if (!Error.IsEmpty())
	{
	...
	TestTrue(TEXT("relay result should load after supervisor run"), FOsvayderUERelayAgentManager::LoadRelayResult(RelayResult, Error));
	TestEqual(TEXT("relay result should report success"),
		RelayResult.TerminalOutcome,
		EOsvayderUERelayTerminalOutcome::Success);
	...
	TestTrue(TEXT("relay progress log should exist"), FFileHelper::LoadFileToString(ProgressJsonl, *FOsvayderUERelayAgentManager::GetRelayProgressPath()));
...
bool FRestartSurvival_RelayAgentAdoptsExternalTerminalResultAndRelaunchesEditor::RunTest(const FString& Parameters)
{
	...
	FOsvayderUERelayResult RelayResult;
	TestTrue(TEXT("relay result should load after external-result supervisor run"), FOsvayderUERelayAgentManager::LoadRelayResult(RelayResult, Error));
	TestEqual(TEXT("relay result should report success"),
		RelayResult.TerminalOutcome,
		EOsvayderUERelayTerminalOutcome::Success);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.RestartSurvival.ReadbackAndDebugSummarySurfaceActivePlanRelayProgressAndCancel",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRestartSurvival_ReadbackAndDebugSummarySurfaceActivePlanRelayProgressAndCancel::RunTest(const FString& Parameters)
{
	...
	FOsvayderUEPlanMechanicEntry WorkMechanic;
	...
	TestTrue(TEXT("relay progress should append"), FOsvayderUERelayAgentManager::AppendProgressEntry(Progress, Error));
	...
	TestTrue(TEXT("relay result should save"), FOsvayderUERelayAgentManager::SaveRelayResult(Result, Error));

	TestTrue(TEXT("cancel request should save"), FOsvayderUERelayAgentManager::WriteCancelRequest(TEXT("monitor test"), Error));

	const TSharedPtr<FJsonObject> Readback = FOsvayderUERestartSurvivalManager::BuildReadbackJson();
	...
	const FString DebugSummary = FOsvayderUERestartSurvivalManager::BuildWidgetDebugSummary();
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.RestartSurvival.ArmEmissionSpawnsOwnerBeforeTransportReset",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRestartSurvival_ArmEmissionSpawnsOwnerBeforeTransportReset::RunTest(const FString& Parameters)
{
	...
	// OsvayderEditorWidget.cpp::TryHandleClosedEditorBuildBlocker routes through
	// FOsvayderUERestartSurvivalManager::ExecuteSpawnBeforeTransportResetSequence
	...
	// FOsvayderCodeSubsystem::CancelCurrentRequest -> TerminateProc on the
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.RestartSurvival.ArmEmissionPreservesEditorExitIntent",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRestartSurvival_ArmEmissionPreservesEditorExitIntent::RunTest(const FString& Parameters)
{
	...
	// OsvayderEditorWidget.cpp:2069 never schedules FPlatformMisc::RequestExit,
	// the editor never exits, and the PS supervisor
	// (OsvayderUE-RestartSurvival.ps1) sits forever in
	...
	// static helper SOsvayderEditorWidget::BuildArmOptionsForClosedEditorBuildBlocker.
	...
	const FClaudeRestartSurvivalStartOptions ArmOptions =
		SOsvayderEditorWidget::BuildArmOptionsForClosedEditorBuildBlocker();
	...
	// intent on the struct default (today default=true at OsvayderEditorWidget.h:26),
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.RestartSurvival.MonitorCloseSemanticShellIntegration",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRestartSurvival_MonitorCloseSemanticShellIntegration::RunTest(const FString& Parameters)
{
	...
	// PS test script OsvayderUE-RestartSurvivalMonitor-CloseSemantic.Tests.ps1
	...
	const FString MonitorScriptPath = FOsvayderUERestartSurvivalManager::GetMonitorScriptPath();
	...
	const FString TestScriptPath = FPaths::Combine(
		...
		TEXT("OsvayderUE-RestartSurvivalMonitor-CloseSemantic.Tests.ps1"));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.RestartSurvival.DetachedBuildCaptureShellIntegration",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRestartSurvival_DetachedBuildCaptureShellIntegration::RunTest(const FString& Parameters)
{
	...
	const FString TestScriptPath = FPaths::Combine(
		...
		TEXT("OsvayderUE-RestartSurvival-Capture.Tests.ps1"));
	if (!IFileManager::Get().FileExists(*TestScriptPath))
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.Widget.Agent2BackgroundBannerVisibilityDecision",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FWidget_Agent2BackgroundBannerVisibilityDecision::RunTest(const FString& Parameters)
{
	...
	auto MakeState = [](EOsvayderUERestartSurvivalPhase Phase, int32 Pid)
	{
		FOsvayderUERestartSurvivalState State;
		State.Phase = Phase;
	...
	for (const EOsvayderUERestartSurvivalPhase Phase : {
		EOsvayderUERestartSurvivalPhase::Detaching,
		EOsvayderUERestartSurvivalPhase::DetachedRunning,
		EOsvayderUERestartSurvivalPhase::AwaitingRelaunch,
		EOsvayderUERestartSurvivalPhase::Relaunching,
		EOsvayderUERestartSurvivalPhase::AwaitingReattach})
	{
		const FOsvayderUERestartSurvivalState State = MakeState(Phase, 12345);
		TestTrue(
			FString::Printf(TEXT("non-terminal phase %s + pid alive → visible"),
				OsvayderUERestartSurvivalPhaseToString(Phase)),
			SOsvayderEditorWidget::ShouldShowAgent2BackgroundBanner(State, true));
	}

	// Terminal phases → hidden regardless of liveness.
	for (const EOsvayderUERestartSurvivalPhase Phase : {
		EOsvayderUERestartSurvivalPhase::Reattached,
		EOsvayderUERestartSurvivalPhase::FailedTerminal,
		EOsvayderUERestartSurvivalPhase::AttachedInEditor})
	{
		const FOsvayderUERestartSurvivalState State = MakeState(Phase, 12345);
		TestFalse(
	...
	{
		const FOsvayderUERestartSurvivalState State = MakeState(
			EOsvayderUERestartSurvivalPhase::DetachedRunning, 98765);
		TestFalse(TEXT("dead-owner non-terminal phase → hidden"),
			SOsvayderEditorWidget::ShouldShowAgent2BackgroundBanner(State, false));
	}
	...
	{
		const FOsvayderUERestartSurvivalState State = MakeState(
			EOsvayderUERestartSurvivalPhase::Detaching, 0);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.Widget.Agent2BannerCancelPayloadShape",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FWidget_Agent2BannerCancelPayloadShape::RunTest(const FString& Parameters)
{
	...
	const FString Payload = SOsvayderEditorWidget::BuildAgent2BannerCancelRequestPayload();
