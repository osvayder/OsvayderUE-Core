...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.ParamValidator.ActorName.ValidNames",
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.ParamValidator.ActorName.InvalidNames",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.ParamValidator.ActorName.LengthLimits",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.ParamValidator.ConsoleCommand.BlockedCommands",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.ParamValidator.ConsoleCommand.ChainAttempts",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.ParamValidator.ConsoleCommand.ValidCommands",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.ParamValidator.BlueprintPath.Security",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
bool FMCPParamValidator_ValidateBlueprintPath_Security::RunTest(const FString& Parameters)
{
	...
	TestFalse("Raw Windows disk paths should be blocked",
		FMCPParamValidator::ValidateBlueprintPath(TEXT("X:\\PublicExample\\Unreal\\SampleProject\\Content\\Packet674Denied"), Error));
	TestTrue("Raw disk path error should classify as invalid package path",
		Error.Contains(TEXT("Invalid package path")));

	const FMCPToolResult RawDiskPathError = FMCPToolResult::Error(Error);
	TestTrue("Raw disk path error should return structured data", RawDiskPathError.Data.IsValid());
	if (!RawDiskPathError.Data.IsValid())
	{
		return false;
	}
	TestEqual("Raw disk path should classify as invalid_path",
		RawDiskPathError.Data->GetStringField(TEXT("error_category")),
		FString(TEXT("invalid_path")));

	TestFalse("Relative package paths should be blocked",
		FMCPParamValidator::ValidateBlueprintPath(TEXT("Game/Blueprints/BP_Relative"), Error));

	TestFalse("Non-mounted absolute package paths should be blocked",
		FMCPParamValidator::ValidateBlueprintPath(TEXT("/PublicExample/Unreal/SampleProject/Content/Packet674Denied"), Error));

	// Valid game paths
	TestTrue("/Game/ paths should be valid",
		FMCPParamValidator::ValidateBlueprintPath(TEXT("/Game/Blueprints/BP_MyActor"), Error));
	TestTrue("/Game/ object paths should be valid",
		FMCPParamValidator::ValidateBlueprintPath(TEXT("/Game/Blueprints/BP_MyActor.BP_MyActor"), Error));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.ParamValidator.PropertyPath.Format",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.ParamValidator.NumericValue.EdgeCases",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.ParamValidator.SanitizeString.RemovesDangerousChars",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.ParamValidator.BlueprintNames.Validation",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
