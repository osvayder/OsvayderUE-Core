 ...
 *
 * Reference:
 *   - Dispatch: `AgentBridge/CODEX_TO_OSVAYDER.md` 2026-04-20 02:00 (628-v2).
...
#include "OsvayderUEEnginePatchProbe.h" // 628-v3 patch probe tests

#if WITH_DEV_AUTOMATION_TESTS

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FLiveCodingLimitLauncher_PreflightScriptEmitsLimit,
	"OsvayderUE.LiveCoding.LimitLauncher.PreflightScriptEmitsLimit",
...
bool FLiveCodingLimitLauncher_PreflightScriptEmitsLimit::RunTest(const FString& /*Parameters*/)
{
	...
	if (!TestTrue(TEXT("OsvayderUE plugin must be discoverable"), Plugin.IsValid()))
	{
		return false;
	}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.LiveCoding.EnginePatch.ProbeDetectsPatchWhenPresent",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FLiveCodingLimitLauncher_ProbeDetectsPatchWhenPresent::RunTest(const FString& /*Parameters*/)
{
	...
	const FString SynthesizedWithPatch = TEXT(
		...
		"    // OsvayderUE 628-v3 patch: raise LiveCoding action limit from environmental\n"
	...
	TestTrue(
		...
		FOsvayderUEEnginePatchProbe::DetectPatchPresence(LineOnly));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.LiveCoding.EnginePatch.ProbeFlagsMissingPatch",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FLiveCodingLimitLauncher_ProbeFlagsMissingPatch::RunTest(const FString& /*Parameters*/)
{
	...
	TestFalse(
		...
		FOsvayderUEEnginePatchProbe::DetectPatchPresence(UnpatchedBaseline));

	// Empty content — must not false-positive.
	TestFalse(
		TEXT("Empty content must not be detected as patch-present"),
		FOsvayderUEEnginePatchProbe::DetectPatchPresence(FString()));
	...
	TestFalse(
		...
		FOsvayderUEEnginePatchProbe::DetectPatchPresence(DifferentLimit));
	...
	// the signature is the FULL phrase "OsvayderUE 628-v3 patch".
	...
	TestFalse(
		...
		FOsvayderUEEnginePatchProbe::DetectPatchPresence(PartialPhrase));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.LiveCoding.EnginePatch.ProbeResolvesEngineSourcePath",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FLiveCodingLimitLauncher_ProbeResolvesEngineSourcePath::RunTest(const FString& /*Parameters*/)
{
	...
	const FString Path = FOsvayderUEEnginePatchProbe::GetEngineSourceAbsolutePath();
