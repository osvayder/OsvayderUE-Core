 ...
 *
 * Reference: dispatch `AgentBridge/CODEX_TO_OSVAYDER.md` 2026-04-20 14:15 (633).
 */
...
namespace
{
	...
	FOsvayderUEActivePlan BuildSynthPlan(const FString& UpdatedAtUtc,
	...
	FOsvayderUEPlanToolCallEntry MakeToolCall(const FString& ToolName,
	                                            const FString& Summary,
	                                            const FString& ResultStatus)
	{
		FOsvayderUEPlanToolCallEntry TC;
	...
	FOsvayderUERestartSurvivalState BuildRestartState(
		const EOsvayderUERestartSurvivalPhase Phase,
	...
	FOsvayderUEActivePlan BuildDeferredPlan(
		const FString& PlanId,
		const FString& UpdatedAtUtc,
		const FString& Status,
		const FString& OriginalTask)
	{
		FOsvayderUEActivePlan Plan = BuildSynthPlan(UpdatedAtUtc, Status, TEXT("incomplete"));
		Plan.PlanId = PlanId;
	...
	{
		FString Error;
		const bool bSaved = FOsvayderUERelayAgentManager::SavePlanToPath(ArchivePath, Plan, Error);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Detector.DetectsInterruptedStaleActive",
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Detector.DoesNotFireOnFreshActive",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Detector.DoesNotFireOnCompletedPlan",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTaskRecovery_FreshSessionDoesNotResumeStopLossedFeatureWorkflow,
	"OsvayderUE.TaskRecovery.FreshSessionDoesNotResumeStopLossedFeatureWorkflow",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FTaskRecovery_FreshSessionDoesNotResumeStopLossedFeatureWorkflow::RunTest(const FString& /*Parameters*/)
{
	...
	FOsvayderUEActivePlan StoppedPlan = BuildSynthPlan(UpdatedAt, TEXT("failed"), TEXT("not_achieved"));
	...
	const FAgentCanonExecution FreshExecution =
		OsvayderUECanonRouting::BuildInitialCanonExecution(
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Detector.CrashReasonDetected",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FTaskRecoveryDetector_CrashReasonDetected::RunTest(const FString& /*Parameters*/)
{
	...
	const FTaskRecoveryDetectionResult Result =
		FOsvayderUETaskRecoveryDetector::ClassifyPlan(Plan, Now, Crashes);
	...
	FOsvayderUEActivePlan FreshPlan = BuildSynthPlan(
		TwoMinsAgo.ToIso8601(), TEXT("active"), TEXT("incomplete"));
	...
	const FTaskRecoveryDetectionResult FreshResult =
		FOsvayderUETaskRecoveryDetector::ClassifyPlan(FreshPlan, Now, FreshCrashes);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.SuppressesDialogDuringActiveRestartSurvival",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FTaskRecovery_SuppressesDialogDuringActiveRestartSurvival::RunTest(const FString& /*Parameters*/)
{
	...
	FOsvayderUEActivePlan Plan = BuildSynthPlan(
		...
		TEXT("active"),
		TEXT("incomplete"));
	const TOptional<FOsvayderUERestartSurvivalState> RestartState = BuildRestartState(
		EOsvayderUERestartSurvivalPhase::DetachedRunning,
		(Now - FTimespan::FromMinutes(30)).ToIso8601());

	const TArray<FDateTime> NoCrashes;
	const FTaskRecoveryDetectionResult Result =
		FOsvayderUETaskRecoveryDetector::ClassifyPlan(Plan, Now, NoCrashes, RestartState);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.SuppressesDialogDuringReattachCompletion",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.FiresDialogAfterFailedRestartSurvival",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.FiresDialogAfterStuckSupervisor",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.FiresDialogAfterCleanRestartSurvivalLongAgo",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.DeferredBrowser.EnumeratesOnlyAbandonedStatuses",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FDeferredBrowser_EnumeratesOnlyAbandonedStatuses::RunTest(const FString& /*Parameters*/)
{
	...
	const TArray<FOsvayderUEDeferredPlanEntry> Entries =
		FOsvayderUEDeferredTasksEnumerator::ListDeferredPlans();
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.DeferredBrowser.ResumeMovesFileAndClearsFields",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FDeferredBrowser_ResumeMovesFileAndClearsFields::RunTest(const FString& /*Parameters*/)
{
	...
	FOsvayderCodeSubsystem::Get().ConsumePendingTaskRecoveryContextForTests();

	const FString ArchiveDir = FOsvayderUERelayAgentManager::GetPlanArchiveDir();
	IFileManager::Get().MakeDirectory(*ArchiveDir, true);

	const FString ArchivePath = FPaths::Combine(ArchiveDir, TEXT("20260420-plan_resume.active_plan.json"));
	const FOsvayderUEActivePlan ArchivedPlan = BuildDeferredPlan(
		TEXT("plan_resume"),
		(Now - FTimespan::FromHours(2)).ToIso8601(),
		TEXT("abandoned_by_user"),
	...
	const FString ExistingActivePlanPath = FOsvayderUERelayAgentManager::GetActivePlanPath();
	const FOsvayderUEActivePlan ExistingActivePlan = BuildDeferredPlan(
		TEXT("plan_existing"),
		(Now - FTimespan::FromMinutes(10)).ToIso8601(),
		TEXT("active"),
	...
	FString Error;
	TestTrue(TEXT("resume should succeed"), FOsvayderUEDeferredTasksBrowser::ResumePlan(Entry, ResumedShortTitle, Error));
	if (!Error.IsEmpty())
	{
		AddInfo(Error);
	}
	...
	TestTrue(TEXT("active plan should load after resume"), FOsvayderUERelayAgentManager::LoadActivePlan(ActivePlan, Error));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.DeferredBrowser.ResumeQueuesContextBlock",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FDeferredBrowser_ResumeQueuesContextBlock::RunTest(const FString& /*Parameters*/)
{
	...
	FString ResumedShortTitle;
	FString Error;
	TestTrue(TEXT("resume should queue context successfully"), FOsvayderUEDeferredTasksBrowser::ResumePlan(Entry, ResumedShortTitle, Error));

	const FString QueuedContext = FOsvayderCodeSubsystem::Get().ConsumePendingTaskRecoveryContextForTests();
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.DeferredBrowser.DeleteRemovesArchiveFile",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FDeferredBrowser_DeleteRemovesArchiveFile::RunTest(const FString& /*Parameters*/)
{
	...
	FString Error;
	TestTrue(TEXT("delete should succeed"), FOsvayderUEDeferredTasksBrowser::DeletePlan(Entry, Error));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Summarizer.GroupsConsecutiveSameTool",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FTaskRecoverySummarizer_GroupsConsecutiveSameTool::RunTest(const FString& /*Parameters*/)
{
	...
	const FTaskRecoverySummary Summary =
		FOsvayderUETaskRecoverySummarizer::BuildRecoverySummary(Plan, Now, TEXT("evidence/plan.json"));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Summarizer.DetectsErrorRepetition",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Summarizer.HandlesEmptyToolCalls",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Summarizer.ElapsedFormatting",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Widget.InjectionFiresOnceThenClears",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Widget.AutoDispatchDecisionReady",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
bool FTaskRecovery_AutoDispatchDecisionReady::RunTest(const FString& /*Parameters*/)
{
	const FTaskRecoveryAutoDispatchDecision Decision =
		SOsvayderEditorWidget::EvaluateTaskRecoveryAutoDispatch(
			false,
			true,
			FString(),
			false,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Widget.AutoDispatchDecisionBlocksBusy",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Widget.AutoDispatchDecisionBlocksBackendUnavailable",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Widget.AutoDispatchDecisionBlocksUnsafeTransportRetry",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.TaskRecovery.Widget.AutoResumePromptTruth",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

bool FTaskRecovery_AutoResumePromptTruth::RunTest(const FString& /*Parameters*/)
{
	const FString Prompt = SOsvayderEditorWidget::BuildTaskRecoveryAutoResumePrompt();
	const FString Marker = SOsvayderEditorWidget::BuildTaskRecoveryAutoResumeVisibleMarker();
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FTaskRecovery_DeferredTasksBrowserEnglishLabels,
	"OsvayderUE.TaskRecovery.DeferredTasksBrowser.EnglishLabels",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)

bool FTaskRecovery_DeferredTasksBrowserEnglishLabels::RunTest(const FString& /*Parameters*/)
{
	TestEqual(
		TEXT("abandoned_by_user label should be English"),
		FOsvayderUEDeferredTasksBrowser::BuildStatusLabel(TEXT("abandoned_by_user")),
		FString(TEXT("deferred by user")));
	TestEqual(
		TEXT("abandoned_for_fresh_session label should be English"),
		FOsvayderUEDeferredTasksBrowser::BuildStatusLabel(TEXT("abandoned_for_fresh_session")),
		FString(TEXT("deferred for new session")));
	TestEqual(
		TEXT("empty status fallback should be English"),
		FOsvayderUEDeferredTasksBrowser::BuildStatusLabel(FString()),
		FString(TEXT("not specified")));
	TestEqual(
		TEXT("empty reason fallback should be English"),
		FOsvayderUEDeferredTasksBrowser::BuildReasonLabel(FString()),
		FString(TEXT("not specified")));

	FOsvayderUEDeferredPlanEntry Entry;
	Entry.UserClosedReason = TEXT("Close as Irrelevant");
	Entry.Status = TEXT("abandoned_by_user");

	FTaskRecoverySummary Summary;
	Summary.ElapsedSincePause = TEXT("5 minutes ago");

	const FString MetaLine = FOsvayderUEDeferredTasksBrowser::BuildMetaLine(Entry, Summary);
	TestTrue(TEXT("meta line should use English deferred label"), MetaLine.Contains(TEXT("Deferred: 5 minutes ago")));
	TestTrue(TEXT("meta line should preserve Close as Irrelevant wording"), MetaLine.Contains(TEXT("Reason: Close as Irrelevant")));
	TestTrue(TEXT("meta line should use English status label"), MetaLine.Contains(TEXT("Status: deferred by user")));
	return true;
}
