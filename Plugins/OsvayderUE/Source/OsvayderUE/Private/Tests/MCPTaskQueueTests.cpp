...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.Create",
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.StartStop",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.SubmitValidTool",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.SubmitInvalidTool",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.SubmitWithTimeout",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.GetTaskStatus",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.GetNonExistentTask",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.CancelPendingTask",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
bool FMCPTaskQueue_CancelPendingTask::RunTest(const FString& Parameters)
{
	...
	FMCPToolResult CancelledResult;
	TestTrue("Cancelled task should expose a terminal result", Queue->GetTaskResult(TaskId, CancelledResult));
	TestFalse("Cancelled task result must not fabricate success", CancelledResult.bSuccess);
	TestTrue("Cancelled task should be classified as cancelled",
		CancelledResult.Data.IsValid()
		&& CancelledResult.Data->GetStringField(TEXT("error_category")) == TEXT("cancelled"));

	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	FMCPTaskQueue_TaskResultCancelledDoesNotFabricateSuccess,
	"OsvayderUE.MCP.TaskQueue.TaskResultCancelledDoesNotFabricateSuccess",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)

bool FMCPTaskQueue_TaskResultCancelledDoesNotFabricateSuccess::RunTest(const FString& Parameters)
{
	FMCPToolRegistry Registry;
	TSharedPtr<FMCPTaskQueue> Queue = Registry.GetTaskQueue();

	TSharedPtr<FJsonObject> SubmitParams = MakeShared<FJsonObject>();
	FGuid TaskId = Queue->SubmitTask(TEXT("get_level_actors"), SubmitParams);
	TestTrue("Should return valid task ID", TaskId.IsValid());
	TestTrue("Pending task cancel should succeed", Queue->CancelTask(TaskId));

	IMCPTool* ResultTool = Registry.FindTool(TEXT("task_result"));
	TestNotNull("task_result tool should exist", ResultTool);
	if (!ResultTool)
	{
		return false;
	}
	...
	ResultParams->SetStringField(TEXT("task_id"), TaskId.ToString());
	FMCPToolResult Result = ResultTool->Execute(ResultParams);

	TestFalse("task_result for cancelled task should fail", Result.bSuccess);
	TestTrue("task_result should include task JSON", Result.Data.IsValid());
	if (!Result.Data.IsValid())
	{
		return false;
	}

	TestEqual("Task status should be cancelled", Result.Data->GetStringField(TEXT("status")), TEXT("cancelled"));
	TestFalse("Task result success field should be false", Result.Data->GetBoolField(TEXT("success")));

	const TSharedPtr<FJsonObject>* ErrorData = nullptr;
	TestTrue("Cancelled task JSON should include structured error data", Result.Data->TryGetObjectField(TEXT("data"), ErrorData) && ErrorData && ErrorData->IsValid());
	if (ErrorData && ErrorData->IsValid())
	{
		TestEqual("Cancelled task error category", (*ErrorData)->GetStringField(TEXT("error_category")), TEXT("cancelled"));
	}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.CancelNonExistentTask",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.GetResultIncomplete",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.GetAllTasks",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.GetStats",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.StatusToString",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.TaskToJson",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.TaskToJsonWithResult",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.TaskIsComplete",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(
	...
	"OsvayderUE.MCP.TaskQueue.ConfigOverride",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter
)
