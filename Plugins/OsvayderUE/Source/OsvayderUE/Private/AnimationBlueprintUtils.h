...
public:
	...
	/**
	 * Human-readable compile status without compiling or saving the asset
	 */
	static FString GetBlueprintCompileStatusString(const UAnimBlueprint* AnimBP);
	...
	/**
	 * Build a read-only animation readiness inventory report for agent preflight checks
	 */
	static TSharedPtr<FJsonObject> BuildAnimationPreflightReport(
		UAnimBlueprint* AnimBP,
