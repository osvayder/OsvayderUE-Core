 ...
 *   - Reuse:    `OsvayderUECommandClassification.h` (P2-derived classifiers).
 */
...
OSVAYDERUE_API const TCHAR* CompileIntentCommandClassToString(ECompileIntentCommandClass Class);

/** 5 inputs the gate inspects. Missing / unknown values default to reasonable fallbacks. */
struct OSVAYDERUE_API FCompileIntentPolicyInputs
{
 ...
 */
struct OSVAYDERUE_API FCompileIntentPolicyDecision
{
	// ----- Core classification ------------------------------------------
 ...
 */
class OSVAYDERUE_API FCompileIntentPolicyGate
{
public:
