...
struct FExecutionReceipt
{
	...
	TSharedPtr<FJsonObject> ToJson() const
	{
		...
		if (CanonExecution.HasAnySignal())
		{
			...
			if (CanonExecution.MandatoryAnimationWorkflowSteps.Num() > 0) CanonObject->SetArrayField(TEXT("mandatory_animation_workflow_steps"), ToJsonArray(CanonExecution.MandatoryAnimationWorkflowSteps));
			if (CanonExecution.ConditionalMandatoryAnimationWorkflowSteps.Num() > 0) CanonObject->SetArrayField(TEXT("conditional_mandatory_animation_workflow_steps"), ToJsonArray(CanonExecution.ConditionalMandatoryAnimationWorkflowSteps));
			if (CanonExecution.CloseoutGateReasonCodes.Num() > 0) CanonObject->SetArrayField(TEXT("closeout_gate_reason_codes"), ToJsonArray(CanonExecution.CloseoutGateReasonCodes));
 ...
 */
class OSVAYDERUE_API FOsvayderUEExecutionLog
{
public:
	static FOsvayderUEExecutionLog& Get()
	{
		static FOsvayderUEExecutionLog Instance;
		return Instance;
	}
	...
	{
		return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("OsvayderUE"), TEXT("diagnostic_trace.jsonl"));
	}
