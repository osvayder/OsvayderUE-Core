...
enum class EOsvayderUERelayTerminalOutcome : uint8
{
	None,
	Success,
	TerminalFail,
...
OSVAYDERUE_API const TCHAR* OsvayderUERelayTerminalOutcomeToString(EOsvayderUERelayTerminalOutcome Outcome);
OSVAYDERUE_API bool ParseOsvayderUERelayTerminalOutcome(const FString& Value, EOsvayderUERelayTerminalOutcome& OutOutcome);

struct OSVAYDERUE_API FOsvayderUERelaySettingsSnapshot
{
...
struct OSVAYDERUE_API FOsvayderUERelayHandoffContext
{
	int32 SchemaVersion = 1;
	FString TaskId;
	...
	FOsvayderUERelaySettingsSnapshot Settings;
};

struct OSVAYDERUE_API FOsvayderUERelayProgressEntry
{
	int32 SchemaVersion = 1;
	FString TaskId;
	FString PlanId;
	...
	bool bIsStale = false;
	EOsvayderUERelayTerminalOutcome TerminalOutcome = EOsvayderUERelayTerminalOutcome::None;
};

struct OSVAYDERUE_API FOsvayderUERelayResult
{
	int32 SchemaVersion = 1;
...
struct OSVAYDERUE_API FOsvayderUEPlanMechanicEntry
{
	int32 SchemaVersion = 1;
	FString MechanicId;
	FString Label;
...
struct OSVAYDERUE_API FOsvayderUEPlanToolCallEntry
{
	int32 SchemaVersion = 1;
	FString ToolCallId;
	FString MechanicId;
...
struct OSVAYDERUE_API FOsvayderUECompileProofState
{
	int32 SchemaVersion = 1;
...
struct OSVAYDERUE_API FOsvayderUEActivePlan
{
	int32 SchemaVersion = 1;
	FString PlanId;
	...
	FOsvayderUERelaySettingsSnapshot Settings;
	FOsvayderUECompileProofState CompileProof;
	FAgentFeatureWorkflowState FeatureWorkflow;
	TArray<FOsvayderUEPlanMechanicEntry> Mechanics;
	TArray<FOsvayderUEPlanToolCallEntry> ToolCalls;
	TArray<FString> CompletedMechanicIds;
...
class OSVAYDERUE_API FOsvayderUERelayAgentManager
{
public:
	...
	static bool LoadActivePlan(FOsvayderUEActivePlan& OutPlan, FString& OutError);
	static bool SaveActivePlan(const FOsvayderUEActivePlan& Plan, FString& OutError);
	static bool DeleteActivePlan(FString& OutError);
	static bool LoadPlanFromPath(const FString& PlanPath, FOsvayderUEActivePlan& OutPlan, FString& OutError);
	static bool SavePlanToPath(const FString& PlanPath, const FOsvayderUEActivePlan& Plan, FString& OutError);
	...
	static bool LoadActivePlanWithFallback(
		...
		FString& OutError);
	static bool LoadProgressEntriesForTask(const FString& TaskId, TArray<FOsvayderUERelayProgressEntry>& OutEntries, FString& OutError);
	static bool ArchiveTerminalArtifacts(const FOsvayderUEActivePlan& Plan, TArray<FString>& OutArchivedPaths, FString& OutError);
	static bool PruneArchivedArtifacts(int32 MaxPlanSnapshots, int32 MaxRelayBundles, FString& OutError);

	static bool LoadHandoffContext(FOsvayderUERelayHandoffContext& OutContext, FString& OutError);
	static bool SaveHandoffContext(const FOsvayderUERelayHandoffContext& Context, FString& OutError);
	static bool DeleteHandoffContext(FString& OutError);

	static bool AppendProgressEntry(const FOsvayderUERelayProgressEntry& Entry, FString& OutError);
	static bool LoadLatestProgressEntry(FOsvayderUERelayProgressEntry& OutEntry, FString& OutError);
	static bool LoadLatestProgressEntryForTask(const FString& TaskId, FOsvayderUERelayProgressEntry& OutEntry, FString& OutError);
	static bool DeleteProgressLog(FString& OutError);

	static bool LoadRelayResult(FOsvayderUERelayResult& OutResult, FString& OutError);
	static bool LoadRelayResultForTask(const FString& TaskId, FOsvayderUERelayResult& OutResult, FString& OutError);
	static bool SaveRelayResult(const FOsvayderUERelayResult& Result, FString& OutError);
	...
	static FOsvayderUERelaySettingsSnapshot BuildCurrentCodexSettingsSnapshot();
};
